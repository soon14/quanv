...
class V_customerserviceModuleSite extends WeModuleSite
{
	...
	public function doMobileChat()
	{
		...
		$xcxopenid = $this->fromgzhidtoxcxid($openid);		//根据公众号openid查询对应的小程序openid
		...
		$xcxtoopenid = $this->fromgzhidtoxcxid($toopenid);		//根据公众号openid查询对应的小程序openid
		if ($openid == $toopenid) {
			$message = '不能和自己聊天！';
			include $this->template('error');
			exit;
	...
	public function fromgzhidtoxcxid($openidstr)
	{
		...
		if (empty($openidstr)){
			...
			if (!empty($memberid)){
				if ($memberid != 0){
					$r = pdo_getcolumn('rr_v_member', array('id' => $memberid,'uniacid' => $_W['uniacid']), 'openid_wa');
				}else{
					...
					$gzh_list = pdo_fetch('select * from ' . tablename('rr_v_member') . ' where ' . $condition . ' limit 1', $params);

					$xcx_list = pdo_fetchall('select * from ' . tablename('rr_v_member') . ' where ' . $where . ' ', $params);
					foreach ($xcx_list as &$row) {
						if($gzh_list['nickname'] == $row['nickname'] && $gzh_list['avatar_byte'] == $row['avatar_byte']){
							pdo_update('rr_v_member', array('memberid' => $gzh_list['id']), array('uniacid' => $_W['uniacid'], 'openid' => $row['openid']));
							pdo_update('rr_v_member', array('memberid' => $row['id']), array('uniacid' => $_W['uniacid'], 'openid' => $gzh_list['openid']));
							$r = $row['openid_wa'];
						}
					}
				}
