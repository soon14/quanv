...
if ($operation == 'display') {
	...
	foreach($list as $k=>$v){
		$biaoqianres = pdo_fetch("SELECT * FROM ".tablename(BEST_BIAOQIAN)." WHERE weid = {$_W['uniacid']} AND fensiopenid = '{$v['fansopenid']}' AND kefuopenid = '{$v['kefuopenid']}'");
		$list[$k]['name'] = $biaoqianres['name'];
		$list[$k]['realname'] = $biaoqianres['realname'];
		$list[$k]['telphone'] = $biaoqianres['telphone'];
		$list[$k]['chat'] = pdo_fetchall("SELECT * FROM ".tablename(BEST_CHAT)." WHERE weid = {$_W['uniacid']} AND fkid = {$v['id']} ORDER BY time DESC");
	}
