...
if ($operation == 'display') {
	...
	if($setting['istplon'] == 1 && $guotime > $setting['kefutplminute'] && $guotime2 > $setting['kefutplminute']){
		...
		if(!empty($tpllist)){
			...
			$concon = $sanfanskefu['fansnickname'].'对您发起了聊天！'.$tplcon.'。';
			$row = array();
	...
	$resArr['error'] = 0;
	$resArr['msg'] = '';
	$resArr['content'] = doReplacecon($data['content'],$data['type'],$data['yuyintime'],$insertchatid);
...
function doReplacecon($content,$msgtype,$yuyintime,$insertchatid){
	...
	if($msgtype == 2){					
		$content = '<img src="'.$content.'" class="sssbbb" data-myid="'.$insertchatid.'" style="max-width:100%;" />';
	}elseif($msgtype == 3){					
