...
class V_customerserviceModuleSite extends WeModuleSite
{
	...
	public function doMobileChat()
	{
		...
		if (empty($_W['fans']['from_user'])) {
			$openid = $_W['clientip'];
			if(!empty($_GPC['fromopenid'])){
				$openid = trim($_GPC['fromopenid']);
				$_W['fans']['from_user'] = $openid;
			}
		} else {
			$openid = $_W['fans']['from_user'];
		}
		...
		if (empty($hasfanskefu)) {
			...
			if (empty($_W['fans']['from_user'])) {
				...
				if(!empty($diag_log)){
					$datafanskefu['fansnickname'] = $diag_log['name'];
			...
			if(!empty($diag_log) && !empty($diag_log['name'])){
				
				pdo_update(BEST_FANSKEFU, array('fansnickname' => $diag_log['name']), array('weid' => $_W['uniacid'], 'fansopenid' => $openid, 'kefuopenid' => $toopenid));
		...
		$chatcon = pdo_fetchall("SELECT * FROM " . tablename(BEST_CHAT) . " WHERE fkid = {$hasfanskefu['id']} AND weid = {$_W['uniacid']} AND fansdel = 0 ORDER BY time ASC ");
		$timestamp = TIMESTAMP;
		$chatcontime = 0;
	...
	public function doMobileServicechat()
	{
		...
		if (empty($hasfanskefu)) {
			...
			if(!empty($diag_log) && !empty($diag_log['name'])){
				...
				pdo_update(BEST_FANSKEFU, array('fansnickname' => $diag_log['name']), array('weid' => $_W['uniacid'], 'fansopenid' => $toopenid, 'kefuopenid' => $openid));
		...
		$chatcon = pdo_fetchall("SELECT * FROM " . tablename(BEST_CHAT) . " WHERE fkid = {$hasfanskefu['id']} AND weid = {$_W['uniacid']} AND kefudel = 0 ORDER BY time ASC ");
		$timestamp = TIMESTAMP;
		$chatcontime = 0;
		foreach ($chatcon as $k => $v) {
			if ($v['time'] - $chatcontime > 7200) {
	...
	public function doMobileAddchat()
	{
		...
		// 	}
		// }
		$hasfanskefu = pdo_fetch("SELECT * FROM " . tablename(BEST_FANSKEFU) . " WHERE weid = {$_W['uniacid']} AND fansopenid = '{$openid}' AND kefuopenid = '{$_GPC['toopenid']}'");
			...
			if($diag_log['doctorid'] > 0){
				if($diag_log['consult_type'] == 1){
					...
					if($consult_total['total'] >= 6){
						$resArr['error'] = 1;
						$resArr['msg'] = '抱歉，您本次咨询已结束，医生不能再回复您的消息，若要继续咨询，您需要续费！';
						echo json_encode($resArr);
						exit;
					}
		...
		if ($setting['istplon'] == 1 && $guotime > $setting['kefutplminute'] && $fanskefu['kfzx'] == 0) {
			...
			$tpllist = pdo_fetch("SELECT id,tplbh FROM" . tablename(BEST_TPLMESSAGE_TPLLIST) . " WHERE tplbh = 'OPENTM202119578' AND uniacid = {$_W['uniacid']}");
			if (!empty($tpllist)) {
				...
				$arrmsg = array('openid' => $data['toopenid'], 'topcolor' => '#980000', 'first' => '您好，您有一条新的咨询待处理', 'firstcolor' => '#990000', 'keyword1' => $data['nickname'], 'keyword1color' => '', 'keyword2' => $tplcon, 'keyword2color' => '', 'remark' => date("Y-m-d H:i:s", TIMESTAMP), 'remarkcolor' => '', 'url' => '');
				$this->sendtemmsg($tpllist['id'], $arrmsg);
			} else {
		...
		$resArr['msg'] = '';
		$resArr['content'] = $this->doReplacecon($data['content'], $data['type'], $data['yuyintime']);
		$resArr['yuyincon'] = $data['type'] == 5 ? $data['yuyintime'] . '\'\'<span class="weidu"></span>' : '';
		$resArr['datetime'] = date("Y-m-d H:i:s", $data['time']);
		$resArr['insertchatid'] = $insertchatid;
		echo json_encode($resArr);
		exit;
	}
	public function doMobileAddchat2()
	{
		...
		if ($setting['istplon'] == 1 && $guotime > $setting['kefutplminute'] && $fanskefu['fszx'] == 0) {
			...
			if (!empty($tpllist)) {
				...
				$arrmsg = array('openid' => $data['toopenid'], 'topcolor' => '#980000', 'first' => $data['nickname'] . '回复了您！', 'firstcolor' => '#990000', 'keyword1' => $lastmsgcon, 'keyword1color' => '', 'keyword2' => $tplcon, 'keyword2color' => '', 'remark' => '回复时间：' . date("Y-m-d H:i:s", TIMESTAMP), 'remarkcolor' => '', 'url' => '');
				$this->sendtemmsg($tpllist['id'], $arrmsg);
			} else {
		...
		$resArr['msg'] = '';
		$resArr['content'] = $this->doReplacecon($data['content'], $data['type'], $data['yuyintime']);
		$resArr['yuyincon'] = $data['type'] == 6 ? '<span class="weidu"></span>' . $data['yuyintime'] . '\'\'' : '';
		$resArr['datetime'] = date("Y-m-d H:i:s", $data['time']);
		$resArr['insertchatid'] = $insertchatid;
		echo json_encode($resArr);
		exit;
	}
	public function doMobileRecallmsg()
	{
		global $_W, $_GPC;
		$openid = $_W['fans']['from_user'];
		if (empty($openid)) {
		...
		$fkid = $_GPC['fkid'];
		$insertchatid = $_GPC['insertchatid'];
		$type = $_GPC['type'];
		if (empty($fkid) || empty($insertchatid) || empty($type)) {
			$resArr['error'] = 1;
			$resArr['msg'] = '请求参数错误！';
			echo json_encode($resArr);
			exit;
		...
		$lastmsg = pdo_fetch("SELECT content,type FROM " . tablename(BEST_CHAT) . " WHERE weid = {$_W['uniacid']} AND id = ".intval($insertchatid)." AND fkid = ".intval($fkid)." ");

		$fanskefu = pdo_fetch("SELECT * FROM " . tablename(BEST_FANSKEFU) . " WHERE weid = {$_W['uniacid']} AND id = ".intval($fkid)." ");

		if ($type == 'kefu') {
			pdo_update(BEST_CHAT, array('content' => '撤回消息', 'kefurecall' => 1, 'type' => 2), array('weid' => $_W['uniacid'], 'id' => intval($insertchatid), 'openid' => $openid, 'fkid' => intval($fkid)));
			if($lastmsg['content'] == $fanskefu['kefulastcon'] && $lastmsg['type'] == $fanskefu['kefumsgtype']){
				pdo_update(BEST_FANSKEFU, array('kefulastcon' => '撤回消息', 'kefurecall' => 1, 'kefumsgtype' => 2), array('weid' => $_W['uniacid'], 'kefuopenid' => $openid, 'id' => intval($fkid)));
			}
		} else {
			pdo_update(BEST_CHAT, array('content' => '撤回消息', 'fansrecall' => 1, 'type' => 1), array('weid' => $_W['uniacid'], 'id' => intval($insertchatid), 'openid' => $openid, 'fkid' => intval($fkid)));
			if($lastmsg['content'] == $fanskefu['lastcon'] && $lastmsg['type'] == $fanskefu['msgtype']){
				pdo_update(BEST_FANSKEFU, array('lastcon' => '撤回消息', 'fansrecall' => 1, 'msgtype' => 1), array('weid' => $_W['uniacid'], 'fansopenid' => $openid, 'id' => intval($fkid)));
			}
		}
