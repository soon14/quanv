...
class TaskProcessor extends PluginProcessor
{
	...
	private function responseSubscribe_new($obj)
	{
		...
		$recordid = pdo_fetchcolumn('select recordid from ' . tablename('rr_v_task_qr') . ' where uniacid = :uniacid and sceneid = :sceneid', array(':uniacid' => $_W['uniacid'], ':sceneid' => $eventkey));
		$openid = pdo_fetchcolumn('select openid from ' . tablename('rr_v_task_record') . ' where uniacid = :uniacid and id = :recordid', array(':uniacid' => $_W['uniacid'], ':recordid' => $recordid));
		...
		if ($member_info['isnew']) {
			...
			if ($agent['isagent']) {
				...
				$a = pdo_update('rr_v_member', array('agentid' => $agent['id'], 'childtime' => time()), array('id' => $my['id']));
			}
	...
	private function responseScan($obj)
	{
		...
		$poster = pdo_fetch('select * from ' . tablename('rr_v_task_poster') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $qr['posterid'], ':uniacid' => $_W['uniacid']));
		...
		if ($member_info['isnew'] || $_SESSION['postercontent']) {
			...
			$scan_count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('rr_v_task_joiner') . ' where uniacid=:uniacid and task_user=:task_user and joiner_id=:joiner_id and join_id=:join_id ', $params);
				...
				if ($member['isagent'] != 1) {
					if (($qrmember['isagent'] == 1) && ($qrmember['status'] == 1)) {
						if (!(empty($poster['bedown']))) {
							if (empty($member['agentid'])) {
								if (empty($member['fixagentid'])) {
									pdo_update('rr_v_member', array('agentid' => $qrmember['id'], 'childtime' => $time), array('id' => $member['id']));
						...
						if (!(empty($poster['beagent']))) {
							...
							pdo_update('rr_v_member', array('isagent' => 1, 'status' => $become_check, 'agenttime' => $time), array('id' => $member['id']));
