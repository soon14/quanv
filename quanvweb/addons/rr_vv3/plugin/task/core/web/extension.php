...
class Extension_RrvV3Page extends PluginWebPage
{
	public $action = '';
	public $taskType = '';
	...
	public function add()
	{
		...
		if ($_W['ispost']) {
			...
			if (empty($id)) {
			 ...
			 else if (pdo_update('rr_v_task', $data, array('id' => $id, 'uniacid' => $_W['uniacid']))) {
			...
			$data = pdo_get('rr_v_task', array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function goods()
	{
		...
		if (!(empty($type))) {
			if ($type == 'goods') {
				...
				$totalsql = 'select count(*) from ' . tablename('rr_v_coupon') . ' where couponname LIKE :title and uniacid=:uniacid ';
				$searchsql = 'select id,couponname,coupontype,enough,thumb,backtype,deduct,backmoney,backcredit,`total`,backredpack,discount,displayorder from ' . tablename('rr_v_coupon') . ' where couponname LIKE :title and uniacid=:uniacid ORDER BY `displayorder` DESC,`id` DESC LIMIT ' . (($page - 1) * $psize) . ',' . $psize;
				$total = pdo_fetchcolumn($totalsql, $params);
				$pagelast = intval(($total - 1) / $psize) + 1;
				$list = pdo_fetchall($searchsql, $params);
	...
	public function delete()
	{
		...
		foreach ($ids as $v ) {
			pdo_delete('rr_v_task', array('id' => intval($v), 'uniacid' => $_W['uniacid']));
		}
