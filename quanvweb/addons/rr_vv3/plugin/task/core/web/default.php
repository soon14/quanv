...
class Default_RrvV3Page extends PluginWebPage
{
	...
	{
		global $_W;
		$defaultcount = pdo_fetchcolumn('select COUNT(*) from ' . tablename('rr_v_task_default') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
		...
		if (0 < $defaultcount) {
			...
			$result = pdo_update('rr_v_task_default', $update_data, array('uniacid' => $_W['uniacid']));
			...
			$result = pdo_insert('rr_v_task_default', $insert_data);
	...
	public function setstart()
	{
		...
		if ($_W['ispost']) {
			...
			if (0 < $defaultcount) {
				...
				$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'rr_vv3:task'));
				...
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'rr_vv3:task', 'module' => 'cover', 'displayorder' => 0, 'status' => 1);
				...
				$cover_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'rr_vv3', 'title' => $default['title'], 'description' => $default['desc'], 'thumb' => $default['thumb'], 'url' => mobileUrl('task'));
				...
				pdo_update('rr_v_task_default', $update_data, array('uniacid' => $_W['uniacid']));
				...
				$default = serialize($default);
				$insert_data = array('uniacid' => $_W['uniacid'], 'data' => $default, 'addtime' => time());
				pdo_insert('rr_v_task_default', $insert_data);
			}
		}


		$set = unserialize($default);
		$bgimg = pdo_get('rr_v_task_default', array('uniacid' => $_W['uniacid']), array('bgimg', 'open'));
