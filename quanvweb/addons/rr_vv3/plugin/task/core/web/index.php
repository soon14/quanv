...
class Index_RrvV3Page extends PluginWebPage
{
	...
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_task_poster') . ' WHERE 1 ' . $condition . ' ORDER BY createtime desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		unset($row);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('rr_v_task_poster') . ' where 1 ' . $condition . ' ', $params);
		$pager = pagination2($total, $pindex, $psize);

		foreach ($list as $key => $val ) {
			$viewcount = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('rr_v_task_joiner') . ' where uniacid=:uniacid and task_id=:task_id and task_type=' . $val['poster_type'], array(':uniacid' => $_W['uniacid'], ':task_id' => $val['id']));
	...
	public function post()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('rr_v_task_poster') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			if (!(empty($id))) {
				if (intval($_GPC['needcount']) < $item['needcount']) {
					$task_count = pdo_fetchcolumn('select COUNT(*) from ' . tablename('rr_v_task_join') . ' where uniacid=:uniacid and task_id=:task_id and task_type=1 and failtime>' . time(), array(':uniacid' => $_W['uniacid'], ':task_id' => $id));
			...
			if (($item['keyword'] != $data['keyword']) && !(empty($keyword))) {
				if ($keyword['name'] != 'rr_vv3:task:' . $id) {
					show_json(0, '关键字已存在!');
				}
			...
			if (!(empty($id))) {
				pdo_update('rr_v_task_poster', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				$updatesql = 'UPDATE ' . tablename('rr_v_task_join') . ' SET  failtime = addtime+' . $data['days'] . ',needcount=:needcount WHERE uniacid = :uniacid AND task_id=:task_id AND task_type=1 AND failtime>' . time();
				...
				pdo_insert('rr_v_task_poster', $data);
				$id = pdo_insertid();
			...
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'rr_vv3', ':name' => 'rr_vv3:task:' . $id));
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'rr_vv3:task:' . $id, 'module' => 'rr_vv3', 'displayorder' => 0, 'status' => $data['status']);
			$keyword_data = array('uniacid' => $_W['uniacid'], 'module' => 'rr_vv3', 'content' => trim($data['keyword']), 'type' => 1, 'displayorder' => 0, 'status' => $data['status']);
			...
			$ruleauto = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'rr_vv3', ':name' => 'rr_vv3:task:auto'));

			if (empty($ruleauto)) {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'rr_vv3:task:auto', 'module' => 'rr_vv3', 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'rr_vv3', 'content' => 'rr_vv3_TASK', 'type' => 1, 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule_keyword', $keyword_data);
			}
	...
	public function delete()
	{
		...
		$posters = pdo_fetchall('SELECT id,title,keyword FROM ' . tablename('rr_v_task_poster') . ' WHERE id in ( ' . $id . ' ) and uniacid=' . $_W['uniacid']);
		...
		foreach ($posters as $poster ) {
			...
			pdo_delete('rr_v_task_poster', array('id' => $poster['id'], 'uniacid' => $_W['uniacid']));
			pdo_delete('rr_v_task_log', array('taskid' => $poster['id'], 'task_type' => 1, 'uniacid' => $_W['uniacid']));
			pdo_delete('rr_v_task_join', array('taskid' => $poster['id'], 'task_type' => 1, 'uniacid' => $_W['uniacid']));
	...
	public function clear()
	{
		...
		global $_GPC;
		load()->func('file');
		@rmdirs(IA_ROOT . '/addons/rr_vv3/data/task/poster/' . $_W['uniacid']);
		@rmdirs(IA_ROOT . '/addons/rr_vv3/data/task/qrcode/' . $_W['uniacid']);
		$acid = pdo_fetchcolumn('SELECT acid FROM ' . tablename('account_wechats') . ' WHERE `uniacid`=:uniacid LIMIT 1', array(':uniacid' => $_W['uniacid']));
		pdo_update('rr_v_task_poster_qr', array('mediaid' => ''), array('acid' => $acid));
	...
	public function delreward()
	{
		...
		if ($type == 1) {
			$item = pdo_fetch('SELECT `id`,`reward_data` FROM ' . tablename('rr_v_task_poster') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
			...
			pdo_update('rr_v_task_poster', array('reward_data' => $item['reward_data']), array('id' => $id, 'uniacid' => $_W['uniacid']));
			$item['reward_data'] = unserialize($item['reward_data']);
	...
	public function main_new()
	{
		...
		$taskS = array();
		$taskS[0] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_task_record') . ' where uniacid = ' . $_W['uniacid']);
		$taskS[1] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_task_record') . ' where uniacid = ' . $_W['uniacid'] . ' and finishtime > \'0000-00-00 00:00:00\' ');
		$taskS[2] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_task_record') . ' where uniacid = ' . $_W['uniacid'] . ' and finishtime = \'0000-00-00 00:00:00\' and stoptime < \'' . $date . '\'');
		$taskS[3] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_task_record') . ' where uniacid = ' . $_W['uniacid'] . ' and finishtime = \'0000-00-00 00:00:00\' and stoptime > \'' . $date . '\'');
		$rewardS = array();
		$rewardS[0] = (int) pdo_fetchcolumn('select sum(reward_data) from ' . tablename('rr_v_task_reward') . ' where uniacid = ' . $_W['uniacid'] . ' and `get` = 1 and reward_type = \'credit\'');
		$rewardS[1] = (double) pdo_fetchcolumn('select sum(reward_data) from ' . tablename('rr_v_task_reward') . ' where uniacid = ' . $_W['uniacid'] . ' and `get` = 1 and reward_type = \'redpacket\'');
		$rewardS[2] = (double) pdo_fetchcolumn('select sum(reward_data) from ' . tablename('rr_v_task_reward') . ' where uniacid = ' . $_W['uniacid'] . ' and `get` = 1 and reward_type = \'balance\'');
		$rewardS[3] = (int) pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_task_reward') . ' where uniacid = ' . $_W['uniacid'] . ' and `get` = 1 and reward_type = \'coupon\'');
	...
	public function notice()
	{
		...
		if ($_W['ispost']) {
			...
			$saveRet = $this->model->taskSave('rr_v_task_set', $data);
	...
	public function setting()
	{
		global $_W;
		global $_GPC;
		$set = pdo_get('rr_v_task_set', array('uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			$saveRet = $this->model->taskSave('rr_v_task_set', $set);
			empty($saveRet) && show_json(0, '保存失败');
			show_json(1);
		}
	...
	protected function saveCover($keyword, $img, $desc, $title)
	{
		...
		if (empty($cid)) {
			pdo_insert('cover_reply', array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'rr_vv3', 'title' => $title, 'description' => $desc, 'thumb' => $img, 'url' => mobileUrl('task', NULL, 1)));
		}
		 else {
			pdo_update('cover_reply', array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'rr_vv3', 'title' => $title, 'description' => $desc, 'thumb' => $img, 'url' => mobileUrl('task', NULL, 1)), array('id' => $cid));
		}
	...
	public function post_new()
	{
		...
		if (!(empty($task['requiregoods']))) {
			...
			$goods = pdo_fetchall('select id,title,thumb from ' . tablename('rr_v_goods') . ' where id in (' . $task['requiregoods'] . ') and status=1 and deleted=0 and uniacid=' . $_W['uniacid'] . ' order by instr(\'' . $task['requiregoods'] . '\',id)');
		}
		...
		if ($_W['ispost']) {
			...
			pdo_update('rr_v_task_type', array('verb' => $diy['verb'], 'unit' => $diy['unit']), array('type_key' => 'poster'));
			...
			$saveRet = $this->model->taskSave('rr_v_task_list', $data);
			...
			$poster_text = pdo_get('rr_v_task_type', array('type_key' => 'poster'), array('verb', 'unit'));
	...
	public function preview()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$sql = 'select rc.*,rw.* from ' . tablename('rr_v_task_record') . ' rc left join ' . tablename('rr_v_task_reward') . ' rw on rc.id = rw.recordid where rc.id = ' . $id . ' and rc.uniacid = ' . $_W['uniacid'];
	...
	public function setdisplayorder()
	{
		...
		pdo_update('rr_v_task_list', array('displayorder' => $value), array('id' => $id, 'uniacid' => $_W['uniacid']));
		show_json(1);
	}
	...
	public function version()
	{
		...
		$new = 1;
		pdo_update('rr_v_task_set', array('isnew' => $new), array('uniacid' => $_W['uniacid']));
		$type = array('INSERT INTO ' . tablename('rr_v_task_type') . ' (`id`, `type_key`, `type_name`, `description`, `verb`, `numeric`, `unit`, `goods`, `theme`, `once`) VALUES (\'1\', \'poster\', \'任务海报\', \'把生成的海报并分享给朋友，朋友扫描并关注公众号即可获得奖励。\', \'转发海报并吸引\', \'1\', \'人关注\', \'0\', \'primary\', \'0\');', 'INSERT INTO ' . tablename('rr_v_task_type') . ' (`id`, `type_key`, `type_name`, `description`, `verb`, `numeric`, `unit`, `goods`, `theme`, `once`) VALUES (\'2\', \'info_phone\', \'绑定手机\', \'在个人中心中，绑定手机号，即可完成任务获得奖励。\', \'绑定手机\', \'0\', \'\', \'0\', \'warning\', \'0\');', 'INSERT INTO ' . tablename('rr_v_task_type') . ' (`id`, `type_key`, `type_name`, `description`, `verb`, `numeric`, `unit`, `goods`, `theme`, `once`) VALUES (\'3\', \'order_first\', \'首次购物\', \'在商城中首次下单，即可获得奖励，必须确认收货。\', \'首次在商城中下单购物\', \'0\', \'\', \'0\', \'warning\', \'0\');', 'INSERT INTO ' . tablename('rr_v_task_type') . ' (`id`, `type_key`, `type_name`, `description`, `verb`, `numeric`, `unit`, `goods`, `theme`, `once`) VALUES (\'4\', \'recharge_full\', \'单笔充值满额\', \'在商城中充值余额，单笔充值满额，即可获得奖励。\', \'单笔充值满\', \'1\', \'元\', \'0\', \'success\', \'1\');', 'INSERT INTO ' . tablename('rr_v_task_type') . ' (`id`, `type_key`, `type_name`, `description`, `verb`, `numeric`, `unit`, `goods`, `theme`, `once`) VALUES (\'5\', \'order_full\', \'单笔满额\', \'在商城中下单，单笔满额即可获得奖励，必须确认收货。\', \'单笔订单满\', \'1\', \'元\', \'0\', \'success\', \'1\');', 'INSERT INTO ' . tablename('rr_v_task_type') . ' (`id`, `type_key`, `type_name`, `description`, `verb`, `numeric`, `unit`, `goods`, `theme`, `once`) VALUES (\'6\', \'order_all\', \'累计消费\', \'在商城中购物消费，累计满额即可获得奖励，无需确认收货。\', \'购物总消费额达到\', \'1\', \'元\', \'0\', \'success\', \'0\');', 'INSERT INTO ' . tablename('rr_v_task_type') . ' (`id`, `type_key`, `type_name`, `description`, `verb`, `numeric`, `unit`, `goods`, `theme`, `once`) VALUES (\'7\', \'pyramid_money\', \'分销佣金\', \'只有分销商可接此任务。累计分销佣金满额，即可完成任务。\', \'分销商获得佣金金额达\', \'1\', \'元\', \'0\', \'primary\', \'0\');', 'INSERT INTO ' . tablename('rr_v_task_type') . ' (`id`, `type_key`, `type_name`, `description`, `verb`, `numeric`, `unit`, `goods`, `theme`, `once`) VALUES (\'8\', \'pyramid_num\', \'下级人数\', \'只有分销商可接此任务。累计下级人数达标，即可完成任务。\', \'分销商推荐下级人数达\', \'1\', \'人\', \'0\', \'primary\', \'0\');', 'INSERT INTO ' . tablename('rr_v_task_type') . ' (`id`, `type_key`, `type_name`, `description`, `verb`, `numeric`, `unit`, `goods`, `theme`, `once`) VALUES (\'9\', \'comment\', \'商品好评\', \'任意给一个商品五星好评，即可完成任务获得奖励。\', \'给商品好评\', \'0\', \'\', \'0\', \'warning\', \'0\');', 'INSERT INTO ' . tablename('rr_v_task_type') . ' (`id`, `type_key`, `type_name`, `description`, `verb`, `numeric`, `unit`, `goods`, `theme`, `once`) VALUES (\'10\', \'post\', \'社区发帖\', \'在社区中发表指定篇帖子，即可完成任务获得奖励。\', \'在论坛中发表\', \'1\', \'篇帖子\', \'0\', \'warning\', \'0\');', 'INSERT INTO ' . tablename('rr_v_task_type') . ' (`id`, `type_key`, `type_name`, `description`, `verb`, `numeric`, `unit`, `goods`, `theme`, `once`) VALUES (\'11\', \'goods\', \'购买指定商品\', \'购买指定商品后即可完成任务，必须确认收货。\', \'购买指定商品\', \'0\', \'\', \'1\', \'info\', \'0\');', 'INSERT INTO ' . tablename('rr_v_task_type') . ' (`id`, `type_key`, `type_name`, `description`, `verb`, `numeric`, `unit`, `goods`, `theme`, `once`) VALUES (\'12\', \'recharge_count\', \'累计充值满额\', \'在商城中充值余额，累计充值满额，即可获得奖励。\', \'累计充值满\', \'1\', \'元\', \'0\', \'success\', \'0\');');

		foreach ($type as $t ) {
			pdo_query($t);
		}

		pdo_delete('rr_v_task_poster', array('uniacid' => $_W['uniacid']));
		pdo_delete('rr_v_task_join', array('uniacid' => $_W['uniacid']));
		pdo_delete('rr_v_task_joiner', array('uniacid' => $_W['uniacid']));
		pdo_delete('rr_v_task_extension_join', array('uniacid' => $_W['uniacid']));
