...
class index_RrvV3Page extends PluginMobilePage
{
	...
	public function main()
	{
		...
		$bgimg = pdo_get('rr_v_task_default', array('uniacid' => $_W['uniacid']), array('bgimg'));
	...
	public function detail()
	{
		...
		if ($poster) {
			$sql = 'SELECT * FROM ' . tablename('rr_v_task_poster') . ' WHERE id = :id';
			$detail = pdo_fetch($sql, array(':id' => $id));
			$reward_data = unserialize($detail['reward_data']);
		}
		 else {
			$sql = 'SELECT * FROM ' . tablename('rr_v_task') . ' WHERE id = :id';
			$detail = pdo_fetch($sql, array(':id' => $id));
			$reward_data = unserialize($detail['reward_data']);
	...
	public function taskposter()
	{
		...
		$now_time = time();
		$task_sql = 'SELECT * FROM ' . tablename('rr_v_task_poster') . ' WHERE timestart<=' . $now_time . ' AND timeend>' . $now_time . ' AND uniacid=' . $_W['uniacid'] . ' AND `status`=1 AND `is_delete`=0 ORDER BY `createtime` DESC';
		...
		$running_sql = 'SELECT `join`.*,`task`.title,`task`.reward_data AS `poster_reward`,`task`.titleicon,`task`.poster_type FROM ' . tablename('rr_v_task_join') . ' AS `join` LEFT JOIN ' . tablename('rr_v_task_poster') . ' AS `task` ON `join`.task_id=`task`.`id` WHERE `join`.`failtime`>' . $now_time . ' AND `join`.`join_user`="' . $openid . '" AND `join`.uniacid=' . $_W['uniacid'] . ' AND `join`.`is_reward` = 0 AND `task`.`is_delete` = 0 ORDER BY `join`.`addtime` DESC LIMIT 0,15';
		...
		$complete_sql = 'SELECT `join`.*,`task`.title,`task`.titleicon,`task`.poster_type FROM ' . tablename('rr_v_task_join') . ' AS `join` LEFT JOIN ' . tablename('rr_v_task_poster') . ' AS `task` ON `join`.task_id=`task`.`id` WHERE `join`.uniacid=' . $_W['uniacid'] . ' AND `join`.`join_user`="' . $openid . '" AND `join`.`is_reward`=1 AND `task`.`is_delete` = 0 ORDER BY `join`.`addtime` DESC LIMIT 0,15';
		...
		$faile_sql = 'SELECT `join`.*,`task`.title,`task`.reward_data AS `poster_reward`,`task`.titleicon,`task`.poster_type FROM ' . tablename('rr_v_task_join') . ' AS `join` LEFT JOIN ' . tablename('rr_v_task_poster') . ' AS `task` ON `join`.task_id=`task`.`id` WHERE `join`.`failtime`<=' . $now_time . ' AND `join`.`join_user`="' . $openid . '" AND `join`.uniacid=' . $_W['uniacid'] . ' AND `join`.`is_reward`=0 AND `task`.`is_delete` = 0 ORDER BY `join`.`addtime` DESC LIMIT 0,15';
	...
	public function getreward()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$rewarded = pdo_get('rr_v_task_extension_join', array('uniacid' => $_W['uniacid'], 'id' => $_GPC['id']));
	...
	private function getpostericon($id)
	{
		global $_W;
		global $_GPC;
		return pdo_fetchcolumn('SELECT titleicon FROM ' . tablename('rr_v_task_poster') . ' WHERE id = :id AND uniacid = :uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	}

	private function checkJoined($taskid)
	{
		global $_W;
		$sql = 'SELECT COUNT(*) FROM ' . tablename('rr_v_task_extension_join') . ' WHERE openid = :openid AND taskid = :taskid';
	...
	{
		global $_W;
		$sql = 'SELECT `data` FROM ' . tablename('rr_v_task_default') . ' WHERE uniacid = :uniacid';
	...
	public function main_new()
	{
		...
		$tableList = tablename('rr_v_task_list');
		$tableRecord = tablename('rr_v_task_record');
		...
		$set = pdo_fetchcolumn('select bg_img from ' . tablename('rr_v_task_set') . ' where uniacid = ' . $_W['uniacid']);
	...
	public function rewardlist()
	{
		...
		$psize = 100;
		$pstart = ($page - 1) * $psize;
		$sql = 'select * from ' . tablename('rr_v_task_reward') . ' where openid = :openid and `get` = 1 and uniacid = :uniacid order by gettime desc limit ' . $pstart . ',' . $psize;
	...
	public function mine()
	{
		...
		$sql = 'select * from ' . tablename('rr_v_task_record') . ' where openid = :openid and uniacid = :uniacid ' . $condition . ' order by id desc';
		$list = pdo_fetchall($sql, array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
		include $this->template();
	}
	...
	public function detail_new()
	{
		...
		if (!(empty($rid))) {
			$sql = 'select * from ' . tablename('rr_v_task_record') . ' where id = :id and uniacid = :uniacid';
			$detail = pdo_fetch($sql, array(':id' => $rid, ':uniacid' => $_W['uniacid']));
			$reward = json_decode($detail['reward_data'], true);
			$reward_goods = pdo_fetchall('select * from ' . tablename('rr_v_task_reward') . ' where recordid = ' . $detail['id'] . ' and openid = \'' . $_W['openid'] . '\' and uniacid = ' . $_W['uniacid'] . ' and isjoiner = 0 and reward_type = \'goods\' and `level` = 0');
			...
			if ($detail['tasktype'] == 'poster') {
				if ($detail['level2'] == 0) {
					...
					$reward_goods1 = pdo_fetchall('select * from ' . tablename('rr_v_task_reward') . ' where recordid = ' . $detail['id'] . ' and openid = \'' . $_W['openid'] . '\' and uniacid = ' . $_W['uniacid'] . ' and isjoiner = 0 and reward_type = \'goods\' and `level` = 1');
					$reward2 = json_decode($detail['reward_data2'], true);
					$reward_goods2 = pdo_fetchall('select * from ' . tablename('rr_v_task_reward') . ' where recordid = ' . $detail['id'] . ' and openid = \'' . $_W['openid'] . '\' and uniacid = ' . $_W['uniacid'] . ' and isjoiner = 0 and reward_type = \'goods\' and `level` = 2');
					$reward3 = $reward;
			...
			$joiner = pdo_fetchall('select DISTINCT openid,headimg,nickname,gettime from ' . tablename('rr_v_task_reward') . ' where isjoiner = 1 and recordid = ' . $rid . ' and `get`=1 and uniacid = ' . $_W['uniacid']);
		}


		if (empty($detail) && !(empty($id))) {
			$sql = 'select * from ' . tablename('rr_v_task_list') . ' where id = :id and uniacid = :uniacid';
	...
	public function sendtowechat()
	{
		...
		$mediaid = pdo_fetchcolumn('select mediaid from ' . tablename('rr_v_task_qr') . ' where recordid = :recordid', array(':recordid' => $recordid));
	...
	public function getred()
	{
		...
		$money = pdo_fetchcolumn('select reward_data from ' . tablename('rr_v_task_reward') . ' where id = ' . $rewardid . ' and `get` = 1 and sent = 0');
