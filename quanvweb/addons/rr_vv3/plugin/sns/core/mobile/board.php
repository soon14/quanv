...
class Board_RrvV3Page extends SnsMobilePage
{
	...
	public function getlist()
	{
		...
		foreach ($list as &$row) {
			...
			$row['postcount'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_sns_post') . ' where pid=:pid and deleted = 0 limit 1', array(':pid' => $row['id']));
	...
	public function relate()
	{
		...
		$sql = 'select id,title,logo,`desc`  from ' . tablename('rr_v_sns_board') . ' where 1 ' . $condition . ' order by displayorder desc';
		$list = pdo_fetchall($sql, $params);
	...
	public function get_boardlist()
	{
		...
		$list = pdo_fetchall('SELECT sb.id,sb.title,sb.logo FROM ' . tablename('rr_v_sns_board') . " as sb\r\n                left join " . tablename('rr_v_sns_category') . " as sc on sc.id = sb.cid\r\n                WHERE 1 " . $condition . ' and sc.enabled = 1  ORDER BY sb.displayorder DESC limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		...
		unset($row);
		$total = pdo_fetchcolumn('SELECT count(1) FROM ' . tablename('rr_v_sns_board') . ' as sb WHERE 1 ' . $condition, $params);
		show_json(1, array('list' => $list, 'pagesize' => $psize, 'total' => $total));
	}
	...
	public function follow()
	{
		...
		$follow = pdo_fetch('select id from ' . tablename('rr_v_sns_board_follow') . ' where bid=:bid and openid=:openid limit 1', array(':bid' => $bid, ':openid' => $_W['openid']));

		if (!empty($follow)) {
			pdo_delete('rr_v_sns_board_follow', array('id' => $follow['id']));
			show_json(1, array('isfollow' => false));
		}
		else {
			$follow = array('uniacid' => $_W['uniacid'], 'bid' => $bid, 'openid' => $_W['openid'], 'createtime' => time());
			pdo_insert('rr_v_sns_board_follow', $follow);
