...
require rr_vv3_PLUGIN . 'sns/core/page_mobile.php';
class Post_RrvV3Page extends SnsMobilePage
{
	public function main()
	{
		...
		pdo_update('rr_v_sns_post', array('views' => $post['views'] + 1), array('id' => $post['id']));
		$goodcount = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_sns_like') . ' where pid=:pid limit 1', array(':pid' => $post['id']));
		$replycount = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_sns_post') . ' where pid=:pid and deleted=0 and checked=1 limit 1', array(':pid' => $post['id']));
		$isgood = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_sns_like') . ' where uniacid=:uniacid and pid=:pid and openid=:openid  limit 1', array(':uniacid' => $_W['uniacid'], ':pid' => $post['id'], ':openid' => $_W['openid']));
		$set = $this->getSet();
		...
		if (!empty($member['sns_level'])) {
			$level = pdo_fetch('select * from ' . tablename('rr_v_sns_level') . ' where id=:id  limit 1', array(':id' => $member['sns_level']));
		}

		$catelist = pdo_fetchall('SELECT id,name FROM ' . tablename('rr_v_sns_complaincate') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY displayorder asc');
	...
	public function checkPost()
	{
		...
		$post = pdo_fetch('select pid,nickname from ' . tablename('rr_v_sns_post') . ' where id = ' . $postid . ' ');
	...
	public function complain()
	{
		...
		$id = intval($_GPC['id']);
		$openid = $_W['openid'];
		$posts = pdo_fetch('SELECT id,pid,openid FROM ' . tablename('rr_v_sns_post') . ' WHERE uniacid = ' . $uniacid . ' AND id = ' . $id . ' AND deleted = 0 ');
		...
		pdo_insert('rr_v_sns_complain', $data);
	...
	public function reply()
	{
		...
		pdo_insert('rr_v_sns_post', $data);
		pdo_update('rr_v_sns_post', array('replytime' => $time), array('id' => $pid, 'uniacid' => $_W['uniacid']));
	...
	public function getlist()
	{
		...
		$sql = 'select id,bid,rpid,title,createtime,content,images ,openid, nickname,avatar,checked from ' . tablename('rr_v_sns_post') . '  where 1 ' . $condition . ' ORDER BY createtime desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_sns_post') . ' where 1 ' . $condition, $params);

		foreach ($list as $key => &$row) {
			$row['avatar'] = tomedia($row['avatar']);
			$row['avatar'] = $this->model->getAvatar($row['avatar']);
			$row['createtime'] = date('Y-m-d H:i', $row['createtime']);
			$row['goodcount'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_sns_like') . ' where pid=:pid limit 1', array(':pid' => $row['id']));
			$row['postcount'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_sns_post') . ' where pid=:pid limit 1', array(':pid' => $row['id']));
			$images = array();
			...
			$isgood = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_sns_like') . ' where uniacid=:uniacid and pid=:pid and openid=:openid limit 1', array(':uniacid' => $_W['uniacid'], ':pid' => $row['id'], ':openid' => $_W['openid']));
			$row['isgood'] = $isgood;
			$row['goodcount'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_sns_like') . ' where uniacid=:uniacid and pid=:pid  limit 1', array(':uniacid' => $_W['uniacid'], ':pid' => $row['id']));
	...
	public function like()
	{
		...
		$isgood = 1;
		$like = pdo_fetch('select id from ' . tablename('rr_v_sns_like') . ' where pid=:pid and openid=:openid limit 1', array(':pid' => $pid, ':openid' => $_W['openid']));

		if (!empty($like)) {
			$isgood = 0;
			pdo_delete('rr_v_sns_like', array('id' => $like['id']));
		}
		else {
			$like = array('uniacid' => $_W['uniacid'], 'pid' => $pid, 'openid' => $_W['openid']);
			pdo_insert('rr_v_sns_like', $like);
		}

		$time = time();
		pdo_update('rr_v_sns_post', array('replytime' => $time), array('id' => $pid, 'uniacid' => $_W['uniacid']));
		$goodcount = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_sns_like') . ' where pid=:pid limit 1', array(':pid' => $pid));
	...
	public function delete()
	{
		...
		pdo_update('rr_v_sns_post', array('deleted' => 1, 'deletedtime' => time()), array('id' => $pid));
	...
	public function check()
	{
		...
		if (!$post['checked']) {
			pdo_update('rr_v_sns_post', array('checked' => 1, 'checktime' => time()), array('id' => $pid));
	...
	public function best()
	{
		...
		if ($post['isboardbest']) {
			$isbest = 0;
			pdo_update('rr_v_sns_post', array('isboardbest' => 0), array('id' => $pid));
			$this->model->setCredit($post['openid'], $bid, SNS_CREDIT_BEST_BOARD_CANCEL);
		}
		else {
			pdo_update('rr_v_sns_post', array('isboardbest' => 1), array('id' => $pid));
	...
	public function top()
	{
		...
		if ($post['isboardtop']) {
			$istop = 0;
			pdo_update('rr_v_sns_post', array('isboardtop' => 0), array('id' => $pid));
			$this->model->setCredit($post['openid'], $bid, SNS_CREDIT_TOP_BOARD_CANCEL);
		}
		else {
			pdo_update('rr_v_sns_post', array('isboardtop' => 1), array('id' => $pid));
	...
	public function allbest()
	{
		...
		if ($post['isbest']) {
			$isbest = 0;
			pdo_update('rr_v_sns_post', array('isbest' => 0), array('id' => $pid));
			$this->model->setCredit($post['openid'], $bid, SNS_CREDIT_BEST_CANCEL);
		}
		else {
			pdo_update('rr_v_sns_post', array('isbest' => 1), array('id' => $pid));
	...
	public function alltop()
	{
		...
		if ($post['istop']) {
			$istop = 0;
			pdo_update('rr_v_sns_post', array('istop' => 0), array('id' => $pid));
			$this->model->setCredit($post['openid'], $bid, SNS_CREDIT_TOP_CANCEL);
		}
		else {
			pdo_update('rr_v_sns_post', array('istop' => 1), array('id' => $pid));
