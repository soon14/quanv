...
class Board_RrvV3Page extends PluginWebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_sns_board') . ' WHERE 1 ' . $condition . '  ORDER BY displayorder DESC limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(1) FROM ' . tablename('rr_v_sns_board') . ' WHERE 1 ' . $condition, $params);
	...
	protected function post()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('select * from ' . tablename('rr_v_sns_board') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				if (!empty($data['keyword'])) {
					$keyword = pdo_fetchcolumn('SELECT keyword FROM ' . tablename('rr_v_sns_board') . ' WHERE id=:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
				...
				pdo_update('rr_v_sns_board', $data, array('id' => $id));
				...
				pdo_insert('rr_v_sns_board', $data);
				$id = pdo_insertid();
				plog('sns.board.add', '添加版块 ID: ' . $id);
			}

			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'rr_vv3', ':name' => 'rr_vv3:sns:board:' . $id));

			if (empty($rule)) {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'rr_vv3:sns:board:' . $id, 'module' => 'rr_vv3', 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'rr_vv3', 'content' => trim($data['keyword']), 'type' => 1, 'displayorder' => 0, 'status' => 1);
		...
		if (!empty($item)) {
			$managers = pdo_fetchall('select m.id,m.openid,m.avatar,m.nickname from ' . tablename('rr_v_sns_manage') . ' sm ' . ' left join ' . tablename('rr_v_member') . ' m on sm.openid = m.openid and sm.uniacid = m.uniacid' . ' where sm.uniacid=:uniacid and sm.bid=:bid ', array(':uniacid' => $_W['uniacid'], ':bid' => $item['id']));
		}
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,title,keyword FROM ' . tablename('rr_v_sns_board') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_delete('rr_v_sns_board', array('id' => $item['id']));
			pdo_delete('rr_v_sns_board_follow', array('bid' => $item['id']));
			pdo_delete('rr_v_sns_post', array('bid' => $item['id']));
	...
	public function displayorder()
	{
		...
		$item = pdo_fetchall('SELECT id,title FROM ' . tablename('rr_v_sns_board') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		if (!empty($item)) {
			pdo_update('rr_v_sns_board', array('displayorder' => $displayorder), array('id' => $id));
	...
	public function status()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('rr_v_sns_board') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_update('rr_v_sns_board', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
