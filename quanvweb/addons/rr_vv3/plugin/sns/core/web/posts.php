...
class Posts_RrvV3Page extends PluginWebPage
{
	public function main()
	{
		...
		$sql = "select p.id,p.title,p.createtime,p.content,p.images ,p.replytime, p.openid, p.nickname,p.avatar,p.isbest,p.isboardbest,p.istop,p.isboardtop,\r\n        p.checked,p.deleted,p.isadmin,b.title as boardtitle,b.logo as boardlogo " . '  from ' . tablename('rr_v_sns_post') . ' p ' . ' left join ' . tablename('rr_v_sns_board') . ' b on p.bid = b.id  and b.uniacid = p.uniacid' . ' left join ' . tablename('rr_v_member') . ' m on m.openid = p.openid and m.uniacid = p.uniacid ' . '  where 1 ' . $condition . ' ORDER BY p.replytime DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_sns_post') . ' p ' . ' left join ' . tablename('rr_v_sns_board') . ' b on p.bid = b.id  and b.uniacid = p.uniacid' . ' left join ' . tablename('rr_v_member') . ' m on m.openid = p.openid and m.uniacid = p.uniacid ' . ' where 1 ' . $condition, $params);

		foreach ($list as &$row) {
			$row['replycount'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_sns_post') . ' where pid=:pid and deleted = 0 limit 1', array(':pid' => $row['id']));
			$images = array();
			...
			$row['thumb'] = $thumb;
			$row['needchecks'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_sns_post') . ' where pid=:pid and checked=0 limit 1', array(':pid' => $row['id']));
		}
	...
	public function post()
	{
		...
		$id = intval($_GPC['id']);
		$uniacid = $_W['uniacid'];
		$item = pdo_fetch("select id,bid,title,content,images,istop,isbest,isboardtop,isboardbest,openid\r\n                from " . tablename('rr_v_sns_post') . ' where id = ' . $id . ' and uniacid = ' . $uniacid . ' ');

		if (!empty($item['images'])) {
			$piclist = array_merge(iunserializer($item['images']));
		}

		$board = pdo_fetchall('select id,title from ' . tablename('rr_v_sns_board') . ' where uniacid = ' . $uniacid . ' order by id desc ');
		$set = $this->set;
		...
		if (isset($set['managers'])) {
			if (!empty($set['managers'])) {
				...
				$managers = pdo_fetchall('select id,nickname,openid from ' . tablename('rr_v_member') . "\r\n                            where openid in (" . implode(',', $openids) . ') and uniacid = ' . $uniacid . ' ');
			}
		}
		...
		if ($_W['ispost']) {
			...
			if (!empty($data['openid'])) {
				$user = pdo_fetch('select avatar,nickname from ' . tablename('rr_v_member') . ' where openid = \'' . $data['openid'] . '\' and uniacid = ' . $uniacid . ' ');
			...
			if (!empty($id)) {
				pdo_update('rr_v_sns_post', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function check()
	{
		...
		$items = pdo_fetchall('SELECT id,title,content,openid, pid,bid FROM ' . tablename('rr_v_sns_post') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
	...
	public function top()
	{
		...
		$items = pdo_fetchall('SELECT id,title,content,openid, pid FROM ' . tablename('rr_v_sns_post') . ' WHERE id in( ' . $id . ' ) and pid=0 AND uniacid=' . $_W['uniacid']);
		...
		foreach ($items as $item) {
			...
			if ($top) {
				...
				pdo_update('rr_v_sns_post', array($field => 1), array('id' => $item['id']));
				...
				pdo_update('rr_v_sns_post', array($field => 0), array('id' => $item['id']));
