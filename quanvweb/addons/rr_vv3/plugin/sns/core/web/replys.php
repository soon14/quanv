...
class Replys_RrvV3Page extends PluginWebPage
{
	public function main()
	{
		...
		if (!empty($pid)) {
			...
			$member['postcount'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_sns_post') . ' where uniacid=:uniacid and openid=:openid and pid=0 and deleted=0 limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
			$member['replycount'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_sns_post') . ' where uniacid=:uniacid and openid=:openid and pid>0 and deleted=0  limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
		...
		$sql = 'select id,bid,pid, rpid,title,createtime,content,images ,openid, nickname,avatar,checked,deleted from ' . tablename('rr_v_sns_post') . '  where 1 ' . $condition . ' ORDER BY createtime asc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		...
		foreach ($list as $key => &$row) {
			...
			if (!empty($rmember['sns_level'])) {
				$rlevel = pdo_fetch('select * from ' . tablename('rr_v_sns_level') . ' where id=:id  limit 1', array(':id' => $rmember['sns_level']));
			}

			$row['member'] = array('id' => $rmember['id'], 'sns_credit' => $rmember['sns_credit'], 'postcount' => pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_sns_post') . ' where uniacid=:uniacid and openid=:openid and pid=0 and deleted=0 limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $rmember['openid'])), 'replycount' => pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_sns_post') . ' where uniacid=:uniacid and openid=:openid and pid>0 and deleted=0 limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $rmember['openid'])));
			$row['level'] = $rlevel;
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,title,pid,openid, content FROM ' . tablename('rr_v_sns_post') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		...
		foreach ($items as $item) {
			...
			if ($deleted) {
				...
				pdo_update('rr_v_sns_post', array('deleted' => 1, 'deletedtime' => time()), array('id' => $item['id']));
				...
				pdo_update('rr_v_sns_post', array('deleted' => 0, 'deletedtime' => 0), array('id' => $item['id']));
	...
	public function delete1()
	{
		...
		$items = pdo_fetchall('SELECT id,title,pid,openid, content,deleted FROM ' . tablename('rr_v_sns_post') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item) {
			$msg = (empty($item['pid']) ? '话题' : '评论');
			$content = $this->model->replaceContent($item['content']);
			plog('sns.posts.delete', '彻底删除' . $msg . ' ID: ' . $item['id'] . ' 标题: ' . $item['title'] . ' 内容: ' . $content);
			pdo_delete('rr_v_sns_post', array('id' => $item['id']));
	...
	public function check()
	{
		...
		$items = pdo_fetchall('SELECT id,title,content,openid, pid FROM ' . tablename('rr_v_sns_post') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		...
		foreach ($items as $item) {
			...
			if ($checked) {
				...
				pdo_update('rr_v_sns_post', array('checked' => 1), array('id' => $item['id']));
				...
				pdo_update('rr_v_sns_post', array('checked' => 0), array('id' => $item['id']));
