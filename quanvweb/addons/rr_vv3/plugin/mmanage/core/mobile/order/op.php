...
class Op_RrvV3Page extends MmanageMobilePage
{
	...
	public function remarksaler()
	{
		...
		if ($_W['ispost']) {
			...
			pdo_update('rr_v_order', array('remarksaler' => $remarksaler), array('id' => $orderid, 'uniacid' => $_W['uniacid']));
	...
	public function changeaddress()
	{
		...
		if ($_W['ispost']) {
			...
			pdo_update('rr_v_order', array('address' => $address_array), array('id' => $orderid, 'uniacid' => $_W['uniacid']));
	...
	public function send()
	{
			...
			if (!empty($sendtype)) {
				...
				if ($goodsids) {
					foreach ($goodsids as $key => $value) {
						pdo_update('rr_v_order_goods', $data, array('orderid' => $item['id'], 'goodsid' => $value, 'uniacid' => $_W['uniacid']));
					}
				}
		...
		if (0 < $item['sendtype']) {
			$noshipped = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.sendtype = 0 and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
			$i = 1;

			while ($i <= $item['sendtype']) {
				$shipped[$i]['sendtype'] = $i;
				$shipped[$i]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.sendtype = ' . $i . ' and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
				++$i;
			}
		}

		$order_goods = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
	...
	public function sendcancel()
	{
			...
			if (0 < $item['sendtype']) {
				...
				if (is_array($sendtype)) {
					foreach ($sendtype as $key => $value) {
						$data['sendtype'] = 0;
						pdo_update('rr_v_order_goods', $data, array('orderid' => $item['id'], 'sendtype' => $value, 'uniacid' => $_W['uniacid']));
		...
		if (0 < $sendtype) {
			$sendgoods = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype=' . $sendtype . ' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
		}
		else {
			if (0 < $item['sendtype']) {
				$i = 1;
				...
				while ($i <= intval($item['sendtype'])) {
					$bundles[$i]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype=' . $i . ' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
	...
	public function changeexpress()
	{
		...
		if ($_W['ispost']) {
			...
			if (0 < $item['sendtype']) {
				...
				if (is_array($sendtype)) {
					foreach ($sendtype as $key => $value) {
						pdo_update('rr_v_order_goods', $change_data, array('orderid' => $orderid, 'sendtype' => $value, 'uniacid' => $_W['uniacid']));
					}
				}
			}
			else {
				pdo_update('rr_v_order', $change_data, array('id' => $orderid, 'uniacid' => $_W['uniacid']));
			}
