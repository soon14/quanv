...
class Index_RrvV3Page extends MmanageMobilePage
{
	...
	public function detail()
	{
		...
		if ($_W['ispost']) {
			pdo_update('rr_v_order', array('remark' => trim($_GPC['remark'])), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function getlist()
	{
		...
		if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
			...
			if ($searchfield == 'ordersn') {
				...
				$sqlcondition = ' inner join ( select  og.orderid from ' . tablename('rr_v_order_goods') . ' og left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and (locate(:keyword,g.title)>0)) gs on gs.orderid=o.id';
			}
			else if ($searchfield == 'goodssn') {
				$sqlcondition = ' inner join ( select og.orderid from ' . tablename('rr_v_order_goods') . ' og left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and (((locate(:keyword,g.goodssn)>0)) or (locate(:keyword,og.goodssn)>0))) gs on gs.orderid=o.id';
			}
			else if ($searchfield == 'goodsoptiontitle') {
				$sqlcondition = ' inner join ( select  og.orderid from ' . tablename('rr_v_order_goods') . ' og left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and (locate(:keyword,og.optionname)>0)) gs on gs.orderid=o.id';
			}
			else {
				if ($searchfield == 'merch') {
					if ($merch_plugin) {
						$condition .= ' AND (locate(:keyword,merch.merchname)>0)';
						$sqlcondition = ' left join ' . tablename('rr_v_merch_user') . ' merch on merch.id = o.merchid and merch.uniacid=o.uniacid';
					}
				}
			}
		}
		...
		if (($condition != ' o.uniacid = :uniacid and and o.deleted=0 and o.isparent=0') || !empty($sqlcondition)) {
			...
			$sql = 'select * from ' . tablename('rr_v_order') . ' where uniacid = :uniacid and deleted=0 and isparent=0 ' . $status_condition . ' GROUP BY id ORDER BY createtime DESC  ';
		...
		unset($value);
		if (($condition != ' o.uniacid = :uniacid and and o.deleted=0 and o.isparent=0') || !empty($sqlcondition)) {
			$t = pdo_fetch('SELECT COUNT(*) as count, ifnull(sum(o.price),0) as sumprice   FROM ' . tablename('rr_v_order') . ' o ' . ' left join ' . tablename('rr_v_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('rr_v_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('rr_v_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('rr_v_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' left join ' . tablename('rr_v_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . ' ' . $sqlcondition . ' WHERE ' . $condition . ' ' . $statuscondition, $paras);
		}
		else {
			$t = pdo_fetch('SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('rr_v_order') . ' WHERE uniacid = :uniacid and deleted=0 and isparent=0 ' . $status_condition, $paras);
		}
