...
class Index_RrvV3Page extends MmanageMobilePage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($item)) {
				pdo_update('rr_v_goods', $data, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function getlist()
	{
		...
		$sql = 'SELECT count(g.id) FROM ' . tablename('rr_v_goods') . 'g' . $condition;
		$total = pdo_fetchcolumn($sql, $params);

		if (0 < $total) {
			$presize = (($pindex - 1) * $psize) - $offset;
			$sql = 'SELECT g.* FROM ' . tablename('rr_v_goods') . 'g' . $condition . " ORDER BY g.`status` DESC, g.`displayorder` DESC,\r\n                g.`id` DESC LIMIT " . $presize . ',' . $psize;
	...
	public function restore()
	{
		...
		foreach ($items as $item) {
			pdo_update('rr_v_goods', array('deleted' => 0, 'status' => 0), array('id' => $item['id']));
