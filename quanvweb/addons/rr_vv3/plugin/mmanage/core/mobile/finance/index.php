...
class Index_RrvV3Page extends MmanageMobilePage
{
	...
	protected function log($type)
	{
		...
		$sql = 'select log.id,m.id as mid, m.realname,m.avatar,m.weixin,log.logno,log.type,log.status,log.rechargetype,log.sendmoney,m.nickname,m.mobile,g.groupname,log.money,log.createtime,l.levelname,log.realmoney,log.deductionmoney,log.charge,log.remark,log.alipay,log.bankname,log.bankcard,log.realname as applyrealname,log.applytype,log.apppay from ' . tablename('rr_v_member_log') . ' log ' . ' left join ' . tablename('rr_v_member') . ' m on m.openid=log.openid' . ' left join ' . tablename('rr_v_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('rr_v_member_level') . ' l on m.level =l.id' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC ';
		...
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_member_log') . ' log ' . ' left join ' . tablename('rr_v_member') . ' m on m.openid=log.openid and m.uniacid= log.uniacid' . ' left join ' . tablename('rr_v_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('rr_v_member_level') . ' l on m.level =l.id' . ' where 1 ' . $condition . ' ', $params);
	...
	protected function credit($type)
	{
		...
		$params = array(':uniacid' => $_W['uniacid'], ':module' => 'rr_vv3', ':credittype' => $type);
		...
		$sql = 'select log.*,m.id as mid, m.realname,m.avatar,m.nickname,m.avatar, m.mobile, m.weixin,u.username from ' . tablename('mc_credits_record') . ' log ' . ' left join ' . tablename('users') . ' u on log.operator<>0 and log.operator<>log.uid and  log.operator=u.uid' . ' left join ' . tablename('rr_v_member') . ' m on m.uid=log.uid' . ' left join ' . tablename('rr_v_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('rr_v_member_level') . ' l on m.level =l.id' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC ';
		...
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('mc_credits_record') . ' log ' . ' left join ' . tablename('users') . ' u on log.operator<>0 and log.operator<>log.uid and  log.operator=u.uid' . ' left join ' . tablename('rr_v_member') . ' m on m.uid=log.uid' . ' left join ' . tablename('rr_v_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('rr_v_member_level') . ' l on m.level =l.id' . ' where 1 ' . $condition . ' ', $params);
		show_json(1, array('list' => $list, 'total' => $total, 'pagesize' => $psize));
	}
	...
	public function refund($tid = 0, $fee = 0, $reason = '')
	{
		...
		$set = $_W['shopset']['shop'];
		$id = intval($_GPC['id']);
		$log = pdo_fetch('select * from ' . tablename('rr_v_member_log') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function wechat()
	{
		...
		if (!empty($data['paytype']['withdraw'])) {
			...
			pdo_update('rr_v_member_log', array('sendmoney' => $result['sendmoney'], 'senddata' => json_encode($result['senddata'])), array('id' => $log['id']));
