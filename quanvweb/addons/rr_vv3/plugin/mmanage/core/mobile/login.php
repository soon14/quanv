...
class Login_RrvV3Page extends MmanageMobilePage
{
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			if ($type == 'wechat') {
				...
				$roleuser = pdo_fetch('SELECT id, uid, username, status FROM' . tablename('rr_v_perm_user') . 'WHERE openid=:openid AND uniacid=:uniacid LIMIT 1', array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
