...
class Index_RrvV3Page extends MmanageMobilePage
{
	public function main()
	{
		global $_W;
		global $_GPC;
		$roleuser = pdo_fetch('SELECT id, uid, username, status, openid, realname, mobile FROM' . tablename('rr_v_perm_user') . 'WHERE uid=:uid AND uniacid=:uniacid', array(':uid' => $_W['uid'], ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			pdo_update('rr_v_perm_user', $data, array('id' => $roleuser['id'], 'uniacid' => $_W['uniacid']));
