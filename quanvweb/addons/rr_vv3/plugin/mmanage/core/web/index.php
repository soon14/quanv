	...
	public function setting()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($arr['keyword'])) {
				...
				$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'rr_vv3', ':name' => 'rr_vv3:mmanage'));

				if (!empty($rule)) {
					pdo_update('rule_keyword', array('content' => $arr['keyword']), array('rid' => $rule['id']));
				}
				else {
					$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'rr_vv3:mmanage', 'module' => 'rr_vv3', 'displayorder' => 0, 'status' => 1);
