...
class UniversalformModel extends PluginModel {
	...
	public function globalData() {
		...
		return array (
				...
				'category' => pdo_fetchall ( 'select * from ' . tablename ( 'rr_v_universalform_category' ) . ' where uniacid=:uniacid order by id desc', array (
						':uniacid' => $_W ['uniacid'] 
				), 'id' ) 
		);
	}
	...
	public function getInsertData($fields, $memberdata) {
		...
		foreach ( $fields as $key => $value ) {
			...
			if ($data_type == 0) {
				...
				$data [$key] = array (
						trim ( $memberdata [$key . '_0'] ),
						trim ( $memberdata [$key . '_1'] ) 
				);
			} else if ($data_type == 9) {
				$data [$key] = array (
	...
	public function getDiyformData($universalform_data, $fields, $member, $flag = 0, $last_flag = 0) {
		global $_W;
		if (! empty ( $universalform_data )) {
			if ($flag) {
				$f_data = iunserializer ( $universalform_data );
			} else {
				$f_data = $universalform_data;
			}
			if ($last_flag) {
				foreach ( $fields as $key => $value ) {
	...
	public function getFormatData($id, $fields, $member) {
		global $_W;
		if (! empty ( $id )) {
			$universalform_data = $this->getOneDiyformData ( $id );
		}
		$f_data = $this->getDiyformData ( $universalform_data, $fields, $member );
		return $f_data;
	}
	public function getLastData($type, $diymode, $universalformid, $cid, $fields, $member) {
		global $_W;
		if (! empty ( $cid )) {
			$table_name = 'rr_v_universalform_temp';
			$sql = 'select * from ' . tablename ( $table_name ) . ' where cid=:cid and universalformid=:universalformid and uniacid=:uniacid and openid=:openid and type=:type order by id desc Limit 1';
			$params = array (
					':cid' => $cid,
					':universalformid' => $universalformid,
					':uniacid' => $_W ['uniacid'],
					':openid' => $member ['openid'],
					':type' => $type 
			);
			$universalform_data = pdo_fetch ( $sql, $params );
			$data = $universalform_data ['universalformdata'];
			if (empty ( $data )) {
				$table_name = 'rr_v_order_goods';
				$sql = 'select * from ' . tablename ( $table_name ) . ' where id=:cid and universalformid=:universalformid and uniacid=:uniacid and openid=:openid order by id desc Limit 1';
				$params = array (
						':cid' => $cid,
						':universalformid' => $universalformid,
						':uniacid' => $_W ['uniacid'],
	...
	public function getLastCartData($goodsid = 0) {
		global $_W;
		$f_data = array ();
		$cartdata = pdo_fetch ( 'select * from ' . tablename ( 'rr_v_member_cart' ) . ' where goodsid=:goodsid and openid=:openid order by id desc limit 1', array (
				':openid' => $_W ['openid'],
				':goodsid' => $goodsid 
		) );
		if (! empty ( $cartdata )) {
			$member = m ( 'member' )->getMember ( $_W ['openid'] );
			$f_data = $this->getDiyformData ( $cartdata ['universalformdata'], iunserializer ( $cartdata ['universalformfields'] ), $member, 1, 1 );
		}
		return $f_data;
	}
	public function getLastOrderData($universalformid, $member) {
		global $_W;
		$f_data = array ();
		if (! empty ( $universalformid )) {
			$order = pdo_fetch ( 'select universalformdata,universalformfields from ' . tablename ( 'rr_v_order' ) . ' where universalformid=:universalformid and openid=:openid order by id desc limit 1', array (
					':universalformid' => $universalformid,
					':openid' => $member ['openid'] 
			) );
			if (! empty ( $order )) {
				$f_data = $this->getDiyformData ( $order ['universalformdata'], iunserializer ( $order ['universalformfields'] ), $member, 1, 1 );
			}
		}
		return $f_data;
	}
	public function addDataNum($id) {
		global $_W;
		pdo_update ( 'rr_v_universalform_type', 'alldata=alldata+1', array (
				'id' => $id 
		) );
	}
	public function getCountData($typeid, $type = 0) {
		global $_W;
		$sql = 'select count(*) from ' . tablename ( 'rr_v_universalform_data' ) . ' where typeid=:typeid and uniacid=:uniacid';
		$params = array (
				':typeid' => $typeid,
				':uniacid' => $_W ['uniacid'] 
		);
	...
	public function getGoodsTemp($goodsid, $universalformid, $openid, $type = 3) {
		global $_W;
		$sql = 'select * from ' . tablename ( 'rr_v_universalform_temp' ) . ' where cid=:cid and universalformid=:universalformid and openid=:openid and type=:type and uniacid=:uniacid Limit 1';
		$params = array (
				':cid' => $goodsid,
				':universalformid' => $universalformid,
				':openid' => $openid,
	...
	public function getCountGoodsUsed($universalformid, $universalformtype = -1) {
		global $_W;
		$sql = 'select count(id) from ' . tablename ( 'rr_v_goods' ) . ' where universalformid=:universalformid and uniacid=:uniacid';
		$params = array (
				':universalformid' => $universalformid,
				':uniacid' => $_W ['uniacid'] 
		);
		if ($universalformtype == - 1) {
			$sql .= ' and universalformtype!=0';
		} else if (0 <= $universalformtype) {
			$sql .= ' and universalformtype=:' . $universalformtype;
			$params [':universalformtype'] = $universalformtype;
		}
		$datacount = pdo_fetchcolumn ( $sql, $params );
		return $datacount;
	}
	public function getDiyformList() {
		global $_W;
		$form_list = pdo_fetchall ( 'select * from ' . tablename ( 'rr_v_universalform_type' ) . ' where uniacid=:uniacid', array (
				':uniacid' => $_W ['uniacid'] 
		) );
		return $form_list;
	}
	public function getDiyformInfo($id, $flag = 1) {
		global $_W;
		$formInfo = pdo_fetch ( 'select * from ' . tablename ( 'rr_v_universalform_type' ) . ' where id=:id and uniacid=:uniacid Limit 1', array (
				':id' => $id,
				':uniacid' => $_W ['uniacid'] 
		) );
	...
	public function getOneDiyformData($id, $flag = 1) {
		global $_W;
		$data = pdo_fetch ( 'select * from ' . tablename ( 'rr_v_universalform_data' ) . ' where id=:id and uniacid=:uniacid limit 1', array (
				':id' => $id,
				':uniacid' => $_W ['uniacid'] 
		) );
		if (! empty ( $data ) && $flag) {
			$data ['universalformfields'] = iunserializer ( $data ['universalformfields'] );
	...
	public function getOneDiyformTemp($id, $flag = 1) {
		global $_W;
		$data = pdo_fetch ( 'select * from ' . tablename ( 'rr_v_universalform_temp' ) . ' where id=:id and uniacid=:uniacid Limit 1', array (
				':id' => $id,
				':uniacid' => $_W ['uniacid'] 
		) );
		if (! empty ( $data ) && $flag) {
			$data ['universalformfields'] = iunserializer ( $data ['universalformfields'] );
			$data ['universalformdata'] = iunserializer ( $data ['universalformdata'] );
		}
		return $data;
	}
	public function isHasDiyform($id) {
		global $_W;
		$sql = 'select count(1) from ' . tablename ( 'rr_v_universalform_type' ) . ' where id=:id and uniacid=:uniacid';
		$params = array (
				':id' => $id,
				':uniacid' => $_W ['uniacid'] 
		);
	...
	public function getPostDatas($fields, $field_data_name = 'field_data') {
		...
		foreach ( $fields as $field => $value ) {
			...
			$universalformfields [$field] = $tp_value;
			++ $key;
		}
		return $universalformfields;
	}
