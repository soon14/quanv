...
class PcModel extends PluginModel 
{
	...
	public function getGoods($id, $member, $optionid = 0) 
	{
		...
		$goods = pdo_fetch('select * from ' . tablename('rr_v_creditshop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		if (empty($goods)) 
		{
	...
	public function sendMessage($id = 0) 
	{
		...
		if (strexists($detailurl, '/addons/rr_vv3/')) 
		{
			$detailurl = str_replace('/addons/rr_vv3/', '/', $detailurl);
		}
		if ($log['status'] == 2) 
		{
			if (!(empty($type))) 
		...
		else if ($log['status'] == 3) 
		{
			...
			if (strexists($detailurl1, '/addons/rr_vv3/')) 
			{
				$detailurl1 = str_replace('/addons/rr_vv3/', '/', $detailurl1);
			}
