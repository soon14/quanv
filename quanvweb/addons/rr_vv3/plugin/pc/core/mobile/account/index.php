...
class Index_RrvV3Page extends PcMobilePage 
{
	...
	public function login() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$pwd = trim($_GPC['pwd']);
			$member = pdo_fetch('select id,openid,mobile,pwd,salt from ' . tablename('rr_v_member') . ' where mobile=:mobile and mobileverify=1 and uniacid=:uniacid limit 1', array(':mobile' => $mobile, ':uniacid' => $_W['uniacid']));
			if (empty($member)) 
			{
				show_json(0, '用户不存在');
			}
	...
	public function logout() 
	{
		global $_W;
		global $_GPC;
		$key = '__rr_vv3_member_session_' . $_W['uniacid'];
