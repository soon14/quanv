...
class Log_RrvV3Page extends PcMobileLoginPage 
{
	...
	public function get_list() 
	{
		...
		$list = pdo_fetchall('select * from ' . tablename('rr_v_member_log') . ' where 1 ' . $condition . ' order by createtime desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_member_log') . ' where 1 ' . $condition, $params);
		foreach ($list as &$row ) 
		{
