...
require rr_vv3_PLUGIN . "pc/core/page_mobile.php";
class Detail_RrvV3Page extends PcMobilePage 
{
	public function main() 
	{
		...
		$params = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_goods_param') . ' WHERE uniacid=:uniacid and goodsid=:goodsid AND title<>"" order by displayorder asc', array(':uniacid' => $uniacid, ':goodsid' => $goods['id']));
		$goods = set_medias($goods, 'thumb');
		$goods['canbuy'] = !(empty($goods['status'])) && empty($goods['deleted']);
		if (!(empty($goods['hasoption']))) 
		{
			$options = pdo_fetchall('select id,stock from ' . tablename('rr_v_goods_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $goods['id'], ':uniacid' => $_W['uniacid']), 'stock');
		...
		else 
		{
			if (isset($options) && (0 < count($options)) && ($goods['hasoption'] !== 0)) 
			{
				...
				pdo_query("delete from " . tablename("rr_v_goods_option") . " where goodsid=" . $id . ' and id not in ( ' . implode(',', $optionids) . ')');
			...
			{
				pdo_query("delete from " . tablename("rr_v_goods_option") . " where goodsid=" . $id);
	...
	public function get_history() 
	{
		...
		$sql = 'SELECT f.id,f.goodsid,g.title,g.thumb,g.marketprice,g.productprice,f.createtime,g.merchid FROM ' . tablename('rr_v_member_history') . ' f ' . ' left join ' . tablename('rr_v_goods') . ' g on f.goodsid = g.id ' . ' where 1 ' . $condition . ' ORDER BY f.`id` DESC LIMIT ' . $psize;
