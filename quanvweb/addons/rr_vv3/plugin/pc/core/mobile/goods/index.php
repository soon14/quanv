...
class Index_RrvV3Page extends PcMobilePage 
{
	...
	private function getList($args = array()) 
	{
		...
		if (!($random)) 
		{
			$sql = 'SELECT id,title,thumb,marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,total,description,bargain FROM ' . tablename('rr_v_goods') . ' where 1 ' . $condition . ' ORDER BY ' . $order . ' ' . $orderby . ' LIMIT ' . (($page - 1) * $pagesize) . ',' . $pagesize;
			$total = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_goods') . ' where 1 ' . $condition . ' ', $params);
		}
		else 
		{
			$sql = 'SELECT id,title,thumb,marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,total,description,bargain FROM ' . tablename('rr_v_goods') . ' where 1 ' . $condition . ' ORDER BY rand() LIMIT ' . $pagesize;
			$total = $pagesize;
		}
		$list = pdo_fetchall($sql, $params);
