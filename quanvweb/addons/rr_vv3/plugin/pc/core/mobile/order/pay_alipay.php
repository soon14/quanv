...
class Pay_Alipay_RrvV3Page extends MobilePage 
{
	...
	public function complete() 
	{
		...
		else 
		{
			...
			{
				$log = pdo_fetch('SELECT * FROM ' . tablename('core_paylog') . ' WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'rr_vv3', ':tid' => $tid));
		...
		if ($log['status'] != 1) 
		{
			...
			$orderid = pdo_fetchcolumn('select id from ' . tablename('rr_v_order') . ' where ordersn=:ordersn and uniacid=:uniacid', array(':ordersn' => $log['tid'], ':uniacid' => $_W['uniacid']));
			if (!(empty($orderid))) 
			{
				...
				pdo_update("rr_v_order", $data_alipay, array('id' => $orderid));
			}
			$ret = array();
			$ret['result'] = 'success';
			$ret['type'] = 'alipay';
	...
	public function recharge_complete() 
	{
		...
		if (!(empty($log)) && empty($log['status'])) 
		{
			pdo_update('rr_v_member_log', array('status' => 1, 'rechargetype' => 'alipay', 'apppay' => (is_h5app() ? 1 : 0), 'transid' => $transid), array('id' => $log['id']));
