...
class DiypageModel extends PluginModel
{
	...
	public function getPageList($type = 'allpage', $condition = NULL, $page = 0)
	{
		...
		$list = pdo_fetchall('select id, `name`, `type`, createtime, lastedittime, keyword from ' . tablename('rr_v_diypage') . ' where merch=:merch ' . $c . ' and uniacid=:uniacid order by `type` desc, id desc ' . $limit, array(':merch' => intval($_W['merchid']), ':uniacid' => $_W['uniacid']));
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('rr_v_diypage') . ' where merch=:merch and uniacid=:uniacid ' . $c, array(':merch' => intval($_W['merchid']), ':uniacid' => $_W['uniacid']));
	...
	public function getPage($id, $mobile = false)
	{
		...
		$page = pdo_fetch('select * from ' . tablename('rr_v_diypage') . $where . ' limit 1 ', $params);
						...
						if ($item['params']['goodsdata'] == '0') {
							if (!empty($item['data']) && is_array($item['data'])) {
								...
								if (!empty($goodsids) && is_array($goodsids)) {
									...
									if ($creditshop) {
										$goods = pdo_fetchall('select id, title, thumb, price as productprice, money as minprice, credit, total, showlevels, showgroups, `type`, goodstype from ' . tablename('rr_v_creditshop_goods') . ' where id in( ' . $newgoodsids . ' ) and status=1 and deleted=0 and uniacid=:uniacid order by displayorder desc ', array(':uniacid' => $_W['uniacid']));
									}
									else {
										$goods = pdo_fetchall('select id, title, subtitle, thumb, productprice, minprice, total, showlevels, showgroups, bargain, merchid, sales, salesreal from ' . tablename('rr_v_goods') . ' where id in( ' . $newgoodsids . ' ) and status=1 and deleted=0 and checked=0 and uniacid=:uniacid order by displayorder desc ', array(':uniacid' => $_W['uniacid']));
									}
							...
							if (!empty($item['params']['cateid'])) {
								if (empty($item['params']['goodstype'])) {
									$category = pdo_fetch('select id,`name`, enabled from ' . tablename('rr_v_category') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $item['params']['cateid'], ':uniacid' => $_W['uniacid']));
								}
								else {
									$category = pdo_fetch('select id,`name`, enabled from ' . tablename('rr_v_creditshop_category') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $item['params']['cateid'], ':uniacid' => $_W['uniacid']));
								}
							...
							if (($item['params']['goodsdata'] == '2') && empty($item['params']['goodstype'])) {
								if (!empty($item['params']['groupid'])) {
									$group = pdo_fetch('select id, `name` from ' . tablename('rr_v_goods_group') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $item['params']['groupid'], ':uniacid' => $_W['uniacid']));
						...
						if (($item['id'] == 'merchgroup') && p('merch')) {
							if ($item['params']['merchdata'] == '0') {
								...
								if (!empty($merchids) && is_array($merchids)) {
									...
									$merchs = pdo_fetchall('select id, merchname, logo, status, `desc` from ' . tablename('rr_v_merch_user') . ' where id in( ' . $newmerchids . ' ) and status=1 and uniacid=:uniacid order by isrecommand desc ', array(':uniacid' => $_W['uniacid']));
								...
								if (!empty($item['params']['cateid'])) {
									$category = pdo_fetch('select id, `catename`, status from ' . tablename('rr_v_merch_category') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $item['params']['cateid'], ':uniacid' => $_W['uniacid']));
								}
								...
								if (!empty($category) && !empty($category['status'])) {
									$merchs = pdo_fetchall('select id, merchname, logo, status, `desc` from ' . tablename('rr_v_merch_user') . ' where cateid=:cateid and status=1 and uniacid=:uniacid order by isrecommand desc ', array(':uniacid' => $_W['uniacid'], ':cateid' => $item['params']['cateid']));
									if (!empty($merchs) && is_array($merchs)) {
										$item['data'] = array();
								...
								if (!empty($item['params']['groupid'])) {
									$group = pdo_fetch('select id, groupname, status from ' . tablename('rr_v_merch_group') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $item['params']['groupid'], ':uniacid' => $_W['uniacid']));
								...
								if (!empty($group) && !empty($group['status'])) {
									$merchs = pdo_fetchall('select id, merchname, logo, status, `desc` from ' . tablename('rr_v_merch_user') . ' where groupid=:groupid and status=1 and uniacid=:uniacid order by isrecommand desc ', array(':uniacid' => $_W['uniacid'], ':groupid' => $item['params']['groupid']));
									if (!empty($merchs) && is_array($merchs)) {
										$item['data'] = array();
								...
								if ($item['params']['merchdata'] == '3') {
									$merchs = pdo_fetchall('select id, merchname, logo, status, `desc` from ' . tablename('rr_v_merch_user') . ' where isrecommand=1 and status=1 and uniacid=:uniacid order by isrecommand desc ', array(':uniacid' => $_W['uniacid']));
									if (!empty($merchs) && is_array($merchs)) {
										$item['data'] = array();
							...
							if (($item['id'] == 'picture') || ($item['id'] == 'picturew')) {
								...
								if (!empty($item['data']) && is_array($item['data'])) {
									...
									if (!empty($couponids) && is_array($couponids)) {
										...
										$coupons = pdo_fetchall('select * from ' . tablename('rr_v_coupon') . ' where id in( ' . $newcouponids . ' ) and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']), 'id');
				...
				foreach ($page['data']['items'] as $itemid => &$item) {
					if ($item['id'] == 'goods') {
						if ($item['params']['goodsdata'] == '0') {
							if (!empty($item['data']) && is_array($item['data'])) {
								...
								if (!empty($goodsids) && is_array($goodsids)) {
									...
									if ($creditshop) {
										$goods = pdo_fetchall('select id, showlevels, showgroups from ' . tablename('rr_v_creditshop_goods') . ' where id in( ' . $newgoodsids . ' ) and status=1 and deleted=0 and uniacid=:uniacid order by displayorder desc ', array(':uniacid' => $_W['uniacid']));
									}
									else {
										$goods = pdo_fetchall('select id, showlevels, showgroups from ' . tablename('rr_v_goods') . ' where id in( ' . $newgoodsids . ' ) and status=1 and deleted=0 and checked=0 and uniacid=:uniacid order by displayorder desc ', array(':uniacid' => $_W['uniacid']));
									}
							...
							if (!empty($cateid)) {
								...
								if (empty($item['params']['goodstype'])) {
									...
									$goods = pdo_fetchall('select id, title, thumb, price as productprice, money as minprice, credit, total, showlevels, showgroups, `type`, goodstype from ' . tablename('rr_v_creditshop_goods') . ' where cate=:cate and status=1 and deleted=0 and uniacid=:uniacid order by ' . $orderby . ' limit ' . $limit, array(':cate' => $cateid, ':uniacid' => $_W['uniacid']));
								}
							...
							if (($item['params']['goodsdata'] == '2') && empty($item['params']['goodstype'])) {
								...
								if (!empty($groupid)) {
									$group = pdo_fetch('SELECT * FROM ' . tablename('rr_v_goods_group') . ' WHERE id=:id and uniacid=:uniacid limit 1 ', array(':id' => $groupid, ':uniacid' => $_W['uniacid']));
								}
								...
								if (!empty($group) && !empty($group['goodsids'])) {
									...
									$goods = pdo_fetchall('select id, title, subtitle, thumb, minprice, sales, salesreal, total, showlevels, showgroups, bargain, productprice from ' . tablename('rr_v_goods') . ' where id in( ' . $goodsids . ' ) and status=1 and `deleted`=0 and `status`=1 and uniacid=:uniacid ' . $orderby . ' limit ' . $limit, array(':uniacid' => $_W['uniacid']));
									if (!empty($goods) && is_array($goods)) {
										foreach ($goods as $index => $good) {
											$showgoods = m('goods')->visit($good, $this->member);
								...
								if (2 < $item['params']['goodsdata']) {
									...
									if (empty($item['params']['goodstype'])) {
										...
										$goods = pdo_fetchall('select id, title, thumb, price as productprice, money as minprice, credit, total, showlevels, showgroups, `type`, goodstype from ' . tablename('rr_v_creditshop_goods') . ' where 1 ' . $condition . ' order by ' . $args['order'] . ' limit ' . $args['pagesize'], $params);
									}
						...
						if ($item['params']['noticedata'] == '0') {
							...
							if (!empty($page['merch'])) {
								$notices = pdo_fetchall('select id, title, link, thumb from ' . tablename('rr_v_merch_notice') . ' where uniacid=:uniacid and merchid=:merchid and status=1 order by displayorder desc limit ' . $limit, array(':uniacid' => $_W['uniacid'], ':merchid' => intval($page['merch'])));
							}
							else {
								$notices = pdo_fetchall('select id, title, link, thumb from ' . tablename('rr_v_notice') . ' where uniacid=:uniacid and status=1 order by displayorder desc limit ' . $limit, array(':uniacid' => $_W['uniacid']));
							}
	...
	public function savePage($id, $data, $update = true)
	{
		...
		if (!empty($keyword) && $update) {
			...
			if (!empty($result)) {
				if ($result['name'] != ('rr_vv3:diypage:' . $id)) {
					show_json(0, '关键字已存在！');
				}
			}
			else {
		...
		if (!empty($id)) {
			...
			pdo_update('rr_v_diypage', $diypage, array('id' => $id, 'uniacid' => $_W['uniacid']));
			$savetype = 'update';
		}
		else {
			...
			pdo_insert('rr_v_diypage', $diypage);
			$id = pdo_insertid();
			$savetype = 'insert';
		}

		if (!empty($keyword) && $update) {
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'rr_vv3', ':name' => 'rr_vv3:diypage:' . $id));

			if (!empty($rule)) {
				pdo_update('rule_keyword', array('content' => $keyword), array('rid' => $rule['id']));
			}
			else {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'rr_vv3:diypage:' . $id, 'module' => 'rr_vv3', 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'rr_vv3', 'content' => $keyword, 'type' => 1, 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule_keyword', $keyword_data);
			}
		}

		if ($update) {
			$item = pdo_fetch('select id, type, name, keyword from ' . tablename('rr_v_diypage') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
	...
	public function delPage($id)
	{
		...
		$items = pdo_fetchall('SELECT id,`name`,`type`,keyword FROM ' . tablename('rr_v_diypage') . ' WHERE id in( ' . $id . ' ) and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));

		foreach ($items as $item) {
			pdo_delete('rr_v_diypage', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			...
			$keyword = pdo_fetch('SELECT * FROM ' . tablename('rule_keyword') . ' WHERE content=:content and module=:module and uniacid=:uniacid limit 1 ', array(':content' => $item['keyword'], ':module' => 'rr_vv3', ':uniacid' => $_W['uniacid']));
	...
	public function saveImg($str)
	{
		...
		$str = preg_replace_callback('/"imgurl"\\:"([^\\\'" ]+)"/', function($matches) {
			...
			if (!strexists($preg, '../addons/rr_vv3')) {
	...
	public function saveTemp($temp)
	{
		...
		pdo_insert('rr_v_diypage_template', $temp);
	...
	public function verify($do, $pagetype)
	{
		...
		if ($do == 'add') {
			...
			if (!empty($tid)) {
				$template = pdo_fetch('select * from ' . tablename('rr_v_diypage_template') . ' where id=:id and (uniacid=:uniacid or uniacid=0) limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $tid));
	...
	public function calculate($str = NULL, $pagetype = false)
	{
		...
		if (strexists($str, 'r=commission')) {
			...
			if ($plugin_commissiom) {
				...
				if (strexists($str, 'r=commission.log')) {
					$this->commission['applycount'] = pdo_fetchcolumn('select count(id) from ' . tablename('rr_v_commission_apply') . ' where mid=:mid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':mid' => $member['id']));
				}

				if (strexists($str, 'r=commission.down')) {
					$level1 = $level2 = $level3 = 0;
					$level1 = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_member') . ' where agentid=:agentid and uniacid=:uniacid limit 1', array(':agentid' => $member['id'], ':uniacid' => $_W['uniacid']));
					if ((2 <= $plugin_commissiom_set['level']) && (0 < count($member['level1_agentids']))) {
						$level2 = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_member') . ' where agentid in( ' . implode(',', array_keys($member['level1_agentids'])) . ') and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
					}

					if ((3 <= $plugin_commissiom_set['level']) && (0 < count($member['level2_agentids']))) {
						$level3 = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_member') . ' where agentid in( ' . implode(',', array_keys($member['level2_agentids'])) . ') and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
					}
		...
		if (strexists($str, 'r=order') || strexists($str, 'r=member.cart') || strexists($str, 'r=coupon.my')) {
			...
			if (strexists($str, 'status=0')) {
				...
				$ordernum['status0'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_order') . ' where ' . $condition . ' and openid=:openid and status=0 and (isparent=1 or (isparent=0 and parentid=0)) and uniacid=:uniacid and istrade=0 and userdeleted=0 limit 1', $params);
			}
			...
			if (strexists($str, 'status=1')) {
				...
				$ordernum['status1'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_order') . ' where ' . $condition . ' and openid=:openid and uniacid=:uniacid limit 1', $params);
			}
			...
			if (strexists($str, 'status=2')) {
				...
				$ordernum['status2'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_order') . ' where ' . $condition . ' and openid=:openid and uniacid=:uniacid limit 1', $params);
			}
			...
			if (strexists($str, 'status=4')) {
				...
				$ordernum['status4'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_order') . ' where ' . $condition . ' and openid=:openid and uniacid=:uniacid limit 1', $params);
			}
			...
			if (strexists($str, 'r=member.cart')) {
				...
				$ordernum['cartnum'] = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('rr_v_member_cart') . ' where ' . $condition . ' and uniacid=:uniacid and openid=:openid ', $params);
			}
			...
			if (strexists($str, 'r=member.favorite')) {
				...
				$ordernum['favorite'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_member_favorite') . ' where ' . $condition . ' and uniacid=:uniacid and openid=:openid ', $params);
			}

			if (strexists($str, 'r=sale.coupon.my')) {
				$time = time();
				$sql = 'select count(*) from ' . tablename('rr_v_coupon_data') . ' d';
	...
	public function url($str)
	{
		...
		if (!empty($_W['openid'])) {
			...
			if (!empty($myid)) {
				$member = pdo_fetch('select id,isagent,status from' . tablename('rr_v_member') . 'where id=' . $myid);
	...
	{
		global $_W;
		$adv = pdo_fetch('SELECT * FROM' . tablename('rr_v_diypage_plu') . 'WHERE id=:id AND status=1 AND `type`=1 AND uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
