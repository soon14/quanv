...
class Category_RrvV3Page extends PluginWebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall('select id, `name` from ' . tablename('rr_v_diypage_template_category') . ' where 1 ' . $condition . ' order by id desc limit ' . (($pindex - 1) * $psize) . ',' . $psize, array(':merch' => intval($_W['merchid']), ':uniacid' => $_W['uniacid']));
	...
	public function delete()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($item)) {
				pdo_delete('rr_v_diypage_template_category', array('id' => $id, 'merch' => intval($_W['merchid'])));
	...
	public function add()
	{
		...
		pdo_insert('rr_v_diypage_template_category', array('name' => $name, 'uniacid' => $_W['uniacid'], 'merch' => intval($_W['merchid'])));
		$id = pdo_insertid();
