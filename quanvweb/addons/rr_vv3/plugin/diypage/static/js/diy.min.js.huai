define(['jquery.ui'], function (ui) {
    ...
    modal.initNavs = function () {
        modal.getNavs();
        var navgroup = {
            0: ['listmenu', 'richtext', 'banner', 'title', 'line', 'blank', 'menu', 'picture', 'picturew', 'icongroup'],
            1: ['search', 'notice', 'goods', 'diymod'],
            2: ['search', 'notice', 'goods', 'diymod'],
            3: ['member', 'bindmobile', 'logout'],
            4: ['memberc', 'blockgroup'],
            5: ['detail_tab', 'detail_swipe', 'detail_info', 'detail_sale', 'detail_spec', 'detail_package', 'detail_shop', 'detail_store', 'detail_buyshow', 'detail_comment', 'detail_pullup', 'detail_navbar', 'goods'],
            99: []
        };
        ...
        $("#navs nav").unbind('click').click(function () {
            ...
            if (modal.selected && modal.selected != 'page' && append) {
                var newItems = {};
    ...
    modal.getNavs = function () {
        modal.navs = {
            notice: {
                name: '公告',
                params: {
                    'iconurl': '../addons/rr_vv3/plugin/diypage/static/images/default/hotdot.jpg',
                    'noticedata': '0',
                    'speed': '4',
                    'noticenum': '5'
                },
    ...
    modal.initEditor = function (scroll) {
        ...
        if (goodsSelector > 0) {
            ...
            _this.attr({'id': 'goods_selector', 'data-url': biz.url('goods/query', null, modal.merch), 'data-callback': 'callbackGoods'});
            _this.unbind('click').click(function () {
                biz.selector.select({name: 'goods'});
                modal.childid = $(this).closest('.item').data('id')
            })
        }
        var goodsSelector = $("#diy-editor .category-selector").length;
        if (goodsSelector > 0) {
            var _this = $("#diy-editor .category-selector");
            _this.attr({
                'id': 'category_selector',
                'data-url': biz.url('goods/category/query', null, modal.merch),
        ...
        var goodsSelector = $("#diy-editor .group-selector").length;
        if (goodsSelector > 0) {
    ...
    modal.initMod = function (item) {
        $.ajax(biz.url('diypage/page/mod/query', null, modal.merch), {type: "get", dataType: "html", cache: false}).done(function (html) {
