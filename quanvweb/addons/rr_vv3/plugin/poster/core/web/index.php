...
class Index_RrvV3Page extends PluginWebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_poster') . ' WHERE 1 ' . $condition . ' ORDER BY isdefault desc,createtime desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);

		foreach ($list as &$row) {
			$row['times'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_poster_scan') . ' where posterid=:posterid and uniacid=:uniacid', array(':posterid' => $row['id'], ':uniacid' => $_W['uniacid']));
			$row['follows'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_poster_log') . ' where posterid=:posterid and uniacid=:uniacid', array(':posterid' => $row['id'], ':uniacid' => $_W['uniacid']));
		}

		unset($row);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('rr_v_poster') . ' where 1 ' . $condition . ' ', $params);
	...
	protected function post()
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('rr_v_poster') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if (($item['keyword2'] != $data['keyword2']) && !empty($keyword)) {
				if ($keyword['name'] != ('rr_vv3:poster:' . $data['type'])) {
					show_json(0, '关键字已存在!');
				}
			}
			...
			if ($data['isdefault'] == 1) {
				pdo_update('rr_v_poster', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'isdefault' => 1, 'type' => $data['type']));
			}

			if (!empty($id)) {
				pdo_update('rr_v_poster', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
			...
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'rr_vv3', ':name' => 'rr_vv3:poster:' . $data['type']));

			if (empty($rule)) {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'rr_vv3:poster:' . $data['type'], 'module' => 'rr_vv3', 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'rr_vv3', 'content' => $data['type'] == 3 ? '^' . trim($data['keyword2']) . '\\+*[0-9]{1,}$' : trim($data['keyword2']), 'type' => $data['type'] == 3 ? 3 : 1, 'displayorder' => 0, 'status' => 1);
			...
			$ruleauto = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'rr_vv3', ':name' => 'rr_vv3:poster:auto'));

			if (empty($ruleauto)) {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'rr_vv3:poster:auto', 'module' => 'rr_vv3', 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'rr_vv3', 'content' => 'rr_vv3_POSTER', 'type' => 1, 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule_keyword', $keyword_data);
			}
	...
	public function delete()
	{
		...
		$posters = pdo_fetchall('SELECT id,title,keyword,keyword2 FROM ' . tablename('rr_v_poster') . ' WHERE id in ( ' . $id . ' ) and uniacid=' . $_W['uniacid']);
		...
		foreach ($posters as $poster) {
			...
			pdo_delete('rr_v_poster', array('id' => $poster['id'], 'uniacid' => $_W['uniacid']));
			pdo_delete('rr_v_poster_log', array('posterid' => $poster['id'], 'uniacid' => $_W['uniacid']));
	...
	public function clear()
	{
		...
		global $_GPC;
		load()->func('file');
		@rmdirs(IA_ROOT . '/addons/rr_vv3/data/poster/' . $_W['uniacid']);
		@rmdirs(IA_ROOT . '/addons/rr_vv3/data/qrcode/' . $_W['uniacid']);
		$acid = pdo_fetchcolumn('SELECT acid FROM ' . tablename('account_wechats') . ' WHERE `uniacid`=:uniacid LIMIT 1', array(':uniacid' => $_W['uniacid']));
	...
	public function setdefault()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$poster = pdo_fetch('SELECT * FROM ' . tablename('rr_v_poster') . ' WHERE id = \'' . $id . '\'');

		if (empty($poster)) {
			show_json(0, '抱歉，海报不存在或是已经被删除！');
		}

		pdo_update('rr_v_poster', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'isdefault' => 1, 'type' => $poster['type']));
		pdo_update('rr_v_poster', array('isdefault' => 1), array('uniacid' => $_W['uniacid'], 'id' => $poster['id']));
