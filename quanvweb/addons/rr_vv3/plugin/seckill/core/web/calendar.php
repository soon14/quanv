...
class Calendar_RrvV3Page extends SeckillWebPage
{
		...
		if (empty($calendar)) {
			...
			while ($i <= $day) {
				...
				if (isset($calendar[$date])) {
					...
					if (!empty($value)) {
						$result[$date] = array('taskid' => $value, 'title' => pdo_fetchcolumn('select title from ' . tablename('rr_v_seckill_task') . ' where id=:id limit 1', array(':id' => $value)));
					}
				}
	...
	public function set()
	{
		...
		$year = date('Y', $time);
		$month = date('m', $time);
		$task = pdo_fetch('select id ,title from ' . tablename('rr_v_seckill_task') . ' where uniacid=:uniacid and id=:id limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $taskid));
