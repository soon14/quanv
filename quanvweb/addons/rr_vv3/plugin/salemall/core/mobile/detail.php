...
class Detail_RrvV3Page extends PluginMobileLoginPage 
{
	...
	public function pay($a = array(), $b = array()) 
	{
		...
		if ($needpay) 
		{
			...
			pdo_update('rr_v_creditshop_log', array('paytype' => ($useweixin ? 1 : 0)), array('id' => $logid));
			if ($useweixin) 
			{
	...
	public function lottery() 
	{
		...
		pdo_update('rr_v_creditshop_goods', array('joins' => $goods['joins'] + 1), array('id' => $id));
		...
		if ((0 < $goods['money']) && empty($log['paystatus'])) 
		{
			if ($log['paytype'] == 0) 
			{
				m('member')->setCredit($openid, 'credit2', -$goods['money'], '' . $_W['shopset']['trade']['credittext'] . '商城抽奖扣除余额度 ' . $goods['money']);
				$update['paystatus'] = 1;
			}
			if ($log['paytype'] == 1) 
			{
		...
		{
			m('member')->setCredit($openid, 'credit1', -$goods['credit'], '' . $_W['shopset']['trade']['credittext'] . '商城抽奖扣除' . $_W['shopset']['trade']['credittext'] . ' ' . $goods['credit']);
			$update['creditpay'] = 1;
		}
		$status = 1;
		if (!(empty($goods['type']))) 
