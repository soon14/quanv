...
class Set_RrvV3Page extends PluginWebPage
{
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'rr_vv3文章营销入口设置'));
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'rr_vv3文章营销入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => 1);
			...
			$sys = pdo_fetch('select * from ' . tablename('rr_v_article_sys') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));

			if (empty($sys)) {
				$arr['uniacid'] = $_W['uniacid'];
				pdo_insert('rr_v_article_sys', $arr);
			}
			else {
				pdo_update('rr_v_article_sys', $arr, array('uniacid' => $_W['uniacid']));
			}
