...
class Index_RrvV3Page extends PluginWebPage
{
	public function main()
	{
		...
		$psize = 20;
		$articles = array();
		$articles = pdo_fetchall('SELECT a.id,a.displayorder, a.article_title,a.article_category,a.article_keyword2,a.article_date,a.article_readnum,a.article_likenum,a.article_state,c.category_name FROM ' . tablename('rr_v_article') . ' a left join ' . tablename('rr_v_article_category') . ' c on c.id=a.article_category  WHERE a.uniacid= :uniacid ' . $select_title . $select_category . ' order by displayorder desc,article_date desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, array(':uniacid' => $_W['uniacid']));
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('rr_v_article') . ' a left join ' . tablename('rr_v_article_category') . ' c on c.id=a.article_category  WHERE a.uniacid= :uniacid ' . $select_title . $select_category, array(':uniacid' => $_W['uniacid']));
		...
		$articlenum = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('rr_v_article') . ' WHERE uniacid= :uniacid ', array(':uniacid' => $_W['uniacid']));
		$categorys = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_article_category') . ' WHERE uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			$article = pdo_fetch('SELECT * FROM ' . tablename('rr_v_article') . ' WHERE id=:aid and uniacid=:uniacid limit 1 ', array(':aid' => $aid, ':uniacid' => $_W['uniacid']));
			...
			if (!empty($data['article_keyword2'])) {
				...
				if (!empty($keyword)) {
					if ($keyword['name'] != ('rr_vv3:article:' . $aid)) {
			...
			if (empty($aid)) {
				...
				pdo_insert('rr_v_article', $data);
				$aid = pdo_insertid();
				plog('article.add', '添加文章 ID: ' . $aid . ' 标题: ' . $data['article_title']);
			}
			else {
				pdo_update('rr_v_article', $data, array('id' => $aid));
				plog('article.edit', '编辑文章 ID: ' . $aid . ' 标题: ' . $article['article_title']);
			}

			if (!empty($data['article_keyword2'])) {
				$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'rr_vv3', ':name' => 'rr_vv3:article:' . $aid));

				if (!empty($rule)) {
					pdo_update('rule_keyword', array('content' => trim($data['article_keyword2'])), array('rid' => $rule['id']));
				}
				else {
					$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'rr_vv3:article:' . $aid, 'module' => 'rr_vv3', 'displayorder' => 0, 'status' => 1);
					pdo_insert('rule', $rule_data);
					$rid = pdo_insertid();
					$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'rr_vv3', 'content' => trim($data['article_keyword2']), 'type' => 1, 'displayorder' => 0, 'status' => 1);
		...
		if (!empty($article)) {
			...
			if ((0 < $article['article_rule_credittotal']) || (0 < $article['article_rule_moneytotal'])) {
				$firstreads = pdo_fetchcolumn('select count(distinct click_user) from ' . tablename('rr_v_article_share') . ' where aid=:aid and uniacid=:uniacid limit 1', array(':aid' => $aid, ':uniacid' => $_W['uniacid']));
				$allreads = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_article_share') . ' where aid=:aid and uniacid=:uniacid limit 1', array(':aid' => $aid, ':uniacid' => $_W['uniacid']));
				$secreads = $allreads - $firstreads;

				if (0 < $article['article_rule_credittotal']) {
					$creditout = pdo_fetchcolumn('select sum(add_credit) from ' . tablename('rr_v_article_share') . ' where aid=:aid and uniacid=:uniacid limit 1', array(':aid' => $aid, ':uniacid' => $_W['uniacid']));
				...
				if (0 < $article['article_rule_moneytotal']) {
					$moneyout = pdo_fetchcolumn('select sum(add_money) from ' . tablename('rr_v_article_share') . ' where aid=:aid and uniacid=:uniacid limit 1', array(':aid' => $aid, ':uniacid' => $_W['uniacid']));
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,article_title,article_keyword2 FROM ' . tablename('rr_v_article') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_delete('rr_v_article', array('id' => $item['id']));
			$keyword = pdo_fetch('SELECT * FROM ' . tablename('rule_keyword') . ' WHERE content=:content and module=:module and uniacid=:uniacid limit 1 ', array(':content' => $item['article_keyword2'], ':module' => 'rr_vv3', ':uniacid' => $_W['uniacid']));
			...
			pdo_delete('rr_v_article_log', array('aid' => $item['id']));
			pdo_delete('rr_v_article_share', array('aid' => $item['id']));
	...
	public function displayorder()
	{
		...
		$item = pdo_fetchall('SELECT id,article_title FROM ' . tablename('rr_v_article') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		if (!empty($item)) {
			pdo_update('rr_v_article', array('displayorder' => $displayorder), array('id' => $id));
	...
	public function state()
	{
		...
		$items = pdo_fetchall('SELECT id,article_title FROM ' . tablename('rr_v_article') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_update('rr_v_article', array('article_state' => intval($_GPC['state'])), array('id' => $item['id']));
	...
	protected function delKey($keyword)
	{
		...
		$keyword = pdo_fetch('SELECT * FROM ' . tablename('rule_keyword') . ' WHERE content=:content and `module`=:module and uniacid=:uniacid limit 1 ', array(':content' => $keyword, ':module' => 'rr_vv3', ':uniacid' => $_W['uniacid']));
