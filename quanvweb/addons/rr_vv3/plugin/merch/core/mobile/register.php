<?php
class Register_RrvV3Page extends PluginMobileLoginPage 
{
	public function main() 
	{
		...
		$reg = pdo_fetch('select * from ' . tablename('rr_v_merch_reg') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
		$user = false;
		if (!(empty($reg['status']))) 
		{
			$user = pdo_fetch('select * from ' . tablename('rr_v_merch_user') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
		}
		...
		if ($_W['ispost']) 
		{
			...
			$usercount1 = pdo_fetchcolumn('select count(1) from ' . tablename('rr_v_merch_reg') . ' where ' . $where1 . ' limit 1', $params1);
			$where2 = ' username=:username';
			$params2 = array(':username' => $uname);
			$usercount2 = pdo_fetchcolumn('select count(1) from ' . tablename('rr_v_merch_account') . ' where ' . $where2 . ' limit 1', $params2);
			...
			{
				$data['applytime'] = time();
				pdo_insert('rr_v_merch_reg', $data);
			}
			else 
			{
				pdo_update('rr_v_merch_reg', $data, array('id' => $reg['id']));
			}
