...
class Index_RrvV3Page extends PluginMobilePage
{
	...
	public function getpage($merchid)
	{
		...
		$advs = pdo_fetchall('select id,advname,link,thumb from ' . tablename('rr_v_merch_adv') . ' where uniacid=:uniacid and merchid=:merchid and enabled=1 order by displayorder desc', array(':uniacid' => $uniacid, ':merchid' => $merchid));
		$navs = pdo_fetchall('select id,navname,url,icon from ' . tablename('rr_v_merch_nav') . ' where uniacid=:uniacid and merchid=:merchid and status=1 order by displayorder desc', array(':uniacid' => $uniacid, ':merchid' => $merchid));
		$cubes = ((is_array($shop['cubes']) ? $shop['cubes'] : array()));
		$banners = pdo_fetchall('select id,bannername,link,thumb from ' . tablename('rr_v_merch_banner') . ' where uniacid=:uniacid and merchid=:merchid and enabled=1 order by displayorder desc', array(':uniacid' => $uniacid, ':merchid' => $merchid));
		...
		if (!(empty($shop['indexrecommands']))) {
			...
			if (!(empty($goodids))) {
				$indexrecommands = pdo_fetchall('select id, title, thumb, marketprice, productprice, minprice, total from ' . tablename('rr_v_goods') . ' where id in( ' . $goodids . ' ) and uniacid=:uniacid and merchid=:merchid and status=1 order by instr(\'' . $goodids . '\',id),merchdisplayorder desc', array(':uniacid' => $uniacid, ':merchid' => $merchid));
			}
		...
		$notices = pdo_fetchall('select id, title, link, thumb from ' . tablename('rr_v_merch_notice') . ' where uniacid=:uniacid and merchid=:merchid and status=1 order by displayorder desc limit 5', array(':uniacid' => $uniacid, ':merchid' => $merchid));
		ob_start();
