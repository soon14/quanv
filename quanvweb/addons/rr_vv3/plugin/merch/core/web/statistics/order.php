...
class Order_RrvV3Page extends PluginWebPage 
{
	public function main() 
	{
		...
		$sql = 'select o.*, a.realname as addressname,m.realname,u.merchname from ' . tablename('rr_v_order') . ' o ' . ' left join ' . tablename('rr_v_member') . ' m on o.openid = m.openid ' . ' left join ' . tablename('rr_v_member_address') . ' a on a.id = o.addressid ' . ' left join ' . tablename('rr_v_merch_user') . ' u on u.id = o.merchid ' . ' where 1 ' . $condition . ' ';
		...
		foreach ($list as &$row ) 
		{
			...
			$row['goods'] = pdo_fetchall('SELECT g.thumb,og.price,og.total,og.realprice,g.title,og.optionname from ' . tablename('rr_v_order_goods') . ' og' . ' left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid  ' . ' where og.uniacid = :uniacid and og.orderid=:orderid order by og.createtime  desc ', array(':uniacid' => $_W['uniacid'], ':orderid' => $row['id']));
			$totalmoney += $row['price'];
		}
		unset($row);
		$total = pdo_fetchall('select o.id from ' . tablename('rr_v_order') . ' o ' . ' left join ' . tablename('rr_v_member') . ' m on o.openid = m.openid ' . ' left join ' . tablename('rr_v_member_address') . ' a on a.id = o.addressid ' . ' left join ' . tablename('rr_v_merch_user') . ' u on u.id = o.merchid ' . ' where 1 ' . $condition, $params);
