...
class Log_RrvV3Page extends MerchWebPage 
{
	...
	public function detail() 
	{
		...
		$id = intval($_GPC['id']);
		$log = pdo_fetch('select * from ' . tablename('rr_v_creditshop_log') . ' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		if (empty($log)) 
		{
		...
		if ($goods['isverify']) 
		{
			if (!(empty($log['storeid']))) 
			{
				if (0 < $merchid) 
				{
					$stores = pdo_fetch('select * from ' . tablename('rr_v_merch_store') . ' where id=:id and uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3)', array(':id' => $log['storeid'], ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
				}
				else 
				{
					$stores = pdo_fetch('select * from ' . tablename('rr_v_store') . ' where id=:id and uniacid=:uniacid and status=1 and type in(2,3)', array(':id' => $log['storeid'], ':uniacid' => $_W['uniacid']));
				}
			}
		}
		else 
	...
	public function doexchange() 
	{
		...
		if ($_W['ispost']) 
		{
			if ($goods['goodstype'] == 0) 
			{
				...
				pdo_update('rr_v_creditshop_log', array('status' => 2, 'usetime' => time(), 'time_send' => time(), 'expresscom' => $_GPC['expresscom'], 'expresssn' => $_GPC['expresssn'], 'express' => $_GPC['express']), array('id' => $id));
