...
class Category_RrvV3Page extends MerchWebPage 
{
	...
	public function main() 
	{
		...
		if (!(empty($_GPC['catid']))) 
		{
			foreach ($_GPC['catid'] as $k => $v ) 
			{
				...
				if (empty($v)) 
				{
					pdo_insert('rr_v_coupon_category', $data);
					$insert_id = pdo_insertid();
					mplog('sale.coupon.category.add', '添加分类 ID: ' . $insert_id);
				}
				else 
				{
					pdo_update('rr_v_coupon_category', $data, array('id' => $v));
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_coupon_category') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' AND merchid = \'' . $_W['merchid'] . '\' ORDER BY displayorder asc');
	...
	public function delete() 
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,name FROM ' . tablename('rr_v_coupon_category') . ' WHERE id = \'' . $id . '\' AND uniacid=' . $_W['uniacid'] . ' AND merchid=' . $_W['merchid']);
		if (!(empty($item))) 
		{
			pdo_delete('rr_v_coupon_category', array('id' => $id));
