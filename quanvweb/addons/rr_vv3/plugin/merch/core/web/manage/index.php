...
class Index_RrvV3Page extends MerchWebPage 
{
	public function main() 
	{
		global $_W;
		global $_GPC;
		$user = pdo_fetch('select `id`,`logo`,`merchname`,`desc` from ' . tablename('rr_v_merch_user') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $_W['uniaccount']['merchid'], ':uniacid' => $_W['uniacid']));
		$order_sql = 'select id,ordersn,createtime,address,price,invoicename from ' . tablename('rr_v_order') . ' where uniacid = :uniacid and merchid=:merchid and isparent=0 and deleted=0 AND ( status = 1 or (status=0 and paytype=3) ) ORDER BY createtime ASC LIMIT 20';
	...
	public function updatepassword() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$item = pdo_fetch('SELECT * FROM ' . tablename('rr_v_merch_account') . ' WHERE id=:id AND uniacid=:uniacid AND merchid=:merchid', array(':id' => $_W['uniaccount']['id'], ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
			...
			pdo_update('rr_v_merch_account', $date, array('id' => $_W['uniaccount']['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
			show_json(1);
		}
