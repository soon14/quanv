...
class Export_RrvV3Page extends MerchWebPage 
{
	...
	public function main() 
	{
		...
		if ($_GPC['export'] == 1) 
		{
			...
			if ($address2index != -1) 
			{
				array_splice($columns, $address2index + 1, 0, array( array('title' => '', 'field' => 'address_city', 'width' => 12), array('title' => '', 'field' => 'address_area', 'width' => 12), array('title' => '', 'field' => 'address_address', 'width' => 24) ));
			}
			$goodsindex = $this->field_index($columns, 'goods_title');
			if ($goodsindex != -1) 
			{
				array_splice($columns, $goodsindex + 1, 0, array( array('title' => '商品编码', 'field' => 'goods_goodssn', 'width' => 12), array('title' => '商品规格', 'field' => 'goods_optiontitle', 'width' => 12), array('title' => '商品数量', 'field' => 'goods_total', 'width' => 12), array('title' => '商品单价(折扣前)', 'field' => 'goods_price1', 'width' => 12), array('title' => '商品单价(折扣后)', 'field' => 'goods_price2', 'width' => 12), array('title' => '商品价格(折扣后)', 'field' => 'goods_rprice1', 'width' => 12), array('title' => '商品价格(折扣后)', 'field' => 'goods_rprice2', 'width' => 12) ));
			}
			...
			$sql = 'select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea,a.address as aaddress, d.dispatchname,m.nickname,m.id as mid,m.realname as mrealname,m.mobile as mmobile,sm.id as salerid,sm.nickname as salernickname,s.salername from ' . tablename('rr_v_order') . ' o' . ' left join ' . tablename('rr_v_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('rr_v_member') . ' m on m.openid=o.openid and m.uniacid =  o.uniacid ' . ' left join ' . tablename('rr_v_member_address') . ' a on a.id=o.addressid ' . ' left join ' . tablename('rr_v_dispatch') . ' d on d.id = o.dispatchid ' . ' left join ' . tablename('rr_v_member') . ' sm on sm.openid = o.verifyopenid and sm.uniacid=o.uniacid' . ' left join ' . tablename('rr_v_merch_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' where ' . $condition . ' ' . $statuscondition . ' ORDER BY o.createtime DESC,o.status DESC  ';
			...
			foreach ($list as &$value ) 
			{
				...
				{
					$value['storeinfo'] = pdo_fetchcolumn('select storename from ' . tablename('rr_v_merch_store') . ' where id=:storeid limit 1 ', array(':storeid' => $value['verifystoreid']));
				}
		...
		$stores = pdo_fetchall('select id,storename from ' . tablename('rr_v_merch_store') . ' where uniacid=:uniacid and merchid = :merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
