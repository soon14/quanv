...
class Printer_RrvV3Page extends MerchWebPage 
{
	public function main() 
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_member_printer_template') . ' WHERE ' . $condition . '  ORDER BY id asc limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('rr_v_member_printer_template') . ' WHERE ' . $condition, $params);
	...
	protected function post() 
	{
		...
		if (!(empty($id))) 
		{
			$list = pdo_fetch('SELECT * FROM ' . tablename('rr_v_member_printer_template') . ' WHERE id=:id and uniacid=:uniacid and merchid=:merchid', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		...
		if ($_W['ispost']) 
		{
			$data = array('uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid'], 'type' => intval($_GPC['type']), 'title' => trim($_GPC['title']), 'print_title' => trim($_GPC['print_title']), 'print_style' => trim($_GPC['print_style']), 'code' => intval($_GPC['code']), 'qrcode' => trim($_GPC['qrcode']));
			...
			if (empty($id)) 
			{
				...
				pdo_insert('rr_v_member_printer_template', $data);
				$id = pdo_insertid();
				mplog('sysset.printer.add', '添加打印机模板 ID: ' . $id . ' 标题: ' . $data['title'] . ' ');
			}
			else 
			{
				pdo_update('rr_v_member_printer_template', $data, array('id' => $id));
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('rr_v_member_printer_template') . ' WHERE id in( ' . $id . ' ) AND uniacid=:uniacid AND merchid=:merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		foreach ($items as $item ) 
		{
			pdo_delete('rr_v_member_printer_template', array('id' => $id));
	...
	public function query() 
	{
		...
		$ds = pdo_fetchall('SELECT id,title FROM ' . tablename('rr_v_member_printer_template') . ' WHERE ' . $condition . ' order by id asc', $params);
		if ($_GPC['suggest']) 
		{
	...
	public function printer_list() 
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_member_printer') . ' WHERE ' . $condition . '  ORDER BY id asc limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('rr_v_member_printer') . ' WHERE ' . $condition, $params);
		$pager = pagination($total, $pindex, $psize);
	...
	protected function printer_post() 
	{
		...
		if (!(empty($id))) 
		{
			$list = pdo_fetch('SELECT * FROM ' . tablename('rr_v_member_printer') . ' WHERE id=:id and uniacid=:uniacid and merchid=:merchid', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
			$print_data = json_decode($list['print_data'], true);
		...
		if ($_W['ispost']) 
		{
			...
			{
				$data['createtime'] = time();
				pdo_insert('rr_v_member_printer', $data);
				$id = pdo_insertid();
				mplog('sysset.printer.printer_add', '添加打印机 ID: ' . $id . ' 标题: ' . $data['title'] . ' ');
			}
			else 
			{
				pdo_update('rr_v_member_printer', $data, array('id' => $id));
	...
	public function printer_delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('rr_v_member_printer') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid'] . ' AND merchid=' . $_W['merchid']);
		foreach ($items as $item ) 
		{
			pdo_delete('rr_v_member_printer', array('id' => $id));
	...
	public function printer_query() 
	{
		...
		$ds = pdo_fetchall('SELECT id,title FROM ' . tablename('rr_v_member_printer') . ' WHERE ' . $condition . ' order by id asc', $params);
		if ($_GPC['suggest']) 
		{
			exit(json_encode(array('value' => $ds)));
		}
	...
	public function set() 
	{
		...
		$order_template = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_member_printer_template') . ' WHERE uniacid=:uniacid AND merchid=:merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		include $this->template();
	}
}
?>