...
class Order_RrvV3Page extends PluginWebPage
{
	public function main()
	{
		...
		foreach ($list as $key => $value) {
			if (!empty($value['address'])) {
				...
				if (!is_array($user)) {
					$user = pdo_fetch('SELECT * FROM ' . tablename('rr_v_member_address') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $value['addressid'], ':uniacid' => $_W['uniacid']));
				}
		...
		$total = pdo_fetchcolumn('SELECT count(1) FROM ' . tablename('rr_v_groups_order') . " as o\r\n\t\t\t\tleft join " . tablename('rr_v_groups_goods') . " as g on g.id = o.goodid\r\n\t\t\t\tleft join " . tablename('rr_v_member') . " m on m.openid=o.openid and m.uniacid =  o.uniacid\r\n\t\t\t\tleft join " . tablename('rr_v_member_address') . " a on a.id=o.addressid\r\n\t\t\t\tWHERE 1 " . $condition . ' ', $params);
	...
	public function detail()
	{
		...
		$order = pdo_fetch('SELECT o.*,g.title,g.category,g.groupsprice,g.singleprice,g.thumb,g.id as gid FROM ' . tablename('rr_v_groups_order') . " as o\r\n\t\t\t\tleft join " . tablename('rr_v_groups_goods') . " as g on g.id = o.goodid\r\n\t\t\t\tWHERE o.id = :orderid ", $params);
		$order = set_medias($order, 'thumb');
		$member = m('member')->getMember($order['openid'], true);

		if ($order['verifytype'] == 0) {
			$verify = pdo_fetch('select * from ' . tablename('rr_v_groups_verify') . ' where orderid = ' . $order['id'] . ' ');

			if (!empty($verify['verifier'])) {
				$saler = m('member')->getMember($verify['verifier']);
				$saler['salername'] = pdo_fetchcolumn('select salername from ' . tablename('rr_v_saler') . ' where openid=:openid and uniacid=:uniacid limit 1 ', array(':uniacid' => $_W['uniacid'], ':openid' => $verify['verifier']));
			}

			if (!empty($order['storeid'])) {
				$store = pdo_fetch('select * from ' . tablename('rr_v_store') . ' where id=:storeid limit 1 ', array(':storeid' => $verify['storeid']));
			}
		}
		else {
			if ($order['verifytype'] == 1) {
				$verifyinfo = pdo_fetchall('select v.*,sm.id as salerid,sm.nickname as salernickname,s.salername,store.storename from ' . tablename('rr_v_groups_verify') . " as v\r\n\t\t\t\t\tleft join " . tablename('rr_v_saler') . " s on s.openid = v.verifier and s.uniacid = v.uniacid\r\n\t\t\t\t\tleft join " . tablename('rr_v_member') . " sm on sm.openid = s.openid and sm.uniacid = s.uniacid\r\n\t\t\t\t\tleft join " . tablename('rr_v_store') . " store on store.id = v.storeid and store.uniacid = v.uniacid\r\n\t\t\t\t\twhere v.uniacid = :uniacid and v.orderid = :orderid ", array(':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
			}
		}
		...
		if (!empty($order['address'])) {
			...
			if (!is_array($user)) {
				$user = pdo_fetch('SELECT * FROM ' . tablename('rr_v_member_address') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $order['addressid'], ':uniacid' => $_W['uniacid']));
			}
	...
	protected function opData()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('rr_v_groups_order') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function delete()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,name FROM ' . tablename('rr_v_groups_order') . ' WHERE id = \'' . $id . '\' AND uniacid=' . $_W['uniacid'] . '');

		if (empty($item)) {
			message('抱歉，订单不存在或是已经被删除！', webUrl('groups/order', array('op' => 'display')), 'error');
		}

		pdo_delete('rr_v_groups_order', array('id' => $id));
	...
	public function remarksaler()
	{
		...
		if ($_W['ispost']) {
			pdo_update('rr_v_groups_order', array('remark' => $_GPC['remark']), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function changeaddress()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				...
				$item = pdo_fetch('SELECT id, orderno, address FROM ' . tablename('rr_v_groups_order') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
				...
				pdo_update('rr_v_groups_order', array('address' => $address_array), array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function pay($a = array(), $b = array())
	{
		...
		if (!empty($item['virtual']) && c('virtual')) {
			...
			pdo_update('rr_v_groups_order', array('status' => 1, 'pay_type' => 'other', 'paytime' => time(), 'starttime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function send()
	{
		...
		if ($_W['ispost']) {
			...
			pdo_update('rr_v_groups_order', array('status' => 2, 'express' => trim($_GPC['express']), 'expresscom' => trim($_GPC['expresscom']), 'expresssn' => trim($_GPC['expresssn']), 'sendtime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function sendcancel()
	{
		...
		if ($_W['ispost']) {
			...
			pdo_update('rr_v_groups_order', array('status' => 1, 'sendtime' => 0, 'remarksend' => $remark), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function changeexpress()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($expresssn)) {
				...
				pdo_update('rr_v_groups_order', $change_data, array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function finish()
	{
		...
		$opdata = $this->opData();
		extract($opdata);
		pdo_update('rr_v_groups_order', array('status' => 3, 'finishtime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
		p('groups')->sendTeamMessage($item['id']);
	...
	public function enabled()
	{
		...
		$items = pdo_fetchall('SELECT id,name FROM ' . tablename('rr_v_groups_order') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_update('rr_v_groups_order', array('enabled' => intval($_GPC['enabled'])), array('id' => $item['id']));
	...
	protected function selectOrderPrice($day = 0)
	{
		...
		$sql = 'select id,price,freight,starttime,openid,creditmoney from ' . tablename('rr_v_groups_order') . ' where uniacid = :uniacid and status > 0 and paytime between :createtime1 and :createtime2 ';
	...
	protected function selectteamPrice($success = 0)
	{
		...
		$sql = 'select id,price,freight,starttime from ' . tablename('rr_v_groups_order') . ' where uniacid = :uniacid and paytime > 0 and is_team = 1 and heads = :heads and success = :success ';
	...
	public function close()
	{
		...
		if ($_W['ispost']) {
			...
			if ((0 < $item['refundstate']) && !empty($item['refundid'])) {
				...
				pdo_update('rr_v_groups_order_refund', $change_refund, array('id' => $item['refundid'], 'uniacid' => $_W['uniacid']));
			}

			pdo_update('rr_v_groups_order', array('status' => -1, 'refundstate' => 0, 'canceltime' => $time, 'remarkclose' => $_GPC['remark']), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
