...
class Verify_RrvV3Page extends PluginWebPage
{
	public function main()
	{
		...
		if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
			...
			if ($searchfield == 'orderno') {
				...
				$sqlcondition = ' left join ' . tablename('rr_v_saler') . " as s on s.openid = v.verifier and s.uniacid=v.uniacid\r\n\t\t\t\tleft join " . tablename('rr_v_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid ';
			}
			else {
				if ($searchfield == 'store') {
					$condition .= ' AND (locate(:keyword,store.storename)>0)';
					$sqlcondition = ' left join ' . tablename('rr_v_store') . ' store on store.id = v.storeid and store.uniacid=o.uniacid ';
				}
			}
		}
