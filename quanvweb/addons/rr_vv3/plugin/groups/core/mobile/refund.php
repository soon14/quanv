...
class Refund_RrvV3Page extends PluginMobileLoginPage
{
	protected function globalData()
	{
		...
		$goodRefund = false;
		$groupsSet = pdo_fetch('select goodsid,refundday from ' . tablename('rr_v_groups_set') . 'where uniacid = :uniacid ', array(':uniacid' => $uniacid));
	...
	public function main()
	{
		...
		try {
			...
			if (0 < $order['refundstate']) {
				if (!empty($refundid)) {
					$refund = pdo_fetch('select * from ' . tablename('rr_v_groups_order_refund') . ' where id=:id and uniacid=:uniacid and orderid=:orderid limit 1', array(':id' => $refundid, ':uniacid' => $uniacid, ':orderid' => $orderid));

					if (!empty($refund['refundaddress'])) {
						$refund['refundaddress'] = iunserializer($refund['refundaddress']);
					}
					else if ($refund['refundaddressid']) {
						$refund['refundaddress'] = pdo_fetch('select * from ' . tablename('rr_v_member_address') . ' where id = :id and uniacid=:uniacid', array(':id' => $refund['refundaddressid'], ':uniacid' => $_W['uniacid']));
					}
					else {
						$refund['refundaddress'] = pdo_fetch('select * from ' . tablename('rr_v_refund_address') . ' where uniacid=:uniacid order by id desc limit 1', array(':uniacid' => $uniacid));
					}
				}
	...
	public function submit()
	{
		...
		if ($order['refundstate'] == 0) {
			...
			pdo_insert('rr_v_groups_order_refund', $refund);
			$refundid = pdo_insertid();
			pdo_update('rr_v_groups_order', array('refundid' => $refundid, 'refundstate' => $refundstate), array('id' => $orderid, 'uniacid' => $uniacid));
		}
		else {
			pdo_update('rr_v_groups_order', array('refundstate' => $refundstate), array('id' => $orderid, 'uniacid' => $uniacid));
			pdo_update('rr_v_groups_order_refund', $refund, array('id' => $refundid, 'uniacid' => $uniacid));
		}
	...
	public function cancel()
	{
		...
		pdo_update('rr_v_groups_order_refund', $change_refund, array('id' => $refundid, 'uniacid' => $uniacid));
		pdo_update('rr_v_groups_order', array('refundstate' => 0), array('id' => $orderid, 'uniacid' => $uniacid));
		show_json(1);
	}
	...
	public function receive()
	{
		...
		pdo_update('rr_v_groups_order_refund', $refund_data, array('id' => $refundid, 'uniacid' => $uniacid));
		$order_data = array();
		$order_data['refundstate'] = 0;
		$order_data['refundstatus'] = -1;
		$order_data['refundtime'] = $time;
		pdo_update('rr_v_groups_order', $order_data, array('id' => $orderid, 'uniacid' => $uniacid));
		show_json(1);
	}
