...
class Team_RrvV3Page extends PluginMobileLoginPage
{
	...
	public function get_list()
	{
		...
		foreach ($orders as $key => $order) {
			...
			$goods = pdo_fetch('SELECT * FROM ' . tablename('rr_v_groups_goods') . 'WHERE id = ' . $order['goodid']);
			$sql2 = 'SELECT * FROM' . tablename('rr_v_groups_order') . 'where teamid = :teamid and success = 1';
			...
			$sql3 = 'SELECT * FROM ' . tablename('rr_v_groups_order') . ' WHERE teamid = :teamid and paytime > 0 and heads = :heads';
	...
	public function detail()
	{
		...
		$myorder = pdo_fetch('SELECT * FROM ' . tablename('rr_v_groups_order') . ' WHERE uniacid = ' . $uniacid . ' and openid = \'' . $_W['openid'] . '\' and teamid = ' . $teamid . ' and paytime>0');
		$params = array(':teamid' => $teamid);
		$orders = pdo_fetchall('select * from ' . tablename('rr_v_groups_order') . ' where uniacid = ' . $uniacid . ' and teamid = :teamid and paytime>0 order by id asc ', $params);
		$profileall = array();
		...
		foreach ($orders as $key => $value) {
			...
			$avatar = pdo_fetch('SELECT openid,avatar,nickname FROM ' . tablename('rr_v_member') . ' WHERE uniacid =\'' . $_W['uniacid'] . '\' and openid = \'' . $value['openid'] . '\'');
			$orders[$key]['openid'] = $avatar['openid'];
			$orders[$key]['nickname'] = $avatar['nickname'];
			$orders[$key]['avatar'] = $avatar['avatar'];

			if ($orders[$key]['avatar'] == '') {
				$orders[$key]['avatar'] = '../addons/rr_vv3/plugin/groups/template/mobile/default/images/user/' . mt_rand(1, 20) . '.jpg';
			}
		}

		$groupsset = pdo_fetch('select description,groups_description,discount,headstype,headsmoney,headsdiscount from ' . tablename('rr_v_groups_set') . "\r\n\t\t\t\t\twhere uniacid = :uniacid ", array(':uniacid' => $uniacid));
		$groupsset['groups_description'] = m('ui')->lazy($groupsset['groups_description']);
		$goods = pdo_fetch('SELECT * FROM' . tablename('rr_v_groups_goods') . 'WHERE  uniacid = ' . $uniacid . ' and id = ' . $order['goodid']);
		...
		$sql = 'SELECT * FROM' . tablename('rr_v_groups_order') . ' where uniacid = :uniacid and teamid=:teamid and status > 0 ';
		...
		if ($n <= 0) {
			pdo_update('rr_v_groups_order', array('success' => 1), array('teamid' => $teamid));
		}
		...
		$tuan_first_order = pdo_fetch('SELECT * FROM ' . tablename('rr_v_groups_order') . ' WHERE teamid = ' . $teamid . ' and heads = 1');
		$hours = $tuan_first_order['endtime'];
		$time = time();
		$date = date('Y-m-d H:i:s', $tuan_first_order['starttime']);
		$endtime = date('Y-m-d H:i:s', strtotime(' ' . $date . ' + ' . $hours . ' hour'));
	...
	public function rules()
	{
		global $_W;
		global $_GPC;
		$set = pdo_fetch('SELECT rules FROM ' . tablename('rr_v_groups_set') . ' WHERE uniacid =\'' . $_W['uniacid'] . '\'');
		include $this->template();
	}
}
