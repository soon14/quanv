...
class Pay_RrvV3Page extends PluginMobileLoginPage
{
	public function main()
	{
		...
		if (!empty($teamid)) {
			...
			$num = pdo_fetchcolumn('select count(1) from ' . tablename('rr_v_groups_order') . ' as o where teamid = :teamid and status > :status and uniacid = :uniacid ', array(':teamid' => $teamid, ':status' => 0, ':uniacid' => $uniacid));
		...
		if (empty($log)) {
			...
			pdo_insert('rr_v_groups_paylog', $log);
			$plid = pdo_insertid();
		}
	...
	public function complete()
	{
		...
		if (is_h5app() && empty($orderid)) {
			...
			$orderid = pdo_fetchcolumn('select id from ' . tablename('rr_v_groups_order') . ' where orderno=:orderno and uniacid=:uniacid and openid=:openid limit 1', array(':orderno' => $ordersn, ':uniacid' => $uniacid, ':openid' => $openid));
		}
		...
		$order = pdo_fetch('select * from ' . tablename('rr_v_groups_order') . ' where id = :orderid and uniacid=:uniacid and openid=:openid', array(':orderid' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
		...
		if ($type == 'credit') {
			...
			pdo_update('rr_v_groups_order', array('pay_type' => 'credit', 'status' => 1, 'paytime' => time(), 'starttime' => time()), array('id' => $orderid));
			...
			if ($type == 'wechat') {
				...
				if (!is_error($payquery) || !is_error($payqueryBorrow)) {
					...
					pdo_update('rr_v_groups_order', array('pay_type' => 'wechat', 'status' => 1, 'paytime' => time(), 'starttime' => time(), 'apppay' => is_h5app() ? 1 : 0), array('id' => $orderid));
	...
	public function orderstatus()
	{
		...
		$order = pdo_fetch('select status from ' . tablename('rr_v_groups_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid));
		show_json(1, $order);
	}
}
