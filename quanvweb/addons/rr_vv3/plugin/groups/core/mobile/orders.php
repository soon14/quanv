...
class Orders_RrvV3Page extends PluginMobileLoginPage
{
	...
	public function detail()
	{
		...
		if (!empty($order['isverify'])) {
			...
			$verifytotal = pdo_fetchcolumn('select count(1) from ' . tablename('rr_v_groups_verify') . ' where orderid = :orderid and openid = :openid and uniacid = :uniacid and verifycode = :verifycode ', array(':orderid' => $order['id'], ':openid' => $order['openid'], ':uniacid' => $order['uniacid'], ':verifycode' => $order['verifycode']));

			if ($order['verifytype'] == 0) {
				$verify = pdo_fetch('select isverify from ' . tablename('rr_v_groups_verify') . ' where orderid = :orderid and openid = :openid and uniacid = :uniacid and verifycode = :verifycode ', array(':orderid' => $order['id'], ':openid' => $order['openid'], ':uniacid' => $order['uniacid'], ':verifycode' => $order['verifycode']));
			}
	...
	public function express()
	{
		...
		$order = pdo_fetch('select * from ' . tablename('rr_v_groups_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
		...
		$goods = pdo_fetch('select *  from ' . tablename('rr_v_groups_goods') . '  where id=:id and uniacid=:uniacid ', array(':uniacid' => $uniacid, ':id' => $order['goodid']));
	...
	public function cancel()
	{
		...
		try {
			$orderid = intval($_GPC['id']);
			$order = pdo_fetch('select id,orderno,openid,status,credit,teamid,groupnum,creditmoney,price,freight,pay_type,discount,success from ' . tablename('rr_v_groups_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
			$total = pdo_fetchcolumn('select count(1) from ' . tablename('rr_v_groups_order') . '  where teamid = :teamid  ', array(':teamid' => $order['teamid']));
			...
			pdo_update('rr_v_groups_order', array('status' => -1, 'canceltime' => time()), array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
	...
	public function delete()
	{
		...
		global $_GPC;
		$orderid = intval($_GPC['id']);
		$order = pdo_fetch('select id,status from ' . tablename('rr_v_groups_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
		...
		pdo_update('rr_v_groups_order', array('deleted' => 1), array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
		show_json(1);
	}
	...
	public function get_list()
	{
		...
		$orders = pdo_fetchall("select o.id,o.orderno,o.createtime,o.price,o.freight,o.creditmoney,o.goodid,o.teamid,o.status,o.is_team,o.success,o.teamid,o.openid,\r\n\t\t\t\tg.title,g.thumb,g.units,g.goodsnum,g.groupsprice,g.singleprice,o.verifynum,o.verifytype,o.isverify,o.uniacid,o.verifycode\r\n\t\t\t\tfrom " . tablename('rr_v_groups_order') . " as o\r\n\t\t\t\tleft join " . tablename('rr_v_groups_goods') . " as g on g.id = o.goodid\r\n\t\t\t\twhere 1 " . $condition . ' order by o.createtime desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('select count(1) from ' . tablename('rr_v_groups_order') . ' as o where 1 ' . $condition, $params);

		foreach ($orders as $key => $value) {
			$verifytotal = pdo_fetchcolumn('select count(1) from ' . tablename('rr_v_groups_verify') . ' where orderid = :orderid and openid = :openid and uniacid = :uniacid and verifycode = :verifycode ', array(':orderid' => $value['id'], ':openid' => $value['openid'], ':uniacid' => $value['uniacid'], ':verifycode' => $value['verifycode']));
	...
	public function confirm()
	{
		...
		if (!empty($teamid)) {
			...
			$num = pdo_fetchcolumn('select count(1) from ' . tablename('rr_v_groups_order') . ' as o where teamid = :teamid and status > :status and goodid = :goodid and uniacid = :uniacid ', array(':teamid' => $teamid, ':status' => 0, ':goodid' => $goods['id'], ':uniacid' => $uniacid));
		...
		$set = pdo_fetch('select discount,headstype,headsmoney,headsdiscount from ' . tablename('rr_v_groups_set') . "\r\n\t\t\t\t\twhere uniacid = :uniacid ", array(':uniacid' => $uniacid));
		...
		if (!empty($goods['isverify'])) {
			...
			while (1) {
				$count = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_groups_order') . ' where verifycode=:verifycode and uniacid=:uniacid limit 1', array(':verifycode' => $verifycode, ':uniacid' => $_W['uniacid']));
			...
			$address = pdo_fetch('select * from ' . tablename('rr_v_member_address') . "\r\n\t\t\t\twhere openid=:openid and deleted=0 and isdefault=1  and uniacid=:uniacid limit 1", array(':uniacid' => $uniacid, ':openid' => $openid));
		}

		$creditdeduct = pdo_fetch('SELECT creditdeduct,groupsdeduct,credit,groupsmoney FROM' . tablename('rr_v_groups_set') . 'WHERE uniacid = :uniacid ', array(':uniacid' => $uniacid));
		...
		if ($_W['ispost']) {
			...
			if ((0 < intval($_GPC['aid'])) && !$isverify) {
				$order_address = pdo_fetch('select * from ' . tablename('rr_v_member_address') . ' where id=:id and openid=:openid and uniacid=:uniacid   limit 1', array(':uniacid' => $uniacid, ':openid' => $openid, ':id' => intval($_GPC['aid'])));
			...
			$order_insert = pdo_insert('rr_v_groups_order', $data);
			...
			if (empty($teamid) && ($type == 'groups')) {
				pdo_update('rr_v_groups_order', array('teamid' => $orderid), array('id' => $orderid));
			}
	...
	public function finish()
	{
		...
		global $_GPC;
		$orderid = intval($_GPC['id']);
		$order = pdo_fetch('select * from ' . tablename('rr_v_groups_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
		...
		if ((0 < $order['refundstate']) && !empty($order['refundid'])) {
			...
			pdo_update('rr_v_groups_order_refund', $change_refund, array('id' => $order['refundid'], 'uniacid' => $_W['uniacid']));
		}

		pdo_update('rr_v_groups_order', array('status' => 3, 'finishtime' => time(), 'refundstate' => 0), array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
		p('groups')->sendTeamMessage($orderid);
		show_json(1);
	}
}
