...
class Index_RrvV3Page extends PluginMobilePage
{
	public function main()
	{
		...
		if ($item['type'] == 0) {
			...
			$room = pdo_fetch('SELECT id,title,introduce,invitation_id FROM ' . tablename('rr_v_live') . ' where uniacid = :uniacid and id=:roomid ', array(':uniacid' => $_W['uniacid'], ':roomid' => $roomid));
	...
	protected function getTemp($item)
	{
		...
		foreach ($item['data']['selected'] as $tempid) {
			if (0 < intval($tempid)) {
				$templist[] = array('bg' => '../addons/rr_vv3/plugin/invitation/static/templist/image_' . $tempid . '_bg.jpg', 'nail' => '../addons/rr_vv3/plugin/invitation/static/templist/image_' . $tempid . '_nail.jpg');
			}
			else {
