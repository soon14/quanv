...
class Index_RrvV3Page extends PluginWebPage
{
	public function main()
	{
		...
		$plugin = pdo_fetch('select `desc` from ' . tablename('rr_v_plugin') . ' where `identity`=:identyty limit  1', array(':identyty' => 'live'));
		$livenum = pdo_fetchcolumn('SELECT count(0) FROM ' . tablename('rr_v_live') . 'WHERE uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
		$livingnum = pdo_fetchcolumn('SELECT count(0) FROM ' . tablename('rr_v_live') . 'WHERE uniacid=:uniacid AND living=1 ', array(':uniacid' => $_W['uniacid']));
		$liveprice = array();
