...
class Setting_RrvV3Page extends PluginWebPage
{
	public function main()
	{
		...
		$data = pdo_fetch('select * from ' . tablename('rr_v_live_setting') . ' where uniacid = ' . $uniacid . ' ');
		...
		if ($_W['ispost']) {
			...
			if (!empty($data)) {
				pdo_update('rr_v_live_setting', $set_data, array('uniacid' => $uniacid));
			}
			else {
				$set_data['uniacid'] = $uniacid;
				pdo_insert('rr_v_live_setting', $set_data);
			}
