...
class LiveModel extends PluginModel
{
	...
	public function selectOrderPrice($day = 0, $roomid = 0)
	{
		...
		$sql = 'select id,price,createtime from ' . tablename('rr_v_order') . ' where uniacid = :uniacid ' . $condition . ' and ismr=0 and isparent=0 and (status > 0 or ( status=0 and paytype=3)) and deleted=0 and createtime between :createtime1 and :createtime2';
	...
	public function getSet()
	{
		global $_W;
		$set = array();
		$set = pdo_fetch('select * from ' . tablename('rr_v_live_setting') . ' where uniacid = :uniacid  ', array(':uniacid' => $_W['uniacid']));
		$plugin = pdo_fetch('select `name` from ' . tablename('rr_v_plugin') . ' where `identity` = \'live\' and isv2 = 1 and status = 1 ');
	...
	public function sendLiveMessage($openid, $type, $roomid)
	{
		...
		$room = pdo_fetch('select * from ' . tablename('rr_v_live') . ' where uniacid = ' . $_W['uniacid'] . ' and id = ' . $roomid . ' ');
		...
		if ($type == 'livefollow') {
			...
			if ($type == 'liveroom') {
				...
				$url = str_replace('addons/rr_vv3/plugin/live/task/', '', $_W['siteroot']);
				$url = $url . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=rr_vv3&do=mobile&r=live.room&id=' . $room['id'];
				$tag = 'livefollow';
				$remark = "\n<a href='" . $url . '\'>点击查看详情</a>';
	...
	public function isLiving($roomid = 0)
	{
		...
		if (!empty($roomid)) {
			$live = pdo_fetch('SELECT id FROM ' . tablename('rr_v_live') . ' WHERE id=:id AND uniacid=:uniacid AND status=1 AND living=1 LIMIT 1', array(':id' => $roomid, ':uniacid' => $_W['uniacid']));
	...
	public function isFavorite($openid = NULL, $roomid = 0)
	{
		...
		if (!empty($openid) && !empty($roomid)) {
			$favorite = pdo_fetch('SELECT id FROM ' . tablename('rr_v_live_favorite') . 'WHERE uniacid=:uniacid AND roomid=:roomid AND openid=:openid AND `deleted`=0 LIMIT 1', array(':uniacid' => $_W['uniacid'], ':roomid' => $roomid, ':openid' => $openid));
	...
	public function emoji2html($str)
	{
		...
		foreach ($emojiList as $index => $emoji) {
			if ($emoji == $str) {
				return '<img class="face" src="../addons/rr_vv3/plugin/live/static/images/face/' . $index . '.gif?v=2" />';
			}
		}
	...
	public function handleRecords($roomid = 0, $manage = false)
	{
		...
		if ($manage) {
			$table_banned = 'rr_v_live_banned_' . $roomid;
			$bannedArr = array();
		}
	...
	{
		return 'rr_v_live_' . $table . '_' . $roomid;
	}
	...
	public function isLiveGoods($goodsid = 0, $liveid = 0)
	{
		...
		$livegoods = pdo_fetch('SELECT * FROM ' . tablename('rr_v_live_goods') . ' WHERE goodsid=:goodsid AND liveid=:liveid AND uniacid=:uniacid', array(':goodsid' => $goodsid, ':liveid' => $liveid, ':uniacid' => $_W['uniacid']));
	...
	public function getLiveOptions($goodsid = 0, $liveid = 0, $options = array())
	{
		...
		if (empty($options)) {
			$options = pdo_fetchall('SELECT id, marketprice FROM ' . tablename('rr_v_goods_option') . ' WHERE goodsid=:goodsid AND uniacid=:uniacid ORDER BY displayorder ASC', array(':goodsid' => $goodsid, ':uniacid' => $_W['uniacid']));
		}
		...
		$liveOptions = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_live_goods_option') . ' WHERE goodsid=:goodsid AND liveid=:liveid AND uniacid=:uniacid', array(':goodsid' => $goodsid, ':liveid' => $liveid, ':uniacid' => $_W['uniacid']), 'optionid');
	...
	public function getAllGoods($liveid = 0)
	{
		...
		if (!empty($liveid)) {
			$goodslist = pdo_fetchall('SELECT lg.*,sg.id as id,lg.id as livegid,sg.marketprice,sg.thumb,sg.title FROM ' . tablename('rr_v_live_goods') . ' lg LEFT JOIN ' . tablename('rr_v_goods') . ' sg ON sg.id=lg.goodsid WHERE lg.liveid=:liveid AND lg.uniacid=:uniacid', array(':liveid' => $liveid, ':uniacid' => $_W['uniacid']));
		}
	...
	{
		$file = rr_vv3_CORE . 'socket/socket.config.php';
