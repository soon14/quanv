...
class Follow_RrvV3Page extends PluginMobileLoginPage
{
	...
	public function get_list()
	{
		...
		$sql = 'SELECT COUNT(1) FROM ' . tablename('rr_v_live_favorite') . " as lf\r\n                    right join " . tablename('rr_v_live') . ' as l on l.id = lf.roomid and l.living = ' . $living . "\r\n            \t\twhere 1 " . $condition;
		$total = pdo_fetchcolumn($sql, $params);
		$list = array();

		if (!empty($total)) {
			$sql = 'SELECT lf.id,lf.roomid,l.title,l.thumb,l.livetime,l.subscribe,l.living,l.covertype,l.cover FROM ' . tablename('rr_v_live_favorite') . " as lf\r\n                    right join " . tablename('rr_v_live') . ' as l on l.id = lf.roomid and l.living = ' . $living . "\r\n            \t\twhere 1 " . $condition . ' ORDER BY lf.id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
