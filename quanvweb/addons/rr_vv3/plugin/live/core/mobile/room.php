...
class Room_RrvV3Page extends PluginMobileLoginPage
{
	public function main()
	{
		...
		if (p('invitation') && (0 < $invitation_id)) {
			...
			pdo_insert('rr_v_invitation_log', $invitation_data);
			pdo_query('update ' . tablename('rr_v_invitation') . ' set scan = scan+1 where id = ' . $invitation_id . ' and uniacid = ' . $uniacid . ' ');
		}
		...
		$room = pdo_fetch('SELECT * FROM ' . tablename('rr_v_live') . ' where uniacid = :uniacid and id = :roomid ', array(':uniacid' => $uniacid, ':roomid' => $roomid));
		...
		if (!empty($room['couponid'])) {
			$coupon = true;
			$room_coupon = pdo_fetchall('select lc.*, sc.couponname, sc.coupontype,sc.backtype ,sc.enough,sc.deduct from ' . tablename('rr_v_live_coupon') . ' lc left join ' . tablename('rr_v_coupon') . ' sc on sc.id=lc.couponid where lc.uniacid=:uniacid and lc.roomid=:roomid ', array(':uniacid' => $_W['uniacid'], ':roomid' => $roomid));
		...
		if (!empty($video) && ($room['livetype'] != 2)) {
			...
			if (!is_error($video_info) && !empty($video_info['hls_url'])) {
				...
				pdo_update('rr_v_live', array('thumb' => $video_info['poster']), array('uniacid' => $uniacid, 'id' => $roomid));
			}
			else {
				$video = '';
			}
		...
		pdo_update('rr_v_live', array('visit' => $room['visit'] + 1), array('uniacid' => $uniacid, 'id' => $roomid));
		$view = pdo_fetch('select * from ' . tablename('rr_v_live_view') . ' where uniacid = ' . $uniacid . ' and openid = \'' . $openid . '\' and roomid = ' . $roomid . ' ');
		$viewing = pdo_fetch('select max(viewing) as viewing from ' . tablename('rr_v_live_view') . ' where uniacid = ' . $uniacid . ' and openid = \'' . $openid . '\' ');

		if (!empty($view)) {
			pdo_update('rr_v_live_view', array('viewing' => $viewing['viewing'] + 1), array('uniacid' => $uniacid, 'openid' => $openid, 'roomid' => $roomid));
		}
		else {
			$view_data = array('uniacid' => $uniacid, 'openid' => $openid, 'roomid' => $roomid);
			$view_data['viewing'] = !empty($viewing) ? $viewing['viewing'] + 1 : 1;
			pdo_insert('rr_v_live_view', $view_data);
		}

		$sysset = m('common')->getSysset();
		$shop = set_medias($sysset['shop'], 'logo');
		$setting = pdo_fetch('select * from ' . tablename('rr_v_live_setting') . ' where uniacid = :uniacid  ', array(':uniacid' => $uniacid));
	...
	public function jurisdiction()
	{
		...
		$room = pdo_fetch('SELECT jurisdictionurl_show,jurisdiction_url FROM ' . tablename('rr_v_live') . ' where id = :roomid and uniacid = :uniacid ', array(':uniacid' => $uniacid, ':roomid' => $roomid));
	...
	public function favorite()
	{
		...
		if (!empty($roomid)) {
			$favorite = pdo_fetch('SELECT * FROM ' . tablename('rr_v_live_favorite') . 'WHERE uniacid=:uniacid AND roomid=:roomid AND openid=:openid LIMIT 1', array(':uniacid' => $_W['uniacid'], ':roomid' => $roomid, ':openid' => $_W['openid']));

			if (empty($favorite)) {
				pdo_insert('rr_v_live_favorite', array('uniacid' => $_W['uniacid'], 'roomid' => $roomid, 'openid' => $_W['openid'], 'createtime' => time()));
				pdo_query('update ' . tablename('rr_v_live') . ' set subscribe = subscribe+1 where id = ' . $roomid . ' and uniacid = ' . intval($_W['uniacid']) . ' ');
				show_json(1, array('favorite' => 1));
			}
			else {
				pdo_update('rr_v_live_favorite', array('deleted' => !empty($favorite['deleted']) ? 0 : 1, 'createtime' => time()), array('id' => $favorite['id']));
	...
	public function roomcoupon()
	{
		...
		if (empty($couponset)) {
			$room_coupon = pdo_fetch('select lc.couponid,lc.coupontotal,lc.couponlimit,l.living,l.iscoupon,c.couponname from ' . tablename('rr_v_live_coupon') . " lc\r\n                    left join " . tablename('rr_v_live') . " as l on l.id = lc.roomid\r\n                    left join " . tablename('rr_v_coupon') . " as c on c.id = lc.couponid\r\n                    where lc.roomid = :roomid and lc.couponid = :couponid and lc.uniacid = :uniacid ", array(':uniacid' => $uniacid, ':roomid' => $roomid, ':couponid' => $couponid));
		...
		pdo_query('update ' . tablename('rr_v_live') . ' set coupon_num = coupon_num+1 where id= ' . $roomid . ' and uniacid = ' . $uniacid . ' ');
		show_json(1);
	}
	...
	public function roompacket()
	{
		...
		if (empty($packetset)) {
			$packet = pdo_fetch('select title,packetmoney,packettotal,packetprice,living,livetime from ' . tablename('rr_v_live') . ' where id = :roomid and status = 1 and uniacid = :uniacid ', array(':uniacid' => $uniacid, ':roomid' => $roomid));
