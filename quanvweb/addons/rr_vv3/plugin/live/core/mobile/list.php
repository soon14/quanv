...
class List_RrvV3Page extends PluginMobileLoginPage
{
	public function main()
	{
		...
		$categorys = pdo_fetchall('select * from ' . tablename('rr_v_live_category') . ' where uniacid = ' . $uniacid . ' and enabled = 1 ');
	...
	public function get_list()
	{
		...
		if (!empty($total)) {
			$sql = 'SELECT id,title,thumb,livetime,covertype,cover,subscribe,living FROM ' . tablename('rr_v_live') . "\r\n            \t\twhere 1 " . $condition . ' ORDER BY displayorder desc,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
			...
			foreach ($list as $key => &$row) {
				...
				$favorite = pdo_fetch('select deleted from ' . tablename('rr_v_live_favorite') . ' where uniacid = ' . $uniacid . ' and openid = \'' . $openid . '\' and roomid = ' . $row['id'] . ' and deleted = 0  ');
