...
class Pay_RrvV3Page extends CashierMobilePage 
{
	public $log;
	...
	public function main() 
	{
		...
		{
			$id = $_GPC['id'];
			$qrid = authcode(base64_decode($id), 'DECODE', 'rr_vv3_cashier');
			if (!(empty($qrid))) 
			{
				$item = pdo_get('rr_v_cashier_qrcode', array('uniacid' => $_W['uniacid'], 'cashierid' => $_W['cashierid'], 'id' => $qrid));
	...
	protected function paytype1() 
	{
		...
		if (is_array($config)) 
		{
			$config['return_url'] = $_W['siteroot'] . 'addons/rr_vv3/payment/alipay/return.php';
		}
		else 
		{
			show_json(0);
			...
			if (!(empty($log['coupon']))) 
			{
				...
				if (!(empty($coupon))) 
				{
					...
					else if ($coupon['backtype'] == 2) 
					{
						...
						else if (!(empty($coupon['backcredit']))) 
						{
							$coupon['backstr'] = '返积分';
							$coupon['css'] = 'credit';
	...
	public function getcoupon() 
	{
		...
		show_json(0, array( 'coupons' => array() ));
	}
}
?>