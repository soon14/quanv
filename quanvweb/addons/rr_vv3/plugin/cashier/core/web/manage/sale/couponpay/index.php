...
class Index_RrvV3Page extends CashierWebPage {
    ...
    public function main()
    {
        ...
        if (!empty($item['goodsids'])){
            $goods = pdo_fetchall('select id,title,thumb from '.tablename('rr_v_goods')." where id in ({$item['goodsids']}) and cashier=1 and deleted=0 and uniacid={$_W['uniacid']}");
        }
        $goods1 = array();
        if (!empty($item['goodsids1'])){
            $goods1 = pdo_fetchall('select id,title,thumb from '.tablename('rr_v_goods')." where id in ({$item['goodsids']}) and cashier=1 and deleted=0 and uniacid={$_W['uniacid']}");
        }
        $goods2 = array();
        if (!empty($item['goodsids2'])){
            $goods2 = pdo_fetchall('select id,title,thumb from '.tablename('rr_v_goods')." where id in ({$item['goodsids']}) and cashier=1 and deleted=0 and uniacid={$_W['uniacid']}");
        }
