...
class Index_RrvV3Page extends CashierWebPage 
{
	public function main() 
	{
		...
		if (!(empty($year)) && !(empty($month)) && !(empty($day))) 
		{
			...
			{
				$nexthour = $hour + 1;
				$dr = array('data' => $hour . '点 - ' . $nexthour . '点', 'count' => pdo_fetchcolumn('SELECT ifnull(' . $countfield . ',0) as cnt FROM ' . tablename('rr_v_cashier_pay_log') . ' WHERE cashierid=:cashierid AND uniacid=:uniacid AND status>=1 AND createtime >=:starttime AND createtime <=:endtime', array(':cashierid' => $_W['cashierid'], ':uniacid' => $_W['uniacid'], ':starttime' => strtotime($year . '-' . $month . '-' . $day . ' ' . $hour . ':00:00'), ':endtime' => strtotime($year . '-' . $month . '-' . $day . ' ' . $hour . ':59:59'))));
				$totalcount += $dr['count'];
		...
		else if (!(empty($year)) && !(empty($month))) 
		{
			...
			while ($d <= $lastday) 
			{
				$dr = array('data' => $d, 'count' => pdo_fetchcolumn('SELECT ifnull(' . $countfield . ',0) as cnt FROM ' . tablename('rr_v_cashier_pay_log') . ' WHERE cashierid=:cashierid AND uniacid=:uniacid AND status>=1 AND createtime >=:starttime AND createtime <=:endtime', array(':cashierid' => $_W['cashierid'], ':uniacid' => $_W['uniacid'], ':starttime' => strtotime($year . '-' . $month . '-' . $d . ' 00:00:00'), ':endtime' => strtotime($year . '-' . $month . '-' . $d . ' 23:59:59'))));
				$totalcount += $dr['count'];
				if ($maxcount < $dr['count']) 
				{
					$maxcount = $dr['count'];
