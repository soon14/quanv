...
if (!class_exists('GlobonusModel')) {
	class GlobonusModel extends PluginModel
	{
		...
		public function getLevels($all = true, $default = false)
		{
			...
			if ($all) {
				$levels = pdo_fetchall('select * from ' . tablename('rr_v_globonus_level') . ' where uniacid=:uniacid order by bonus asc', array(':uniacid' => $_W['uniacid']));
			}
			else {
				$levels = pdo_fetchall('select * from ' . tablename('rr_v_globonus_level') . ' where uniacid=:uniacid and (ordermoney>0 or commissionmoney>0 or bonusmoney>0) order by bonus asc', array(':uniacid' => $_W['uniacid']));
			}
		...
		public function getBonus($openid = '', $params = array())
		{
			...
			if (in_array('ok', $params)) {
				$ret['ok'] = pdo_fetchcolumn('select ifnull(sum(paymoney),0) from ' . tablename('rr_v_globonus_billp') . ' where openid=:openid and status=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}

			if (in_array('lock', $params)) {
				$billdData = pdo_fetchall('select id from ' . tablename('rr_v_globonus_bill') . ' where 1 and uniacid = ' . intval($_W['uniacid']));
				$id = '';
				...
				if (!empty($billdData)) {
					...
					$id = implode(',', $ids);
					$ret['lock'] = pdo_fetchcolumn('select ifnull(sum(paymoney),0) from ' . tablename('rr_v_globonus_billp') . ' where openid=:openid and status<>1 and uniacid=:uniacid  and billid in(' . $id . ') limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
				}
			}
		...
		public function getLevel($openid)
		{
			...
			$level = pdo_fetch('select * from ' . tablename('rr_v_globonus_level') . ' where uniacid=:uniacid and id=:id limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $member['partnerlevel']));
			return $level;
		}
		...
		public function upgradeLevelByOrder($openid)
		{
			...
			if (($leveltype == 4) || ($leveltype == 5)) {
				...
				if ($leveltype == 4) {
					$newlevel = pdo_fetch('select * from ' . tablename('rr_v_globonus_level') . ' where uniacid=:uniacid  and ' . $ordermoney . ' >= ordermoney and ordermoney>0  order by ordermoney desc limit 1', array(':uniacid' => $_W['uniacid']));
					...
					if ($leveltype == 5) {
						$newlevel = pdo_fetch('select * from ' . tablename('rr_v_globonus_level') . ' where uniacid=:uniacid  and ' . $ordercount . ' >= ordercount and ordercount>0  order by ordercount desc limit 1', array(':uniacid' => $_W['uniacid']));
				...
				pdo_update('rr_v_member', array('partnerlevel' => $newlevel['id']), array('id' => $m['id']));
				...
				if ((0 <= $leveltype) && ($leveltype <= 3)) {
					...
					foreach ($agents as $agent) {
						...
						pdo_update('rr_v_member', array('partnerlevel' => $newlevel['id']), array('id' => $agent['id']));
			...
			if (($leveltype == 6) || ($leveltype == 8)) {
				...
				foreach ($agents as $agent) {
					...
					if ($leveltype == 6) {
						...
						$newlevel = pdo_fetch('select * from ' . tablename('rr_v_globonus_level') . ' where uniacid=:uniacid  and ' . $downcount . ' >= downcount and downcount>0  order by downcount desc limit 1', array(':uniacid' => $_W['uniacid']));
					}
					else {
						if ($leveltype == 8) {
		...
		public function upgradeLevelByCommissionOK($openid)
		{
			...
			$newlevel = pdo_fetch('select * from ' . tablename('rr_v_globonus_level') . ' where uniacid=:uniacid  and ' . $commissionmoney . ' >= commissionmoney and commissionmoney>0  order by commissionmoney desc limit 1', array(':uniacid' => $_W['uniacid']));
		...
		public function upgradeLevelByBonus($openid)
		{
			...
			$newlevel = pdo_fetch('select * from ' . tablename('rr_v_globonus_level') . ' where uniacid=:uniacid  and ' . $bonusmoney['ok'] . ' >= bonusmoney and bonusmoney>0  order by bonusmoney desc limit 1', array(':uniacid' => $_W['uniacid']));
		...
		public function getBonusData($year = 0, $month = 0, $week = 0, $openid = '')
		{
			...
			$bill = pdo_fetch('select * from ' . tablename('rr_v_globonus_bill') . ' where uniacid=:uniacid and `year`=:year and `month`=:month and `week`=:week limit 1', array(':uniacid' => $_W['uniacid'], ':year' => $year, ':month' => $month, ':week' => $week));
			...
			$orders = pdo_fetchall('select id,openid,price from ' . tablename('rr_v_order') . ' where uniacid=' . $_W['uniacid'] . ' and status=3 and isglobonus=0 and finishtime + ' . $settletimes . '>= ' . $starttime . ' and  finishtime + ' . $settletimes . '<=' . $endtime . ' ' . $pcondition, array(), 'id');
			$pcondition = '';
			...
			$partners = pdo_fetchall('select m.id,m.openid,m.partnerlevel,l.bonus from ' . tablename('rr_v_member') . ' m ' . '  left join ' . tablename('rr_v_globonus_level') . ' l on l.id = m.partnerlevel ' . '  where m.uniacid=:uniacid and  m.ispartner=1 and m.partnerstatus=1 ' . $pcondition, array(':uniacid' => $_W['uniacid']));
		...
		{
			global $_W;
			return array('total0' => pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_globonus_bill') . ' where uniacid=:uniacid and status=0 limit 1', array(':uniacid' => $_W['uniacid'])), 'total1' => pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_globonus_bill') . ' where uniacid=:uniacid and status=1 limit 1', array(':uniacid' => $_W['uniacid'])), 'total2' => pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_globonus_bill') . ' where uniacid=:uniacid and status=2  limit 1', array(':uniacid' => $_W['uniacid'])));
		}
	}
}
