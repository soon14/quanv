...
if (!class_exists('CreditshopModel')) {
	class CreditshopModel extends PluginModel
	{
		...
		public function payResult($logno, $type, $total_fee, $app = false)
		{
			...
			if (!empty($log) && ($log['paystatus'] < 1)) {
				if (!empty($goods) && ($total_fee == ($goods['money'] + $goods['dispatch']))) {
					pdo_update('rr_v_creditshop_log', $record, array('id' => $log['id']));
				}
			}
		}
		...
		public function packetmoney($goodsid)
		{
			...
			$uniacid = $_W['uniacid'];
			$money = 0;
			$goods = pdo_fetch('select * from ' . tablename('rr_v_creditshop_goods') . ' where id = ' . $goodsid . ' and uniacid = ' . $uniacid . ' ');
			$size = pdo_fetchcolumn('select count(1) from ' . tablename('rr_v_creditshop_log') . ' where goodsid = ' . $goodsid . ' and uniacid = ' . $uniacid . ' and status = 2 ');
		...
		public function getGoods($id, $member, $optionid = 0)
		{
			...
			$goods = pdo_fetch('select * from ' . tablename('rr_v_creditshop_goods') . ' where id=:id ' . $condition . ' limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
			...
			if (empty($goods['canbuy'])) {
				...
				if ($goods['goodstype'] == 3) {
					...
					$logcount = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_creditshop_log') . '  where goodsid=:goodsid and status>=2  and uniacid=:uniacid  ', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
					$goods['logcount'] = $logcount;

					if ($goods['joins'] < $logcount) {
						pdo_update('rr_v_creditshop_goods', array('joins' => $logcount), array('id' => $id));
					}
				}
				else {
					$goods['canbuy'] = false;
				...
				if ($goods['hasoption'] && $optionid) {
					$option = pdo_fetch('select total,credit,money,title as optiontitle,weight from ' . tablename('rr_v_creditshop_option') . ' where uniacid = ' . $_W['uniacid'] . ' and id = ' . $optionid . ' and goodsid = ' . $id . ' ');
				...
				if ($goods['canbuy']) {
					if (0 < $goods['totalday']) {
						$logcount = pdo_fetchcolumn('select count(*)  from ' . tablename('rr_v_creditshop_log') . '  where goodsid=:goodsid and status>=2 and  date_format(from_UNIXTIME(`createtime`),\'%Y-%m-%d\') = date_format(now(),\'%Y-%m-%d\') and uniacid=:uniacid  ', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
				...
				if ($goods['canbuy']) {
					if (0 < $goods['chanceday']) {
						$logcount = pdo_fetchcolumn('select count(*)  from ' . tablename('rr_v_creditshop_log') . '  where goodsid=:goodsid and openid=:openid and status>0 and  date_format(from_UNIXTIME(`createtime`),\'%Y-%m-%d\') = date_format(now(),\'%Y-%m-%d\') and uniacid=:uniacid  ', array(':goodsid' => $id, ':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
				...
				if ($goods['canbuy']) {
					if (0 < $goods['chance']) {
						$logcount = pdo_fetchcolumn('select count(*)  from ' . tablename('rr_v_creditshop_log') . '  where goodsid=:goodsid and openid=:openid and status>0 and  uniacid=:uniacid  ', array(':goodsid' => $id, ':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
				...
				if ($goods['canbuy']) {
					if (0 < $goods['usermaxbuy']) {
						$logcount = pdo_fetchcolumn('select ifnull(sum(total),0)  from ' . tablename('rr_v_creditshop_log') . '  where goodsid=:goodsid and openid=:openid  and uniacid=:uniacid ', array(':goodsid' => $id, ':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
		...
		{
			global $_W;
			$ecount = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_creditshop_log') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
			...
			while (1) {
				$c = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_creditshop_log') . ' where uniacid=:uniacid and eno=:eno limit 1', array(':uniacid' => $_W['uniacid'], ':eno' => $eno));
		...
		public function sendMessage($id = 0)
		{
			...
			$log = pdo_fetch('select * from ' . tablename('rr_v_creditshop_log') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
			...
			if (strexists($detailurl, '/addons/rr_v/')) {
				$detailurl = str_replace('/addons/rr_v/', '/', $detailurl);
			}
			...
			if ($log['status'] == 2) {
				...
				if (($log['dispatchstatus'] == 1) || ($log['dispatchstatus'] == -1)) {
					...
					if (!empty($log['addressid'])) {
						$address = pdo_fetch('select id,realname,mobile,address,province,city,area from ' . tablename('rr_v_member_address') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log['addressid'], ':uniacid' => $_W['uniacid']));
				...
				if ($log['status'] == 3) {
					...
					if (strexists($detailurl1, '/addons/rr_v/')) {
						$detailurl1 = str_replace('/addons/rr_v/', '/', $detailurl1);
					}
		...
		public function createQrcode($logid = 0)
		{
			global $_W;
			global $_GPC;
			$path = IA_ROOT . '/addons/rr_vv3/data/creditshop/' . $_W['uniacid'];
			...
			return $_W['siteroot'] . '/addons/rr_vv3/data/creditshop/' . $_W['uniacid'] . '/' . $file;
		}
	...
	'creditshop' => array(
		...
		public function allow($logid, $times = 0, $verifycode = '', $openid = '')
		{
			...
			$goods = pdo_fetch('select * from ' . tablename('rr_v_creditshop_goods') . ' where uniacid=:uniacid and id = :goodsid ', array(':uniacid' => $uniacid, ':goodsid' => $log['goodsid']));
			...
			if (empty($merchid)) {
				...
				if ($merch_plugin) {
					$saler = pdo_fetch('select * from ' . tablename('rr_v_merch_saler') . ' where openid=:openid and uniacid=:uniacid and merchid=:merchid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid, ':merchid' => $merchid));
				}
			}
			...
			if ($goods['verifytype'] == 0) {
				$verifynum = pdo_fetchcolumn('select COUNT(1) from ' . tablename('rr_v_creditshop_verify') . ' where uniacid = :uniacid and logid = :logid ', array(':uniacid' => $uniacid, ':logid' => $logid));
			...
			if (!empty($saler['storeid'])) {
				if (0 < $merchid) {
					$stores = pdo_fetch('select * from ' . tablename('rr_v_merch_store') . ' where id=:id and uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3)', array(':id' => $saler['storeid'], ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
				}
				else {
					$stores = pdo_fetch('select * from ' . tablename('rr_v_store') . ' where id=:id and uniacid=:uniacid and status=1 and type in(2,3)', array(':id' => $saler['storeid'], ':uniacid' => $_W['uniacid']));
				}
			}
		...
		public function verify($logid = 0, $times = 0, $verifycode = '', $openid = '')
		{
			...
			if ($goods['isverify']) {
				if ($goods['verifytype'] == 0) {
					pdo_update('rr_v_creditshop_log', array('status' => 3, 'verifytime' => time(), 'verifyopenid' => $openid, 'time_finish' => time()), array('id' => $logid));
					...
					pdo_insert('rr_v_creditshop_verify', $data);
				}
				else {
					if ($goods['verifytype'] == 1) {
						if ($log['status'] != 3) {
							pdo_update('rr_v_creditshop_log', array('status' => 3, 'usetime' => time(), 'verifyopenid' => $openid, 'time_finish' => time()), array('id' => $logid));
						}
