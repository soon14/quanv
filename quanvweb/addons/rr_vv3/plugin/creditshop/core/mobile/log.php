...
class Log_RrvV3Page extends PluginMobileLoginPage
{
	...
	public function getlist()
	{
		...
		$sql = 'SELECT COUNT(*) FROM ' . tablename('rr_v_creditshop_log') . " log\r\n                left join " . tablename('rr_v_creditshop_goods') . " g on log.goodsid = g.id\r\n                where 1 " . $condition;
		$total = pdo_fetchcolumn($sql, $params);
		$list = array();

		if (!empty($total)) {
			$sql = 'SELECT log.id,log.logno,log.goodsid,log.status,log.eno,log.paystatus,g.title,g.type,g.thumb,g.credit,g.money,g.isverify,g.goodstype,log.addressid,log.storeid,' . 'g.goodstype,log.time_send,log.time_finish,log.iscomment,op.title as optiontitleg,g.merchid ' . ' FROM ' . tablename('rr_v_creditshop_log') . ' log ' . ' left join ' . tablename('rr_v_creditshop_goods') . ' g on log.goodsid = g.id ' . ' left join ' . tablename('rr_v_creditshop_option') . ' op on op.id = log.optionid ' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
	...
	public function detail()
	{
		...
		$id = intval($_GPC['id']);
		$log = pdo_fetch('select * from ' . tablename('rr_v_creditshop_log') . ' where id=:id and openid=:openid ' . $condition . ' limit 1', array(':id' => $id, ':openid' => $openid, ':uniacid' => $uniacid));
		...
		if (!empty($log['addressid'])) {
			$address = pdo_fetch('select * from ' . tablename('rr_v_member_address') . ' where id=:id and openid=:openid and uniacid=:uniacid limit 1', array(':id' => $log['addressid'], ':uniacid' => $uniacid, ':openid' => $openid));
		...
		if (!empty($goods['isverify'])) {
			$verifytotal = pdo_fetchcolumn('select count(1) from ' . tablename('rr_v_creditshop_verify') . ' where logid = :id and openid=:openid ' . $condition . ' and verifycode = :verifycode ', array(':id' => $id, ':openid' => $log['openid'], ':uniacid' => $log['uniacid'], ':verifycode' => $log['eno']));

			if ($goods['verifytype'] == 0) {
				$verify = pdo_fetch('select isverify from ' . tablename('rr_v_creditshop_verify') . ' where logid = :id and openid=:openid ' . $condition . ' and verifycode = :verifycode ', array(':id' => $log['id'], ':openid' => $log['openid'], ':uniacid' => $log['uniacid'], ':verifycode' => $log['eno']));
			}
	...
	public function Receivepacket()
	{
		...
		$log = pdo_fetch('select * from ' . tablename('rr_v_creditshop_log') . ' where id = ' . $logid . ' ' . $condition . ' ');
		...
		$goods = pdo_fetch('select surplusmoney from ' . tablename('rr_v_creditshop_goods') . ' where id = ' . $log['goodsid'] . ' ' . $condition . ' ');
		...
		if (is_error($err)) {
			...
			pdo_update('rr_v_creditshop_goods', $updategoods, array('id' => $log['goodsid']));
		}
	...
	public function express()
	{
		...
		$log = pdo_fetch('select * from ' . tablename('rr_v_creditshop_log') . ' where id=:logid ' . $condition . ' and openid=:openid limit 1', array(':logid' => $logid, ':uniacid' => $uniacid, ':openid' => $openid));
		...
		$goods = pdo_fetch('select *  from ' . tablename('rr_v_creditshop_goods') . '  where id=:id ' . $condition . ' ', array(':uniacid' => $uniacid, ':id' => $log['goodsid']));
	...
	public function finish()
	{
		...
		$log = pdo_fetch('select * from ' . tablename('rr_v_creditshop_log') . ' where id=:id ' . $condition . ' and openid=:openid limit 1', array(':id' => $logid, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
		...
		pdo_update('rr_v_creditshop_log', array('time_finish' => time()), array('id' => $logid, 'uniacid' => $_W['uniacid']));
		show_json(1);
	}
	...
	public function paydispatch()
	{
		...
		if ($goods['dispatch'] <= 0) {
			pdo_update('rr_v_creditshop_log', array('dispatchstatus' => 1, 'addressid' => $addressid), array('id' => $log['id']));
		...
		if ($paytype == 'wechat') {
			...
			if (empty($dispatchno)) {
				...
				pdo_update('rr_v_creditshop_log', array('dispatchno' => $dispatchno, 'addressid' => $addressid), array('id' => $log['id']));
			}
	...
	public function dispatch_complete()
	{
		...
		if ($lastlog['dispatchstatus'] < 1) {
			...
			pdo_update('rr_v_creditshop_log', $record, array('dispatchno' => $tid));
	...
	public function wechat_dispatch_complete()
	{
		...
		if (empty($log)) {
			...
			$log = pdo_fetch('select * from ' . tablename('rr_v_creditshop_log') . ' where dispatchno=:logno and uniacid=:uniacid limit 1', array(':logno' => $logno, ':uniacid' => $_W['uniacid']));
		}
		...
		if (!empty($log)) {
			...
			if (!is_error($payquery) || !is_error($payqueryborrow)) {
				if ($log['dispatchstatus'] < 1) {
					$record = array();
					$record['dispatchstatus'] = '1';
					pdo_update('rr_v_creditshop_log', $record, array('dispatchno' => $log['dispatchno']));
					...
					if (is_h5app()) {
						pdo_update('rr_v_creditshop_log', array('apppay' => 1), array('dispatchno' => $log['dispatchno']));
					}
				}
	...
	public function setstore()
	{
		...
		if (!empty($upgrade)) {
			pdo_update('rr_v_creditshop_log', $upgrade, array('id' => $log['id']));
		}

		if (!empty($upgrade1)) {
			pdo_update('rr_v_member', $upgrade1, array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
