...
require rr_vv3_PLUGIN . 'creditshop/core/page_mobile.php';
class Detail_RrvV3Page extends CreditshopMobilePage
{
	public function main()
	{
		...
		if (!empty($goods)) {
			pdo_update('rr_v_creditshop_goods', array('views' => $goods['views'] + 1), array('id' => $id));
		...
		$logtotal = 0;
		$logtotal = pdo_fetchcolumn('select count(1) from ' . tablename('rr_v_creditshop_log') . ' where uniacid = ' . $uniacid . ' and goodsid = ' . $id . ' and status > 0 ');
		$replys = array();
		$replys = pdo_fetchall('select * from ' . tablename('rr_v_creditshop_comment') . "\r\n                where uniacid = " . $uniacid . ' and goodsid = ' . $id . ' and checked = 1 and deleted = 0 order by `time` desc limit 2 ');
	...
	public function getlistlog()
	{
		...
		$logtotal = 0;
		$logtotal = pdo_fetchcolumn('select count(1) from ' . tablename('rr_v_creditshop_log') . ' where uniacid = ' . $uniacid . ' and goodsid = ' . $goodsid . ' and status > 0 ');
	...
	public function option()
	{
		...
		$goods = pdo_fetch('select id,thumb,credit,money,total,title from ' . tablename('rr_v_creditshop_goods') . ' where id= ' . $id . ' and uniacid = ' . $uniacid . ' ');
		$goods = set_medias($goods, 'thumb');
		$specs = false;
		$options = false;
		$specs = pdo_fetchall('select * from ' . tablename('rr_v_creditshop_spec') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));

		foreach ($specs as &$spec) {
			$spec['items'] = pdo_fetchall('select * from ' . tablename('rr_v_creditshop_spec_item') . ' where specid=:specid and `show`=1 order by displayorder asc', array(':specid' => $spec['id']));
		}

		unset($spec);
		$options = pdo_fetchall('select * from ' . tablename('rr_v_creditshop_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function pay($a = array(), $b = array())
	{
		...
		if ($goods['hasoption'] && $optionid) {
			$option = pdo_fetch('select total from ' . tablename('rr_v_creditshop_option') . ' where uniacid = ' . $uniacid . ' and id = ' . $optionid . ' and goodsid = ' . $id . ' ');
		...
		if ((0 < $goods['money']) || $goods['dispatch']) {
			pdo_delete('rr_v_creditshop_log', array('goodsid' => $id, 'openid' => $openid, 'status' => 0, 'paystatus' => 0));
			$needpay = true;
			$lastlog = pdo_fetch('select * from ' . tablename('rr_v_creditshop_log') . ' where goodsid=:goodsid and openid=:openid  and status=0 and paystatus=1 and uniacid=:uniacid limit 1', array(':goodsid' => $id, ':openid' => $openid, ':uniacid' => $uniacid));
			...
			pdo_delete('rr_v_creditshop_log', array('goodsid' => $id, 'openid' => $openid, 'status' => 0));
		}
		...
		if (!empty($addressid)) {
			$address = pdo_fetch('select id,realname,mobile,address,province,city,area from ' . tablename('rr_v_member_address') . "\r\n            where id=:id and uniacid=:uniacid limit 1", array(':id' => $addressid, ':uniacid' => $_W['uniacid']));
		...
		pdo_insert('rr_v_creditshop_log', $log);
		$logid = pdo_insertid();
		...
		if ($needpay) {
			...
			if ($paytype == 'balance') {
				...
				pdo_update('rr_v_creditshop_log', array('paytype' => $paystatus), array('id' => $logid));
			}
	...
	public function wechat_complete()
	{
		...
		$log = pdo_fetch('select * from ' . tablename('rr_v_creditshop_log') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $logid, ':uniacid' => $_W['uniacid']));

		if (empty($log)) {
			$logno = intval($_GPC['logno']);
			$log = pdo_fetch('select * from ' . tablename('rr_v_creditshop_log') . ' where logno=:logno and uniacid=:uniacid limit 1', array(':logno' => $logno, ':uniacid' => $_W['uniacid']));
		}
		...
		if (!empty($log)) {
			...
			if (!is_error($payquery) || !is_error($payqueryborrow)) {
				if ($log['status'] < 1) {
					...
					$record['paystatus'] = 1;
					$record['paytype'] = 1;
					pdo_update('rr_v_creditshop_log', $record, array('logno' => $log['logno']));
					$creditlog = pdo_fetch('select id from ' . tablename('rr_v_creditshop_log') . "\r\n                    where logno=:logno and openid=:openid  and status=0 and paystatus=1 and uniacid=:uniacid limit 1", array(':logno' => $log['logno'], ':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));

					if (is_h5app()) {
						pdo_update('rr_v_creditshop_log', array('apppay' => 1), array('logno' => $log['logno']));
					}
				}
	...
	public function creditshop_complete()
	{
		...
		if ($lastlog['status'] < 1) {
			...
			$record['paystatus'] = 1;
			$record['paystatus'] = 1;
			pdo_update('rr_v_creditshop_log', $record, array('logno' => $tid));
			...
			if (is_h5app()) {
				pdo_update('rr_v_creditshop_log', array('apppay' => 1), array('logno' => $tid));
			}
		}
	...
	public function lottery()
	{
		...
		$member = m('member')->getMember($openid);
		$goodsid = intval($_GPC['goodsid']);
		$log = pdo_fetch('select * from ' . tablename('rr_v_creditshop_log') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $logid, ':uniacid' => $uniacid));

		if (empty($log)) {
			$logno = $_GPC['logno'];
			$log = pdo_fetch('select * from ' . tablename('rr_v_creditshop_log') . ' where logno=:logno and uniacid=:uniacid limit 1', array(':logno' => $logno, ':uniacid' => $uniacid));
		}
		...
		if ((0 < $goods['credit']) && empty($log['creditpay'])) {
			...
			$update['creditpay'] = 1;
			pdo_query('update ' . tablename('rr_v_creditshop_goods') . ' set joins=joins+1 where id=' . $log['goodsid']);
		}
		...
		pdo_update('rr_v_creditshop_log', $update, array('id' => $log['id']));
		...
		if ($status == 2) {
			...
			$update['status'] = $status;
			pdo_update('rr_v_creditshop_log', $update, array('id' => $logid));
			$this->model->sendMessage($logid);

			if ($status == 3) {
				pdo_query('update ' . tablename('rr_v_creditshop_goods') . ' set total=total-1 where id=' . $log['goodsid']);
			}
			...
			if (($goods['goodstype'] == 3) && ($status == 2)) {
				pdo_query('update ' . tablename('rr_v_creditshop_goods') . ' set packetsurplus=packetsurplus-1 where id=' . $log['goodsid']);
			}

			if ($goods['hasoption'] && $log['optionid']) {
				pdo_query('update ' . tablename('rr_v_creditshop_option') . ' set total=total-1 where id=' . $log['optionid']);
			}
		}
