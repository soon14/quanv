...
class Index_RrvV3Page extends CreditshopMobilePage
{
	public function main()
	{
		...
		$advs = pdo_fetchall('select id,advname,link,thumb from ' . tablename('rr_v_creditshop_adv') . ' where ' . $contation . ' and enabled=1 order by displayorder desc', array(':uniacid' => $uniacid));
		...
		if (0 < intval($_GPC['merchid'])) {
			...
			if (!empty($merch_category)) {
				...
				foreach ($merch_category as $index => $row) {
					if (0 < $row) {
						$list = pdo_fetch('select id,name,thumb,isrecommand from ' . tablename('rr_v_creditshop_category') . "\r\n\t\t\t\t\t\twhere id = " . $index . ' and uniacid=:uniacid and  enabled=1 ', array(':uniacid' => $uniacid));
			...
			$category = pdo_fetchall('select id,name,thumb,isrecommand from ' . tablename('rr_v_creditshop_category') . ' where uniacid=:uniacid and  enabled=1 order by displayorder desc', array(':uniacid' => $uniacid));
			$category = set_medias($category, 'thumb');
		}

		array_values($category);
		$lotterydraws = pdo_fetchall('select id, title,goodstype, subtitle, credit, money, thumb,`type`,price from ' . tablename('rr_v_creditshop_goods') . "\r\n\t\t\t\twhere " . $contation . ' and isrecommand = 1 and `type` = 1 and  status=1 and deleted=0 order by displayorder,id desc limit 4', array(':uniacid' => $uniacid));
