...
class Log_RrvV3Page extends PluginWebPage
{
	...
	protected function getData($type)
	{
		...
		$sql = "select log.*, m.nickname,m.avatar,m.realname as mrealname,m.mobile as mmobile, g.title,g.thumb,g.thumb,g.credit,g.money,g.type as goodstype,g.isverify,\r\n\t\t\tg.goodstype as iscoupon,s.storename,s.address as storeaddress,g.dispatch,g.goodstype,g.type,g.merchid as gmerchid,g.hasoption\r\n\t\t\tfrom " . tablename('rr_v_creditshop_log') . ' log ' . ' left join ' . tablename('rr_v_member') . ' m on m.openid = log.openid and m.uniacid=log.uniacid' . ' left join ' . tablename('rr_v_member_address') . ' a on a.id = log.addressid' . ' left join ' . tablename('rr_v_store') . ' s on s.id = log.storeid' . ' left join ' . tablename('rr_v_creditshop_goods') . ' g on g.id = log.goodsid' . ' where 1 ' . $condition . ' group by log.id ORDER BY log.createtime desc ';
		...
		foreach ($list as $key => &$row) {
			if (($row['hasoption'] == 1) && (0 < $row['optionid'])) {
				$option = pdo_fetch('select total,credit,money,title as optiontitle,weight from ' . tablename('rr_v_creditshop_option') . ' where uniacid = ' . $_W['uniacid'] . ' and id = ' . $row['optionid'] . ' and goodsid = ' . $row['goodsid'] . ' ');
		...
		unset($row);
		$total = pdo_fetchcolumn('select count(log.id) from' . tablename('rr_v_creditshop_log') . ' log ' . ' left join ' . tablename('rr_v_member') . ' m on m.openid = log.openid and m.uniacid=log.uniacid' . ' left join ' . tablename('rr_v_store') . ' s on s.id = log.storeid' . ' left join ' . tablename('rr_v_member_address') . ' a on a.id = log.addressid' . ' left join ' . tablename('rr_v_creditshop_goods') . ' g on g.id = log.goodsid' . ' where 1 ' . $condition, $params);

		foreach ($list as &$row) {
			$row['address'] = array();

			if (!empty($row['addressid'])) {
				$row['address'] = pdo_fetch('select realname,mobile,address,province,city,area from ' . tablename('rr_v_member_address') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $row['addressid'], ':uniacid' => $_W['uniacid']));
			}
			else {
				if (0 < intval($row['gmerchid'])) {
					$stores = pdo_fetch('select * from ' . tablename('rr_v_merch_store') . ' where id=:id and uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3)', array(':id' => $row['storeid'], ':uniacid' => $_W['uniacid'], ':merchid' => $row['gmerchid']));
				}
				else {
					$stores = pdo_fetch('select * from ' . tablename('rr_v_store') . ' where id=:id and uniacid=:uniacid and status=1 and type in(2,3)', array(':id' => $row['storeid'], ':uniacid' => $_W['uniacid']));
				}
			...
			if ($row['optionid']) {
				$option = pdo_fetch('select title from ' . tablename('rr_v_creditshop_option') . ' where id = ' . $row['optionid'] . ' and uniacid = ' . $_W['uniacid'] . ' ');
			...
			$canexchange = true;
			$verifynum = pdo_fetchcolumn('select count(1) from ' . tablename('rr_v_creditshop_verify') . ' where logid = ' . $row['id'] . ' and uniacid = ' . $_W['uniacid'] . ' ');
	...
	public function detail()
	{
		...
		if (!empty($goods['isverify'])) {
			if (!empty($log['storeid'])) {
				$store = pdo_fetch('select id,storename,address  from ' . tablename('rr_v_store') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log['storeid'], ':uniacid' => $_W['uniacid']));
			}
		}
		else {
			...
			if (!is_array($address)) {
				$address = pdo_fetch('select realname,mobile,address,province,city,area from ' . tablename('rr_v_member_address') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log['addressid'], ':uniacid' => $_W['uniacid']));
			}
		}
	...
	public function doexchange()
	{
		...
		$log = pdo_fetch('select * from ' . tablename('rr_v_creditshop_log') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		$verifytotal = pdo_fetchcolumn('select count(1) from ' . tablename('rr_v_creditshop_verify') . ' where logid=:logid and uniacid=:uniacid', array(':logid' => $id, ':uniacid' => $_W['uniacid']));
		$set = m('common')->getPluginset('creditshop');
			...
			if ($goods['goodstype'] == 0) {
				...
				if (0 < $goods['isverify']) {
					if ($goods['verifytype'] == 0) {
						pdo_update('rr_v_creditshop_log', array('status' => 3, 'usetime' => time(), 'verifyopenid' => 'system', 'time_finish' => time()), array('id' => $id));
					...
					pdo_update('rr_v_creditshop_log', array('status' => 3, 'usetime' => time(), 'time_send' => time(), 'expresscom' => $_GPC['expresscom'], 'expresssn' => $_GPC['expresssn'], 'express' => $_GPC['express']), array('id' => $id));
				}
			}
			else {
				if ($goods['goodstype'] == 3) {
					...
					$goods = pdo_fetch('select surplusmoney from ' . tablename('rr_v_creditshop_goods') . ' where id = ' . $log['goodsid'] . ' ');
