...
class Setting_RrvV3Page extends PluginWebPage 
{
	...
	public function download() 
	{
		global $_GPC;
		global $_W;
		$dir = IA_ROOT . '/addons/rr_vv3/data/qrcode/' . $_W['uniacid'] . '/exchange';
	...
	public function other() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (pdo_get('rr_v_exchange_setting', array('uniacid' => $_W)) == false) 
			{
				pdo_insert('rr_v_exchange_setting', array('uniacid' => $_W['uniacid'], 'mistake' => $mistake, 'freeze' => $freeze, 'grouplimit' => $grouplimit, 'alllimit' => $alllimit));
			}
			else 
			{
				pdo_update('rr_v_exchange_setting', array('mistake' => $mistake, 'freeze' => $freeze, 'grouplimit' => $grouplimit, 'alllimit' => $alllimit), array('uniacid' => $_W['uniacid']));
			}
			show_json(1, '保存成功');
		}
		else 
		{
			$res = pdo_fetch('SELECT * FROM ' . tablename('rr_v_exchange_setting') . ' WHERE uniacid = :uniacid ', array(':uniacid' => $_W['uniacid']));
		}
		include $this->template();
	}
}
