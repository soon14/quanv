...
class Index_RrvV3Page extends PluginMobileLoginPage 
{
	public function main() 
	{
		...
		{
			$keyresult = pdo_fetch('SELECT groupid FROM ' . tablename('rr_v_exchange_code') . ' WHERE `key` = :code AND uniacid = :uniacid', array(':code' => $key, ':uniacid' => $_W['uniacid']));
			if (empty($keyresult)) 
			{
		...
		if (!(empty($codetype))) 
		{
			$res = pdo_fetch('SELECT * FROM ' . tablename('rr_v_exchange_group') . ' WHERE id = :id AND uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], ':id' => $id));
	...
	{
		global $_W;
		$set = pdo_fetch('SELECT * FROM ' . tablename('rr_v_exchange_setting') . ' WHERE uniacid = :uniacid', array(':uniacid' => $_W['uniacid']));
		$this->counterror($set);
		$time = strtotime('now');
		$time = date('Y-m-d');
		$time1 = $time . ' 00:00:00';
		...
		{
			$exchangelimit = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('rr_v_exchange_record') . ' WHERE openid =:openid AND uniacid = :uniacid AND `time` > :timea AND `time` <= :timeb', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid'], ':timea' => $time1, ':timeb' => $time2));
		...
		{
			$exchangelimit2 = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('rr_v_exchange_record') . ' WHERE openid =:openid AND uniacid = :uniacid AND `time` > :timea AND `time` <= :timeb AND groupid = :groupid', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid'], ':timea' => $time1, ':timeb' => $time2, ':groupid' => $_SESSION['exchangeGroupId']));
		...
		$return = array();
		$table1 = tablename('rr_v_exchange_group');
		$table2 = tablename('rr_v_exchange_code');
		...
		if ($_SESSION['exchangeGroupId'] != $codeResult['groupid']) 
		{
			...
			pdo_query('UPDATE ' . tablename('rr_v_exchange_query') . ' SET `errorcount` = `errorcount` + 1 WHERE openid = :openid', array(':openid' => $_W['openid']));
			return $return;
		}
		if ($codeResult === false) 
		{
		...
		pdo_query('UPDATE ' . tablename('rr_v_exchange_query') . ' SET `errorcount` = 0 AND `unfreeze`=0 WHERE openid = :openid', array(':openid' => $_W['openid']));
		...
		pdo_query('UPDATE ' . tablename('rr_v_exchange_code') . ' SET openid = :openid , `count` = `count` + 1 WHERE openid != :openid AND uniacid = :uniacid AND `key`=:key', array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid'], ':key' => $key));
	...
	public function balance() 
	{
		...
		if ($exchange == '1') 
		{
			...
			if ($balance_res === 1) 
			{
				...
				{
					pdo_update('rr_v_exchange_code', array('status' => 2), array('key' => $key, 'uniacid' => $_W['uniacid'], 'status' => 1, 'repeatcount' => 1));
				}
				...
				pdo_insert('rr_v_exchange_record', $record);
	...
	public function coupon() 
	{
		...
		if ($exchange == '1') 
		{
			...
			$allCoupon = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_coupon') . ' WHERE ' . $condition . ' and uniacid=:uniacid and merchid=0', array(':uniacid' => $_W['uniacid']));
	...
	public function goods() 
	{
		...
		else 
		{
			...
			if ($goods_arr['goods'] != false) 
			{
				...
				{
					$goodsList[$k] = pdo_fetch('SELECT * FROM ' . tablename('rr_v_goods') . ' WHERE id = :id AND uniacid = :uniacid', array(':id' => $v, ':uniacid' => $_W['uniacid']));
				}
			}
			if ($goods_arr['option'] != false) 
			{
				...
				{
					$optionList[$k] = pdo_fetch('SELECT * FROM ' . tablename('rr_v_goods') . ' WHERE id = :id AND uniacid = :uniacid', array(':id' => $k, ':uniacid' => $_W['uniacid']));
					$optionstr = implode('-', $v);
	...
	public function group() 
	{
		...
		if ($exchange == '1') 
		{
			...
			if ($balance_res === 1) 
			{
				pdo_update('rr_v_exchange_code', array('balancestatus' => 2), array('key' => $key, 'uniacid' => $_W['uniacid']));
				$info = m('member')->getInfo($_W['openid']);
				$record_exist = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('rr_v_exchange_record') . ' WHERE `key`=:key AND uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], 'key' => $key));
				...
				else 
				{
					...
					pdo_update('rr_v_exchange_record', $record, array('key' => $key, 'uniacid' => $_W['uniacid']));
				}
				...
				{
					pdo_update('rr_v_exchange_code', array('status' => 2), array('key' => $key, 'uniacid' => $_W['uniacid']));
					pdo_query('UPDATE ' . tablename('rr_v_exchange_group') . ' SET `use` = `use` + 1 WHERE uniacid = :uniacid AND `id` = :id', array(':uniacid' => $_W['uniacid'], ':id' => $codeResult['groupid']));
				}
		...
		else if ($exchange == '2') 
		{
			...
			if (!(is_error($result))) 
			{
				pdo_update('rr_v_exchange_code', array('redstatus' => 2), array('key' => $key, 'uniacid' => $_W['uniacid']));
				$info = m('member')->getInfo($_W['openid']);
				$record_exist = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('rr_v_exchange_record') . ' WHERE `key`=:key AND uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], 'key' => $key));
				if (empty($record_exist)) 
				{
		...
		else if ($exchange == '3') 
		{
			...
			if ($balance_res === 1) 
			{
				pdo_update('rr_v_exchange_code', array('scorestatus' => 2), array('key' => $key, 'uniacid' => $_W['uniacid']));
				$info = m('member')->getInfo($_W['openid']);
				$record_exist = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('rr_v_exchange_record') . ' WHERE `key`=:key AND uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], 'key' => $key));
				if (empty($record_exist)) 
				{
		...
		else if ($exchange == '4') 
		{
			...
			$allCoupon = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_coupon') . ' WHERE ' . $condition . ' and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
			if (empty($allCoupon[0])) 
			{
				show_json(0, '没有可兑换的优惠券');
				$this->message('没有可兑换的优惠券');
			...
			$this->model->sendMessage($resp, 1, $m);
			pdo_update('rr_v_exchange_code', array('couponstatus' => 2), array('key' => $key, 'uniacid' => $_W['uniacid']));
			$info = m('member')->getInfo($_W['openid']);
			$record_exist = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('rr_v_exchange_record') . ' WHERE `key`=:key AND uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], 'key' => $key));
			if (empty($record_exist)) 
			{
	...
	private function chongzhi($type, $id, $changetype, $num, $remark, $balancetype = 0) 
	{
		...
		if ($type == 'credit2') 
		{
			...
			pdo_insert('rr_v_member_log', $data);
			$logid = pdo_insertid();
	...
	public function modal() 
	{
		...
		$goods = pdo_fetch('SELECT * FROM ' . tablename('rr_v_goods') . ' WHERE id = :id AND uniacid = :uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		$optionstr = trim($_GPC['option']);
		$optionid = explode('-', $optionstr);
		$count = 0;
		foreach ($optionid as $k => $v ) 
		{
			$option[$k] = pdo_fetch('SELECT * FROM ' . tablename('rr_v_goods_option') . ' WHERE id = :id AND uniacid = :uniacid', array(':id' => $v, ':uniacid' => $_W['uniacid']));
		}
		include $this->template();
	}
	public function calculate() 
	{
		...
		global $_W;
		@session_start();
		pdo_delete('rr_v_exchange_cart', array('openid' => $_W['openid'], 'selected' => 1));
		$key = $_SESSION['exchange_key'];
		$exchange = pdo_fetch('SELECT * FROM ' . tablename('rr_v_exchange_code') . ' WHERE `key` = :key AND uniacid = :uniacid', array(':key' => $key, ':uniacid' => $_W['uniacid']));
		$group = pdo_fetch('SELECT * FROM ' . tablename('rr_v_exchange_group') . ' WHERE id = :id AND uniacid = :uniacid', array(':id' => $exchange['groupid'], ':uniacid' => $_W['uniacid']));
		...
		{
			$price = pdo_fetch('SELECT * FROM ' . tablename('rr_v_goods') . ' WHERE id = :id AND uniacid=:uniacid', array(':id' => $gv, ':uniacid' => $_W['uniacid']));
			pdo_insert('rr_v_exchange_cart', array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid'], 'goodsid' => $price['id'], 'marketprice' => $price['marketprice'], 'optionid' => 0, 'merchid' => $price['merchid'], 'title' => $price['title'], 'groupid' => $exchange['groupid'], 'serial' => $exchange['serial']));
			if ($price == false) 
			{
				continue;
			}
		...
		foreach ($option_str as $k => $v ) 
		{
			...
			{
				$price = pdo_fetch('SELECT * FROM ' . tablename('rr_v_goods_option') . ' WHERE id = :id AND uniacid=:uniacid', array(':id' => $v2, ':uniacid' => $_W['uniacid']));
				pdo_insert('rr_v_exchange_cart', array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid'], 'goodsid' => $price['goodsid'], 'marketprice' => $price['marketprice'], 'optionid' => $price['id'], 'merchid' => $price['merchid'], 'title' => $price['title'], 'groupid' => $exchange['groupid'], 'serial' => $exchange['serial']));
	...
	public function groupexchange() 
	{
		...
		global $_W;
		$key = trim($_GPC['key']);
		$code = pdo_fetch('SELECT * FROM ' . tablename('rr_v_exchange_code') . ' WHERE `key`=:key AND uniacid = :uniacid', array(':key' => $key, ':uniacid' => $_W['uniacid']));
		if ($code == false) 
		{
			show_json(0, '没有这个兑换码');
		}
		$group = pdo_fetch('SELECT * FROM ' . tablename('rr_v_exchange_group') . ' WHERE id =:id AND uniacid = :uniacid', array(':id' => $code['groupid'], ':uniacid' => $_W['uniacid']));
		if ($group == false) 
		{
			show_json(0, '没有这个兑换组');
		}
	...
	public function counterror($set) 
	{
		...
		$query = pdo_fetch('SELECT * FROM ' . tablename('rr_v_exchange_query') . ' WHERE openid = :openid AND uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
		if (empty($query)) 
		{
			pdo_insert('rr_v_exchange_query', array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid'], 'querytime' => time()));
		}
		if ($query['querytime'] < strtotime(date('Y-m-d', time()) . ' 00:00:00')) 
		{
			pdo_update('rr_v_exchange_query', array('errorcount' => 0, 'querytime' => time()));
		}
		...
		{
			pdo_update('rr_v_exchange_query', array('unfreeze' => time() + ($set['freeze'] * 86400)), array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
			pdo_update('rr_v_exchange_query', array('errorcount' => 0, 'unfreeze' => time() + ($set['freeze'] * 86400)), array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
