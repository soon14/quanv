...
class Category_RrvV3Page extends PluginWebPage
{
	public function main()
	{
		global $_W;
		global $_GPC;
		$list = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_article_category') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY displayorder desc ,id desc');
	...
	public function save()
	{
		...
		if (!empty($_GPC['cate'])) {
			foreach ($_GPC['cate'] as $id => $cate ) {
				...
				if (!empty($id) && !empty($data['category_name'])) {
					pdo_update('rr_v_article_category', $data, array('id' => $id));
		...
		if (!empty($_GPC['cate_new'])) {
			foreach ($_GPC['cate_new'] as $cate_new ) {
				...
				pdo_insert('rr_v_article_category', array('category_name' => $cate_new, 'uniacid' => $_W['uniacid']));
	...
	public function delete()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,category_name FROM ' . tablename('rr_v_article_category') . ' WHERE id = \'' . $id . '\' AND uniacid=' . $_W['uniacid'] . '');

		if (!empty($item)) {
			pdo_delete('rr_v_article_category', array('id' => $id));
