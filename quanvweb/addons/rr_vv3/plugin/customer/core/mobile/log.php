...
if ($_W['isajax']) {
	if ($operation == 'display') {
		...
		if (!(empty($total))) {
			$sql = 'SELECT log.id,log.goodsid,log.status,log.eno,log.paystatus,g.title,g.type,g.thumb,g.credit,g.money FROM ' . tablename('rr_v_creditshop_log') . ' log ' . ' left join ' . tablename('rr_v_creditshop_goods') . ' g on log.goodsid = g.id ' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		...
		if (!(empty($log['addressid']))) {
			$address = pdo_fetch('select id,realname,mobile,address,province,city,area from ' . tablename('rr_v_member_address') . ' where id=:id and openid=:openid and uniacid=:uniacid limit 1', array(':id' => $log['addressid'], ':uniacid' => $uniacid, ':openid' => $openid));
		}
