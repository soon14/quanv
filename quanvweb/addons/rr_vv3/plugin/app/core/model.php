...
if (!class_exists('AppModel')) {
	class AppModel extends PluginModel
	{
		...
		public function createAuth()
		{
			...
			$resp = ihttp_request(rr_vv3_AUTH_URL . 'app', array('ip' => $ip, 'id' => $id, 'code' => $auth['code'], 'domain' => $domain, 'appid' => $_W['account']['key']), NULL, 1);
		...
		public function diyMenu($menuid)
		{
			...
			$menu = pdo_fetch('select * from ' . tablename('rr_v_diypage_menu') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		public function getUrl($url)
		{
			...
			if (strexists($url, './index.php?') && strexists($url, 'rr_vv3') && strexists($url, 'mobile')) {
				$parse = parse_url($url);
				...
				if (($newVars['m'] != 'rr_vv3') || ($newVars['do'] != 'mobile')) {
					return $url;
				}
		...
		{
			$file = rr_vv3_PLUGIN . 'app/static/iconfont.json';
