...
class Refund_RrvV3Page extends AppMobilePage
{
	protected function globalData()
	{
		...
		$order = pdo_fetch('select id,status,price,refundid,goodsprice,dispatchprice,deductprice,deductcredit2,finishtime,isverify,`virtual`,refundstate,merchid from ' . tablename('rr_v_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
		...
		if ($order['status'] == 2) {
			$goods = pdo_fetchall('select og.goodsid, og.price, og.total, og.optionname, g.cannotrefund, g.thumb, g.title from' . tablename('rr_v_order_goods') . ' og left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid where og.orderid=' . $order['id']);
		...
		if ($order['cannotrefund']) {
			app_error(AppError::$OrderCanNotRefund, '此订单不可退换货');
		}
	...
	public function main()
	{
		...
		if (0 < $order['refundstate']) {
			if (!empty($refundid)) {
				$refund = pdo_fetch('select * from ' . tablename('rr_v_order_refund') . ' where id=:id and uniacid=:uniacid and orderid=:orderid limit 1', array(':id' => $refundid, ':uniacid' => $uniacid, ':orderid' => $orderid));
		...
		app_json(array('title' => ($order['status'] == 1 ? '退款' : '售后') . '申请', 'refundtype' => $refund['rtype'], 'refundreason' => ($refund['rtype'] == '2' ? '换货' : '退款') . '原因', 'refundexplain' => ($refund['rtype'] == '2' ? '换货' : '退款') . '说明', 'price' => $show_price, 'content' => isset($refund['content']) ? $refund['content'] : '', 'order' => $order, 'refund' => $refund, 'images' => is_array($refund['imgs']) ? $refund['imgs'] : array(), 'imgs' => !empty($images) ? $images : array(), 'express_list' => $express_list, 'rtypeIndex' => !empty($refund) ? $refund['rtype'] : 0, 'reasonIndex' => !empty($reasonIndex) ? $reasonIndex : 0));
	}
	...
	public function submit()
	{
		...
		if ($order['refundstate'] == 0) {
			...
			pdo_insert('rr_v_order_refund', $refund);
			$refundid = pdo_insertid();
			pdo_update('rr_v_order', array('refundid' => $refundid, 'refundstate' => $refundstate), array('id' => $orderid, 'uniacid' => $uniacid));
		}
		else {
			pdo_update('rr_v_order', array('refundstate' => $refundstate), array('id' => $orderid, 'uniacid' => $uniacid));
			pdo_update('rr_v_order_refund', $refund, array('id' => $refundid, 'uniacid' => $uniacid));
		}
	...
	public function cancel()
	{
		...
		pdo_update('rr_v_order_refund', $change_refund, array('id' => $refundid, 'uniacid' => $uniacid));
		pdo_update('rr_v_order', array('refundstate' => 0), array('id' => $orderid, 'uniacid' => $uniacid));
		app_json();
	}
	...
	public function receive()
	{
		...
		pdo_update('rr_v_order_refund', $refund_data, array('id' => $refundid, 'uniacid' => $uniacid));
		$order_data = array();
		$order_data['refundstate'] = 0;
		$order_data['status'] = -1;
		$order_data['refundtime'] = $time;
		pdo_update('rr_v_order', $order_data, array('id' => $orderid, 'uniacid' => $uniacid));
		show_json(1);
	}
