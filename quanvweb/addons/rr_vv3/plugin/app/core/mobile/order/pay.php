...
class Pay_RrvV3Page extends AppMobilePage
{
	public function main()
	{
		...
		if (!empty($set['pay']['wxapp']) && (0 < $order['price']) && $this->iswxapp) {
			...
			$payinfo = array('openid' => $openid, 'title' => $set['shop']['name'] . '订单', 'tid' => $tid, 'fee' => $order['price']);
	...
	public function complete()
	{
		...
		if ($type == 'credit') {
			...
			if (!is_error($payquery)) {
				...
				pdo_update('rr_v_order', array('apppay' => 2), array('id' => $order['id']));
				$this->success($orderid);
			}
	...
	protected function success($orderid)
	{
		...
		app_json(array(
	'order'   => array('id' => $orderid, 'isverify' => $order['isverify'], 'virtual' => $order['virtual'], 'isvirtual' => $order['isvirtual'], 'isvirtualsend' => $order['isvirtualsend'], 'status' => $order['paytype'] == 3 ? '订单提交支付' : '订单支付成功', 'text' => $text, 'price' => $order['price']),
