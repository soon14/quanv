...
class Withdraw_RrvV3Page extends AppMobilePage
{
	public function main()
	{
		...
		if ($set['withdrawcashweixin'] == 1) {
			$type_array[] = array('type' => 1, 'title' => '提现到微信钱包');

			if (!empty($last_data)) {
				if ($last_data['applytype'] != 1) {
					$type_last = $this->getLastApply($openid, 1);
	...
	public function submit()
	{
		...
		pdo_insert('rr_v_member_log', $apply);
		$logid = pdo_insertid();
	...
	public function getLastApply($openid, $applytype = -1)
	{
		...
		$sql = 'select applytype,alipay,bankname,bankcard,realname from ' . tablename('rr_v_member_log') . ' where openid=:openid and uniacid=:uniacid';
	...
	public function log()
	{
		...
		$condition = ' and uniacid=:uniacid and type=:type and money<>0 and openid = :openid';
		$params = array(':uniacid' => $uniacid, ':type' => 1, ':openid' => $openid);

		$sql = 'select * from ' . tablename('rr_v_member_log') . ' where 1 ' . $condition . ' ORDER BY createtime DESC ';
		$sql .= $limit;
