...
class Index_RrvV3Page extends Base_RrvV3Page
{
	public function main()
	{
		...
		$member['applycount'] = pdo_fetchcolumn('select count(id) from ' . tablename('rr_v_commission_apply') . ' where mid=:mid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':mid' => $member['id']));
		$openselect = false;
		...
		$return_set = array('texts' => $this->set['texts'], 'closed_qrcode' => (int) $this->set['closed_qrcode']);
		$return_member = array('avatar' => (string) $member['avatar'], 'nickname' => (string) $member['nickname'], 'commission_pay' => (double) $member['commission_pay'], 'commission_ok' => (double) $member['commission_ok'], 'commission_total' => (double) $member['commission_total'], 'ordercount0' => (double) $member['ordercount0'], 'applycount' => (double) $member['applycount'], 'downcount' => (double) $member['downcount']);
		$data = array('set' => $return_set, 'member' => $return_member, 'levelname' => empty($level) ? (empty($this->set['levelname']) ? '普通等级' : $this->set['levelname']) : $level['levelname'], 'agentname' => empty($up) ? '总店' : $up['nickname'], 'cansettle' => $cansettle);
		app_json($data);
	}
	...
	public function withdraw()
	{
		...
		if (!empty($agentid)) {
			$data = pdo_fetch('select sum(deductionmoney) as sumcharge from ' . tablename('rr_v_commission_log') . ' where mid=:mid and uniacid=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':mid' => $agentid));
		...
		$data = array('set' => $set, 'member' => $member, 'cansettle' => $cansettle);
		app_json($data);
	}
	...
	public function apply()
	{
		...
		if (1 <= $level) {
			$level1_orders = pdo_fetchall('select distinct o.id from ' . tablename('rr_v_order') . ' o ' . ' left join  ' . tablename('rr_v_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid=:agentid and o.status>=3  and og.status1=0 and og.nocommission=0 and (' . $time . ' - o.finishtime > ' . $day_times . ') and o.uniacid=:uniacid  group by o.id', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
		...
		if (2 <= $level) {
			if (0 < $member['level1']) {
				$level2_orders = pdo_fetchall('select distinct o.id from ' . tablename('rr_v_order') . ' o ' . ' left join  ' . tablename('rr_v_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($member['level1_agentids'])) . ')  and o.status>=3  and og.status2=0 and og.nocommission=0 and (' . $time . ' - o.finishtime > ' . $day_times . ') and o.uniacid=:uniacid  group by o.id', array(':uniacid' => $_W['uniacid']));
		...
		if (3 <= $level) {
			if (0 < $member['level2']) {
				$level3_orders = pdo_fetchall('select distinct o.id from ' . tablename('rr_v_order') . ' o ' . ' left join  ' . tablename('rr_v_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($member['level2_agentids'])) . ')  and o.status>=3  and  og.status3=0 and og.nocommission=0 and (' . $time . ' - o.finishtime > ' . $day_times . ')   and o.uniacid=:uniacid  group by o.id', array(':uniacid' => $_W['uniacid']));
		...
		if ($this->set['cashother'] == 1) {
			...
			if ($this->set['cashcard'] == 1) {
				...
				$banklist = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_commission_bank') . ' WHERE 1 ' . $condition . '  ORDER BY displayorder DESC', $params);
			}
		}
		...
		if ($_W['ispost']) {
			...
			foreach ($orderids as $o) {
				pdo_update('rr_v_order_goods', array('status' . $o['level'] => 1, 'applytime' . $o['level'] => $time), array('orderid' => $o['orderid'], 'uniacid' => $_W['uniacid']));
			}
			...
			pdo_insert('rr_v_commission_apply', $apply);
	...
	public function register()
	{
		...
		if ($_W['ispost']) {
			...
			if ($template_flag == 1) {
				...
				pdo_update('rr_v_member', $m_data, array('id' => $member['id']));
				...
				pdo_update('rr_v_member', $data, array('id' => $member['id']));
