...
class Index_RrvV3Page extends PluginMobileLoginPage
{
	public function main()
	{
		...
		$share_res = pdo_fetch('SELECT * FROM ' . tablename('rr_v_bargain_account') . 'WHERE id = :id', array(':id' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			$res2 = pdo_fetchall('SELECT * FROM' . tablename('rr_v_bargain_goods') . 'WHERE account_id = :account_id AND status =\'0\' ORDER BY id DESC', array(':account_id' => $_W['uniacid']));
			...
			foreach ($res2 as $i => $value) {
				...
				$res[$i] = $res2[$i];
				$res3 = pdo_fetch('SELECT * FROM' . tablename('rr_v_goods') . 'WHERE id = :id AND deleted = \'0\' AND status = \'1\' AND title LIKE :title ', array(':id' => $res[$i]['goods_id'], ':title' => $keywords));
		...
		foreach ($res2 as $i => $value) {
			...
			$res[$i] = $res2[$i];
			$res3 = pdo_fetch('SELECT * FROM' . tablename('rr_v_goods') . 'WHERE id = :id AND deleted = \'0\' AND status = \'1\'', array(':id' => $res[$i]['goods_id']));
	...
	public function purchase()
	{
		...
		$mid = m('member')->getMid();
		$_W['shopshare'] = array('title' => $share['title'], 'desc' => $share['content'], 'link' => mobileUrl('bargain', array('mid' => $mid), true), 'imgUrl' => $share['logo']);
		$act = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_bargain_actor') . ' WHERE openid= :openid AND account_id = :account_id AND status = \'1\' ORDER BY id DESC', array(':openid' => $_W['openid'], ':account_id' => $_W['uniacid']));
		$i = 0;

		while ($i < count($act)) {
			$goods[$i] = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_bargain_goods') . ' WHERE id=:id', array(':id' => $act[$i]['goods_id']));
			$rr_detail = pdo_fetch('SELECT * FROM ' . tablename('rr_v_goods') . ' WHERE id = :id AND status = \'1\'', array(':id' => $goods[$i][0]['goods_id']));
			...
			$goods[$i][0]['title'] = $rr_detail['title'];
			$goods[$i][0]['title2'] = $rr_detail['subtitle'];
			$goods[$i][0]['start_price'] = $rr_detail['marketprice'];
			$goods[$i][0]['sold'] = $rr_detail['sales'];
			$goods[$i][0]['stock'] = $rr_detail['total'];
			$goods[$i][0]['images'] = json_encode(array($rr_detail['thumb']));
			$goods[$i][0]['content'] = $rr_detail['content'];
	...
	public function act()
	{
		...
		$mid = m('member')->getMid();
		$_W['shopshare'] = array('title' => $share['title'], 'desc' => $share['content'], 'link' => mobileUrl('bargain', array('mid' => $mid), true), 'imgUrl' => $share['logo']);
		$act = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_bargain_actor') . ' WHERE openid= :openid AND account_id = :account_id AND status = \'0\' ORDER BY id DESC', array(':openid' => $_W['openid'], ':account_id' => $_W['uniacid']));
		$i = 0;

		while ($i < count($act)) {
			$goods[$i] = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_bargain_goods') . ' WHERE id=:id AND status = \'0\'', array(':id' => $act[$i]['goods_id']));
			$rr_detail = pdo_fetch('SELECT * FROM ' . tablename('rr_v_goods') . ' WHERE id = :id AND status = \'1\' AND deleted=\'0\'', array(':id' => $goods[$i][0]['goods_id']));
	...
	public function detail()
	{
		...
		$res = pdo_fetch('SELECT * FROM ' . tablename('rr_v_bargain_goods') . ' WHERE id = :id AND status=\'0\'', array(':id' => $id));
		...
		$rr_detail = pdo_fetch('SELECT * FROM ' . tablename('rr_v_goods') . ' WHERE id = :id AND status = \'1\' AND deleted = \'0\'', array(':id' => $res['goods_id']));
		...
		$res['title'] = $rr_detail['title'];
		$res['title2'] = $rr_detail['subtitle'];

		if (substr($rr_detail['marketprice'], -3, 3) == '.00') {
			$rr_detail['marketprice'] = intval($rr_detail['marketprice']);
		}
		...
		$res['start_price'] = $rr_detail['marketprice'];
		$res['sold'] = $rr_detail['sales'];
		$res['stock'] = $rr_detail['total'];
		$res['images'] = unserialize($rr_detail['thumb_url']);
		$res['content'] = $rr_detail['content'];
		...
		if (!empty($share_res['goods_logo'])) {
			...
			$share['logo'] = tomedia($rr_detail['thumb']);
		}
		...
		if (!empty($res['initiate'])) {
			$act_id = pdo_get('rr_v_bargain_actor', array('goods_id' => $id, 'openid' => $_W['openid'], 'status' => 0), 'id');
	...
	public function bargain()
	{
		...
		$account_set = pdo_get('rr_v_bargain_account', array('id' => $_W['uniacid']), array('partin', 'rule', 'sharestyle'));
		$res = pdo_fetch('SELECT * FROM ' . tablename('rr_v_bargain_actor') . ' WHERE id = :id', array(':id' => $id));
		$res2 = pdo_fetch('SELECT * FROM ' . tablename('rr_v_bargain_goods') . ' WHERE id = :id AND status=\'0\'', array(':id' => $res['goods_id']));
		$rr_detail = pdo_fetch('SELECT * FROM ' . tablename('rr_v_goods') . ' WHERE id = :id AND status = \'1\' AND bargain > 0 AND deleted=0', array(':id' => $res2['goods_id']));
		if (!$rr_detail['id'] || !$res['id'] || !$res2['id']) {
			include $this->template('bargain/lost');
			return NULL;
		}

		$res2['title'] = $rr_detail['title'];
		$res2['title2'] = $rr_detail['subtitle'];
		$res2['start_price'] = $rr_detail['marketprice'];
		...
		$res2['sold'] = $rr_detail['sales'];
		$res2['stock'] = $rr_detail['total'];
		$res2['images'] = json_encode(array($rr_detail['thumb']));
		$res2['content'] = $rr_detail['content'];
		if (!$res || !$res2) {
			include $this->template('bargain/lost');
			return NULL;
		}
		...
		$res3 = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_bargain_record') . ' WHERE actor_id = :actor_id ORDER BY id DESC LIMIT 0,10', array(':actor_id' => $id));
		$res4 = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_bargain_actor') . ' WHERE bargain_price <= \'' . $account_set['partin'] . '\' AND account_id=:uniacid and goods_id = ' . $res2['id'] . ' ORDER BY bargain_price ASC LIMIT 10', array(':uniacid' => $_W['uniacid']));
		$min_price = $res2['end_price'];
		$max_price = $res2['start_price'];

		if (pdo_fetch('SELECT * FROM ' . tablename('rr_v_bargain_actor') . ' WHERE id = :id AND status = \'1\'', array(':id' => $id))) {
			if ($trade_swi != 2) {
				$trade_swi = 1;
			}
		}
		...
		if ($ajax == 151) {
			...
			$account_set['partin'] *= -1;
			$myself_swi = 0;
			$myself_count = pdo_get('rr_v_bargain_record', array('openid' => $_W['openid'], 'actor_id' => $res['id']), 'id');
	...
	public function join()
	{
		...
		$res = pdo_fetch('SELECT * FROM ' . tablename('rr_v_bargain_goods') . ' WHERE id = :id', array(':id' => $goods_id));
		...
		$goods_detail = pdo_fetch('SELECT * FROM ' . tablename('rr_v_goods') . ' WHERE id = :id AND status=\'1\'', array(':id' => $res['goods_id']));
		...
		if (!empty($user_info['openid'])) {
			$if = pdo_insert('rr_v_bargain_actor', $data);
			$id = pdo_insertid();
			pdo_query('UPDATE ' . tablename('rr_v_bargain_goods') . ' SET act_times=act_times+1 WHERE id= :id', array(':id' => $goods_id));
		}
		else {
			exit('拒绝访问');
		}
	...
	public function cut($actor_id, $time3, $min_price, $each_time, $total_time, $max_price, $probability_json)
	{
		...
		$min *= 100;
		$max *= 100;
		$record_res = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_bargain_record') . ' WHERE actor_id=:actor_id AND openid= :openid', array(':actor_id' => $actor_id, ':openid' => $_W['openid']));
		$all_record = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_bargain_record') . ' WHERE actor_id= :actor_id', array(':actor_id' => $actor_id));
		...
		$now_price = pdo_fetch('SELECT * FROM ' . tablename('rr_v_bargain_actor') . ' WHERE id = :id', array(':id' => $actor_id));
		...
		$res_ins = pdo_insert('rr_v_bargain_record', $insert_data);
		$res_act = pdo_query('UPDATE ' . tablename('rr_v_bargain_actor') . ' SET now_price = now_price + :now_price ,update_time = :update_time , bargain_price = bargain_price + :bargain_price WHERE id= :id', array(':now_price' => $cut_price, ':update_time' => $time, ':bargain_price' => $cut_price, ':id' => $actor_id));
		...
		if ($cut_price <= 0) {
			...
			pdo_query('UPDATE ' . tablename('rr_v_bargain_actor') . ' SET bargain_times = bargain_times + 1 WHERE id = :id', array(':id' => $actor_id));
	...
	public function rule()
	{
		...
		$rule = pdo_get('rr_v_bargain_goods', array('id' => $id, 'account_id' => $_W['uniacid']), array('rule'));

		if (empty($rule['rule'])) {
			$rule = pdo_get('rr_v_bargain_account', array('id' => $_W['uniacid']), array('rule'));
		}
	...
	public function sendNotice(array $params)
	{
		...
		if (!empty($templateid)) {
			$template = pdo_fetch('select * from ' . tablename('rr_v_member_message_template') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $templateid, ':uniacid' => $_W['uniacid']));
			...
			if (!empty($template)) {
				...
				if (is_error($ret)) {
					$templatetype = pdo_fetch('select templateid  from ' . tablename('rr_v_member_message_template_type') . ' where typecode=:typecode  limit 1', array(':typecode' => $tag));
