...
if (!(class_exists('AbonusModel'))) 
{
	class AbonusModel extends PluginModel 
	{
		...
		public function getLevels($all = true, $default = false) 
		{
			...
			if ($all) 
			{
				$levels = pdo_fetchall('select * from ' . tablename('rr_v_abonus_level') . ' where uniacid=:uniacid order by id asc', array(':uniacid' => $_W['uniacid']));
			}
			else 
			{
				$levels = pdo_fetchall('select * from ' . tablename('rr_v_abonus_level') . ' where uniacid=:uniacid and (ordermoney>0 or bonusmoney>0) order by bonus asc', array(':uniacid' => $_W['uniacid']));
			}
			if ($default) 
			{
		...
		public function getBonus($openid = '', $params = array()) 
		{
			...
			{
				$ret['ok'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)+ifnull(sum(paymoney2),0)  + ifnull(sum(paymoney3),0)  from ' . tablename('rr_v_abonus_billp') . ' where openid=:openid and status=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}
			if (in_array('ok1', $params)) 
			{
				$ret['ok1'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)  from ' . tablename('rr_v_abonus_billp') . ' where openid=:openid and status=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}
			if (in_array('ok2', $params)) 
			{
				$ret['ok2'] = pdo_fetchcolumn('select ifnull(sum(paymoney2),0)   from ' . tablename('rr_v_abonus_billp') . ' where openid=:openid and status=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}
			if (in_array('ok3', $params)) 
			{
				$ret['ok3'] = pdo_fetchcolumn('select ifnull(sum(paymoney3),0)   from ' . tablename('rr_v_abonus_billp') . ' where openid=:openid and status=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}
			if (in_array('lock', $params)) 
			{
				$ret['lock'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)+ifnull(sum(paymoney2),0)  + ifnull(sum(paymoney3),0)  from ' . tablename('rr_v_abonus_billp') . ' where openid=:openid and status<>1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}
			if (in_array('lock1', $params)) 
			{
				$ret['lock1'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)  from ' . tablename('rr_v_abonus_billp') . ' where openid=:openid and status<>1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}
			if (in_array('lock2', $params)) 
			{
				$ret['lock2'] = pdo_fetchcolumn('select ifnull(sum(paymoney2),0)   from ' . tablename('rr_v_abonus_billp') . ' where openid=:openid and status<>1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}
			if (in_array('lock3', $params)) 
			{
				$ret['lock3'] = pdo_fetchcolumn('select ifnull(sum(paymoney3),0)  from ' . tablename('rr_v_abonus_billp') . ' where openid=:openid and status<>1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}
			if (in_array('total', $params)) 
			{
				$ret['total'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)+ifnull(sum(paymoney2),0)  + ifnull(sum(paymoney3),0)  from ' . tablename('rr_v_abonus_billp') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}
			if (in_array('total1', $params)) 
			{
				$ret['total1'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)  from ' . tablename('rr_v_abonus_billp') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}
			if (in_array('total2', $params)) 
			{
				$ret['total2'] = pdo_fetchcolumn('select ifnull(sum(paymoney2),0)   from ' . tablename('rr_v_abonus_billp') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}
			if (in_array('total3', $params)) 
			{
				$ret['total3'] = pdo_fetchcolumn('select ifnull(sum(paymoney3),0)  from ' . tablename('rr_v_abonus_billp') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			}
			return $ret;
		}
		...
		public function getLevel($openid) 
		{
			...
			$level = pdo_fetch('select * from ' . tablename('rr_v_abonus_level') . ' where uniacid=:uniacid and id=:id limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $member['aagentlevel']));
			return $level;
		}
		...
		public function upgradeLevelByOrder($openid) 
		{
			...
			if (($leveltype == 4) || ($leveltype == 5)) 
			{
				...
				if ($leveltype == 4) 
				{
					$newlevel = pdo_fetch('select * from ' . tablename('rr_v_abonus_level') . ' where uniacid=:uniacid  and ' . $ordermoney . ' >= ordermoney and ordermoney>0  order by ordermoney desc limit 1', array(':uniacid' => $_W['uniacid']));
					if (empty($newlevel)) 
					{
						return;
					}
					if (!(empty($oldlevel['id']))) 
					{
						...
						if ($newlevel['ordermoney'] < $oldlevel['ordermoney']) 
						{
							...
							if ($leveltype == 5) 
							{
								$newlevel = pdo_fetch('select * from ' . tablename('rr_v_abonus_level') . ' where uniacid=:uniacid  and ' . $ordercount . ' >= ordercount and ordercount>0  order by ordercount desc limit 1', array(':uniacid' => $_W['uniacid']));
								if (empty($newlevel)) 
								{
									return;
								}
				...
				pdo_update('rr_v_member', array('aagentlevel' => $newlevel['id']), array('id' => $m['id']));
			...
			else if ((0 <= $leveltype) && ($leveltype <= 3)) 
			{
				...
				foreach ($agents as $agent ) 
				{
					...
					pdo_update('rr_v_member', array('aagentlevel' => $newlevel['id']), array('id' => $agent['id']));
			...
			if (($leveltype == 6) || ($leveltype == 8)) 
			{
				...
				foreach ($agents as $agent ) 
				{
					...
					if ($leveltype == 6) 
					{
						...
						$newlevel = pdo_fetch('select * from ' . tablename('rr_v_abonus_level') . ' where uniacid=:uniacid  and ' . $downcount . ' >= downcount and downcount>0  order by downcount desc limit 1', array(':uniacid' => $_W['uniacid']));
					}
					else if ($leveltype == 8) 
					{
		...
		public function upgradeLevelByCommissionOK($openid) 
		{
			...
			$newlevel = pdo_fetch('select * from ' . tablename('rr_v_abonus_level') . ' where uniacid=:uniacid  and ' . $commissionmoney . ' >= commissionmoney and commissionmoney>0  order by commissionmoney desc limit 1', array(':uniacid' => $_W['uniacid']));
			if (empty($newlevel)) 
			{
				return;
			}
		...
		public function upgradeLevelByBonus($openid) 
		{
			...
			$newlevel = pdo_fetch('select * from ' . tablename('rr_v_abonus_level') . ' where uniacid=:uniacid  and ' . $bonusmoney['ok'] . ' >= bonusmoney and bonusmoney>0  order by bonusmoney desc limit 1', array(':uniacid' => $_W['uniacid']));
			if (empty($newlevel)) 
			{
				return;
			}
		...
		public function getBonusData($year = 0, $month = 0, $week = 0, $openid = '') 
		{
			...
			$bill = pdo_fetch('select * from ' . tablename('rr_v_abonus_bill') . ' where uniacid=:uniacid and `year`=:year and `month`=:month and `week`=:week limit 1', array(':uniacid' => $_W['uniacid'], ':year' => $year, ':month' => $month, ':week' => $week));
			...
			$ordermoney = 0;
			$bonusmoney = 0;
			$orders = pdo_fetchall('select id,openid,price,addressid, address from ' . tablename('rr_v_order') . ' where uniacid=' . $_W['uniacid'] . ' and addressid<>0 and status=3 and isabonus=0 and finishtime + ' . $settletimes . '>= ' . $starttime . ' and  finishtime + ' . $settletimes . '<=' . $endtime, array(), 'id');
			$pcondition = '';
			if (!(empty($openid))) 
			{
				$pcondition = ' and m.openid=\'' . $openid . '\'';
			}
			$aagents = pdo_fetchall('select m.id,m.openid,m.aagentlevel,m.aagenttype,m.aagentprovinces,m.aagentcitys,m.aagentareas, l.bonus1,l.bonus2,l.bonus3 from ' . tablename('rr_v_member') . ' m ' . '  left join ' . tablename('rr_v_abonus_level') . ' l on l.id = m.aagentlevel ' . '  where m.uniacid=:uniacid and  m.isaagent=1 and m.aagentstatus=1 ' . $pcondition, array(':uniacid' => $_W['uniacid']));
			$aagentcount1 = 0;
			$aagentcount2 = 0;
			$aagentcount3 = 0;
			...
			foreach ($orders as $o ) 
			{
				...
				foreach ($aagents as &$a ) 
				{
					...
					if (!(is_array($address))) 
					{
						$address = pdo_fetch('select * from ' . tablename('rr_v_member_address') . ' where id=:id limit 1', array(':id' => $o['addressid']));
					}
		...
		{
			global $_W;
			return array('total0' => pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_abonus_bill') . ' where uniacid=:uniacid and status=0 limit 1', array(':uniacid' => $_W['uniacid'])), 'total1' => pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_abonus_bill') . ' where uniacid=:uniacid and status=1 limit 1', array(':uniacid' => $_W['uniacid'])), 'total2' => pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_abonus_bill') . ' where uniacid=:uniacid and status=2  limit 1', array(':uniacid' => $_W['uniacid'])));
		}
	}
}
?>