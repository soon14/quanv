...
class Agent_RrvV3Page extends PluginWebPage 
{
	public function main() 
	{
		...
		$sql = 'select dm.*,dm.nickname,dm.avatar,l.levelname,p.nickname as parentname,p.avatar as parentavatar from ' . tablename('rr_v_member') . ' dm ' . ' left join ' . tablename('rr_v_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('rr_v_abonus_level') . ' l on l.id = dm.aagentlevel' . ' where dm.uniacid = ' . $_W['uniacid'] . ' and dm.isaagent =1  ' . $condition . ' ORDER BY dm.aagenttime desc';
		if (empty($_GPC['export'])) 
		{
			$sql .= ' limit ' . (($pindex - 1) * $psize) . ',' . $psize;
		}
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('select count(dm.id) from' . tablename('rr_v_member') . ' dm  ' . ' left join ' . tablename('rr_v_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('rr_v_abonus_level') . ' l on l.id = dm.aagentlevel' . ' where dm.uniacid =' . $_W['uniacid'] . ' and dm.isaagent =1  ' . $condition, $params);
		foreach ($list as &$row ) 
		{
	...
	public function delete() 
	{
		...
		{
			pdo_update('rr_v_member', array('isaagent' => 0, 'aagentstatus' => 0), array('id' => $member['id']));
	...
	public function check() 
	{
		...
		$members = pdo_fetchall('SELECT id,openid,nickname,realname,mobile,aagentstatus FROM ' . tablename('rr_v_member') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		$time = time();
		foreach ($members as $member ) 
		{
			...
			if ($status == 1) 
			{
				pdo_update('rr_v_member', array('aagentstatus' => 1, 'aagenttime' => $time), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
			...
			{
				pdo_update('rr_v_member', array('aagentstatus' => 0, 'aagenttime' => 0), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
	...
	public function aagentblack() 
	{
		...
		foreach ($members as $member ) 
		{
			...
			if ($aagentblack == 1) 
			{
				pdo_update('rr_v_member', array('isaagent' => 1, 'aagentstatus' => 0, 'aagentblack' => 1), array('id' => $_GPC['id']));
				plog('abonus.agent.aagentblack', '设置黑名单 <br/>区域代理信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
			else 
			{
				pdo_update('rr_v_member', array('isaagent' => 1, 'aagentstatus' => 1, 'aagentblack' => 0), array('id' => $_GPC['id']));
