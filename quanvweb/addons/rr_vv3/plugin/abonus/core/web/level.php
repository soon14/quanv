...
class Level_RrvV3Page extends PluginWebPage 
{
	public function main() 
	{
		...
		$others = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_abonus_level') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY id asc');
	...
	protected function post() 
	{
		...
		{
			$level = pdo_fetch('SELECT * FROM ' . tablename('rr_v_abonus_level') . ' WHERE id=:id and uniacid=:uniacid limit 1', array(':id' => intval($id), ':uniacid' => $_W['uniacid']));
		}
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				...
				else 
				{
					...
					pdo_update('rr_v_abonus_level', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
					plog('abonus.level.edit', '修改代理商等级 ID: ' . $id . $updatecontent);
				}
			}
			else 
			{
				pdo_insert('rr_v_abonus_level', $data);
				$id = pdo_insertid();
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,levelname FROM ' . tablename('rr_v_abonus_level') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_delete('rr_v_abonus_level', array('id' => $item['id']));
