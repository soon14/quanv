...
class Set_RrvV3Page extends PluginWebPage
{
	public function main()
	{
		global $_W;
		global $_GPC;
		$item = pdo_fetch('select * from ' . tablename('rr_v_polyapi_key') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if (empty($item)) {
				...
				pdo_insert('rr_v_polyapi_key', $data);
			}
			 else {
				$data['updatetime'] = $time;
				pdo_update('rr_v_polyapi_key', $data, array('uniacid' => $_W['uniacid']));
			}
	...
	public function update()
	{
		global $_W;
		global $_GPC;
		$item = pdo_fetch('select code from ' . tablename('rr_v_express') . ' where express=:express limit 1', array(':express' => 'shunfeng'));
		...
		if (empty($item['code'])) {
			...
			foreach ($data as $k => $v ) {
				pdo_update('rr_v_express', array('code' => $v), array('express' => $k));
			}
		}
