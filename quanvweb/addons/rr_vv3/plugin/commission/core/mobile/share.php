...
require rr_vv3_PLUGIN . 'commission/core/page_mobile.php';
class Share_RrvV3Page extends CommissionMobilePage
{
	public function main()
	{
		...
		$qrcode = '';
		$path = $_W['siteroot'] . 'addons/rr_vv3/data/poster/' . $_W['uniacid'] . '/';

		if (!empty($goodsid)) {
			$goods = pdo_fetch('select * from ' . tablename('rr_v_goods') . ' where uniacid=:uniacid and id=:id limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $goodsid));
			...
			if (!empty($goods)) {
				...
				$qrcode = $path . $md5 . '.jpg';
				if (p('poster') && !is_file(IA_ROOT . '/addons/rr_vv3/data/poster/' . $_W['uniacid'] . '/' . $md5 . '.jpg')) {
					$poster = pdo_fetch('select bg,data from ' . tablename('rr_v_poster') . ' where uniacid=:uniacid and type=3 and isdefault=1', array(':uniacid' => $_W['uniacid']));
		...
		if (!$share_goods) {
			...
			$qrcode = $path . $md5 . '.jpg';
			if (p('poster') && !is_file(IA_ROOT . '/addons/rr_vv3/data/poster/' . $_W['uniacid'] . '/' . $md5 . '.jpg')) {
				$poster = pdo_fetch('select bg,`data` from ' . tablename('rr_v_poster') . ' where uniacid=:uniacid and `type`=:type and isdefault=1 limit 1', array(':uniacid' => $_W['uniacid'], ':type' => empty($set['qrcode']) ? 2 : 4));
