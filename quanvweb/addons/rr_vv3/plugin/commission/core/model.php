...
if (!class_exists('CommissionModel')) {
	class CommissionModel extends PluginModel
	{
		...
		public function calculate($orderid = 0, $update = true)
		{
			...
			$order = pdo_fetch('select agentid,price,goodsprice,deductcredit2,discountprice,isdiscountprice,dispatchprice,changeprice,ispackage,packageid from ' . tablename('rr_v_order') . ' where id=:id limit 1', array(':id' => $orderid));
			...
			$hascommission = false;
			$goods = pdo_fetchall("select og.id,og.realprice,og.total,g.hasoption,og.goodsid,og.optionid,g.hascommission,g.nocommission, g.commission1_rate,g.commission1_pay,g.commission2_rate,g.commission2_pay,\r\n                  g.commission3_rate,g.commission3_pay,g.commission,og.commissions,og.seckill,og.seckill_taskid,og.seckill_timeid from " . tablename('rr_v_order_goods') . '  og ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id = og.goodsid' . ' where og.orderid=:orderid and og.uniacid=:uniacid', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
			...
			if (0 < $set['level']) {
				foreach ($goods as &$cinfo) {
						...
						if (empty($cinfo['nocommission'])) {
							...
							if (0 < $order['ispackage']) {
								...
								if (!empty($cinfo['optionid'])) {
									$packoption = pdo_fetch('select commission1,commission2,commission3 from ' . tablename('rr_v_package_goods_option') . "\r\n                                            where uniacid = " . $_W['uniacid'] . ' and pid = ' . $order['packageid'] . ' and optionid = ' . $cinfo['optionid'] . ' ');
								}
								else {
									$packoption = pdo_fetch('select commission1,commission2,commission3 from ' . tablename('rr_v_package_goods') . "\r\n                                            where uniacid = " . $_W['uniacid'] . ' and pid = ' . $order['packageid'] . ' and goodsid = ' . $cinfo['goodsid'] . ' ');
								}
					...
					if ($update) {
						...
						pdo_update('rr_v_order_goods', array('commission1' => iserializer($cinfo['commission1']), 'commission2' => iserializer($cinfo['commission2']), 'commission3' => iserializer($cinfo['commission3']), 'commissions' => iserializer($commissions), 'nocommission' => $cinfo['nocommission']), array('id' => $cinfo['id']));
					}
				}
			...
			if (!$hascommission) {
				pdo_update('rr_v_order', array('agentid' => 0), array('id' => $orderid));
			}
		...
		public function getOrderCommissions($orderid = 0, $ogid = 0)
		{
			global $_W;
			$set = $this->getSet();
			$agentid = pdo_fetchcolumn('select agentid from ' . tablename('rr_v_order') . ' where id=:id limit 1', array(':id' => $orderid));
			$goods = pdo_fetch('select commission1,commission2,commission3 from ' . tablename('rr_v_order_goods') . ' where id=:id and orderid=:orderid and uniacid=:uniacid and nocommission=0 limit 1', array(':id' => $ogid, ':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
			$commissions = array('level1' => 0, 'level2' => 0, 'level3' => 0);
		...
		public function getInfo($openid, $options = NULL)
		{
			...
			if (1 <= $level) {
				if (in_array('ordercount0', $options)) {
					$level1_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from ' . tablename('rr_v_order') . ' o ' . ' left join  ' . tablename('rr_v_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid=:agentid and o.status>=0 and og.status1>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
				...
				if (in_array('ordercount', $options)) {
					$level1_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from ' . tablename('rr_v_order') . ' o ' . ' left join  ' . tablename('rr_v_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid=:agentid and o.status>=1 and og.status1>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
				...
				if (in_array('ordercount3', $options)) {
					$level1_ordercount3 = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from ' . tablename('rr_v_order') . ' o ' . ' left join  ' . tablename('rr_v_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid=:agentid and o.status>=3 and og.status1>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
				...
				if (in_array('total', $options)) {
					$level1_commissions = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('rr_v_order_goods') . ' og ' . ' left join  ' . tablename('rr_v_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
				...
				if (in_array('ok', $options)) {
					$level1_commissions = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('rr_v_order_goods') . ' og ' . ' left join  ' . tablename('rr_v_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and og.nocommission=0 and (' . $time . ' - o.finishtime > ' . $day_times . ') and og.status1=0  and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
				...
				if (in_array('lock', $options)) {
					$level1_commissions1 = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('rr_v_order_goods') . ' og ' . ' left join  ' . tablename('rr_v_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and og.nocommission=0 and (' . $time . ' - o.finishtime <= ' . $day_times . ')  and og.status1=0  and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
				...
				if (in_array('apply', $options)) {
					$level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('rr_v_order_goods') . ' og ' . ' left join  ' . tablename('rr_v_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and og.status1=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
				...
				if (in_array('check', $options)) {
					$level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('rr_v_order_goods') . ' og ' . ' left join  ' . tablename('rr_v_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and og.status1=2 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
				...
				if (in_array('pay', $options)) {
					$level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('rr_v_order_goods') . ' og ' . ' left join  ' . tablename('rr_v_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and og.status1=3 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
				...
				if (in_array('wait', $options)) {
					$level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('rr_v_order_goods') . ' og ' . ' left join  ' . tablename('rr_v_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and (o.status=2 or o.status=1) and og.status1=0  and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
				...
				$level1_agentids = pdo_fetchall('select id from ' . tablename('rr_v_member') . ' where agentid=:agentid and isagent=1 and status=1 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']), 'id');
			...
			if (2 <= $level) {
				if (0 < $level1) {
					if (in_array('ordercount0', $options)) {
						$level2_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from ' . tablename('rr_v_order') . ' o ' . ' left join  ' . tablename('rr_v_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=0 and og.status2>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1', array(':uniacid' => $_W['uniacid']));
					...
					if (in_array('ordercount3', $options)) {
						$level2_ordercount3 = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from ' . tablename('rr_v_order') . ' o ' . ' left join  ' . tablename('rr_v_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=3 and og.status2>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1', array(':uniacid' => $_W['uniacid']));
					...
					if (in_array('total', $options)) {
						$level2_commissions = pdo_fetchall('select og.commission2,og.commissions from ' . tablename('rr_v_order_goods') . ' og ' . ' left join  ' . tablename('rr_v_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid']));
					...
					if (in_array('lock', $options)) {
						$level2_commissions1 = pdo_fetchall('select og.commission2,og.commissions  from ' . tablename('rr_v_order_goods') . ' og ' . ' left join  ' . tablename('rr_v_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and (' . $time . ' - o.finishtime <= ' . $day_times . ') and og.status2=0 and o.status>=3 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid']));
					...
					if (in_array('apply', $options)) {
						$level2_commissions2 = pdo_fetchall('select og.commission2,og.commissions  from ' . tablename('rr_v_order_goods') . ' og ' . ' left join  ' . tablename('rr_v_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=3 and og.status2=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid']));
					...
					if (in_array('check', $options)) {
						$level2_commissions3 = pdo_fetchall('select og.commission2,og.commissions  from ' . tablename('rr_v_order_goods') . ' og ' . ' left join  ' . tablename('rr_v_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=3 and og.status2=2 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid']));
					...
					$level2_agentids = pdo_fetchall('select id from ' . tablename('rr_v_member') . ' where agentid in( ' . implode(',', array_keys($level1_agentids)) . ') and isagent=1 and status=1 and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']), 'id');
			...
			if (3 <= $level) {
				if (0 < $level2) {
					if (in_array('ordercount0', $options)) {
						$level3_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct og.orderid) as ordercount from ' . tablename('rr_v_order') . ' o ' . ' left join  ' . tablename('rr_v_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and o.status>=0 and og.status3>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1', array(':uniacid' => $_W['uniacid']));
					...
					if (in_array('ordercount3', $options)) {
						$level3_ordercount3 = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct og.orderid) as ordercount from ' . tablename('rr_v_order') . ' o ' . ' left join  ' . tablename('rr_v_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and o.status>=3 and og.status3>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1', array(':uniacid' => $_W['uniacid']));
					...
					if (in_array('total', $options)) {
						$level3_commissions = pdo_fetchall('select og.commission3,og.commissions  from ' . tablename('rr_v_order_goods') . ' og ' . ' left join  ' . tablename('rr_v_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid']));
					...
					$level3_agentids = pdo_fetchall('select id from ' . tablename('rr_v_member') . ' where uniacid=:uniacid and agentid in( ' . implode(',', array_keys($level2_agentids)) . ') and isagent=1 and status=1', array(':uniacid' => $_W['uniacid']), 'id');
		...
		public function getAgents($orderid = 0)
		{
			...
			global $_GPC;
			$agents = array();
			$order = pdo_fetch('select id,agentid,openid from ' . tablename('rr_v_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
		...
		public function getAgentsDownNum($openid = NULL)
		{
			...
			$level1 = (int) pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_member') . ' where agentid=:agentid and uniacid=:uniacid limit 1', array(':agentid' => $member['id'], ':uniacid' => $_W['uniacid']));
			if ((2 <= $set['level']) && (0 < $levelcount1)) {
				$level2 = (int) pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_member') . ' where agentid in( ' . implode(',', array_keys($member['level1_agentids'])) . ') and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
			}

			if ((3 <= $set['level']) && (0 < $levelcount2)) {
				$level3 = (int) pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_member') . ' where agentid in( ' . implode(',', array_keys($member['level2_agentids'])) . ') and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
			}
		...
		{
			global $_W;
			$path = IA_ROOT . '/addons/rr_vv3/data/qrcode/' . $_W['uniacid'];
			...
			return $_W['siteroot'] . 'addons/rr_vv3/data/qrcode/' . $_W['uniacid'] . '/' . $file;
		}
		...
		public function createGoodsImage($goods)
		{
			...
			$path = IA_ROOT . '/addons/rr_vv3/data/poster/' . $_W['uniacid'] . '/';
			...
			if (!is_file($path . $file)) {
				set_time_limit(0);
				$font = IA_ROOT . '/addons/rr_vv3/static/fonts/msyh.ttf';
				$target = imagecreatetruecolor(640, 1225);
				$bg = imagecreatefromjpeg(IA_ROOT . '/addons/rr_vv3/plugin/commission/static/images/poster.jpg');
			...
			return $_W['siteroot'] . 'addons/rr_vv3/data/poster/' . $_W['uniacid'] . '/' . $file;
		}
		...
		public function checkAgent($openid = '')
		{
			...
			if ($parent_is_agent) {
				if ($parent['openid'] != $openid) {
					$clickcount = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_commission_clickcount') . ' where uniacid=:uniacid and openid=:openid and from_openid=:from_openid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid, ':from_openid' => $parent['openid']));

					if ($clickcount <= 0) {
						$click = array('uniacid' => $_W['uniacid'], 'openid' => $openid, 'from_openid' => $parent['openid'], 'clicktime' => time());
						pdo_insert('rr_v_commission_clickcount', $click);
						pdo_update('rr_v_member', array('clickcount' => $parent['clickcount'] + 1), array('uniacid' => $_W['uniacid'], 'id' => $parent['id']));
			...
			$first = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_member') . ' where id<>:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $member['id']));

			if ($first <= 0) {
				pdo_update('rr_v_member', array('isagent' => 1, 'status' => 1, 'agenttime' => time(), 'agentblack' => 0), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
				return NULL;
			}
					...
					if (empty($become_child)) {
						if (empty($member['fixagentid'])) {
							...
							if ($author) {
								...
								pdo_update('rr_v_member', array('agentid' => $parent['id'], 'childtime' => $time, 'authorid' => $authorid), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
							}
							else {
								pdo_update('rr_v_member', array('agentid' => $parent['id'], 'childtime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
							}
								...
								if (!empty($author_set['become']) && (($author_set['become'] == '2') || ($author_set['become'] == '5'))) {
									...
									if ($can_author) {
										...
										if (empty($member['authorblack'])) {
											pdo_update('rr_v_member', array('authorstatus' => $become_check, 'isauthor' => 1, 'authortime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $parent['id']));
						...
						pdo_update('rr_v_member', array('inviter' => $parent['id']), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
					}
				}
			}
			...
			if (empty($set['become'])) {
				...
				$ordercount = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_order') . ' where uniacid=:uniacid and openid=:openid and status>=' . $order_status . ' limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
				...
				$moneycount = pdo_fetchcolumn('select sum(goodsprice) from ' . tablename('rr_v_order') . ' where uniacid=:uniacid and openid=:openid and status>=' . $order_status . ' limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
				...
				if ($set['become'] == 4) {
					$goods = pdo_fetch('select id,title,thumb,marketprice from' . tablename('rr_v_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $set['become_goodsid'], ':uniacid' => $_W['uniacid']));
					$goodscount = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_order_goods') . ' og ' . '  left join ' . tablename('rr_v_order') . ' o on o.id = og.orderid' . ' where og.goodsid=:goodsid and o.openid=:openid and o.status>=' . $order_status . '  limit 1', array(':goodsid' => $set['become_goodsid'], ':openid' => $openid));
			...
			if ($to_check_agent) {
				pdo_update('rr_v_member', array('isagent' => 1, 'status' => $become_check, 'agenttime' => $become_check == 1 ? $time : 0), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
		...
		public function checkOrderConfirm($orderid = '0')
		{
			...
			$order = pdo_fetch('select id,openid,ordersn,goodsprice,agentid,paytime from ' . tablename('rr_v_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
			...
			if (!empty($agentid)) {
				pdo_update('rr_v_order', array('agentid' => $agentid), array('id' => $orderid));
			}
		...
		public function checkOrderPay($orderid = '0')
		{
			...
			$order = pdo_fetch('select id,openid,ordersn,goodsprice,agentid,paytime from ' . tablename('rr_v_order') . ' where id=:id and status>=1 and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
					...
					if (empty($member['agentid']) && ($member['id'] != $parent['id'])) {
						if (empty($member['fixagentid'])) {
							...
							if (empty($order['agentid'])) {
								...
								pdo_update('rr_v_order', array('agentid' => $parent['id']), array('id' => $orderid));
								$this->calculate($orderid);
							}
						}
					}
			...
			if (!$isagent) {
				if ((intval($set['become']) == 4) && !empty($set['become_goodsid'])) {
					if (empty($set['become_order'])) {
						$order_goods = pdo_fetchall('select goodsid from ' . tablename('rr_v_order_goods') . ' where orderid=:orderid and uniacid=:uniacid  ', array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']), 'goodsid');

						if (in_array($set['become_goodsid'], array_keys($order_goods))) {
							if (empty($member['agentblack'])) {
								pdo_update('rr_v_member', array('status' => $become_check, 'isagent' => 1, 'agenttime' => $become_check == 1 ? $time : 0), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
					...
					if (($set['become'] == 2) || ($set['become'] == 3)) {
						if (empty($set['become_order'])) {
							...
							if ($set['become'] == '2') {
								$ordercount = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_order') . ' where openid=:openid and status>=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
								$can = intval($set['become_ordercount']) <= $ordercount;
							}
							else {
								if ($set['become'] == '3') {
									$moneycount = pdo_fetchcolumn('select sum(og.realprice) from ' . tablename('rr_v_order_goods') . ' og left join ' . tablename('rr_v_order') . ' o on og.orderid=o.id  where o.openid=:openid and o.status>=1 and o.uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
							...
							if ($can) {
								if (empty($member['agentblack'])) {
									pdo_update('rr_v_member', array('status' => $become_check, 'isagent' => 1, 'agenttime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
			...
			if (!empty($member['agentid'])) {
				$parent = m('member')->getMember($member['agentid']);
				if (!empty($parent) && ($parent['isagent'] == 1) && ($parent['status'] == 1)) {
					$order_goods = pdo_fetchall('select g.id,g.title,og.total,og.price,og.realprice, og.optionname as optiontitle,g.noticeopenid,g.noticetype,og.commission1,og.commissions  from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']));
					$goods = '';
					$commission_total1 = 0;
					$commission_total2 = 0;
					$commission_total3 = 0;
				...
				if ((intval($set['become']) == 4) && !empty($set['become_goodsid'])) {
					if (empty($set['become_order'])) {
						...
						if (in_array($set['become_goodsid'], array_keys($order_goods))) {
							if (empty($member['partnerblack'])) {
								pdo_update('rr_v_member', array('partnerstatus' => $become_check, 'ispartner' => 1, 'partnertime' => $become_check == 1 ? $time : 0), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
					...
					if (($set['become'] == 2) || ($set['become'] == 3)) {
						if (empty($set['become_order'])) {
							...
							if ($can) {
								if (empty($member['partnerblack'])) {
									pdo_update('rr_v_member', array('partnerstatus' => $become_check, 'ispartner' => 1, 'partnertime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
		...
		public function checkOrderFinish($orderid = '')
		{
			...
			$order = pdo_fetch('select id,openid, ordersn,goodsprice,agentid,finishtime from ' . tablename('rr_v_order') . ' where id=:id and status>=3 and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
			...
			if (!$isagent && ($set['become_order'] == '1')) {
				if (($set['become'] == '4') && !empty($set['become_goodsid'])) {
					...
					if (($set['become'] == 2) || ($set['become'] == 3)) {
						...
						if ($set['become'] == '2') {
							$ordercount = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_order') . ' where openid=:openid and status>=3 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
							$can = intval($set['become_ordercount']) <= $ordercount;
						}
						else {
							if ($set['become'] == '3') {
								$moneycount = pdo_fetchcolumn('select sum(goodsprice) from ' . tablename('rr_v_order') . ' where openid=:openid and status>=3 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
								$can = floatval($set['become_moneycount']) <= $moneycount;
							}
						}
			...
			if (!empty($member['agentid'])) {
				$parent = m('member')->getMember($member['agentid']);
				if (!empty($parent) && ($parent['isagent'] == 1) && ($parent['status'] == 1)) {
					$order_goods = pdo_fetchall('select g.id,g.title,og.total,og.realprice,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype,og.commission1,og.commissions from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']));
					$goods = '';
					$commission_total1 = 0;
					$commission_total2 = 0;
					$commission_total3 = 0;
					...
					if (!empty($set['open'])) {
						...
						if ($isauthor) {
							...
							if ($set['become_order'] == '1') {
								...
								if ($can) {
									if (empty($member['authorblack'])) {
										pdo_update('rr_v_member', array('authorstatus' => $become_check, 'isauthor' => 1, 'authortime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
		...
		public function getLevels($all = true, $default = false)
		{
			...
			if ($all) {
				$levels = pdo_fetchall('select * from ' . tablename('rr_v_commission_level') . ' where uniacid=:uniacid order by commission1 asc', array(':uniacid' => $_W['uniacid']));
			}
			else {
				$levels = pdo_fetchall('select * from ' . tablename('rr_v_commission_level') . ' where uniacid=:uniacid and (ordermoney>0 or commissionmoney>0) order by commission1 asc', array(':uniacid' => $_W['uniacid']));
			}
		...
		public function getLevel($openid)
		{
			...
			$level = pdo_fetch('select * from ' . tablename('rr_v_commission_level') . ' where uniacid=:uniacid and id=:id limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $member['agentlevel']));
			return $level;
		}
		...
		public function upgradeLevelByOrder($openid)
		{
			...
			if (($leveltype == 4) || ($leveltype == 5)) {
				...
				$orders = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct og.orderid) as ordercount from ' . tablename('rr_v_order') . ' o ' . ' left join  ' . tablename('rr_v_order_goods') . ' og on og.orderid=o.id ' . ' where o.openid=:openid and o.status>=3 and o.uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
				$ordermoney = $orders['ordermoney'];
				$ordercount = $orders['ordercount'];

				if ($leveltype == 4) {
					$newlevel = pdo_fetch('select * from ' . tablename('rr_v_commission_level') . ' where uniacid=:uniacid  and ' . $ordermoney . ' >= ordermoney and ordermoney>0  order by ordermoney desc limit 1', array(':uniacid' => $_W['uniacid']));
					...
					if ($leveltype == 5) {
						$newlevel = pdo_fetch('select * from ' . tablename('rr_v_commission_level') . ' where uniacid=:uniacid  and ' . $ordercount . ' >= ordercount and ordercount>0  order by ordercount desc limit 1', array(':uniacid' => $_W['uniacid']));
				...
				pdo_update('rr_v_member', array('agentlevel' => $newlevel['id']), array('id' => $m['id']));
				...
				if ((0 <= $leveltype) && ($leveltype <= 3)) {
					...
					foreach ($agents as $agent) {
						...
						pdo_update('rr_v_member', array('agentlevel' => $newlevel['id']), array('id' => $agent['id']));
		...
		public function upgradeLevelByAgent($openid)
		{
			...
			if (($leveltype == 6) || ($leveltype == 8)) {
				...
				foreach ($agents as $agent) {
					...
					if ($leveltype == 6) {
						$downs1 = pdo_fetchall('select id from ' . tablename('rr_v_member') . ' where agentid=:agentid and uniacid=:uniacid ', array(':agentid' => $m['id'], ':uniacid' => $_W['uniacid']), 'id');
						$downcount += count($downs1);

						if (!empty($downs1)) {
							$downs2 = pdo_fetchall('select id from ' . tablename('rr_v_member') . ' where agentid in( ' . implode(',', array_keys($downs1)) . ') and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']), 'id');
							$downcount += count($downs2);

							if (!empty($downs2)) {
								$downs3 = pdo_fetchall('select id from ' . tablename('rr_v_member') . ' where agentid in( ' . implode(',', array_keys($downs2)) . ') and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']), 'id');
								$downcount += count($downs3);
							}
						}

						$newlevel = pdo_fetch('select * from ' . tablename('rr_v_commission_level') . ' where uniacid=:uniacid  and ' . $downcount . ' >= downcount and downcount>0  order by downcount desc limit 1', array(':uniacid' => $_W['uniacid']));
					}
					else {
						if ($leveltype == 8) {
				...
				if ($leveltype == 7) {
					$downcount = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_member') . ' where agentid=:agentid and uniacid=:uniacid ', array(':agentid' => $m['id'], ':uniacid' => $_W['uniacid']));
					$newlevel = pdo_fetch('select * from ' . tablename('rr_v_commission_level') . ' where uniacid=:uniacid  and ' . $downcount . ' >= downcount and downcount>0  order by downcount desc limit 1', array(':uniacid' => $_W['uniacid']));
				}
				else {
					if ($leveltype == 9) {
		...
		public function upgradeLevelByCommissionOK($openid)
		{
			...
			$newlevel = pdo_fetch('select * from ' . tablename('rr_v_commission_level') . ' where uniacid=:uniacid  and ' . $commissionmoney . ' >= commissionmoney and commissionmoney>0  order by commissionmoney desc limit 1', array(':uniacid' => $_W['uniacid']));
		...
		{
			global $_W;
			return array('total1' => pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_commission_apply') . ' where status=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'])), 'total2' => pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_commission_apply') . ' where status=2 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'])), 'total3' => pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_commission_apply') . ' where status=3 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'])), 'total_1' => pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_commission_apply') . ' where status=-1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'])));
		}
		...
		protected function sendNotice($touser, $tm, $tag, $datas, $member, $msg)
		{
			...
			if (!empty($tm['is_advanced']) && !empty($tm[$tag])) {
				$advanced_template = pdo_fetch('select * from ' . tablename('rr_v_member_message_template') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $tm[$tag], ':uniacid' => $_W['uniacid']));
		...
		public function getLastApply($mid, $type = -1)
		{
			...
			$sql = 'select type,alipay,bankname,bankcard,realname from ' . tablename('rr_v_commission_apply') . ' where mid=:mid and uniacid=:uniacid';
		...
		public function getRepurchase($openid, array $time)
		{
			...
			foreach ($time as $value) {
				...
				if (!empty($repurchase_price)) {
					$order_price = (double) pdo_fetchcolumn('SELECT SUM(price) as price FROM ' . tablename('rr_v_order') . ' WHERE `uniacid`=:uniacid AND `openid`=:openid AND `status`>2 AND `createtime` BETWEEN :time1 AND :time2', array(':uniacid' => $_W['uniacid'], ':openid' => $openid, ':time1' => $time1, ':time2' => $time2));
					$year_month = explode('-', $value);
					$year_month[0] = (int) $year_month[0];
					$year_month[1] = (int) $year_month[1];
					$residue_price = (double) pdo_fetchcolumn('SELECT SUM(repurchase) FROM ' . tablename('rr_v_commission_repurchase') . ' WHERE `uniacid`=:uniacid AND `openid`=:openid AND `year`=:year AND `month`=:month', array(':uniacid' => $_W['uniacid'], ':openid' => $openid, ':year' => $year_month[0], ':month' => $year_month[1]));
		...
		public function getAllDown($openid)
		{
			...
			if ($uid == 0) {
				$info = pdo_fetch('select id,openid,uniacid,agentid,isagent,status from ' . tablename('rr_v_member') . ' where  openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			...
			$agents = pdo_fetchall('select id,openid,uniacid,agentid,isagent,nickname,agenttime,createtime,avatar,status,realname,mobile,weixin from ' . tablename('rr_v_member') . ' where uniacid=:uniacid and agentid=:agentid', array(':uniacid' => $_W['uniacid'], ':agentid' => $uid));
			$ids = array();
			$openids = array();
			$users = array();
		...
		public function getAllDownOrder($openid, $start = 0, $end = 0)
		{
			...
			if (!empty($down['openids'])) {
				$order = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_order') . ' WHERE uniacid=:uniacid AND openid IN (\'' . implode('\',\'', $down['openids']) . '\') AND createtime BETWEEN :time1 AND :time2 AND ccard>0', array(':uniacid' => $_W['uniacid'], ':time1' => $start, ':time2' => $end));
