...
class Rank_RrvV3Page extends PluginWebPage
{
	...
	{
		global $_W;
		$result = pdo_fetchall('SELECT openid FROM ' . tablename('rr_v_member') . ' WHERE uniacid = :uniacid AND status=1 AND agentblack=0', array(':uniacid' => $_W['uniacid']));
		...
		if (!empty($openid)) {
			if (is_array($openid)) {
				foreach ($openid as $val) {
					...
					pdo_update('rr_v_member', $data, array('openid' => $val, 'uniacid' => $_W['uniacid']));
				}
			...
			$data = array('commission_total' => $member['commission_total']);
			pdo_update('rr_v_member', $data, array('openid' => $openid, 'uniacid' => $_W['uniacid']));
			show_json(1, $data);
		}
