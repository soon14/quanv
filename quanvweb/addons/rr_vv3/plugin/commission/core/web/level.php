...
class Level_RrvV3Page extends PluginWebPage
{
	public function main()
	{
		...
		$others = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_commission_level') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY commission1 asc');
	...
	protected function post()
	{
		...
		if ($id == 'default') {
			...
			$level = pdo_fetch('SELECT * FROM ' . tablename('rr_v_commission_level') . ' WHERE id=:id and uniacid=:uniacid limit 1', array(':id' => intval($id), ':uniacid' => $_W['uniacid']));
		}
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				if ($id == 'default') {
					...
					pdo_update('rr_v_commission_level', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
					plog('commission.level.edit', '修改分销商等级 ID: ' . $id . $updatecontent);
				}
			}
			else {
				pdo_insert('rr_v_commission_level', $data);
				$id = pdo_insertid();
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,levelname FROM ' . tablename('rr_v_commission_level') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_delete('rr_v_commission_level', array('id' => $item['id']));
