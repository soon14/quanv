...
class Index_RrvV3Page extends PluginWebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('rr_message_mass_task') . ' WHERE 1 ' . $condition . '  ORDER BY id asc limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('rr_message_mass_task') . ' WHERE 1 ' . $condition, $params);
		...
		foreach ($list as &$item) {
			...
			$counts = pdo_fetchall('select `status`,COUNT(*) as num from ' . tablename('rr_message_mass_sign') . ' WHERE taskid=' . $item['id'] . '  GROUP BY `status`');
			$num = 0;
			$item['nosend'] = 0;
			$item['sendsuccess'] = 0;
			$item['sendfail'] = 0;
	...
	public function showsign()
	{
		...
		$list = pdo_fetchall('SELECT *  FROM ' . tablename('rr_message_mass_sign') . ' WHERE 1 ' . $condition . '  ORDER BY id asc limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('rr_message_mass_sign') . ' WHERE 1 ' . $condition, $params);
		$pager = pagination2($total, $pindex, $psize);
		include $this->template();
	}
	...
	protected function post()
	{
		...
		if (!empty($id)) {
			$item = pdo_fetch('SELECT * FROM ' . tablename('rr_message_mass_task') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $_GPC['id'], ':uniacid' => $_W['uniacid']));

			if (!empty($item['templateid'])) {
				$send = pdo_fetch('SELECT *  FROM ' . tablename('rr_message_mass_template') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $item['templateid'], ':uniacid' => $_W['uniacid']));
			}
		...
		$list3 = pdo_fetchall('SELECT *  FROM ' . tablename('rr_v_commission_level') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY id asc');
		...
		if ($_W['ispost']) {
			...
			if (empty($id)) {
				pdo_insert('rr_message_mass_task', $data);
				$id = pdo_insertid();
				plog('messages.template.add', '添加群发模板 ID: ' . $id . ' 标题: ' . $data['title'] . ' ');
			}
			else {
				pdo_update('rr_message_mass_task', $data, array('id' => $id));
	...
	public function build()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,sendlimittype,send_openid,send_level,send_group,send_agentlevel FROM ' . tablename('rr_message_mass_task') . ' WHERE id =:id AND uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));

		if (!empty($item)) {
			$params = array(':taskid' => $item['id'], ':uniacid' => $_W['uniacid']);
			$sql = 'insert into  ' . tablename('rr_message_mass_sign') . '  (uniacid,openid,nickname,taskid,`status`) ';
			$sql .= ' SELECT  :uniacid,sm.openid,sm.nickname,:taskid,1 from ' . tablename('rr_v_member') . ' sm  inner join ' . tablename('mc_mapping_fans') . ' mf  on sm.openid=mf.openid and mf.follow=1 where sm.uniacid=:uniacid ';
			...
			pdo_delete('rr_message_mass_sign', array('taskid' => $id, 'uniacid' => $_W['uniacid']));
			pdo_query($sql, $params);
			$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('rr_message_mass_sign') . 'WHERE taskid =\'' . $id . '\' and uniacid = \'' . $_W['uniacid'] . '\'');
			$data = array('sendnum' => intval($total), 'successnum' => 0, 'failnum' => 0, 'status' => 1);
			pdo_update('rr_message_mass_task', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
		}
	...
	public function reset()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,title FROM ' . tablename('rr_message_mass_task') . ' WHERE id =:id AND uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));

		if (!empty($item)) {
			$data = array('status' => 1);
			pdo_update('rr_message_mass_task', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
			$data['log'] = '';
			pdo_update('rr_message_mass_sign', $data, array('taskid' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function delete()
	{
		...
		if (!empty($item)) {
			pdo_delete('rr_message_mass_task', array('id' => $id, 'uniacid' => $_W['uniacid']));
			pdo_delete('rr_message_mass_sign', array('taskid' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function run()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$successnum = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('rr_message_mass_sign') . 'WHERE taskid =\'' . $id . '\'  and status =3  and uniacid = \'' . $_W['uniacid'] . '\'');
		$failnum = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('rr_message_mass_sign') . 'WHERE taskid =\'' . $id . '\'  and status =4  and uniacid = \'' . $_W['uniacid'] . '\'');
		$sendnum = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('rr_message_mass_sign') . 'WHERE taskid =\'' . $id . '\'   and uniacid = \'' . $_W['uniacid'] . '\'');
		$data = array('sendnum' => intval($sendnum), 'successnum' => intval($successnum), 'failnum' => intval($failnum));
		pdo_update('rr_message_mass_task', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
		$item = pdo_fetch('SELECT  *   FROM ' . tablename('rr_message_mass_task') . ' WHERE id =:id AND uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function fetch()
	{
		...
		$items = pdo_fetchall('SELECT s.id,s.taskid,s.openid,s.nickname,t.messagetype,t.templateid,t.resptitle,t.respthumb,t.respdesc,t.respurl,t.customertype,t.resdesc2  FROM ' . tablename('rr_message_mass_sign') . ' s inner join ' . tablename('rr_message_mass_task') . " t\r\n          on s.taskid = t.id  WHERE s.taskid =:id AND s.uniacid=:uniacid and s.status =1 order by id limit  " . $pagecount, array(':id' => $id, ':uniacid' => $_W['uniacid']));
		$successnum = 0;
		$failnum = 0;
		$count = count($items);
		$hasnext = 1;
		...
		foreach ($items as $item) {
			...
			pdo_update('rr_message_mass_sign', $data, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
		}
	...
	public function sendTplNotice($openid, $templateid, $nickname)
	{
		...
		if (!empty($templateid)) {
			$template = pdo_fetch('SELECT * FROM ' . tablename('rr_message_mass_template') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $templateid, ':uniacid' => $_W['uniacid']));
