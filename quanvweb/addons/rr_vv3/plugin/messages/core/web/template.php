...
class Template_RrvV3Page extends PluginWebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('rr_message_mass_template') . ' WHERE 1 ' . $condition . '  ORDER BY id asc limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('rr_message_mass_template') . ' WHERE 1 ' . $condition, $params);
	...
	protected function post()
	{
		...
		if (!empty($_GPC['id'])) {
			$list = pdo_fetch('SELECT * FROM ' . tablename('rr_message_mass_template') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $_GPC['id'], ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if (empty($id)) {
				pdo_insert('rr_message_mass_template', $insert);
				$id = pdo_insertid();
				plog('sysset.tmessage.delete', '添加群发模板 ID: ' . $id . ' 标题: ' . $insert['title'] . ' ');
			}
			else {
				pdo_update('rr_message_mass_template', $insert, array('id' => $id));
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('rr_message_mass_template') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_delete('rr_message_mass_template', array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function query()
	{
		...
		$ds = pdo_fetchall('SELECT id,title FROM ' . tablename('rr_message_mass_template') . ' WHERE 1 ' . $condition . ' order by id asc', $params);
	...
	public function check()
	{
		...
		if ($_W['ispost']) {
			...
			$member = pdo_fetch('select *  from ' . tablename('rr_v_member') . ' where openid=:openid and uniacid = :uniacid ', array(':openid' => $check_openid, ':uniacid' => $_W['uniacid']));
