...
class aliApy
{
		...
		if (!empty($log) && ($log['status'] == '0')) {
			...
			if (!is_error($site)) {
				...
				if (method_exists($site, $method)) {
					...
					pdo_update('rr_v_order', array('paytype' => 22), array('uniacid' => $log['uniacid'], 'ordersn' => $log['tid']));
					...
					if ($result) {
						...
						pdo_update('rr_v_order', array('paytype' => 22, 'apppay' => $this->isapp ? 1 : 0, 'transid' => $this->post['trade_no']), array('ordersn' => $log['tid'], 'uniacid' => $log['uniacid']));
						exit('success');
					}
				}
			}
	...
	public function threen()
	{
		...
		$log = pdo_fetch('SELECT * FROM ' . tablename('rr_v_threen_log') . ' WHERE `uniacid`=:uniacid and id=:id limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $logno));
	...
	public function recharge()
	{
		...
		if (!empty($log) && empty($log['status'])) {
			pdo_update('rr_v_member_log', array('status' => 1, 'rechargetype' => 'alipay', 'apppay' => $this->isapp ? 1 : 0, 'transid' => $this->post['trade_no']), array('id' => $log['id']));
	...
	{
		global $_W;
		$apply = pdo_fetch('select * from ' . tablename('rr_v_commission_apply') . ' where id=:id limit 1', array(':id' => $id));
	...
	{
		$log = pdo_fetch('select * from ' . tablename('rr_v_member_log') . ' where id=:id limit 1', array(':id' => $id));
		...
		pdo_update('rr_v_member_log', array('status' => 1), array('id' => $id, 'uniacid' => $_W['uniacid']));
		m('notice')->sendMemberLogMessage($log['id']);
	...
	{
		$item = pdo_fetch('SELECT * FROM ' . tablename('rr_v_order') . ' WHERE id = :id limit 1', array(':id' => $id));
		...
		$time = time();
		$goods = pdo_fetchall('SELECT g.id,g.credit, o.total,o.realprice FROM ' . tablename('rr_v_order_goods') . ' o left join ' . tablename('rr_v_goods') . ' g on o.goodsid=g.id ' . ' WHERE o.orderid=:orderid and o.uniacid=:uniacid', array(':orderid' => $item['id'], ':uniacid' => $item['uniacid']));
		...
		pdo_update('rr_v_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => $time), array('id' => $item['id'], 'uniacid' => $item['uniacid']));

		foreach ($goods as $g) {
			$salesreal = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_order') . ' o on o.id = og.orderid ' . ' where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid limit 1', array(':goodsid' => $g['id'], ':uniacid' => $item['uniacid']));
	...
	public function batch_refund_notify_rc($id, $money)
	{
		...
		$GLOBALS['_W']['uniacid'] = $log['uniacid'];
		$_W['uniacid'] = $log['uniacid'];
		pdo_update('rr_v_member_log', array('status' => 3), array('id' => $id, 'uniacid' => $_W['uniacid']));
