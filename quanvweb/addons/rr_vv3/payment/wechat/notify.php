...
class EweiShopWechatPay
{
	...
	public function order()
	{
		...
		$tid = substr($tid, 0, 22);
		$order = pdo_fetch('SELECT * FROM ' . tablename('rr_v_order') . ' WHERE ordersn = :ordersn AND uniacid = :uniacid', array(':ordersn' => $tid, ':uniacid' => $_W['uniacid']));
		$sql = 'SELECT * FROM ' . tablename('core_paylog') . ' WHERE `module`=:module AND `tid`=:tid  limit 1';
		$params = array();
		$params[':tid'] = $tid;
		$params[':module'] = 'rr_vv3';
		$log = pdo_fetch($sql, $params);
		if (!empty($log) && (($log['status'] == '0') || $ispeerpay) && (($log['fee'] == $this->total_fee) || $ispeerpay)) {
			pdo_update('rr_v_order', array('paytype' => $paytype, 'isborrow' => $isborrow, 'borrowopenid' => $borrowopenid, 'apppay' => $this->isapp ? 1 : 0), array('ordersn' => $log['tid'], 'uniacid' => $log['uniacid']));
	...
	public function recharge()
	{
		...
		$log = pdo_fetch('SELECT * FROM ' . tablename('rr_v_member_log') . ' WHERE `uniacid`=:uniacid and `logno`=:logno limit 1', array(':uniacid' => $_W['uniacid'], ':logno' => $logno));
		$OK = !empty($log) && empty($log['status']) && ($log['money'] == $this->total_fee);

		if ($OK) {
			pdo_update('rr_v_member_log', array('status' => 1, 'rechargetype' => 'wechat', 'isborrow' => $isborrow, 'borrowopenid' => $borrowopenid, 'apppay' => $this->isapp ? 1 : 0), array('id' => $log['id']));
	...
	public function creditShopFreight()
	{
		...
		$log = pdo_fetch('SELECT * FROM ' . tablename('rr_v_creditshop_log') . ' WHERE `dispatchno`=:dispatchno and `uniacid`=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':dispatchno' => $dispatchno));
		if (!empty($log) && ($log['dispatchstatus'] < 0)) {
			pdo_update('rr_v_creditshop_log', array('dispatchstatus' => 1), array('dispatchno' => $dispatchno));
		}
	}
	...
	public function coupon()
	{
		...
		$coupon = pdo_fetchcolumn('select money from ' . tablename('rr_v_coupon') . ' where id=:id limit 1', array(':id' => $log['couponid']));
	...
	public function groups()
	{
		...
		if ($this->is_jie) {
			pdo_update('rr_v_groups_order', array('isborrow' => '1', 'borrowopenid' => $this->get['openid']), array('orderno' => $orderno, 'uniacid' => $_W['uniacid']));
		}
	...
	public function threen()
	{
		...
		if ($this->is_jie) {
			pdo_update('rr_v_threen_log', array('isborrow' => '1', 'borrowopenid' => $this->get['openid']), array('logno' => $orderno, 'uniacid' => $_W['uniacid']));
		}
	...
	{
		global $_W;
		$setting = pdo_fetch('select * from ' . tablename('rr_v_system_grant_setting') . ' where id = 1 limit 1 ');
		...
		if (0 < $setting['weixin']) {
			...
			if ($this->sign == $this->get['sign']) {
				$order = pdo_fetch('select * from ' . tablename('rr_v_system_grant_order') . ' where logno = \'' . $this->get['out_trade_no'] . '\'');
				pdo_update('rr_v_system_grant_order', array('paytime' => time(), 'paystatus' => 1), array('logno' => $this->get['out_trade_no']));
				...
				foreach ($plugind as $key => $value) {
					$plugin = pdo_fetch('select `identity` from ' . tablename('rr_v_plugin') . ' where id = ' . $value . ' ');
					$data['identity'] = $plugin['identity'];
					$data['pluginid'] = $value;
					pdo_insert('rr_v_system_grant_log', $data);
					$id = pdo_insertid();
	...
	{
		global $_W;
		$setting = pdo_fetch('select * from ' . tablename('rr_v_system_plugingrant_setting') . ' where 1 = 1 limit 1 ');
		...
		if (0 < $setting['weixin']) {
			...
			if ($this->sign == $this->get['sign']) {
				$order = pdo_fetch('select * from ' . tablename('rr_v_system_plugingrant_order') . ' where logno = \'' . $this->get['out_trade_no'] . '\'');
				pdo_update('rr_v_system_plugingrant_order', array('paytime' => time(), 'paystatus' => 1), array('logno' => $this->get['out_trade_no']));
				$plugind = explode(',', $order['pluginid']);
				...
				foreach ($plugind as $key => $value) {
					...
					$data['identity'] = $plugin['identity'];
					$data['pluginid'] = $value;
					pdo_query('update ' . tablename('rr_v_system_plugingrant_plugin') . ' set sales = sales + 1 where pluginid = ' . $value . ' ');
					pdo_insert('rr_v_system_plugingrant_log', $data);
					$id = pdo_insertid();
	...
	public function mr()
	{
		...
		if (p('mr')) {
			$price = pdo_fetchcolumn('select payprice from ' . tablename('rr_v_mr_order') . ' where ordersn=:ordersn limit 1', array(':ordersn' => $ordersn));

			if ($price == $this->total_fee) {
				if ($isborrow == 1) {
					pdo_update('rr_v_order', array('isborrow' => $isborrow, 'borrowopenid' => $borrowopenid), array('ordersn' => $ordersn));
				}
		...
		if (!empty($log) && ($log['status'] == '0') && ($log['fee'] == $this->total_fee)) {
			...
			if (!is_error($site)) {
				...
				if (method_exists($site, $method)) {
					...
					$ret['fee'] = $log['fee'];
					$ret['tag'] = $log['tag'];
					pdo_update('rr_v_order', array('paytype' => 21, 'apppay' => 2), array('ordersn' => $log['tid'], 'uniacid' => $log['uniacid']));
					$result = $site->$method($ret);
	...
	public function wxapp_recharge()
	{
		...
		if ($OK) {
			pdo_update('rr_v_member_log', array('status' => 1, 'rechargetype' => 'wechat', 'apppay' => 2), array('id' => $log['id']));
			$shopset = m('common')->getSysset('shop');
			m('member')->setCredit($log['openid'], 'credit2', $log['money'], array(0, $shopset['name'] . '会员充值:wechatnotify:credit2:' . $log['money']));
			m('member')->setRechargeCredit($log['openid'], $log['money']);
			com_run('sale::setRechargeActivity', $log);
			...
			if ($log['money'] == $this->total_fee) {
				pdo_update('rr_v_member_log', array('rechargetype' => 'wechat', 'apppay' => 2), array('id' => $log['id']));
			}
		}
	}
...
require IA_ROOT . '/addons/rr_vv3/core/inc/functions.php';
require IA_ROOT . '/addons/rr_vv3/core/inc/plugin_model.php';
require IA_ROOT . '/addons/rr_vv3/core/inc/com_model.php';
