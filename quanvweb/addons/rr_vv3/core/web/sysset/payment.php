...
class Payment_RrvV3Page extends WebPage
{
	...
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_payment') . ' WHERE 1 ' . $condition . '  ORDER BY id asc limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('rr_v_payment') . ' WHERE 1 ' . $condition, $params);
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (empty($id)) {
				...
				pdo_insert('rr_v_payment', $data);
				$id = pdo_insertid();
				plog('sysset.payment.add', '添加支付信息一条 ID: ' . $id . ' 标题: ' . $data['title'] . ' ');
			}
			else {
				pdo_update('rr_v_payment', $data, array('id' => $id));
		...
		if (!empty($id)) {
			$data = pdo_fetch('SELECT * FROM ' . tablename('rr_v_payment') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		}
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('rr_v_payment') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_delete('rr_v_payment', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function query()
	{
		...
		$ds = pdo_fetchall('SELECT id,title FROM ' . tablename('rr_v_member_printer_template') . ' WHERE 1 ' . $condition . ' order by id asc', $params);
