...
class Printer_RrvV3Page extends WebPage
{
	...
	protected function post()
	{
		...
		if (!empty($id)) {
			$list = pdo_fetch('SELECT * FROM ' . tablename('rr_v_member_printer_template') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('rr_v_member_printer_template') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid'] . ' AND merchid=0');
	...
	protected function printer_post()
	{
		...
		if (!empty($id)) {
			$list = pdo_fetch('SELECT * FROM ' . tablename('rr_v_member_printer') . ' WHERE id=:id and uniacid=:uniacid and merchid=0', array(':id' => $id, ':uniacid' => $_W['uniacid']));
			$print_data = json_decode($list['print_data'], true);
	...
	public function printer_delete()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('rr_v_member_printer') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid'] . '  and merchid=0');
