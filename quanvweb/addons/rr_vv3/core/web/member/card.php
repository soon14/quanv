...
class Card_RrvV3Page extends WebPage
{
	...
	public function activationset()
	{
		...
		if (!empty($item['couponid'])) {
			$coupon = pdo_fetch('SELECT id,couponname as title , thumb  FROM ' . tablename('rr_v_coupon') . ' WHERE uniacid = ' . $_W['uniacid'] . ' and id =' . $item['couponid']);
		}

		$levels = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_member_level') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY level asc');
