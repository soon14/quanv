...
class Index_RrvV3Page extends WebPage
{
	...
	protected function selectMemberCreate($day = 0)
	{
		...
		$sql = 'select count(*) from ' . tablename('rr_v_member') . ' where uniacid = :uniacid and createtime between :createtime1 and :createtime2';
	...
	protected function ajaxnewmember($day = 0)
	{
		...
		$member_count = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_member') . ' where uniacid=:uniacid', $param);
	...
	protected function ajaxmembergender()
	{
		...
		$sql_member = 'select gender,count(gender) as gender_num from ' . tablename('rr_v_member') . ' where uniacid = :uniacid group by gender';
	...
	{
		global $_W;
		$levels = pdo_fetchall('select * from ' . tablename('rr_v_member_level') . ' where uniacid=:uniacid order by level asc', array(':uniacid' => $_W['uniacid']), 'id');
		$levelname = array();
		...
		$levelname[0] = '普通等级';
		ksort($levelname);
		$sql_level = 'select level,count(level) as level_num from ' . tablename('rr_v_member') . ' where uniacid = :uniacid group by level';
	...
	{
		global $_W;
		$province = pdo_fetchall('select province,count(province) as province_num from ' . tablename('rr_v_member') . ' where uniacid = :uniacid group by province', array(':uniacid' => $_W['uniacid']));
		$result = array();
