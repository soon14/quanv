...
class List_RrvV3Page extends WebPage
{
	public function main()
	{
		...
		$sql = 'select * from ' . tablename('rr_v_member') . ' dm ' . $join . ' where 1 ' . $condition . '  ORDER BY id DESC,dm.nickname,dm.MD5nickname';
		...
		if (!empty($list_group)) {
			$res_group = pdo_fetchall('select id,groupname from ' . tablename('rr_v_member_group') . ' where id in (' . implode(',', $list_group) . ')', array(), 'id');
		}

		isset($list_level) && ($list_level = array_values(array_filter($list_level)));

		if (!empty($list_level)) {
			$res_level = pdo_fetchall('select id,levelname from ' . tablename('rr_v_member_level') . ' where id in (' . implode(',', $list_level) . ')', array(), 'id');
		}

		isset($list_agent) && ($list_agent = array_values(array_filter($list_agent)));

		if (!empty($list_agent)) {
			$res_agent = pdo_fetchall('select id,nickname as agentnickname,avatar as agentavatar from ' . tablename('rr_v_member') . ' where id in (' . implode(',', $list_agent) . ')', array(), 'id');
		}

		isset($list_fans) && ($list_fans = array_values(array_filter($list_fans)));

		if (!empty($list_fans)) {
			$res_fans = pdo_fetchall('select fanid,openid,follow as followed, unfollowtime, tag from ' . tablename('mc_mapping_fans') . ' where openid in (\'' . implode('\',\'', $list_fans) . '\')', array(), 'openid');
		}
		...
		foreach ($list as &$row) {
			...
			$row['ordercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $row['openid']));
			$row['ordermoney'] = pdo_fetchcolumn('select sum(price) from ' . tablename('rr_v_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $row['openid']));
			$row['credit1'] = m('member')->getCredit($row['openid'], 'credit1');
			$row['credit2'] = m('member')->getCredit($row['openid'], 'credit2');
			if(empty($row['nickname']) && !empty($row['wxnickname'])){
				// $row['nickname'] = strip_emoji(base64_decode($row['wxnickname']));
				$row['nickname'] = base64_decode($row['wxnickname']);
			}
	...
	'columns' => array(
		...
		$membertotal = pdo_fetchcolumn('select count(*) from' . tablename('rr_v_member') . ' where uniacid='.$_W['uniacid'].' ');
	...
	public function detail()
	{
		...
		if(empty($member['nickname']) && !empty($member['wxnickname'])){
			$member['nickname'] = base64_decode($member['wxnickname']);
		}
		...
		$member['self_ordercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
		$member['self_ordermoney'] = pdo_fetchcolumn('select sum(price) from ' . tablename('rr_v_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));

		if (!empty($member['agentid'])) {
			$parentagent = m('member')->getMember($member['agentid']);
		}

		$order = pdo_fetch('select finishtime from ' . tablename('rr_v_order') . ' where uniacid=:uniacid and openid=:openid and status>=1 Limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
		...
		if ($plugin_sns) {
			...
			$sns_member = pdo_fetch('select * from ' . tablename('rr_v_sns_member') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $member['openid'], ':uniacid' => $_W['uniacid']));
			$sns_member['postcount'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_sns_post') . ' where uniacid=:uniacid and openid=:openid and pid=0 and deleted = 0 and checked=1', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
			$sns_member['replycount'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_sns_post') . ' where uniacid=:uniacid and openid=:openid and pid>0 and deleted = 0 and checked=1', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
		...
		if ($_W['ispost']) {
			...
			if ($openbind) {
				if (!empty($data['mobileverify'])) {
					...
					$m = pdo_fetch('select id from ' . tablename('rr_v_member') . ' where mobile=:mobile and mobileverify=1 and uniacid=:uniaicd limit 1 ', array(':mobile' => $data['mobile'], ':uniaicd' => $_W['uniacid']));
					if (!empty($m) && ($m['id'] != $id)) {
			...
			$userData['openid'] = $member['openid'];
			$userData['createtime'] = time();
			$doc = pdo_fetch('select id from ' . tablename('rr_v_member_doctors') . ' where openid=:openid and uniacid=:uniaicd limit 1 ', array(':openid' => $member['openid'], ':uniaicd' => $_W['uniacid']));
			...
			if($data['identity'] == 1){
				...
				if (!empty($doc))
				{
					$userData['isdelete'] = 0;
					$userData['isaudit'] = 0;
					pdo_update('rr_v_member_doctors', $userData, array('openid' => $member['openid'], 'uniacid' => $_W['uniacid']));
				}else{
					pdo_insert('rr_v_member_doctors', $userData);
				}
				$data['identity'] = 0;
				$data['isaudit'] = 1;
				pdo_update('rr_v_member', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
			...
			if ($hascommission) {
				if (cv('commission.agent.edit')) {
					...
					if (!empty($adata)) {
						...
						pdo_update('rr_v_member', $adata, array('id' => $id, 'uniacid' => $_W['uniacid']));
			...
			if ($hasglobonus) {
				if (cv('globonus.partner.check')) {
					...
					if (!empty($gdata)) {
						...
						pdo_update('rr_v_member', $gdata, array('id' => $id, 'uniacid' => $_W['uniacid']));
					}
				}
			}
			...
			if ($hasauthor) {
				if (cv('author.partner.check')) {
					...
					if (!empty($author_data)) {
						...
						pdo_update('rr_v_member', $author_data, array('id' => $id, 'uniacid' => $_W['uniacid']));
					}
				}
			}
			...
			if ($hasabonus) {
				if (cv('abonus.agent.check')) {
					...
					if (!empty($aadata)) {
						...
						pdo_update('rr_v_member', $aadata, array('id' => $id, 'uniacid' => $_W['uniacid']));
					}
				}
			}
			...
			if ($hassns) {
				if (cv('sns.member.edit')) {
					...
					if (!empty($snsdata)) {
						...
						pdo_update('rr_v_sns_member', $snsdata, array('id' => $sns_member['id'], 'uniacid' => $_W['uniacid']));
					}
				}
			}
		...
		$order = pdo_fetch('select finishtime from ' . tablename('rr_v_order') . ' where uniacid=:uniacid and openid=:openid and status=3 order by id desc limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
		$member['last_ordertime'] = $order['finishtime'];
	...
	public function delete()
	{
		...
		$members = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_member') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($members as $member) {
			pdo_delete('rr_v_member', array('id' => $member['id']));
			...
			if(!empty($member['memberid'])){
				pdo_update('rr_v_member', array('memberid' => ''), array('id' => $member['memberid'], 'uniacid' => $_W['uniacid']));
			}
			if(strexists($member['openid'],'sns_wa_')){
				pdo_delete('rr_v_member_doctors', array('openid' => $member['openid'], 'uniacid' => $_W['uniacid']));
				pdo_delete('rr_v_member_patients', array('openid' => $member['openid'], 'uniacid' => $_W['uniacid']));
			}
		}
	...
	public function setblack()
	{
		...
		$members = pdo_fetchall('select id,openid,nickname,realname,mobile from ' . tablename('rr_v_member') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		$black = intval($_GPC['isblack']);

		foreach ($members as $member) {
			if (!empty($black)) {
				pdo_update('rr_v_member', array('isblack' => 1), array('id' => $member['id']));
				plog('member.list.edit', '设置黑名单 <br/>用户信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
			else {
				pdo_update('rr_v_member', array('isblack' => 0), array('id' => $member['id']));
	...
	public function changelevel()
	{
		...
		if ($_W['ispost']) {
			...
			if ($toggle == 'group') {
				if (!empty($levelid)) {
					$group = pdo_fetch('select * from ' . tablename('rr_v_member_group') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $levelid, ':uniacid' => $_W['uniacid']));
				...
				if (!empty($levelid)) {
					$level = pdo_fetch('select * from ' . tablename('rr_v_member_level') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $levelid, ':uniacid' => $_W['uniacid']));
			...
			$changeids = array();
			$members = pdo_fetchall('select id,openid,nickname,realname,mobile from ' . tablename('rr_v_member') . ' WHERE id in( ' . $idsstr . ' ) AND uniacid=' . $_W['uniacid']);

			if (!empty($members)) {
				foreach ($members as $member) {
					pdo_update('rr_v_member', $arr, array('id' => $member['id']));
					$changeids[] = $member['id'];
				}
			}
	...
	public function query()
	{
		...
		$ds = pdo_fetchall('SELECT id,avatar,nickname,openid,realname,mobile FROM ' . tablename('rr_v_member') . ' WHERE 1 ' . $condition . ' order by createtime desc', $params);
