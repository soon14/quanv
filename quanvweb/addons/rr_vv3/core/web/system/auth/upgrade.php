...
class Upgrade_RrvV3Page extends SystemPage
{
	public function main()
	{
		global $_W;
		$auth = get_auth();
		$versionfile = IA_ROOT . '/addons/rr_vv3/version.php';
		$updatedate = date('Y-m-d H:i', filemtime($versionfile));
		$version = rr_vv3_VERSION;
		$release = rr_vv3_RELEASE;
		...
		$uni = pdo_fetchall('SELECT uniacid FROM ' . tablename('rr_v_sysset'), array(), 'uniacid');
		$options = array();
		if (!empty($uni) && ((date('w') % 7) == 1)) {
			...
			$members = pdo_fetchall('SELECT COUNT(*) as count,uniacid FROM ' . tablename('rr_v_member') . '  GROUP BY uniacid', array(), 'uniacid');
			$orders = pdo_fetchall('SELECT SUM(price) as price,uniacid,COUNT(*) as `count` FROM ' . tablename('rr_v_order') . ' WHERE `status`=3 AND paytime>' . (strtotime(date('y-m-d')) - (3600 * 24 * 7)) . ' GROUP BY uniacid', array(), 'uniacid');
		...
		$plugins = scandir(rr_vv3_PLUGIN);
		array_shift($plugins);
		array_shift($plugins);
		$resp = ihttp_request(rr_vv3_AUTH_URL, array('ip' => $ip, 'id' => $id, 'code' => $auth['code'], 'domain' => $domain, 'options' => $options, 'dirplugin' => $plugins));
		...
		$auth_encode = base64_encode(authcode($domain . '|' . $id . '|' . $auth['code'], 'ENCODE', 'rr_vv3_apps'));
	...
	public function check()
	{
		global $_W;
		global $_GPC;
		$plugins = pdo_fetchall('select `identity` from ' . tablename('rr_v_plugin'), array(), 'identity');
		load()->func('db');
		load()->func('communication');
		set_time_limit(0);
		$auth = get_auth();
		$version = (defined('rr_vv3_VERSION') ? rr_vv3_VERSION : '2.0.0');
		$release = (defined('rr_vv3_RELEASE') ? rr_vv3_RELEASE : '201605010000');
		$resp = ihttp_post(rr_vv3_AUTH_URL . 'check', array('ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', trim($_W['siteroot'], '/'))), 'version' => $version, 'release' => $release, 'manual' => 1, 'plugins' => array_keys($plugins), 'phpversion' => PHP_VERSION_ID));
		$templatefiles = '';
		$result = @json_decode($resp['content'], true);
		...
		if (is_file(rr_vv3_PATH . 'tmp')) {
			@unlink(rr_vv3_PATH . 'tmp');
		}
	...
	public function process()
	{
		...
		if ($action == 'database') {
			...
			if (!empty($path)) {
				$resp = ihttp_post(rr_vv3_AUTH_URL . 'download', array('ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', rtrim($_W['siteroot'], '/'))), 'path' => $path, 'phpversion' => PHP_VERSION_ID));
				...
				if (is_array($ret)) {
					...
					if (!is_dir(rr_vv3_PATH . $dirpath)) {
						mkdirs(rr_vv3_PATH . $dirpath);
						@chmod(rr_vv3_PATH . $dirpath, 511);
					}

					$content = base64_decode($ret['content']);
					file_put_contents(rr_vv3_PATH . $path, $content);

					if (isset($ret['path1'])) {
						$path1 = $ret['path1'];
						$dirpath1 = dirname($path1);

						if (!is_dir(rr_vv3_PATH . $dirpath1)) {
							mkdirs(rr_vv3_PATH . $dirpath1);
							@chmod(rr_vv3_PATH . $dirpath1, 511);
						}

						$content1 = base64_decode($ret['content1']);
						file_put_contents(rr_vv3_PATH . $path1, $content1);
					}
					...
					if (isset($ret['path2'])) {
						...
						if (!is_dir(rr_vv3_PATH . $dirpath2)) {
							mkdirs(rr_vv3_PATH . $dirpath2);
							@chmod(rr_vv3_PATH . $dirpath2, 511);
						}

						$content2 = base64_decode($ret['content2']);
						file_put_contents(rr_vv3_PATH . $path2, $content2);
					}
	...
	{
		load()->func('file');
		file_put_contents(rr_vv3_PATH . 'version.php', '<?php if(!defined(\'IN_IA\')) {exit(\'Access Denied\');}if(!defined(\'rr_vv3_VERSION\')) {define(\'rr_vv3_VERSION\', \'' . $version . '\');}if(!defined(\'rr_vv3_RELEASE\')) {define(\'rr_vv3_RELEASE\', \'' . $release . '\');}');
		cache_delete('cloud:modules:upgradev2');
		$time = time();
		global $my_scenfiles;
		my_scandir(IA_ROOT . '/addons/rr_vv3');

		foreach ($my_scenfiles as $file) {
			if (!strexists($file, '/rr_vv3/data/') && !strexists($file, 'version.php')) {
				@touch($file, $time);
			}
		}

		rmdirs(IA_ROOT . '/addons/rr_vv3/tmp');
	}

	public function checkversion()
	{
		file_put_contents(IA_ROOT . '/addons/rr_vv3/version.php', '<?php if(!defined(\'IN_IA\')) {exit(\'Access Denied\');}if(!defined(\'rr_vv3_VERSION\')) {define(\'rr_vv3_VERSION\', \'2.0.0\');}if(!defined(\'rr_vv3_RELEASE\')) {define(\'rr_vv3_RELEASE\', \'201605010000\');}');
	...
	public function log()
	{
		...
		$psize = 15;
		load()->func('communication');
		$resp = ihttp_post(rr_vv3_AUTH_URL . 'log', array('ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', trim($_W['siteroot'], '/'))), 'version' => $version, 'release' => $release, 'manual' => 1, 'plugins' => array_keys($plugins), 'pindex' => $pindex, 'psize' => $psize));
