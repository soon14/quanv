...
class Index_RrvV3Page extends SystemPage
{
	public function main()
	{
		...
		if ($_W['ispost']) {
			if (!empty($_GPC['displayorder'])) {
				foreach ($_GPC['displayorder'] as $id => $displayorder) {
					pdo_update('rr_v_plugin', array('status' => $_GPC['status'][$id], 'displayorder' => $displayorder, 'name' => $_GPC['name'][$id], 'thumb' => $_GPC['thumb'][$id], 'desc' => $_GPC['desc'][$id]), array('id' => $id));
				}
		...
		$list = pdo_fetchall('select * from ' . tablename('rr_v_plugin') . ' where 1 ' . $condition . ' order by displayorder asc', $params);
		$total = count($list);
	...
	public function apps()
	{
		...
		$auth = base64_encode(authcode($domain . '|' . $id . '|' . $authcode, 'ENCODE', 'rr_vv3_apps'));
