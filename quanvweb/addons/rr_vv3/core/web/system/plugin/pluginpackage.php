...
class Pluginpackage_RrvV3Page extends SystemPage
{
	public function main()
	{
		...
		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('rr_v_system_plugingrant_package') . " as gp\n                left join " . tablename('rr_v_plugin') . " as p on p.id = gp.pluginid\n                WHERE 1 " . $condition . ' ', $params);
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				pdo_update('rr_v_system_plugingrant_package', $data, array('id' => $id));
			}
			else {
				pdo_insert('rr_v_system_plugingrant_package', $data);
				$id = pdo_insertid();
			}
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,text FROM ' . tablename('rr_v_system_plugingrant_package') . ' WHERE id in( ' . $id . ' ) ');

		foreach ($items as $item) {
			pdo_delete('rr_v_system_plugingrant_package', array('id' => $item['id']));
	...
	public function property()
	{
		...
		if (in_array($type, array('state', 'rec', 'displayorder'))) {
			pdo_update('rr_v_system_plugingrant_package', array($type => $value), array('id' => $id));
			$statusstr = '';
	...
	public function query()
	{
		...
		$pluginid = '';
		$grantPlugin = array();
		$grantPlugin = pdo_fetchall('select pluginid from ' . tablename('rr_v_system_plugingrant_package') . ' where 1 ');
		...
		$plugins = pdo_fetchall('select id,`name` as title,thumb,`desc` from ' . tablename('rr_v_plugin') . ' where 1 ' . $condition . ' order by displayorder asc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('rr_v_plugin') . ' WHERE 1 ' . $condition . ' ', $params);
