...
class Index_RrvV3Page extends ComWebPage
{
	...
	public function main()
	{
		...
		$sql = 'SELECT * FROM ' . tablename('rr_v_coupon') . ' ' . ' where  1 and ' . $condition . ' ORDER BY displayorder DESC,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$list = pdo_fetchall($sql, $params);

		foreach ($list as &$row) {
			$row['gettotal'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_coupon_data') . ' where couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid']));
			$row['usetotal'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_coupon_data') . ' where used = 1 and couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid']));
			$row['pwdjoins'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_coupon_guess') . ' where couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid']));
			$row['pwdoks'] = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_coupon_guess') . ' where couponid=:couponid and uniacid=:uniacid and ok=1 limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid']));
		...
		unset($row);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('rr_v_coupon') . ' where 1 and ' . $condition, $params);
		$pager = pagination2($total, $pindex, $psize);
		$category = pdo_fetchall('select * from ' . tablename('rr_v_coupon_category') . ' where uniacid=:uniacid and merchid=0 order by id desc', array(':uniacid' => $_W['uniacid']), 'id');
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				if (!empty($data['pwdkey2'])) {
					$pwdkey2 = pdo_fetch('SELECT pwdkey2 FROM ' . tablename('rr_v_coupon') . ' WHERE id=:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
				...
				pdo_update('rr_v_coupon', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				...
				pdo_insert('rr_v_coupon', $data);
				$id = pdo_insertid();
				plog('sale.coupon.add', '添加优惠券 ID: ' . $id . '  <br/>优惠券名称: ' . $data['couponname']);
			}

			$key = 'rr_vv3:com:coupon:' . $id;
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'rr_vv3', ':name' => $key));

			if (!empty($data['pwdkey2'])) {
				if (empty($rule)) {
					$rule_data = array('uniacid' => $_W['uniacid'], 'name' => $key, 'module' => 'rr_vv3', 'displayorder' => 0, 'status' => $data['pwdopen']);
					pdo_insert('rule', $rule_data);
					$rid = pdo_insertid();
					$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'rr_vv3', 'content' => $data['pwdkey2'], 'type' => 1, 'displayorder' => 0, 'status' => $data['pwdopen']);
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('rr_v_coupon') . ' WHERE id =:id and uniacid=:uniacid and merchid=0 limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		...
		if (empty($item)) {
			...
			if (($item['limitgoodtype'] == 1) || ($item['limitgoodtype'] == 2)) {
				if ($item['limitgoodids']) {
					$goods = pdo_fetchall('SELECT id,title,thumb FROM ' . tablename('rr_v_goods') . ' WHERE uniacid = :uniacid and id in (' . $item['limitgoodids'] . ') ', array(':uniacid' => $_W['uniacid']));
				}
			}
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,couponname FROM ' . tablename('rr_v_coupon') . ' WHERE id in( ' . $id . ' ) and merchid=0 AND uniacid=' . $_W['uniacid']);
		$key = 'rr_vv3:com:coupon:' . $id;
		$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'rr_vv3', ':name' => $key));
		...
		foreach ($items as $item) {
			pdo_delete('rr_v_coupon', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			pdo_delete('rr_v_coupon_data', array('couponid' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function displayorder()
	{
		...
		foreach ($items as $item) {
			pdo_update('rr_v_coupon', array('displayorder' => $displayorder), array('id' => $id));
	...
	public function query()
	{
		...
		$time = time();
		$ds = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_coupon') . '  WHERE 1 ' . $condition . ' ORDER BY id asc', $params);
	...
	public function querygoods()
	{
		...
		$ds = pdo_fetchall('SELECT id,title,thumb FROM ' . tablename('rr_v_goods') . ' WHERE 1 ' . $condition . ' order by createtime desc', $params);
	...
	public function send()
	{
		...
		$coupon = pdo_fetch('SELECT * FROM ' . tablename('rr_v_coupon') . ' WHERE id=:id and uniacid=:uniacid and merchid=0', array(':id' => $couponid, ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if ($class1 == 1) {
				...
				$members = pdo_fetchall('SELECT openid FROM ' . tablename('rr_v_member') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\'' . $where, array(), 'openid');

				if (!empty($_GPC['send_level'])) {
					$levelname = pdo_fetchcolumn('select levelname from ' . tablename('rr_v_member_level') . ' where id=:id limit 1', array(':id' => $_GPC['send_level']));
				}
				else {
					$levelname = '全部';
				}
				...
				if (!empty($_GPC['send_group'])) {
					$groupname = pdo_fetchcolumn('select groupname from ' . tablename('rr_v_member_group') . ' where id=:id limit 1', array(':id' => $_GPC['send_group']));
				}
				else {
					$groupname = '全部分组';
				}
				...
				$members = pdo_fetchall('SELECT openid FROM ' . tablename('rr_v_member') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' and isagent=1 and status=1 ' . $where, array(), 'openid');

				if ($_GPC['send_agentlevel'] != '') {
					$levelname = pdo_fetchcolumn('select levelname from ' . tablename('rr_v_commission_level') . ' where id=:id limit 1', array(':id' => $_GPC['send_agentlevel']));
				}
				else {
					$levelname = '全部';
				}
				...
				$members = pdo_fetchall('SELECT openid FROM ' . tablename('rr_v_member') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' and ispartner=1 and partnerstatus=1 ' . $where, array(), 'openid');

				if ($_GPC['send_partnerlevels'] != '') {
					$levelname = pdo_fetchcolumn('select levelname from ' . tablename('rr_v_globonus_level') . ' where id=:id limit 1', array(':id' => $_GPC['send_partnerlevels']));
				}
				else {
					$levelname = '全部';
				}
				...
				if ($class1 == 7) {
					...
					$members = pdo_fetchall('SELECT openid FROM ' . tablename('rr_v_member') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' and isaagent=1 and aagentstatus=1 ' . $where, array(), 'openid');

					if ($_GPC['send_aagentlevels'] != '') {
						$levelname = pdo_fetchcolumn('select levelname from ' . tablename('rr_v_abonus_level') . ' where id=:id limit 1', array(':id' => $_GPC['send_aagentlevels']));
					}
					else {
						$levelname = '全部';
					}
			...
			$members = pdo_fetchall('select id,openid,nickname from ' . tablename('rr_v_member') . ' where openid in (' . implode(',', $mopenids) . ') and uniacid=' . $_W['uniacid']);
			...
			pdo_update('rr_v_coupon', $upgrade, array('id' => $coupon['id']));
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_member_level') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY level asc');
		$list2 = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_member_group') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY id asc');
		$coupons = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_coupon') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY id asc');
		$hascommission = false;
		$plugin_com = p('commission');
