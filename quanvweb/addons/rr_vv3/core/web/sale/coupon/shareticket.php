...
class Shareticket_RrvV3Page extends WebPage
{
	public function main()
	{
		...
		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('rr_v_sendticket_share') . ' WHERE 1 ' . $condition . ' ', $params);
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			if (empty($id)) {
				$rathersql = 'SELECT * FROM ' . tablename('rr_v_sendticket_share') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND enough = "' . $_GPC['enough'] . '"';
				...
				$rathersql = 'SELECT * FROM ' . tablename('rr_v_sendticket_share') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND enough = "' . $_GPC['enough'] . '" AND id <> ' . intval($id);
				$ratherlist = pdo_fetchall($rathersql);
			...
			if ($_GPC['issync'] == 0) {
				if (!empty($_GPC['couponid']) && is_array($_GPC['couponid'])) {
					...
					foreach ($_GPC['couponid'] as $ck => $cv) {
						...
						$csql = 'SELECT * FROM ' . tablename('rr_v_coupon') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND id = ' . $cv;
			...
			if (!empty($id)) {
				pdo_update('rr_v_sendticket_share', $data, array('id' => $id));
				plog('sale.sendticket.share.edit', '编辑分享活动 ID: ' . $id . ' <br/>分享名称: ' . $data['sharetitle']);
			}
			else {
				pdo_insert('rr_v_sendticket_share', $data);
				$id = pdo_insertid();
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('rr_v_sendticket_share') . ' WHERE uniacid = ' . $uniacid . ' and id = ' . $id . ' ');
	...
	public function status()
	{
		...
		$items = pdo_fetchall('SELECT id,sharetitle FROM ' . tablename('rr_v_sendticket_share') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_update('rr_v_sendticket_share', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
	...
	public function delete1()
	{
		...
		foreach ($items as $item) {
			pdo_delete('rr_v_sendticket_share', array('id' => $item['id']));
	...
	public function change()
	{
		...
		$gift = pdo_fetch('select id from ' . tablename('rr_v_sendticket_share') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));

		if (empty($gift)) {
			show_json(0, array('message' => '参数错误'));
		}

		pdo_update('rr_v_sendticket_share', array($type => $value), array('id' => $id));
		show_json(1);
	}
	...
	public function querycoupon($couponid)
	{
		...
		foreach ($couponid as $ck => $cv) {
			...
			$cpsql = 'SELECT * FROM ' . tablename('rr_v_coupon') . $where;
