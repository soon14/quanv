...
class Setticket_RrvV3Page extends WebPage
{
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			if (empty($id)) {
				if (is_array($_GPC['couponid'])) {
					...
					foreach ($_GPC['couponid'] as $cpk => $cpv) {
						...
						$sql = 'SELECT * FROM ' . tablename('rr_v_coupon') . $where;
			...
			if (empty($id)) {
				pdo_insert('rr_v_sendticket', $data);
				plog('sendticket.set', '设置发券内容');
			}
			else {
				$params = array('id' => $id);
				pdo_update('rr_v_sendticket', $data, $params);
			}

			show_json(1);
		}

		$sql = 'SELECT * FROM ' . tablename('rr_v_sendticket') . ' WHERE uniacid = ' . intval($_W['uniacid']);
		$item = pdo_fetch($sql);
		...
		if (!empty($item)) {
			...
			foreach ($cpids as $cpidk => $cpidv) {
				$cpsql = 'SELECT * FROM ' . tablename('rr_v_coupon') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND id = ' . intval($cpidv);
