...
class Category_RrvV3Page extends ComWebPage
{
	...
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_coupon_category') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' and merchid=0 ORDER BY displayorder asc');
	...
	public function delete()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,name FROM ' . tablename('rr_v_coupon_category') . ' WHERE id = \'' . $id . '\' and merchid=0 AND uniacid=' . $_W['uniacid'] . '');
