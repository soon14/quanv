...
class Index_RrvV3Page extends ComWebPage
{
	...
	public function enoughfree()
	{
		...
		if (!empty($data['goodsids'])) {
			$goods = pdo_fetchall('SELECT id,uniacid,title,thumb FROM ' . tablename('rr_v_goods') . ' WHERE uniacid=:uniacid AND id IN (' . implode(',', $data['goodsids']) . ')', array(':uniacid' => $_W['uniacid']));
		}
