...
class Index_RrvV3Page extends ComWebPage
{
	...
	public function main()
	{
		...
		$sql = 'SELECT * FROM ' . tablename('rr_v_wxcard') . ' ' . ' where  1 and ' . $condition . ' ORDER BY id DESC,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$list = pdo_fetchall($sql, $data);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('rr_v_wxcard') . ' where 1 and ' . $condition, $data);
	...
	protected function post()
	{
		...
		if (!empty($id)) {
			$item = pdo_fetch('SELECT * FROM ' . tablename('rr_v_wxcard') . ' WHERE id =:id and uniacid=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		}
		...
		if ($_W['ispost']) {
			...
			if (!empty($item)) {
				...
				pdo_update('rr_v_wxcard', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,card_id,title FROM ' . tablename('rr_v_wxcard') . ' WHERE id in( ' . $id . ' ) and merchid=0 AND uniacid=' . $_W['uniacid']);
		...
		foreach ($items as $item) {
			...
			pdo_delete('rr_v_wxcard', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function stock()
	{
		...
		$sql = 'select id,uniacid,logo_url, card_id,title,total_quantity,quantity from ' . tablename('rr_v_wxcard');
