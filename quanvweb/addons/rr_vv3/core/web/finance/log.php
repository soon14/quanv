...
class Log_RrvV3Page extends WebPage
{
	protected function main($type = 0)
	{
		...
		if ($condition1 != '') {
			$member_sql = ' and openid IN (SELECT openid FROM ims_rr_v_member WHERE uniacid = :uniacid ' . $condition1 . ') OR openid IN (SELECT CONCAT(\'sns_wa_\',openid_wa) FROM ims_rr_v_member WHERE uniacid = :uniacid ' . $condition1 . ')';
		}
		...
		$sql = 'select log.id,log.openid,log.logno,log.type,log.status,log.rechargetype,log.sendmoney,log.money,log.createtime,log.realmoney,log.deductionmoney,log.charge,log.remark,log.alipay,log.bankname,log.bankcard,log.realname as applyrealname,log.applytype,m.nickname,m.id as mid,m.avatar,m.level,m.groupid,m.realname,m.mobile,g.groupname,l.levelname from ' . tablename('rr_v_member_log') . ' log ' . ' left join ' . tablename('rr_v_member') . ' m on m.openid = log.openid ' . ' left join ' . tablename('rr_v_member_group') . ' g on g.id = m.groupid ' . ' left join ' . tablename('rr_v_member_level') . ' l on l.id = m.level ' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC ';
		...
		$apply_type = array(1 => '微信钱包', 2 => '支付宝', 3 => '银行卡');
		...
		if (!empty($list)) {
			...
			$members_sql = 'select id as mid, realname,avatar,weixin,nickname,mobile,openid,openid_wa from ' . tablename('rr_v_member') . ' where uniacid=:uniacid and openid IN (\'' . implode('\',\'', array_unique($openids)) . '\') OR openid_wa IN (\'' . implode('\',\'', array_unique($openids)) . '\')';
		...
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_member_log') . ' log ' . ' where 1 ' . $condition . ' ' . $member_sql, $params);
