...
class Credit_RrvV3Page extends WebPage
{
	protected function main($type = 'credit1')
	{
		...
		$params = array(':uniacid' => $_W['uniacid'], ':module1' => 'rr_vv3', ':module2' => 'rr_v', ':credittype' => $type);
		...
		$sql = 'select log.*,m.id as mid, m.realname,m.avatar,m.nickname,m.avatar, m.mobile, m.weixin,u.username from ' . tablename('mc_credits_record') . ' log ' . ' left join ' . tablename('users') . ' u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid' . ' left join ' . tablename('rr_v_member') . ' m on m.uid=log.uid' . ' left join ' . tablename('rr_v_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('rr_v_member_level') . ' l on m.level =l.id' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC ';
