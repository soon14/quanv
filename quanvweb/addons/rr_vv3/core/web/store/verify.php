...
class Verify_RrvV3Page extends ComWebPage
{
	...
	public function verifygoods()
	{
		...
		if (!empty($verifygood['storeid'])) {
			$store = pdo_fetch('select * from ' . tablename('rr_v_store') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $verifygood['storeid'], ':uniacid' => $_W['uniacid']));
		}
