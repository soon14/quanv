<?php
class Page_RrvV3Page extends ComWebPage
{
	/**
     * 门店页面编辑
     */
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_newstore_diypage') . 'WHERE ' . $condition . $limit, $params);
		$total = pdo_fetchcolumn('SELECT count(1) FROM ' . tablename('rr_v_newstore_diypage') . 'WHERE ' . $condition, $params);
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id, `name` FROM ' . tablename('rr_v_newstore_diypage') . ' WHERE id in( ' . $id . ' ) AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));

		if (!empty($items)) {
			foreach ($items as $item) {
				pdo_delete('rr_v_newstore_diypage', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function status()
	{
		...
		if (!empty($items)) {
			foreach ($items as $item) {
				pdo_update('rr_v_newstore_diypage', array('status' => intval($_GPC['status'])), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
