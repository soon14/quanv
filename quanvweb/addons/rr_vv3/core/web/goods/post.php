...
$id = intval($_GPC['id']);
$item = pdo_fetch('SELECT * FROM ' . tablename('rr_v_goods') . ' WHERE id = :id and uniacid = :uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
if (!empty($item) && ($item['type'] == 5) && !empty($item['opencard']) && !empty($item['cardid'])) {
	$card = pdo_fetch('SELECT * FROM ' . tablename('rr_v_goods_cards') . ' WHERE id = :id and uniacid = :uniacid', array(':id' => $item['cardid'], ':uniacid' => $_W['uniacid']));
}
...
$commission_level = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_commission_level') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY commission1 asc');
...
if ($_W['ispost']) {
	...
	if (is_array($_GPC['cates'])) {
		...
		foreach ($cates as $key => $cid) {
			$c = pdo_fetch('select level from ' . tablename('rr_v_category') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $cid, ':uniacid' => $_W['uniacid']));
			...
			if ($key == 0) {
				if ($c['level'] == 1) {
					...
					$crow = pdo_fetch('select parentid from ' . tablename('rr_v_category') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $cid, ':uniacid' => $_W['uniacid']));
					...
					if ($c['level'] == 3) {
						$tcateid = $cid;
						$tcate = pdo_fetch('select id,parentid from ' . tablename('rr_v_category') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $cid, ':uniacid' => $_W['uniacid']));
						$ccateid = $tcate['parentid'];
						$ccate = pdo_fetch('select id,parentid from ' . tablename('rr_v_category') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $ccateid, ':uniacid' => $_W['uniacid']));
		...
		if ($_GPC['diyformtype'] == 2) {
			...
			if ($_GPC['diysave'] == 1) {
				...
				if (empty($is_save)) {
					...
					pdo_insert('rr_v_diyform_type', $insert);
					$data['diysaveid'] = pdo_insertid();
				}
				else {
					pdo_update('rr_v_diyform_type', $insert, array('id' => $diysaveid));
				}
			}
			...
			if (!empty($opencard)) {
				...
				if (!empty($card)) {
					...
					if ($change) {
						...
						pdo_update('rr_v_goods_cards', $carddata, array('id' => $card['id']));
					}
					...
					pdo_insert('rr_v_goods_cards', $carddata);
					$cardid = pdo_insertid();
				}
	...
	while ($k < $len) {
		...
		if (!is_numeric($get_param_id)) {
			pdo_insert('rr_v_goods_param', $a);
			$param_id = pdo_insertid();
		}
		else {
			pdo_update('rr_v_goods_param', $a, array('id' => $get_param_id));
	...
	if (0 < count($paramids)) {
		pdo_query('delete from ' . tablename('rr_v_goods_param') . ' where goodsid=' . $id . ' and id not in ( ' . implode(',', $paramids) . ')');
	}
	else {
		pdo_query('delete from ' . tablename('rr_v_goods_param') . ' where goodsid=' . $id);
	}
	...
	while ($k < $len) {
		...
		if (is_numeric($get_spec_id)) {
			pdo_update('rr_v_goods_spec', $a, array('id' => $get_spec_id));
			$spec_id = $get_spec_id;
		}
		else {
			pdo_insert('rr_v_goods_spec', $a);
		...
		while ($n < $itemlen) {
			...
			if (is_numeric($get_item_id)) {
				pdo_update('rr_v_goods_spec_item', $d, array('id' => $get_item_id));
				$item_id = $get_item_id;
			}
			else {
				pdo_insert('rr_v_goods_spec_item', $d);
		...
		if (0 < count($itemids)) {
			pdo_query('delete from ' . tablename('rr_v_goods_spec_item') . ' where uniacid=' . $_W['uniacid'] . ' and specid=' . $spec_id . ' and id not in (' . implode(',', $itemids) . ')');
		}
		else {
			pdo_query('delete from ' . tablename('rr_v_goods_spec_item') . ' where uniacid=' . $_W['uniacid'] . ' and specid=' . $spec_id);
		}

		pdo_update('rr_v_goods_spec', array('content' => serialize($itemids)), array('id' => $spec_id));
		$specids[] = $spec_id;
		++$k;
	}

	if (0 < count($specids)) {
		pdo_query('delete from ' . tablename('rr_v_goods_spec') . ' where uniacid=' . $_W['uniacid'] . ' and goodsid=' . $id . ' and id not in (' . implode(',', $specids) . ')');
	}
	else {
		pdo_query('delete from ' . tablename('rr_v_goods_spec') . ' where uniacid=' . $_W['uniacid'] . ' and goodsid=' . $id);
	}
	...
	while ($k < $len) {
		...
		if (empty($get_option_id)) {
			pdo_insert('rr_v_goods_option', $a);
			$option_id = pdo_insertid();
		}
		else {
			pdo_update('rr_v_goods_option', $a, array('id' => $get_option_id));
	...
	pdo_update('rr_v_goods', array('discounts' => $discounts_json), array('id' => $id));
	$has_merch = 0;
	...
	pdo_update('rr_v_goods', array('isdiscount_discounts' => $is_discounts_json), array('id' => $id));
	...
	pdo_update('rr_v_goods', $commission_arr, array('id' => $id));
	...
	$sqlgoods = 'SELECT id,title,thumb,marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,total,description,merchsale FROM ' . tablename('rr_v_goods') . ' where id=:id and uniacid=:uniacid limit 1';
	$goodsinfo = pdo_fetch($sqlgoods, array(':id' => $id, ':uniacid' => $_W['uniacid']));
	$goodsinfo = m('goods')->getOneMinPrice($goodsinfo);
	pdo_update('rr_v_goods', array('minprice' => $goodsinfo['minprice'], 'maxprice' => $goodsinfo['maxprice']), array('id' => $id, 'uniacid' => $_W['uniacid']));
	if (($data['type'] == 3) && $com_virtual) {
		$com_virtual->updateGoodsStock($id);
	}
	else {
		if (($data['hasoption'] !== 0) && ($data['totalcnf'] != 2) && empty($data['unite_total'])) {
			pdo_update('rr_v_goods', array('total' => $totalstocks), array('id' => $id));
		}
	}
...
if (!empty($id)) {
	...
	$allspecs = pdo_fetchall('select * from ' . tablename('rr_v_goods_spec') . ' where goodsid=:id order by displayorder asc', array(':id' => $id));
	...
	unset($s);
	$params = pdo_fetchall('select * from ' . tablename('rr_v_goods_param') . ' where goodsid=:id order by displayorder asc', array(':id' => $id));
	...
	$options = pdo_fetchall('select * from ' . tablename('rr_v_goods_option') . ' where goodsid=:id order by id asc', array(':id' => $id));
	$specs = array();
...
if ($com_virtual) {
	$virtual_types = pdo_fetchall('select * from ' . tablename('rr_v_virtual_type') . ' where uniacid=:uniacid and merchid=:merchid order by id asc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
}

if ($merchid == 0) {
	$details = pdo_fetchall('select detail_logo,detail_shopname,detail_btntext1, detail_btnurl1 ,detail_btntext2,detail_btnurl2,detail_totaltitle from ' . tablename('rr_v_goods') . ' where uniacid=:uniacid and detail_shopname<>\'\' group by detail_shopname', array(':uniacid' => $_W['uniacid']));
