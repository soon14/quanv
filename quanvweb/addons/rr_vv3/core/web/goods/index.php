...
class Index_RrvV3Page extends WebPage
{
	public function main($goodsfrom = 'sale')
	{
		...
		if (!empty($_GPC['keyword'])) {
			...
			$sqlcondition = ' left join ' . tablename('rr_v_goods_option') . ' op on g.id = op.goodsid';

			if ($merch_plugin) {
				$sqlcondition .= ' left join ' . tablename('rr_v_merch_user') . ' merch on merch.id = g.merchid and merch.uniacid=g.uniacid';
			}
		...
		$sql = 'SELECT g.id FROM ' . tablename('rr_v_goods') . 'g' . $sqlcondition . $condition . $groupcondition;
		$total_all = pdo_fetchall($sql, $params);
		$total = count($total_all);
		unset($total_all);

		if (!empty($total)) {
			$sql = 'SELECT g.* FROM ' . tablename('rr_v_goods') . 'g' . $sqlcondition . $condition . $groupcondition . " ORDER BY g.`status` DESC, g.`displayorder` DESC,\r\n                g.`id` DESC LIMIT " . (($pindex - 1) * $psize) . ',' . $psize;
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('rr_v_goods') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_update('rr_v_goods', array('deleted' => 1), array('id' => $item['id']));
	...
	public function status()
	{
		...
		$items = pdo_fetchall('SELECT id,title,status,isstatustime,statustimestart,statustimeend FROM ' . tablename('rr_v_goods') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		...
		foreach ($items as $item) {
			...
			pdo_update('rr_v_goods', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
	...
	public function checked()
	{
		...
		foreach ($items as $item) {
			pdo_update('rr_v_goods', array('checked' => intval($_GPC['checked'])), array('id' => $item['id']));
	...
	public function delete1()
	{
		...
		foreach ($items as $item) {
			pdo_delete('rr_v_goods', array('id' => $item['id']));
	...
	public function restore()
	{
		...
		foreach ($items as $item) {
			pdo_update('rr_v_goods', array('deleted' => 0), array('id' => $item['id']));
	...
	public function property()
	{
		...
		if (in_array($type, array('new', 'hot', 'recommand', 'discount', 'time', 'sendfree', 'nodiscount'))) {
			pdo_update('rr_v_goods', array('is' . $type => $data), array('id' => $id, 'uniacid' => $_W['uniacid']));
		...
		if (in_array($type, array('status'))) {
			pdo_update('rr_v_goods', array($type => $data), array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function change()
	{
		...
		$goods = pdo_fetch('select id,hasoption,marketprice,dowpayment from ' . tablename('rr_v_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		...
		pdo_update('rr_v_goods', array($type => $value), array('id' => $id));

		if ($goods['hasoption'] == 0) {
			$sql = 'update ' . tablename('rr_v_goods') . ' set minprice = marketprice,maxprice = marketprice where id = ' . $goods['id'] . ' and hasoption=0;';
			pdo_query($sql);
		}
	...
	public function query()
	{
		...
		$ds = pdo_fetchall('SELECT id,title,thumb,marketprice,productprice,share_title,share_icon,description,minprice,costprice,total,sales,islive,liveprice FROM ' . tablename('rr_v_goods') . ' WHERE 1 ' . $condition . ' order by createtime desc', $params);
	...
	{
		global $_W;
		$sql = 'update ' . tablename('rr_v_goods') . " g set \r\ng.minprice = (select min(marketprice) from " . tablename('rr_v_goods_option') . 
			" where g.id = goodsid),\r\ng.maxprice = (select max(marketprice) from " . tablename('rr_v_goods_option') . " 
			where g.id = goodsid)\r\nwhere g.hasoption=1 and g.uniacid=" . $_W['uniacid'] . ";\r\nupdate " . tablename('rr_v_goods') . '
			 set minprice = marketprice,maxprice = marketprice where hasoption=0 and uniacid=' . $_W['uniacid'] . ';';
		pdo_run($sql);
		show_json(1);
	}
}
