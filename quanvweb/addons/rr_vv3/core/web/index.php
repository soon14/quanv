...
class Index_RrvV3Page extends WebPage
{
	...
	public function switchversion()
	{
		...
		$id = intval($_GPC['id']);
		$set = pdo_fetch('SELECT * FROM ' . tablename('rr_v_version') . ' WHERE uid=:uid AND `type`=0', array(':uid' => $_W['uid']));
		$data = array('version' => !empty($_W['shopversion']) ? 0 : 1);

		if (empty($set)) {
			$data['uid'] = $_W['uid'];
			pdo_insert('rr_v_version', $data);
		}
		else {
			pdo_update('rr_v_version', $data, array('id' => $set['id']));
		}
