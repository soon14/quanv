...
class Goods_RrvV3Page extends WebPage
{
	public function main()
	{
		...
		$sql = 'select og.price,og.total,o.createtime,o.ordersn,g.title,g.thumb,g.goodssn,op.goodssn as optiongoodssn,op.title as optiontitle from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_order') . ' o on o.id = og.orderid ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id = og.goodsid ' . ' left join ' . tablename('rr_v_goods_option') . ' op on op.id = og.optionid ' . ' where 1 ' . $condition . ' order by ' . $orderby . ' desc ';
		...
		unset($row);
		$total = pdo_fetchcolumn('select  count(*) from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_order') . ' o on o.id = og.orderid ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id = og.goodsid ' . ' where 1 ' . $condition, $params);
