...
class O2o_orderusers_RrvV3Page extends WebPage
{
	public function main()
	{
		...
		$total = pdo_fetchcolumn('select  count(1) from ' . tablename('rr_v_store') . ' where uniacid=:uniacid  ' . $condition2, $params2);
		$sql = 'select *  from  ' . tablename('rr_v_store') . ' where uniacid=:uniacid ' . $condition2 . ' ';
		...
		foreach ($list as &$item) {
			$sql = 'select count(a.openid) from (SELECT  openid   from  ' . tablename('rr_v_order') . '    where uniacid=:uniacid and storeid = :storeid  ' . $condition . '  and isnewstore=1 GROUP BY openid) a';
			$params[':storeid'] = $item['id'];
			$params[':uniacid'] = $_W['uniacid'];
			$usercount = pdo_fetchcolumn($sql, $params);
			$item['usercount'] = $usercount;
			$sql = 'SELECT  SUM(price)   from     ' . tablename('rr_v_order') . '  where   uniacid=:uniacid and storeid = :storeid ' . $condition . '  and isnewstore=1 ';
