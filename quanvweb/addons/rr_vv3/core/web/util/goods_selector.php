...
class Goods_selector_RrvV3Page extends WebPage
{
	/**
     * 商品选择器
     */
	public function main($page = 0)
	{
		...
		$query_sql = 'select ' . $query_field . ' from ' . tablename('rr_v_goods') . ' where uniacid = :uniacid ' . $condition . ' ' . $goodsgroup_where . $keywords;
		$count_field = 'count(*)';
	...
	public function op()
	{
		...
		$id = intval($_GPC['id']);
		$sql = 'select * from ' . tablename('rr_v_goods') . ' where id = ' . $id;
		$goods = pdo_fetch($sql);
		...
		if (empty($_GPC['nooption'])) {
			$sql = 'select * from ' . tablename('rr_v_goods_option') . ' where goodsid = ' . $id;
