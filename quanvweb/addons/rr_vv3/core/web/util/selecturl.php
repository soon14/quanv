...
class Selecturl_RrvV3Page extends WebPage
{
	...
	public function main()
	{
		...
		if (0 < $syscate['level']) {
			$categorys = pdo_fetchall('SELECT id,name,parentid FROM ' . tablename('rr_v_category') . ' WHERE enabled=:enabled and uniacid= :uniacid  ', array(':uniacid' => $_W['uniacid'], ':enabled' => '1'));
		}
		...
		$allUrls = array(
			array(
				...
				'name' => '首页导航链接',
				'list' => array(
					array('name' => '图文咨询', 'url' => mobileUrl('doctor/consult/consult', NULL, $full), 'url_wxapp' => '/pages/doctor/consult/consult'),
					array('name' => '当面咨询', 'url' => mobileUrl('doctor/order/order', NULL, $full), 'url_wxapp' => '/pages/doctor/order/order'),
					array('name' => '医生集团', 'url' => mobileUrl('doctor/doctor/doctor', NULL, $full), 'url_wxapp' => '/pages/doctor/doctor/doctor'),
					array('name' => '线下讲座', 'url' => mobileUrl('doctor/chair/chair', NULL, $full), 'url_wxapp' => '/pages/doctor/chair/chair'),
					array('name' => '我的视频', 'url' => mobileUrl('doctor/myViewScreen/myViewScreen', NULL, $full), 'url_wxapp' => '/pages/doctor/myViewScreen/myViewScreen'),
					array('name' => '我的文章', 'url' => mobileUrl('doctor/myEssay/myEssay', NULL, $full), 'url_wxapp' => '/pages/doctor/myEssay/myEssay')
					)
				),
			// 'list' => array(
	...
	public function query()
	{
		...
		if (!empty($kw) && !empty($type)) {
			if ($type == 'good') {
				$list = pdo_fetchall('SELECT id,title,productprice,marketprice,thumb,sales,unit,minprice FROM ' . tablename('rr_v_goods') . ' WHERE uniacid= :uniacid and status=:status and deleted=0 AND title LIKE :title ', array(':title' => '%' . $kw . '%', ':uniacid' => $_W['uniacid'], ':status' => '1'));
				$list = set_medias($list, 'thumb');
			}
			else if ($type == 'article') {
				$list = pdo_fetchall('select id,article_title from ' . tablename('rr_v_article') . ' where article_title LIKE :title and article_state=1 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':title' => '%' . $kw . '%'));
			}
			else if ($type == 'coupon') {
				$list = pdo_fetchall('select id,couponname,coupontype from ' . tablename('rr_v_coupon') . ' where couponname LIKE :title and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':title' => '%' . $kw . '%'));
			}
			else if ($type == 'groups') {
				$list = pdo_fetchall('select id,title from ' . tablename('rr_v_groups_goods') . ' where title LIKE :title and status=1 and deleted=0 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':title' => '%' . $kw . '%'));
			}
			else if ($type == 'sns') {
				$list_board = pdo_fetchall('select id,title from ' . tablename('rr_v_sns_board') . ' where title LIKE :title and status=1 and enabled=0 and uniacid=:uniacid order by id desc ', array(':uniacid' => $_W['uniacid'], ':title' => '%' . $kw . '%'));
				$list_post = pdo_fetchall('select id,title from ' . tablename('rr_v_sns_post') . ' where title LIKE :title and checked=1 and deleted=0 and uniacid=:uniacid order by id desc ', array(':uniacid' => $_W['uniacid'], ':title' => '%' . $kw . '%'));
				$list = array();
				...
				if ($type == 'creditshop') {
					$list = pdo_fetchall('select id, thumb, title, price, credit, money from ' . tablename('rr_v_creditshop_goods') . ' where title LIKE :title and status=1 and deleted=0 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':title' => '%' . $kw . '%'));
				}
			}
		}
