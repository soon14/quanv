	...
	protected function orderData($status, $st)
	{
		...
		if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
			...
			if ($searchfield == 'ordersn') {
				...
				$sqlcondition = ' inner join ( select  DISTINCT(og.orderid) from ' . tablename('rr_v_order_goods') . ' og left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and (locate(:keyword,og.optionname)>0)) gs on gs.orderid=o.id';
			}
			else {
		...
		if (!empty($list)) {
			...
			foreach ($list as &$value) {
				...
				foreach ($order_goods as &$og) {
					...
					if($og['goodstype'] == '2'){
						$videos = pdo_fetch('SELECT id,videoname,img_url FROM ' .tablename('rr_v_member_videos').' where uniacid = :uniacid and id = :id ',array(':id' => $og['goodsid'],':uniacid' => $uniacid));
						if(!empty($videos)){
							$og['title'] = $videos['videoname'];
							$og['thumb'] = $videos['img_url'];
						}
					
					}elseif($og['goodstype'] == '3'){
						$articles = pdo_fetch('SELECT id,cover_url,article_title FROM ' .tablename('rr_v_articles').' where uniacid = :uniacid and id = :id ',array(':id' => $og['goodsid'],':uniacid' => $uniacid));
						if(!empty($articles)){
							$og['title'] = $articles['article_title'];
							$og['thumb'] = $articles['cover_url'];
						}

					}elseif($og['goodstype'] == '4'){
						$lectures = pdo_fetch('SELECT id,cover_url,lecture_title FROM ' .tablename('rr_v_lectures').' where uniacid = :uniacid and id = :id ',array(':id' => $og['goodsid'],':uniacid' => $uniacid));
						if(!empty($lectures)){
							$og['title'] = $lectures['lecture_title'];
							$og['thumb'] = $lectures['cover_url'];
						}
