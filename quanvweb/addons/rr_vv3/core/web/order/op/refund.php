...
class Refund_RrvV3Page extends WebPage
{
	protected function opData()
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('rr_v_order') . ' WHERE id = :id and uniacid=:uniacid Limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function submit()
	{
		...
		if ($_W['ispost']) {
			...
			if (($refund['status'] < 0) || ($refund['status'] == 1)) {
				pdo_update('rr_v_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			...
			if ($refundstatus == 0) {
				...
				if ($raid == 0) {
					$raddress = pdo_fetch('select * from ' . tablename('rr_v_refund_address') . ' where isdefault=1 and uniacid=:uniacid and merchid=0 limit 1', array(':uniacid' => $uniacid));
				}
				else {
					$raddress = pdo_fetch('select * from ' . tablename('rr_v_refund_address') . ' where id=:id and uniacid=:uniacid and merchid=0 limit 1', array(':id' => $raid, ':uniacid' => $uniacid));
				}

				if (empty($raddress)) {
					$raddress = pdo_fetch('select * from ' . tablename('rr_v_refund_address') . ' where uniacid=:uniacid and merchid=0 order by id desc limit 1', array(':uniacid' => $uniacid));
				}
				...
				pdo_update('rr_v_order', $order_data, array('id' => $item['id'], 'uniacid' => $uniacid));
				m('notice')->sendOrderMessage($item['id'], true);
			}
				...
				pdo_update('rr_v_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => $time), array('id' => $item['id'], 'uniacid' => $uniacid));

				foreach ($goods as $g) {
					$salesreal = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_order') . ' o on o.id = og.orderid ' . ' where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid limit 1', array(':goodsid' => $g['id'], ':uniacid' => $uniacid));
				...
				pdo_update('rr_v_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $uniacid));
				m('notice')->sendOrderMessage($item['id'], true);
			}
			else {
				if ($refundstatus == 2) {
					...
					pdo_update('rr_v_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => $time), array('id' => $item['id'], 'uniacid' => $uniacid));
					$goods = pdo_fetchall('SELECT g.id,g.credit, o.total,o.realprice FROM ' . tablename('rr_v_order_goods') . ' o left join ' . tablename('rr_v_goods') . ' g on o.goodsid=g.id ' . ' WHERE o.orderid=:orderid and o.uniacid=:uniacid', array(':orderid' => $item['id'], ':uniacid' => $uniacid));
					$credits = m('order')->getGoodsCredit($goods);
		...
		$refund_address = pdo_fetchall('select * from ' . tablename('rr_v_refund_address') . ' where uniacid=:uniacid and merchid=0', array(':uniacid' => $_W['uniacid']));
