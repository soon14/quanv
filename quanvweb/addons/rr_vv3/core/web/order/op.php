...
class Op_RrvV3Page extends WebPage
{
	public function delete()
	{
		...
		pdo_update('rr_v_order', array('deleted' => 1), array('id' => $orderid, 'uniacid' => $_W['uniacid']));
	...
	public function changeprice()
	{
		...
		if ($_W['ispost']) {
			...
			foreach ($changegoodsprice as $ogid => $change) {
				$og = pdo_fetch('select price,realprice from ' . tablename('rr_v_order_goods') . ' where id=:ogid and uniacid=:uniacid limit 1', array(':ogid' => $ogid, ':uniacid' => $_W['uniacid']));
			...
			if (!empty($orderupdate)) {
				pdo_update('rr_v_order', $orderupdate, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			}
			...
			foreach ($changegoodsprice as $ogid => $change) {
				$og = pdo_fetch('select price,realprice,changeprice from ' . tablename('rr_v_order_goods') . ' where id=:ogid and uniacid=:uniacid limit 1', array(':ogid' => $ogid, ':uniacid' => $_W['uniacid']));

				if (!empty($og)) {
					$realprice = $og['realprice'] + $change;
					$changeprice = $og['changeprice'] + $change;
					pdo_update('rr_v_order_goods', array('realprice' => $realprice, 'changeprice' => $changeprice), array('id' => $ogid));
				}
			}
	...
	public function pay($a = array(), $b = array())
	{
		...
		if (!empty($item['virtual']) && com('virtual')) {
			...
			pdo_update('rr_v_order', array('status' => 1, 'paytype' => 11, 'paytime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function close()
	{
		...
		if ($_W['ispost']) {
			...
			if ((0 < $item['refundstate']) && !empty($item['refundid'])) {
				...
				pdo_update('rr_v_order_refund', $change_refund, array('id' => $item['refundid'], 'uniacid' => $_W['uniacid']));
			}
			...
			pdo_update('rr_v_order', array('status' => -1, 'refundstate' => 0, 'canceltime' => $time, 'remarkclose' => $_GPC['remark']), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function paycancel()
	{
		...
		if ($_W['ispost']) {
			m('order')->setStocksAndCredits($item['id'], 2);
			pdo_update('rr_v_order', array('status' => 0, 'cancelpaytime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function finish()
	{
		...
		$opdata = $this->opData();
		extract($opdata);
		pdo_update('rr_v_order', array('status' => 3, 'finishtime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function fetchcancel()
	{
		...
		pdo_update('rr_v_order', array('status' => 1, 'finishtime' => 0), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function sendcancel()
	{
		...
		if ($_W['ispost']) {
			...
			if (0 < $item['sendtype']) {
				...
				$data['sendtype'] = 0;
				pdo_update('rr_v_order_goods', $data, array('orderid' => $item['id'], 'sendtype' => $sendtype, 'uniacid' => $_W['uniacid']));
				$order = pdo_fetch('select sendtype from ' . tablename('rr_v_order') . ' where id = ' . $item['id'] . ' and uniacid = ' . $_W['uniacid'] . ' ');
				pdo_update('rr_v_order', array('sendtype' => $order['sendtype'] - 1, 'status' => 1), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			}
			else {
				$data['status'] = 1;
				pdo_update('rr_v_order', $data, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			}
		...
		if (0 < $sendtype) {
			$sendgoods = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype=' . $sendtype . ' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
		}
		else {
			if (0 < $item['sendtype']) {
				$i = 1;
				...
				while ($i <= intval($item['sendtype'])) {
					$bundles[$i]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype=' . $i . ' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
	...
	public function fetch()
	{
		...
		pdo_update('rr_v_order', $d, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
		m('order')->fullback($item['id']);
		...
		if (!empty($item['refundid'])) {
			$refund = pdo_fetch('select * from ' . tablename('rr_v_order_refund') . ' where id=:id limit 1', array(':id' => $item['refundid']));

			if (!empty($refund)) {
				pdo_update('rr_v_order_refund', array('status' => -1), array('id' => $item['refundid']));
				pdo_update('rr_v_order', array('refundstate' => 0), array('id' => $item['id']));
			}
		}
	...
	public function send()
	{
		...
		if ($_W['ispost']) {
			...
			if ((intval($_GPC['sendtype']) == 1) || (0 < $item['sendtype'])) {
				...
				$ogoods = array();
				$ogoods = pdo_fetchall('select sendtype from ' . tablename('rr_v_order_goods') . "\r\n                    where orderid = " . $item['id'] . ' and uniacid = ' . $_W['uniacid'] . ' order by sendtype desc ');
				...
				foreach ($goodsid as $key => $value) {
					pdo_update('rr_v_order_goods', $data, array('id' => $value, 'uniacid' => $_W['uniacid']));
				}

				$send_goods = pdo_fetch('select * from ' . tablename('rr_v_order_goods') . "\r\n                    where orderid = " . $item['id'] . ' and sendtype = 0 and uniacid = ' . $_W['uniacid'] . ' limit 1 ');

				if (empty($send_goods)) {
					$senddata['status'] = 2;
				}

				pdo_update('rr_v_order', $senddata, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			}
			else {
				$data['status'] = 2;
				pdo_update('rr_v_order', $data, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			...
			if (!empty($item['refundid'])) {
				...
				if (!empty($refund)) {
					pdo_update('rr_v_order_refund', array('status' => -1, 'endtime' => $time), array('id' => $item['refundid']));
					pdo_update('rr_v_order', array('refundstate' => 0), array('id' => $item['id']));
				}
			}
		...
		if (0 < $item['sendtype']) {
			$noshipped = pdo_fetchall('select og.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.sendtype = 0 and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
			$i = 1;

			while ($i <= $item['sendtype']) {
				$shipped[$i]['sendtype'] = $i;
				$shipped[$i]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.sendtype = ' . $i . ' and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
				++$i;
			}
		}

		$order_goods = pdo_fetchall('select og.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
		$address = iunserializer($item['address']);

		if (!is_array($address)) {
			$address = pdo_fetch('SELECT * FROM ' . tablename('rr_v_member_address') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $item['addressid'], ':uniacid' => $_W['uniacid']));
		}
	...
	public function remarksaler()
	{
		...
		if ($_W['ispost']) {
			pdo_update('rr_v_order', array('remarksaler' => $_GPC['remark']), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if (!empty($expresssn)) {
				...
				if (0 < $item['sendtype']) {
					...
					pdo_update('rr_v_order_goods', $change_data, array('orderid' => $id, 'sendtype' => $sendtype, 'uniacid' => $_W['uniacid']));
				}
				else {
					pdo_update('rr_v_order', $change_data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				}
	...
	public function changeaddress()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				...
				$item = pdo_fetch('SELECT id, ordersn, address,openid FROM ' . tablename('rr_v_order') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
				...
				pdo_update('rr_v_order', array('address' => $address_array), array('id' => $id, 'uniacid' => $_W['uniacid']));
