...
if (!function_exists('m')) {
	function m($name = '')
	{
		...
		$model = rr_vv3_CORE . 'model/' . strtolower($name) . '.php';
		...
		require_once $model;
		$class_name = ucfirst($name) . '_RrvV3Model';
...
if (!function_exists('d')) {
	function d($name = '')
	{
		...
		$model = rr_vv3_CORE . 'data/' . strtolower($name) . '.php';

		if (!is_file($model)) {
			exit(' Data Model ' . $name . ' Not Found!');
		}

		require_once rr_vv3_INC . 'data_model.php';
		require_once $model;
		$class_name = ucfirst($name) . '_RrvV3DataModel';
		$_modules[$name] = new $class_name();
		return $_modules[$name];
	}
}
...
if (!function_exists('socket')) {
	function socket($name)
	{
		...
		$socket = rr_vv3_PLUGIN . strtolower($name) . '/core/socket.php';

		if (!is_file($socket)) {
			return false;
		}

		require_once rr_vv3_CORE . 'inc/plugin_model.php';
		require_once $socket;
...
if (!function_exists('p')) {
	function p($name = '')
	{
		...
		$model = rr_vv3_PLUGIN . strtolower($name) . '/core/model.php';
...
if (!function_exists('com')) {
	function com($name = '')
	{
		...
		$model = rr_vv3_CORE . 'com/' . strtolower($name) . '.php';

		if (!is_file($model)) {
			return false;
		}

		require_once rr_vv3_CORE . 'inc/com_model.php';
		require_once $model;
		$class_name = ucfirst($name) . '_RrvV3ComModel';
...
if (!function_exists('get_auth')) {
	...
	{
		global $_W;
		$set = pdo_fetch('select sets from ' . tablename('rr_v_sysset') . ' order by id asc limit 1');
...
if (!function_exists('rc')) {
	function rc($plugin = '')
	{
		...
		$resp = ihttp_request(rr_vv3_AUTH_URL, array('ip' => $ip, 'id' => $id, 'code' => $auth['code'], 'domain' => $domain, 'plugin' => $plugin), NULL, 1);
	...
	function tpl_selector_new($name, $options = array())
	{
		...
		foreach ($options['items'] as $item) {
			if ($options['type'] == 'image') {
				...
				$html .= "\r\n                    <tr class='multi-product-item' data-" . $options['key'] . '=\'' . $item['goodsid'] . "' >\r\n                        <input type='hidden' class='form-control img-textname' readonly='' value='" . $item[$options['text']] . "'>\r\n                       <input type='hidden'  value='" . $item['goodsid'] . '\' name=\'' . $id . "'>\r\n                        <td style='width:80px;'>\r\n                            <img src='" . tomedia($item[$options['thumb']]) . "' style='width:70px;border:1px solid #ccc;padding:1px' onerror=\"this.src='../addons/rr_vv3/static/images/nopic.png'\">\r\n                        </td>\r\n                        <td style='width:220px;'>" . $item[$options['text']] . "</td>\r\n                        <td>";
...
if (!function_exists('mobileUrl')) {
	function mobileUrl($do = '', $query = NULL, $full = false)
	{
		...
		$query = array_merge(array('m' => 'rr_vv3'), $query);
...
if (!function_exists('tpl_form_field_image2')) {
	function tpl_form_field_image2($name, $value = '', $default = '', $options = array())
	{
		...
		if (empty($default)) {
			$default = '../addons/rr_vv3/static/images/nopic.png';
		}
		...
		if (!empty($value)) {
			...
			$val = '../addons/rr_vv3/static/images/default-pic.jpg';
		}
...
if (!function_exists('tpl_form_field_multi_image2')) {
	function tpl_form_field_multi_image2($name, $value = array(), $options = array())
	{
		...
		if (!defined('TPL_INIT_MULTI_IMAGE')) {
			$s = "\r\n<script type=\"text/javascript\">\r\n\tfunction uploadMultiImage(elm) {\r\n\t\tvar name = \$(elm).next().val();\r\n\t\tutil.image( \"\", function(urls){\r\n\t\t\t\$.each(urls, function(idx, url){\r\n\t\t\t\t\$(elm).parent().parent().next().append('<div class=\"multi-item\"><img onerror=\"this.src=\\'../addons/rr_vv3/static/images/nopic.png\\'; this.title=\\'图片未找到.\\'\" src=\"'+url.url+'\" class=\"img-responsive img-thumbnail\"><input type=\"hidden\" name=\"'+name+'[]\" value=\"'+url.attachment+'\"><em class=\"close\" title=\"删除这张图片\" onclick=\"deleteMultiImage(this)\">×</em></div>');\r\n\t\t\t});\r\n\t\t}, " . json_encode($options) . ");\r\n\t}\r\n\tfunction deleteMultiImage(elm){\r\n\t\trequire([\"jquery\"], function(\$){\r\n\t\t\t\$(elm).parent().remove();\r\n\t\t});\r\n\t}\r\n</script>";
		...
		if (is_array($value) && (0 < count($value))) {
			foreach ($value as $row) {
				$s .= "\r\n<div class=\"multi-item\">\r\n\t<img src=\"" . tomedia($row) . "\" onerror=\"this.src='../addons/rr_vv3/static/images/nopic.png'; this.title='图片未找到.'\" class=\"img-responsive img-thumbnail\">\r\n\t<input type=\"hidden\" name=\"" . $name . '[]" value="' . $row . "\" >\r\n\t<em class=\"close\" title=\"删除这张图片\" onclick=\"deleteMultiImage(this)\">×</em>\r\n</div>";
			}
		}
...
if (!(function_exists('tpl_goods_selector'))) {
	...
	function tpl_goods_selector($name, $data_gs = '', $option = array())
	{
		...
		$path = IA_ROOT . '/addons/rr_vv3/template/web_v3/util/tpl_goods_selector.html';
		include $path;
...
function cloud_upgrade($type, $post_data = array(), $timeout = 60)
{
    ...
    $resp = ihttp_request("http://0359.0359.link/api/api2.php", $post_data, $extra, $ip, $timeout);
