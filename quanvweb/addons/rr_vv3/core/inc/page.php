...
class Page extends WeModuleSite
{
	public function runTasks()
	{
		...
		if (($lasttime + $interval) <= $current) {
			...
			ihttp_request(rr_vv3_TASK_URL . 'order/receive.php', NULL, NULL, 1);
		}
		...
		if (($lasttime + $interval) <= $current) {
			...
			ihttp_request(rr_vv3_TASK_URL . 'order/close.php', NULL, NULL, 1);
		}
		...
		if (($lasttime + $interval) <= $current) {
			...
			ihttp_request(rr_vv3_TASK_URL . 'order/fullback.php', NULL, NULL, 1);
		}
		...
		if (($lasttime + $interval) <= $current) {
			...
			ihttp_request(rr_vv3_TASK_URL . 'goods/status.php', NULL, NULL, 1);
		}
		...
		if (com('coupon')) {
			...
			if (($lasttime + $interval) <= $current) {
				...
				ihttp_request(rr_vv3_TASK_URL . 'order/willclose.php', NULL, NULL, 1);
			}
		}
		...
		if (com('coupon')) {
			...
			if (($lasttime + $interval) <= $current) {
				...
				ihttp_request(rr_vv3_TASK_URL . 'coupon/back.php', NULL, NULL, 1);
			}
		}
		...
		if (p('groups')) {
			...
			if (($groups_order_lasttime + $groups_order_interval) <= $groups_order_current) {
				...
				ihttp_request($_W['siteroot'] . 'addons/rr_vv3/plugin/groups/task/order.php', NULL, NULL, 1);
			}
			...
			if (($groups_team_lasttime + $groups_team_interval) <= $groups_team_current) {
				...
				ihttp_request($_W['siteroot'] . 'addons/rr_vv3/plugin/groups/task/refund.php', NULL, NULL, 1);
			}
			...
			if (($groups_receive_lasttime + $groups_receive_interval) <= $groups_receive_current) {
				...
				ihttp_request($_W['siteroot'] . 'addons/rr_vv3/plugin/groups/task/receive.php', NULL, NULL, 1);
			}
		}
		...
		if (p('seckill')) {
			...
			if (($lasttime + $interval) <= $current) {
				...
				ihttp_request($_W['siteroot'] . 'addons/rr_vv3/plugin/seckill/task/receive.php', NULL, NULL, 1);
			}
		}
	...
	public function template($filename = '', $type = TEMPLATE_INCLUDEPATH, $account = false)
	{
		...
		$name = 'rr_vv3';
		$moduleroot = IA_ROOT . '/addons/rr_vv3';
