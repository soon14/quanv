...
class MobilePage extends Page
{
	...
	public function __construct()
	{
		...
		if (!empty($_GPC['merchid']) && $merch_plugin && $merch_data['is_openmerch']) {
			$this->merch_user = pdo_fetch('select * from ' . tablename('rr_v_merch_user') . ' where id=:id limit 1', array(':id' => intval($_GPC['merchid'])));
		}
	}
	...
	public function followBar($diypage = false, $merch = false)
	{
		...
		if ((!$followed && ($memberid != $mid)) || (!empty($diyfollowbar['params']['showtype']) && !empty($diyfollowbar['params']['isopen']))) {
			...
			if (!empty($mid) && ($memberid != $mid)) {
				if (!empty($_SESSION[rr_vv3_PREFIX . '_shareid']) && ($_SESSION[rr_vv3_PREFIX . '_shareid'] == $mid)) {
					$mid = $_SESSION[rr_vv3_PREFIX . '_shareid'];
				}

				$member = m('member')->getMember($mid);

				if (!empty($member)) {
					$_SESSION[rr_v_PREFIX . '_shareid'] = $mid;
					$friend = true;
	...
	public function MemberBar($diypage = false, $merch = false)
	{
		...
		if (!empty($data['become_goodsid'])) {
			$goods = pdo_fetch('select id,title,thumb from ' . tablename('rr_v_goods') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $data['become_goodsid'], ':uniacid' => $_W['uniacid']));
		}
		else {
			return NULL;
		}
	...
	public function footerMenus($diymenuid = NULL, $ismerch = false, $texts = array())
	{
		...
		$cartcount = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('rr_v_member_cart') . ' where uniacid=:uniacid and openid=:openid and deleted=0 and isnewstore=0  and selected =1', $params);
		$commission = array();
		...
		if (!empty($diymenuid)) {
			$menu = pdo_fetch('SELECT * FROM ' . tablename('rr_v_diypage_menu') . ' WHERE id=:id and uniacid=:uniacid limit 1 ', array(':id' => $diymenuid, ':uniacid' => $_W['uniacid']));
	...
	public function diyDanmu($diy = false)
	{
		...
		if (empty($danmu['params']['datatype'])) {
			...
			$danmu['data'] = pdo_fetchall('SELECT nickname, avatar as imgurl FROM' . tablename('rr_v_member') . ' WHERE uniacid=:uniacid AND nickname!=\'\' AND avatar!=\'\' ' . $condition . ' ORDER BY rand() LIMIT 10', array(':uniacid' => $_W['uniacid']));
			...
			$danmu['data'] = pdo_fetchall('SELECT m.nickname, m.avatar as imgurl, o.createtime as time FROM' . tablename('rr_v_order') . ' o LEFT JOIN ' . tablename('rr_v_member') . ' m ON m.openid=o.openid WHERE o.uniacid=:uniacid AND m.nickname!=\'\' AND m.avatar!=\'\' ORDER BY o.createtime DESC LIMIT 10', array(':uniacid' => $_W['uniacid']));
		}
		else {
