...
class Index_RrvV3Page extends MobilePage
{
	...
	public function getlist()
	{
		...
		$psize = 10;
		$time = time();
		$sql = 'select vg.*,g.title,g.subtitle,g.thumb,c.card_id  from ' . tablename('rr_v_verifygoods') . "   vg\r\n\t\t inner join " . tablename('rr_v_order_goods') . " og on vg.ordergoodsid = og.id\r\n\t\t left  join " . tablename('rr_v_order') . " o on vg.orderid = o.id\r\n\t\t left  join " . tablename('rr_v_order_refund') . " orf on o.refundid = orf.id\r\n\t\t inner join " . tablename('rr_v_goods') . " g on og.goodsid = g.id\r\n\t\t left  join " . tablename('rr_v_goods_cards') . " c on c.id = g.cardid\r\n\t\t where   vg.uniacid=:uniacid and vg.openid=:openid and vg.invalid =0 and (o.refundid=0 or orf.status<0) and o.status>0";
	...
	public function detail()
	{
		...
		if (empty($verifycode) || ($item['codeinvalidtime'] < time())) {
			...
			while (1) {
				$count = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_verifygoods') . ' where verifycode=:verifycode and uniacid=:uniacid limit 1', array(':verifycode' => $verifycode, ':uniacid' => $_W['uniacid']));
			...
			pdo_update('rr_v_verifygoods', $data, array('id' => $item['id']));
		}
