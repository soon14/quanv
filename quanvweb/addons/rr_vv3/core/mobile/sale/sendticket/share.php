...
class Share_RrvV3Page extends MobileLoginPage
{
	...
	public function enjoy()
	{
		...
		if (!empty($_GPC['sid'])) {
			$asql = 'SELECT * FROM ' . tablename('rr_v_sendticket_share') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND id = ' . intval($_GPC['sid']) . ' AND status = 1 AND (expiration = 0 OR (expiration = 1 AND endtime >= ' . TIMESTAMP . '))';
		...
		$csql = 'SELECT * FROM ' . tablename('rr_v_coupon_data') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND shareident = "' . trim($_GPC['ident']) . '" AND openid = "' . $openid . '"';
		...
		if (!empty($clist)) {
			...
			if (!empty($cpid)) {
				...
				foreach ($cpid as $ck => $cv) {
					$couponsql = 'SELECT * FROM ' . tablename('rr_v_coupon') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND id = ' . $cv;
		...
		$storesql = 'SELECT * FROM ' . tablename('rr_v_sysset') . ' WHERE uniacid = ' . intval($_W['uniacid']);
	...
	{
		global $_W;
		$sql = 'SELECT * FROM ' . tablename('rr_v_coupon') . ' WHERE  uniacid = ' . intval($_W['uniacid']) . ' AND id IN (' . $cpids . ')';
	...
	public function sendTicket($openid, $couponid, $gettype = 0, $num, $ident = '')
	{
		...
		while ($i < $num) {
			...
			$datas = pdo_insert('rr_v_coupon_data', $data);
			++$i;
		}
	...
	public function shareCoupon()
	{
		...
		if ($shareOpenid == $openid) {
			...
			pdo_update('rr_v_order', $data, array('id' => intval($_GPC['orderid'])));
	...
	public function shareSuccess()
	{
		...
		global $_GPC;
		$openid = $_W['openid'];
		$orderSql = 'SELECT isshare FROM ' . tablename('rr_v_order') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND id = ' . intval($_GPC['orderid']);
		...
		$couponDataSql = 'SELECT * FROM ' . tablename('rr_v_coupon_data') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND shareident = "' . $_GPC['ident'] . '"';
