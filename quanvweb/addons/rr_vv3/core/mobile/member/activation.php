...
class Activation_RrvV3Page extends MobileLoginPage
{
	public function main()
	{
		...
		$item = pdo_fetch('select * from ' . tablename('rr_v_member') . ' where uniacid=:uniacid and openid =:openid limit 1 ', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
		...
		if (empty($CardActivation['openactive'])) {
			pdo_update('rr_v_member', $arr, array('openid' => $_W['openid'], 'uniacid' => $_W['uniacid']));
			...
			if (is_wxerror($result)) {
				...
				pdo_update('rr_v_member', array('membercardactive' => 1), array('openid' => $_W['openid'], 'uniacid' => $_W['uniacid']));
	...
	public function submit()
	{
		...
		if (!empty($CardActivation['openactive'])) {
			if (!empty($CardActivation['sms_active']) && !empty($CardActivation['mobile'])) {
				@session_start();
				$key = '__rr_vv3_member_verifycodesession_' . $_W['uniacid'] . '_' . trim($_GPC['mobile']);
				$code = $_SESSION[$key];
	...
	public function verifycode()
	{
		...
		$member = pdo_fetch('select id,openid,mobile,pwd,salt from ' . tablename('rr_v_member') . ' where mobile=:mobile and openid <>:openid  and mobileverify=1 and uniacid=:uniacid limit 1', array(':mobile' => $mobile, ':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
