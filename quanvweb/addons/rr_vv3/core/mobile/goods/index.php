...
class Index_RrvV3Page extends MobilePage
{
	...
	public function gift()
	{
		...
		$gift = pdo_fetch('select * from ' . tablename('rr_v_gift') . ' where uniacid = ' . $uniacid . ' and id = ' . $giftid . ' and starttime <= ' . time() . ' and endtime >= ' . time() . ' and status = 1 ');
		$giftgoodsid = explode(',', $gift['giftgoodsid']);
		$giftgoods = array();

		if (!(empty($giftgoodsid))) {
			foreach ($giftgoodsid as $key => $value ) {
				$giftgoods[$key] = pdo_fetch('select id,status,title,thumb,marketprice from ' . tablename('rr_v_goods') . ' where uniacid = ' . $uniacid . ' and deleted = 0 and total > 0 and id = ' . $value . ' and status = 2 ');
			}
