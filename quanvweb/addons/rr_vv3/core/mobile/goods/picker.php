...
class Picker_RrvV3Page extends MobilePage
{
	public function main()
	{
		...
		if (!empty($goods) && $goods['hasoption']) {
			...
			foreach ($specs as &$spec) {
				$spec['items'] = pdo_fetchall('select * from ' . tablename('rr_v_goods_spec_item') . ' where specid=:specid and `show`=1 order by displayorder asc', array(':specid' => $spec['id']));
			}
