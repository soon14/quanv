...
class Verifygoods_RrvV3Page extends MobilePage
{
	public function detail()
	{
		...
		$saler = pdo_fetch('select * from ' . tablename('rr_v_saler') . ' where  status=1  and openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
		...
		if (!empty($item['limitnum'])) {
			$verifygoodlogs = pdo_fetchall('select *  from ' . tablename('rr_v_verifygoods_log') . '    where verifygoodsid =:id  ', array(':id' => $item['id']));
			$verifynum = 0;
	...
	public function search()
	{
		...
		$verifygood = pdo_fetch('select *  from ' . tablename('rr_v_verifygoods') . ' where uniacid=:uniacid and  verifycode=:verifycode  limit 1 ', array(':uniacid' => $_W['uniacid'], ':verifycode' => $verifycode));
	...
	public function complete()
	{
		...
		if (!empty($verifygood['limitnum'])) {
			$verifygoodlogs = pdo_fetchall('select *  from ' . tablename('rr_v_verifygoods_log') . '    where verifygoodsid =:id  ', array(':id' => $verifygood['id']));
			$verifynum = 0;
		...
		pdo_insert('rr_v_verifygoods_log', $data);
		$logid = pdo_insertid();
		m('notice')->sendVerifygoodMessage($logid);
		pdo_query('update ' . tablename('rr_v_verifygoods') . ' set used=:used , verifycode=null ,codeinvalidtime=0 where id=:id', array(':id' => $verifygood['id'], ':used' => $used));
		...
		if ($isonlyverifygood) {
			$status = pdo_fetchcolumn('select status  from ' . tablename('rr_v_order') . ' where uniacid=:uniacid and id=:id  limit 1 ', array(':uniacid' => $_W['uniacid'], ':id' => $verifygood['orderid']));
	...
	public function finishorder($id)
	{
		...
		pdo_update('rr_v_order', array('status' => 3, 'sendtime' => time(), 'finishtime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
