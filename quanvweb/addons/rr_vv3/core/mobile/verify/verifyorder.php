...
class Verifyorder_RrvV3Page extends MobilePage
{
	public function orderData()
	{
		...
		if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
			...
			if ($searchfield == 'ordersn') {
				...
				$sqlcondition = ' left join ' . tablename('rr_v_store') . ' store on store.id = o.verifystoreid and store.uniacid=o.uniacid';
			}
			else if ($searchfield == 'goodstitle') {
				$sqlcondition = ' inner join ( select DISTINCT(og.orderid) from ' . tablename('rr_v_order_goods') . ' og left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and (locate(:keyword,g.title)>0)) gs on gs.orderid=o.id';
			}
			else {
				if ($searchfield == 'goodssn') {
					$sqlcondition = ' inner join ( select DISTINCT(og.orderid) from ' . tablename('rr_v_order_goods') . ' og left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and (((locate(:keyword,g.goodssn)>0)) or (locate(:keyword,og.goodssn)>0))) gs on gs.orderid=o.id';
				}
			}
		}
		...
		if (($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 and o.istrade=0 ') || !empty($sqlcondition)) {
			...
			$sql = 'select * from ' . tablename('rr_v_order') . ' where uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 ' . $status_condition . ' GROUP BY id ORDER BY createtime DESC  ';
			...
			if (!empty($list)) {
				...
				$refundid_array = pdo_fetchall('SELECT id,rtype,status as rstatus FROM ' . tablename('rr_v_order_refund') . ' WHERE id IN (' . $refundid . ')', array(), 'id');
				$openid_array = pdo_fetchall('SELECT openid,nickname,id as mid,realname as mrealname,mobile as mmobile FROM ' . tablename('rr_v_member') . ' WHERE openid IN (' . $openid . ') AND uniacid=' . $_W['uniacid'], array(), 'openid');
				$addressid_array = pdo_fetchall('SELECT id,realname as arealname,mobile as amobile,province as aprovince ,city as acity , area as aarea,address as aaddress FROM ' . tablename('rr_v_member_address') . ' WHERE id IN (' . $addressid . ')', array(), 'id');
				$dispatchid_array = pdo_fetchall('SELECT id,dispatchname FROM ' . tablename('rr_v_dispatch') . ' WHERE id IN (' . $dispatchid . ')', array(), 'id');
				$verifyopenid_array = pdo_fetchall('SELECT sm.id as salerid,sm.nickname as salernickname,sm.openid,s.salername FROM ' . tablename('rr_v_saler') . ' s LEFT JOIN ' . tablename('rr_v_member') . ' sm ON sm.openid = s.openid and sm.uniacid=s.uniacid WHERE s.openid IN (' . $verifyopenid . ')', array(), 'openid');
			...
			foreach ($list as &$value) {
				...
				if (!empty($agentid) && (0 < $level)) {
					...
					if ($value['agentid'] == $agentid) {
						$value['level'] = 1;
						$level1_commissions = pdo_fetchall('select commission1,commissions  from ' . tablename('rr_v_order_goods') . ' og ' . ' left join  ' . tablename('rr_v_order') . ' o on o.id = og.orderid ' . ' where og.orderid=:orderid and o.agentid= ' . $agentid . '  and o.uniacid=:uniacid', array(':orderid' => $value['id'], ':uniacid' => $uniacid));
						...
						if (0 < $agent['level2']) {
							$level2_commissions = pdo_fetchall('select commission2,commissions  from ' . tablename('rr_v_order_goods') . ' og ' . ' left join  ' . tablename('rr_v_order') . ' o on o.id = og.orderid ' . ' where og.orderid=:orderid and  o.agentid in ( ' . implode(',', array_keys($agent['level1_agentids'])) . ')  and o.uniacid=:uniacid', array(':orderid' => $value['id'], ':uniacid' => $uniacid));
						...
						if (in_array($value['agentid'], array_keys($agent['level2_agentids']))) {
							...
							if (0 < $agent['level3']) {
								$level3_commissions = pdo_fetchall('select commission3,commissions from ' . tablename('rr_v_order_goods') . ' og ' . ' left join  ' . tablename('rr_v_order') . ' o on o.id = og.orderid ' . ' where og.orderid=:orderid and  o.agentid in ( ' . implode(',', array_keys($agent['level2_agentids'])) . ')  and o.uniacid=:uniacid', array(':orderid' => $value['id'], ':uniacid' => $uniacid));
		...
		if ($_GPC['export'] == 1) {
			...
			foreach ($list as &$row) {
				...
				if (!empty($row['verifystoreid'])) {
					if (0 < $row['merchid']) {
						$row['storeinfo'] = pdo_fetchcolumn('select storename from ' . tablename('rr_v_merch_store') . ' where id=:storeid limit 1 ', array(':storeid' => $row['verifystoreid']));
					}
					else {
						$row['storeinfo'] = pdo_fetchcolumn('select storename from ' . tablename('rr_v_store') . ' where id=:storeid limit 1 ', array(':storeid' => $row['verifystoreid']));
					}
				}
				else {
					if (!empty($row['verifyinfo'])) {
				...
				if (!empty($row['storeid'])) {
					if (0 < $row['merchid']) {
						$row['pickname'] = pdo_fetchcolumn('select storename from ' . tablename('rr_v_merch_store') . ' where id=:storeid limit 1 ', array(':storeid' => $row['storeid']));
					}
					else {
						$row['pickname'] = pdo_fetchcolumn('select storename from ' . tablename('rr_v_store') . ' where id=:storeid limit 1 ', array(':storeid' => $row['storeid']));
					}
				}
					...
					if (is_array($verifyinfo)) {
						if (empty($row['dispatchtype'])) {
							...
							if ($v['verified'] || ($row['verifytype'] == 1)) {
								$v['storename'] = pdo_fetchcolumn('select storename from ' . tablename('rr_v_merch_store') . ' where id=:id limit 1', array(':id' => $v['verifystoreid']));
								...
								$v['nickname'] = pdo_fetchcolumn('select nickname from ' . tablename('rr_v_member') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $v['verifyopenid'], ':uniacid' => $_W['uniacid']));
								$v['salername'] = pdo_fetchcolumn('select salername from ' . tablename('rr_v_merch_saler') . ' where openid=:openid and uniacid=:uniacid and merchid = :merchid limit 1', array(':openid' => $v['verifyopenid'], ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		...
		if (($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0') || !empty($sqlcondition)) {
			$t = pdo_fetch('SELECT COUNT(DISTINCT(o.id)) as count, ifnull(sum(o.price),0) as sumprice   FROM ' . tablename('rr_v_order') . ' o ' . ' left join ' . tablename('rr_v_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('rr_v_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('rr_v_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('rr_v_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' left join ' . tablename('rr_v_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . ' ' . $sqlcondition . ' WHERE ' . $condition . ' ' . $statuscondition, $paras);
		}
		else {
			$t = pdo_fetch('SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('rr_v_order') . ' WHERE uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 ' . $status_condition, $paras);
		}

		$total = $t['count'];
		$totalmoney = $t['sumprice'];
		$pager = pagination2($total, $pindex, $psize);
		$stores = pdo_fetchall('select id,storename from ' . tablename('rr_v_store') . ' where uniacid=:uniacid ', array(':uniacid' => $uniacid));
	...
	public function log()
	{
		...
		if (P('newstore')) {
			$openid = $_W['openid'];
			$salers = pdo_fetch('select r.perms  from ' . tablename('rr_v_saler') . ' s inner join ' . tablename('rr_v_newstore_perm_role') . ' r on s.roleid = r.id  where  s.openid=:openid and r.deleted =0  limit 1', array(':openid' => $openid));
	...
	protected function selectOrderPrice($day = 0)
	{
		...
		$sql = 'SELECT o.id,o.price,o.createtime,s.openid,s.salername FROM' . tablename('rr_v_order') . ' o inner JOIN ' . tablename('rr_v_saler') . " s ON o.verifyopenid=s.openid   WHERE \r\n        o.uniacid=:uniacid AND o.ismr=0  AND   o.status =3  AND o.deleted=0 AND o.createtime between :createtime1 and :createtime2";
	...
	public function detail()
	{
		...
		$id = intval($_GPC['id']);
		$p = p('commission');
		$item = pdo_fetch('SELECT * FROM ' . tablename('rr_v_order') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		$item['statusvalue'] = $item['status'];
		$item['paytypevalue'] = $item['paytype'];
		$order_goods = array();

		if (0 < $item['sendtype']) {
			$order_goods = pdo_fetchall('SELECT orderid,goodsid,sendtype,expresssn,expresscom,express,sendtime FROM ' . tablename('rr_v_order_goods') . "\r\n            WHERE orderid = " . $id . ' and sendtime > 0 and uniacid=' . $_W['uniacid'] . ' and sendtype > 0 group by sendtype order by sendtime desc ');

			foreach ($order_goods as $key => $value) {
				$order_goods[$key]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype=' . $value['sendtype'] . ' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $id));
			}
		...
		$dispatch = pdo_fetch('SELECT * FROM ' . tablename('rr_v_dispatch') . ' WHERE id = :id and uniacid=:uniacid and merchid=0', array(':id' => $item['dispatchid'], ':uniacid' => $_W['uniacid']));
		...
		if (empty($item['addressid'])) {
			...
			if (!is_array($user)) {
				$user = pdo_fetch('SELECT * FROM ' . tablename('rr_v_member_address') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $item['addressid'], ':uniacid' => $_W['uniacid']));
			}
		...
		$refund = pdo_fetch('SELECT * FROM ' . tablename('rr_v_order_refund') . ' WHERE orderid = :orderid and uniacid=:uniacid order by id desc', array(':orderid' => $item['id'], ':uniacid' => $_W['uniacid']));
		$diyformfields = '';
		...
		if (com('verify')) {
			...
			if (!empty($item['verifyopenid'])) {
				...
				$saler['salername'] = pdo_fetchcolumn('select salername from ' . tablename('rr_v_saler') . ' where openid=:openid and uniacid=:uniacid limit 1 ', array(':uniacid' => $_W['uniacid'], ':openid' => $item['verifyopenid']));
			}

			if (!empty($item['verifystoreid'])) {
				$store = pdo_fetch('select * from ' . tablename('rr_v_store') . ' where id=:storeid limit 1 ', array(':storeid' => $item['verifystoreid']));
			}
					...
					if (empty($item['dispatchtype'])) {
						foreach ($verifyinfo as &$v) {
							if ($v['verified'] || ($item['verifytype'] == 1)) {
								$v['storename'] = pdo_fetchcolumn('select storename from ' . tablename('rr_v_store') . ' where id=:id limit 1', array(':id' => $v['verifystoreid']));
								...
								$v['nickname'] = pdo_fetchcolumn('select nickname from ' . tablename('rr_v_member') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $v['verifyopenid'], ':uniacid' => $_W['uniacid']));
								$v['salername'] = pdo_fetchcolumn('select salername from ' . tablename('rr_v_saler') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $v['verifyopenid'], ':uniacid' => $_W['uniacid']));
							}
						}
