...
class Create_RrvV3Page extends MobileLoginPage
{
	...
	public function main()
	{
		...
		if (!$packageid) {
			...
			if (p('bargain') && !empty($bargain_id)) {
				...
				$bargain_act = pdo_fetch('SELECT *  FROM ' . tablename('rr_v_bargain_actor') . ' WHERE id = :id AND openid = :openid AND status = \'0\'', array(':id' => $bargain_id, ':openid' => $_W['openid']));

				if (empty($bargain_act)) {
					exit('没有这个商品!');
				}

				$bargain_act_id = pdo_fetch('SELECT *  FROM ' . tablename('rr_v_bargain_goods') . ' WHERE id = \'' . $bargain_act['goods_id'] . '\'');

				if (empty($bargain_act_id)) {
					exit('没有这个商品!');
				}

				$if_bargain = pdo_fetch('SELECT bargain FROM ' . tablename('rr_v_goods') . ' WHERE id = :id AND uniacid = :uniacid ', array(':id' => $bargain_act_id['goods_id'], ':uniacid' => $_W['uniacid']));
			...
			if (empty($id)) {
				if (!empty($quickid)) {
					$sql = 'SELECT c.goodsid,c.total,g.maxbuy,g.type,g.intervalfloor,g.intervalprice,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype' . ',g.weight,o.weight as optionweight,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,o.title as optiontitle,c.optionid,' . ' g.storeids,g.isverify,g.deduct,g.manydeduct,g.virtual,o.virtual as optionvirtual,discounts,' . ' g.deduct2,g.ednum,g.edmoney,g.edareas,g.edareas_code,g.diyformtype,g.diyformid,diymode,g.dispatchtype,g.dispatchid,g.dispatchprice,g.minbuy ' . ' ,g.isdiscount,g.isdiscount_time,g.isdiscount_discounts,g.cates,g.isfullback, ' . ' g.virtualsend,invoice,o.specs,g.merchid,g.checked,g.merchsale,g.unite_total,' . ' g.buyagain,g.buyagain_islong,g.buyagain_condition, g.buyagain_sale, g.hasoption, g.threen' . ' FROM ' . tablename('rr_v_quick_cart') . ' c ' . ' left join ' . tablename('rr_v_goods') . ' g on c.goodsid = g.id ' . ' left join ' . tablename('rr_v_goods_option') . ' o on c.optionid = o.id ' . ' where c.openid=:openid and c.selected=1 and  c.deleted=0 and c.uniacid=:uniacid and c.quickid=' . $quickid . '  order by c.id desc';
					$goods = pdo_fetchall($sql, array(':uniacid' => $uniacid, ':openid' => $openid));
				}
				else if (empty($exchangeOrder)) {
					$sql = 'SELECT c.goodsid,c.total,g.maxbuy,g.type,g.intervalfloor,g.intervalprice,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype' . ',g.weight,o.weight as optionweight,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,o.title as optiontitle,c.optionid,' . ' g.storeids,g.isverify,g.deduct,g.manydeduct,g.virtual,o.virtual as optionvirtual,discounts,' . ' g.deduct2,g.ednum,g.edmoney,g.edareas,g.diyformtype,g.diyformid,diymode,g.dispatchtype,g.dispatchid,g.dispatchprice,g.minbuy ' . ' ,g.isdiscount,g.isdiscount_time,g.isdiscount_discounts,g.cates, ' . ' g.virtualsend,invoice,o.specs,g.merchid,g.checked,g.merchsale,' . ' g.buyagain,g.buyagain_islong,g.buyagain_condition, g.buyagain_sale, g.hasoption' . ' FROM ' . tablename('rr_v_member_cart') . ' c ' . ' left join ' . tablename('rr_v_goods') . ' g on c.goodsid = g.id ' . ' left join ' . tablename('rr_v_goods_option') . ' o on c.optionid = o.id ' . ' where c.openid=:openid and c.selected=1 and  c.deleted=0 and c.uniacid=:uniacid  order by c.id desc';
					$goods = pdo_fetchall($sql, array(':uniacid' => $uniacid, ':openid' => $openid));
				}
				else {
					if (p('exchange')) {
						$sql = 'SELECT c.goodsid,c.total,g.maxbuy,g.type,g.intervalfloor,g.intervalprice,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype' . ',g.weight,o.weight as optionweight,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,o.title as optiontitle,c.optionid,' . ' g.storeids,g.isverify,g.deduct,g.manydeduct,g.virtual,o.virtual as optionvirtual,discounts,' . ' g.deduct2,g.ednum,g.edmoney,g.edareas,g.edareas_code,g.diyformtype,g.diyformid,diymode,g.dispatchtype,g.dispatchid,g.dispatchprice,g.minbuy ' . ' ,g.isdiscount,g.isdiscount_time,g.isdiscount_discounts,g.cates,g.isfullback, ' . ' g.virtualsend,invoice,o.specs,g.merchid,g.checked,g.merchsale,g.unite_total,' . ' g.buyagain,g.buyagain_islong,g.buyagain_condition, g.buyagain_sale, g.hasoption' . ' FROM ' . tablename('rr_v_exchange_cart') . ' c ' . ' left join ' . tablename('rr_v_goods') . ' g on c.goodsid = g.id ' . ' left join ' . tablename('rr_v_goods_option') . ' o on c.optionid = o.id ' . ' where c.openid=:openid and c.selected=1 and  c.deleted=0 and c.uniacid=:uniacid  order by c.id desc';
						$goods = pdo_fetchall($sql, array(':uniacid' => $uniacid, ':openid' => $openid));
					}
				}
				...
				if (empty($goods)) {
					...
					foreach ($goods as $k => $v) {
						...
						if ($v['isfullback']) {
							$fullbackgoods = pdo_fetch('SELECT * FROM ' . tablename('rr_v_fullback_goods') . ' WHERE uniacid = ' . $uniacid . ' and goodsid = ' . $v['goodsid'] . ' limit 1 ');
						}
				...
				if (!empty($id) && !empty($iswholesale)) {
					...
					foreach ($optionsdata as $option) {
						...
						if (0 < $option['optionid']) {
							$option = pdo_fetch('select id,title,marketprice,presellprice,goodssn,productsn,`virtual`,stock,weight,specs from ' . tablename('rr_v_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $id, ':id' => $option['optionid']));
					...
					$sql = 'SELECT id as goodsid,type,title,weight,issendfree,isnodiscount,ispresell,presellprice,' . ' thumb,marketprice,liveprice,islive,storeids,isverify,deduct,hasoption,preselltimeend,presellsendstatrttime,presellsendtime,presellsendtype,' . ' manydeduct,`virtual`,maxbuy,usermaxbuy,discounts,total as stock,deduct2,showlevels,' . ' ednum,edmoney,edareas,edareas_code,unite_total,' . ' diyformtype,diyformid,diymode,dispatchtype,dispatchid,dispatchprice,cates,minbuy, ' . ' isdiscount,isdiscount_time,isdiscount_discounts,isfullback, ' . ' virtualsend,invoice,needfollow,followtip,followurl,merchid,checked,merchsale, ' . ' buyagain,buyagain_islong,buyagain_condition, buyagain_sale' . $threensql . $ishotelsql . ' FROM ' . tablename('rr_v_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
					$data = pdo_fetch($sql, array(':uniacid' => $uniacid, ':id' => $id));
					...
					if ($data['seckillinfo'] && ($data['seckillinfo']['status'] == 0)) {
						...
						if ($giftid) {
							$gift = pdo_fetch('select id,title,thumb,activity,giftgoodsid,goodsid from ' . tablename('rr_v_gift') . "\r\n                where uniacid = " . $uniacid . ' and id = ' . $giftid . ' and status = 1 and starttime <= ' . time() . ' and endtime >= ' . time() . ' ');
							...
							if (!empty($gift['giftgoodsid'])) {
								...
								if ($giftGoodsid) {
									foreach ($giftGoodsid as $key => $value) {
										$giftGood[$key] = pdo_fetch('select id,title,thumb,marketprice from ' . tablename('rr_v_goods') . ' where uniacid = ' . $uniacid . ' and total > 0 and status = 2 and id = ' . $value . ' and deleted = 0 ');
									}
					...
					if ($data['isfullback']) {
						$fullbackgoods = pdo_fetch('SELECT * FROM ' . tablename('rr_v_fullback_goods') . ' WHERE uniacid = ' . $uniacid . ' and goodsid = ' . $data['goodsid'] . ' limit 1 ');
					}
			...
			foreach ($goods as &$g) {
				if ($g['seckillinfo'] && ($g['seckillinfo']['status'] == 0)) {
					...
					if (p('task')) {
						...
						if (!empty($task_id)) {
							$rewarded = pdo_fetchcolumn('SELECT `rewarded` FROM ' . tablename('rr_v_task_extension_join') . ' WHERE id = :id AND openid = :openid AND  uniacid = :uniacid', array(':id' => $task_id, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
				...
				if (!empty($g['seckillinfo']) && ($g['seckillinfo']['status'] == 0)) {
					...
					if (0 < $g['usermaxbuy']) {
						$order_goodscount = pdo_fetchcolumn('select ifnull(sum(og.total),0)  from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_order') . ' o on og.orderid=o.id ' . ' where og.goodsid=:goodsid and  o.status>=0 and o.openid=:openid  and og.uniacid=:uniacid ', array(':goodsid' => $g['goodsid'], ':uniacid' => $uniacid, ':openid' => $openid));
			...
			if ($hasinvoice) {
				$invoicename = pdo_fetchcolumn('select invoicename from ' . tablename('rr_v_order') . ' where openid=:openid and uniacid=:uniacid and ifnull(invoicename,\'\')<>\'\'', array(':openid' => $openid, ':uniacid' => $uniacid));
			...
			if (!$isverify && !$isvirtual && !$ismerch) {
				if (0 < $merch_id) {
					$carrier_list = pdo_fetchall('select * from ' . tablename('rr_v_merch_store') . ' where  uniacid=:uniacid and merchid=:merchid and status=1 and type in(1,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merch_id));
				}
				else {
					$carrier_list = pdo_fetchall('select * from ' . tablename('rr_v_store') . ' where  uniacid=:uniacid and status=1 and type in(1,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid']));
				}
			}
			...
			if ($isverify) {
				...
				if (empty($storeids)) {
					if (0 < $merchid) {
						$stores = pdo_fetchall('select * from ' . tablename('rr_v_merch_store') . ' where  uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
					}
					else {
						$stores = pdo_fetchall('select * from ' . tablename('rr_v_store') . ' where  uniacid=:uniacid and status=1 and type in(2,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid']));
					}
				}
				else if (0 < $merchid) {
					$stores = pdo_fetchall('select * from ' . tablename('rr_v_merch_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
				}
				else {
					$stores = pdo_fetchall('select * from ' . tablename('rr_v_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and status=1 and type in(2,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid']));
				}
			}
			else {
				$address = pdo_fetch('select * from ' . tablename('rr_v_member_address') . ' where openid=:openid and deleted=0 and isdefault=1  and uniacid=:uniacid limit 1', array(':uniacid' => $uniacid, ':openid' => $openid));
			...
			foreach ($goods as $g) {
				...
				if ($g['seckillinfo'] && ($g['seckillinfo']['status'] == 0)) {
					...
					$gift = array();
					$giftdata = pdo_fetch('select giftgoodsid from ' . tablename('rr_v_gift') . ' where uniacid = ' . $uniacid . ' and id = ' . $giftid . ' and status = 1 and starttime <= ' . time() . ' and endtime >= ' . time() . ' ');

					if ($giftdata['giftgoodsid']) {
						$giftgoodsid = explode(',', $giftdata['giftgoodsid']);

						foreach ($giftgoodsid as $key => $value) {
							$gift[$key] = pdo_fetch('select id as goodsid,title,thumb from ' . tablename('rr_v_goods') . ' where uniacid = ' . $uniacid . ' and total > 0 and status = 2 and id = ' . $value . ' and deleted = 0 ');
					...
					foreach ($gifts as $key => $value) {
						...
						foreach ($giftgoods as $k => $val) {
							$giftgoodsdetail = pdo_fetch('select id,title,thumb,marketprice from ' . tablename('rr_v_goods') . ' where uniacid = ' . $uniacid . ' and deleted = 0 and total > 0 and status = 2 and id = ' . $val . ' ');
			...
			$package = pdo_fetch('SELECT * FROM ' . tablename('rr_v_package') . ' WHERE uniacid = ' . $uniacid . ' and id = ' . $packageid . ' ');
			...
			foreach ($g as $key => $value) {
				$goods[$key] = pdo_fetch('select id,title,thumb,marketprice from ' . tablename('rr_v_goods') . "\r\n                            where id = " . $value['goodsid'] . ' and uniacid = ' . $uniacid . ' ');
				$option = array();
				$packagegoods = array();

				if (0 < $value['optionid']) {
					$option = pdo_fetch('select title,packageprice from ' . tablename('rr_v_package_goods_option') . "\r\n                            where optionid = " . $value['optionid'] . ' and goodsid=' . $value['goodsid'] . ' and uniacid = ' . $uniacid . ' and pid = ' . $packageid . ' ');
					$goods[$key]['packageprice'] = $option['packageprice'];
				}
				else {
					$packagegoods = pdo_fetch('select title,packageprice from ' . tablename('rr_v_package_goods') . "\r\n                            where goodsid=" . $value['goodsid'] . ' and uniacid = ' . $uniacid . ' and pid = ' . $packageid . ' ');
	...
	public function caculatecoupon($contype, $couponid, $wxid, $wxcardid, $wxcode, $goodsarr, $totalprice, $discountprice, $isdiscountprice, $isSubmit = 0, $discountprice_array = array(), $merchisdiscountprice = 0)
	{
		...
		if ($contype == 1) {
			$sql = 'select id,uniacid,card_type,logo_url,title, card_id,least_cost,reduce_cost,discount,merchid,limitgoodtype,limitgoodcatetype,limitgoodcateids,limitgoodids,merchid,limitdiscounttype  from ' . tablename('rr_v_wxcard');
	...
	public function caculate()
	{
		...
		$address = pdo_fetch('select * from ' . tablename('rr_v_member_address') . ' where  id=:id and openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $uniacid, ':openid' => $openid, ':id' => $addressid));
		$member = m('member')->getMember($openid, true);
		$level = m('member')->getLevel($openid);
		...
		if (is_array($goodsarr)) {
			...
			foreach ($goodsarr as &$g) {
				...
				if (!empty($optionid)) {
					$option = pdo_fetch('select id,title,marketprice,presellprice,goodssn,productsn,stock,`virtual`,weight,liveprice,islive from ' . tablename('rr_v_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $goodsid, ':id' => $optionid));
	...
	public function submit()
	{
		...
		if (!empty($packageid)) {
			$package = pdo_fetch('SELECT id,title,price,freight,cash,starttime,endtime FROM ' . tablename('rr_v_package') . "\r\n                    WHERE uniacid = " . $uniacid . ' and id = ' . $packageid . ' and deleted = 0 and status = 1  ORDER BY id DESC');
			...
			$packgoods = pdo_fetchall('SELECT id,title,thumb,packageprice,`option`,goodsid FROM ' . tablename('rr_v_package_goods') . "\r\n                    WHERE uniacid = " . $uniacid . ' and pid = ' . $packageid . '  ORDER BY id DESC');
		...
		if ($giftid) {
			...
			if ($giftdata['giftgoodsid']) {
				...
				foreach ($giftgoodsid as $key => $value) {
					$gift[$key] = pdo_fetch('select id as goodsid,title,thumb from ' . tablename('rr_v_goods') . ' where uniacid = ' . $uniacid . ' and deleted = 0 and status = 2 and id = ' . $value . ' and deleted = 0 ');
				}
		...
		foreach ($goods as $g) {
			...
			if ($data['seckillinfo'] && ($data['seckillinfo']['status'] == 0)) {
				...
				if (0 < $data['usermaxbuy']) {
					$order_goodscount = pdo_fetchcolumn('select ifnull(sum(og.total),0)  from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_order') . ' o on og.orderid=o.id ' . ' where og.goodsid=:goodsid and  o.status>=0 and o.openid=:openid  and og.uniacid=:uniacid ', array(':goodsid' => $data['goodsid'], ':uniacid' => $uniacid, ':openid' => $openid));
			...
			if (!empty($optionid)) {
				$option = pdo_fetch('select id,title,marketprice,liveprice,presellprice,goodssn,productsn,stock,`virtual`,weight' . $sql_condition . ' from ' . tablename('rr_v_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $goodsid, ':id' => $optionid));
				...
				if (!empty($option)) {
					if (!empty($_SESSION['exchange']) && p('exchange')) {
						if ($option['exchange_stock'] <= 0) {
							...
							pdo_query('UPDATE ' . tablename('rr_v_goods_option') . ' SET exchange_stock = exchange_stock - 1 WHERE id = :id AND uniacid = :uniacid', array(':id' => $optionid, ':uniacid' => $_W['uniacid']));
						}
					}
					else {
						if ($data['seckillinfo'] && ($data['seckillinfo']['status'] == 0)) {
					...
					if ($data['type'] != 4) {
						...
						if ($packageid) {
							$packageoption = pdo_fetch('select packageprice from ' . tablename('rr_v_package_goods_option') . "\r\n                                where uniacid = " . $uniacid . ' and goodsid = ' . $goodsid . ' and optionid = ' . $optionid . ' and pid = ' . $packageid . ' ');
				...
				if ($packageid) {
					$pg = pdo_fetch('select packageprice from ' . tablename('rr_v_package_goods') . "\r\n                                where uniacid = " . $uniacid . ' and goodsid = ' . $goodsid . ' and pid = ' . $packageid . ' ');
				...
				if (!empty($_SESSION['exchange']) && p('exchange')) {
					if (0 < $data['exchange_stock']) {
						pdo_query('UPDATE ' . tablename('rr_v_goods') . ' SET exchange_stock = exchange_stock - 1 WHERE id = :id AND uniacid = :uniacid', array(':id' => $data['goodsid'], ':uniacid' => $_W['uniacid']));
					}
					else {
			...
			if (intval($_GPC['fromcart']) == 1) {
				if ($diyform_plugin) {
					$cartdata = pdo_fetch('select id,diyformdataid,diyformfields,diyformdata from ' . tablename('rr_v_member_cart') . ' ' . ' where goodsid=:goodsid and optionid=:optionid and openid=:openid and deleted=0 order by id desc limit 1', array(':goodsid' => $data['goodsid'], ':optionid' => intval($data['optionid']), ':openid' => $openid));
		...
		if ($isverify || $dispatchtype) {
			if ($isverify) {
				if (($verifytype == 0) || ($verifytype == 1)) {
					...
					while (1) {
						$count = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_order') . ' where verifycode=:verifycode and uniacid=:uniacid limit 1', array(':verifycode' => $verifycode, ':uniacid' => $_W['uniacid']));
					...
					if ($verifytype == 2) {
						...
						while ($i <= $totaltimes) {
							...
							while (1) {
								$count = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_order') . ' where concat(verifycodes,\'|\' + verifycode +\'|\' ) like :verifycodes and uniacid=:uniacid limit 1', array(':verifycodes' => '%' . $verifycode . '%', ':uniacid' => $_W['uniacid']));
		...
		if (!empty($goods[0]['bargain_id']) && p('bargain')) {
			$bargain_act = pdo_fetch('SELECT * FROM ' . tablename('rr_v_bargain_actor') . ' WHERE id = :id AND openid = :openid ', array(':id' => $goods[0]['bargain_id'], ':openid' => $_W['openid']));
			...
			if (!pdo_update('rr_v_bargain_actor', array('status' => 1), array('id' => $goods[0]['bargain_id'], 'openid' => $_W['openid']))) {
				exit('下单失败');
			}
		...
		if ($taskgoodsprice) {
			...
			pdo_update('rr_v_task_extension_join', array('rewarded' => serialize($taskGoodsInfo0)), array('id' => $task_id, 'uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
		...
		if (!empty($goods[0]['bargain_id']) && p('bargain')) {
			pdo_update('rr_v_bargain_actor', array('order' => $orderid), array('id' => $goods[0]['bargain_id'], 'openid' => $_W['openid']));
		}
		...
		if ($multiple_order == 0) {
			...
			foreach ($allgoods as $goods) {
				...
				if (!empty($bargain_act) && p('bargain')) {
					...
					pdo_query('UPDATE ' . tablename('rr_v_goods') . ' SET sales = sales + 1 WHERE id = :id AND uniacid = :uniacid', array(':id' => $goods['goodsid'], ':uniacid' => $uniacid));
				}
		...
		if (!empty($_SESSION['exchange']) && p('exchange')) {
			$codeResult = pdo_fetch('SELECT * FROM ' . tablename('rr_v_exchange_code') . ' WHERE `key` = :key AND uniacid =:uniacid', array(':key' => $_SESSION['exchange_key'], ':uniacid' => $_W['uniacid']));

			if ($codeResult['status'] == 2) {
				show_json(0, '兑换失败:此兑换码已兑换');
			}

			$groupResult = pdo_fetch('SELECT * FROM ' . tablename('rr_v_exchange_group') . ' WHERE id = :id AND uniacid = :uniacid ', array(':uniacid' => $_W['uniacid'], ':id' => $codeResult['groupid']));
			$record_exsit = pdo_fetch('SELECT * FROM ' . tablename('rr_v_exchange_record') . ' WHERE `key`=:key AND uniacid = :uniacid', array(':key' => $_SESSION['exchange_key'], ':uniacid' => $_W['uniacid']));
			...
			if (empty($record_exsit)) {
				...
				pdo_insert('rr_v_exchange_record', $data);
			}
			...
			if (empty($_SESSION['exchange_key']) || is_null($_SESSION['exchangeprice']) || is_null($_SESSION['exchangepostage'])) {
				...
				if ($groupResult['mode'] != 6) {
					$exchange_res = pdo_update('rr_v_exchange_code', array('status' => 2), array('key' => $_SESSION['exchange_key'], 'status' => 1, 'uniacid' => $_W['uniacid']));
					pdo_query('UPDATE ' . tablename('rr_v_exchange_group') . ' SET `use` = `use` + 1 WHERE id = :id AND uniacid = :uniacid', array(':id' => $_SESSION['groupid'], ':uniacid' => $_W['uniacid']));
				}
				else {
					pdo_update('rr_v_exchange_code', array('goodsstatus' => 2), array('key' => $_SESSION['exchange_key'], 'uniacid' => $_W['uniacid']));
					if ((($codeResult['balancestatus'] == 2) || empty($groupResult['balance_type'])) && (($codeResult['scorestatus'] == 2) || empty($groupResult['score_type'])) && (($codeResult['redstatus'] == 2) || empty($groupResult['red_type'])) && (($codeResult['couponstatus'] == 2) || empty($groupResult['coupon_type']))) {
						pdo_update('rr_v_exchange_code', array('status' => 2), array('key' => $_SESSION['exchange_key'], 'status' => 1, 'uniacid' => $_W['uniacid']));
					}
				}
			}
		}
		...
		if (!is_null($_SESSION['exchangeprice']) && !empty($_SESSION['exchange']) && p('exchange')) {
			...
			if ($groupResult['mode'] != 6) {
				...
				pdo_update('rr_v_exchange_record', $exchangedata, array('key' => $_SESSION['exchange_key'], 'uniacid' => $_W['uniacid']));
			}
			else {
			...
			pdo_update('rr_v_exchange_cart', array('selected' => 0), array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid'], 'selected' => 1));
		}
		...
		if (is_array($carrier)) {
			...
			pdo_update('rr_v_member', $up, array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
		...
		if ($_GPC['fromcart'] == 1) {
			pdo_query('update ' . tablename('rr_v_member_cart') . ' set deleted=1 where  openid=:openid and uniacid=:uniacid and selected=1 ', array(':uniacid' => $uniacid, ':openid' => $openid));
		}

		if (p('quick') && !empty($_GPC['fromquick'])) {
			pdo_update('rr_v_quick_cart', array('deleted' => 1), array('quickid' => intval($_GPC['fromquick']), 'uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
		}
		...
		if (empty($virtualid)) {
			...
			if (isset($allgoods[0])) {
				$vgoods = $allgoods[0];
				pdo_update('rr_v_goods', array('sales' => $vgoods['sales'] + $vgoods['total']), array('id' => $vgoods['goodsid']));
			}
		}
	...
	protected function singleDiyformData($id = 0)
	{
		...
		if ($diyform_plugin && !empty($id)) {
			$sql = 'SELECT id as goodsid,type,diyformtype,diyformid,diymode,diyfields FROM ' . tablename('rr_v_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
	...
	public function diyform()
	{
		...
		if (empty($goods_temp)) {
			pdo_insert('rr_v_diyform_temp', $insert);
			$gdid = pdo_insertid();
		}
		else {
			pdo_update('rr_v_diyform_temp', $insert, array('id' => $goods_temp['id']));
			$gdid = $goods_temp['id'];
		}

		if (!empty($cartid)) {
			$cart_data = array('diyformid' => $insert['diyformid'], 'diyformfields' => $insert['diyformfields'], 'diyformdata' => $insert['diyformdata']);
			pdo_update('rr_v_member_cart', $cart_data, array('id' => $cartid));
		}
