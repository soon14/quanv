...
class Index_RrvV3Page extends MobileLoginPage
{
	...
	public function get_list()
	{
		...
		foreach ($list as &$row) {
			...
			if ((0 < $row['refundstate']) && !empty($row['refundid'])) {
				$refund = pdo_fetch('select * from ' . tablename('rr_v_order_refund') . ' where id=:id and uniacid=:uniacid and orderid=:orderid limit 1', array(':id' => $row['refundid'], ':uniacid' => $uniacid, ':orderid' => $row['id']));
	...
	public function detail()
	{
		...
		$goodsid_array = array();
		$goods = pdo_fetchall("select og.goodsid,og.price,g.title,g.thumb,g.status, g.cannotrefund, og.total,g.credit,og.optionid,\r\n            og.optionname as optiontitle,g.isverify,g.storeids,og.seckill,g.isfullback,\r\n            og.seckill_taskid" . $diyformfields . $condition1 . ',og.prohibitrefund  from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid ' . ' where ' . $scondition . ' and og.uniacid=:uniacid ', $param);
		$prohibitrefund = false;

		foreach ($goods as &$g) {
			if ($g['isfullback']) {
				$fullbackgoods = pdo_fetch('SELECT * FROM ' . tablename('rr_v_fullback_goods') . ' WHERE uniacid = ' . $uniacid . ' and goodsid = ' . $g['goodsid'] . ' limit 1 ');

				if ($g['optionid']) {
					$option = pdo_fetch("select `day`,allfullbackprice,fullbackprice,allfullbackratio,fullbackratio,isfullback\r\n                      from " . tablename('rr_v_goods_option') . ' where id = ' . $g['optionid'] . ' and uniacid = ' . $uniacid . ' ');
		...
		if ((0 < $order['sendtype']) && (1 <= $order['status'])) {
			$order_goods = pdo_fetchall('select orderid,goodsid,sendtype,expresscom,expresssn,express,sendtime from ' . tablename('rr_v_order_goods') . "\r\n            where orderid = " . $orderid . ' and uniacid = ' . $uniacid . ' and sendtype > 0 group by sendtype order by sendtime asc ');
			$expresslist = m('util')->getExpressList($order['express'], $order['expresssn']);
	...
	public function express()
	{
		...
		if ((0 < $order['sendtype']) && ($sendtype == 0)) {
			...
			while ($i <= intval($order['sendtype'])) {
				...
				$bundlelist[$i]['goods'] = pdo_fetchall("select g.title,g.thumb,og.total,og.optionname as optiontitle,og.expresssn,og.express,\r\n                    og.sendtype,og.expresscom,og.sendtime from " . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid ' . ' where og.orderid=:orderid and og.sendtype = ' . $i . ' and og.uniacid=:uniacid ', array(':uniacid' => $uniacid, ':orderid' => $orderid));
