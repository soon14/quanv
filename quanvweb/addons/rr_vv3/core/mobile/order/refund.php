...
class Refund_RrvV3Page extends MobileLoginPage
{
	protected function globalData()
	{
		...
		if ($order['status'] == 2) {
			$goods = pdo_fetchall('select og.goodsid, og.price, og.total, og.optionname, g.cannotrefund, g.thumb, g.title,g.isfullback from' . tablename('rr_v_order_goods') . ' og left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid where og.orderid=' . $order['id']);
		...
		$fullback_log = pdo_fetch('select * from ' . tablename('rr_v_fullback_log') . ' where orderid = ' . $orderid . ' and uniacid = ' . $uniacid . ' ');

		if ($fullback_log) {
			$fullbackgoods = pdo_fetch('select refund from ' . tablename('rr_v_fullback_goods') . ' where goodsid = ' . $fullback_log['goodsid'] . ' and uniacid = ' . $uniacid . ' ');
