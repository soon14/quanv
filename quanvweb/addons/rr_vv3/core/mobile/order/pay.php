...
class Pay_RrvV3Page extends MobileLoginPage
{
	public function main()
	{
		...
		if (!empty($ispeerpay)) {
			if (pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('rr_v_order_peerpay_payinfo') . ' WHERE openid = :openid AND pid = :pid', array(':openid' => $_W['openid'], ':pid' => $ispeerpay['id']))) {
				$this->message('每人只能代付一次');
			}

			$peerpayMessage = trim($_GPC['peerpaymessage']);
			$peerpay_info = (double) pdo_fetchcolumn('select SUM(price) from ' . tablename('rr_v_order_peerpay_payinfo') . ' where pid=:pid limit 1', array(':pid' => $ispeerpay['id']));
			...
			if (empty($peerprice) || ($peerprice <= 0)) {
				...
				$openid = pdo_fetchcolumn('SELECT openid FROM ' . tablename('rr_v_order') . ' WHERE id = :id AND uniacid = :uniacid LIMIT 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
			}
		}
		...
		$seckill_goods = pdo_fetchall('select goodsid,optionid,seckill from  ' . tablename('rr_v_order_goods') . ' where orderid=:orderid and uniacid=:uniacid and seckill=1 ', array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
		if (!empty($log) && ($log['status'] == '0')) {
		...
		if (empty($log)) {
			$log = array('uniacid' => $uniacid, 'openid' => $member['uid'], 'module' => 'rr_vv3', 'tid' => $order['ordersn'], 'fee' => $order['price'], 'status' => 0);
	...
	public function orderstatus()
	{
		...
		$uniacid = $_W['uniacid'];
		$orderid = intval($_GPC['id']);
		$order = pdo_fetch('select status from ' . tablename('rr_v_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid));
	...
	public function complete()
	{
		...
		if (!empty($ispeerpay)) {
			...
			$openid = pdo_fetchcolumn('select openid from ' . tablename('rr_v_order') . ' where id=:orderid and uniacid=:uniacid limit 1', array(':orderid' => $orderid, ':uniacid' => $uniacid));
		...
		if (is_h5app() && empty($orderid)) {
			...
			$orderid = pdo_fetchcolumn('select id from ' . tablename('rr_v_order') . ' where ordersn=:ordersn and uniacid=:uniacid and openid=:openid limit 1', array(':ordersn' => $ordersn, ':uniacid' => $uniacid, ':openid' => $openid));
		}
		...
		if (0 < $order['ispackage']) {
			$package = pdo_fetch('SELECT * FROM ' . tablename('rr_v_package') . ' WHERE uniacid = ' . $uniacid . ' and id = ' . $order['packageid'] . ' ');
		...
		if ((empty($order['isnewstore']) || empty($order['storeid'])) && empty($order['istrade'])) {
			...
			$sql = 'select og.id,g.title, og.goodsid,og.optionid,ng.stotal as stock,og.total as buycount,ng.gstatus as status,ng.deleted,g.maxbuy,g.usermaxbuy,g.istime,g.timestart,g.timeend,g.buylevels,g.buygroups,g.totalcnf,og.seckill,og.storeid ' . ' from ' . tablename('rr_v_order_goods') . ' og left join  ' . tablename('rr_v_goods') . ' g  on g.id=og.goodsid and g.uniacid=og.uniacid' . ' inner join ' . tablename('rr_v_newstore_goods') . ' ng on g.id=ng.goodsid   ' . ' where og.orderid=:orderid and og.uniacid=:uniacid and ng.storeid=:storeid';
			...
			foreach ($order_goods as $data) {
				...
				if ($data['totalcnf'] == 1) {
					if (!empty($data['optionid'])) {
						$option = pdo_fetch('select id,marketprice,stock from ' . tablename('rr_v_newstore_goods_option') . ' where optionid=:optionid and goodsid=:goodsid and uniacid=:uniacid and storeid=:storeid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $data['goodsid'], ':optionid' => $data['optionid'], ':storeid' => $data['storeid']));
		...
		if ($type == 'credit') {
			...
			if ($type == 'wechat') {
				...
				if (!is_error($payquery) || !is_error($payquery_jie) || !empty($ispeerpay)) {
					...
					if (is_h5app()) {
						pdo_update('rr_v_order', array('apppay' => 1), array('id' => $order['id']));
					}
	...
	public function success()
	{
		...
		if (!isset($_SESSION[rr_vv3_PREFIX . '_order_pay_complete'])) {
			if (empty($order['istrade'])) {
				header('location: ' . mobileUrl('order'));
			}
			else {
		...
		unset($_SESSION[rr_vv3_PREFIX . '_order_pay_complete']);
		...
		if (!empty($ispeerpay)) {
			...
			$peerprice = pdo_fetch('SELECT `price` FROM ' . tablename('rr_v_order_peerpay_payinfo') . ' WHERE uid = :uid ORDER BY id DESC LIMIT 1', array(':uid' => $peerpayuid['id']));
			...
			$goods = pdo_fetchall('select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,g.storeids from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid ' . ' where og.orderid=:orderid and og.uniacid=:uniacid ', array(':uniacid' => $uniacid, ':orderid' => $orderid));
			$address = false;
			...
			if (!empty($order['addressid'])) {
				...
				if (!is_array($address)) {
					$address = pdo_fetch('select * from  ' . tablename('rr_v_member_address') . ' where id=:id limit 1', array(':id' => $order['addressid']));
				}
			}
			...
			if (!empty($order['storeid'])) {
				if (0 < $merchid) {
					$store = pdo_fetch('select * from  ' . tablename('rr_v_merch_store') . ' where id=:id limit 1', array(':id' => $order['storeid']));
				}
				else {
					$store = pdo_fetch('select * from  ' . tablename('rr_v_store') . ' where id=:id limit 1', array(':id' => $order['storeid']));
				}
			}
			...
			if ($order['isverify']) {
				...
				if (p('newstore') && !empty($order['isnewstore'])) {
					$stores = pdo_fetchall('select * from ' . tablename('rr_v_store') . ' where id = :id  and uniacid=:uniacid and status=1  order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':id' => $order['storeid']));
				}
		...
		if (!empty($order['virtual']) && !empty($order['virtual_str'])) {
			...
			$virtualtemp = pdo_fetch('SELECT linktext, linkurl FROM ' . tablename('rr_v_virtual_type') . ' WHERE id=:id AND uniacid=:uniacid LIMIT 1', array(':id' => $order['virtual'], ':uniacid' => $_W['uniacid']));
		}
	...
	public function peerpay()
	{
		...
		$peerpay = (int) pdo_fetchcolumn('select orderid from ' . tablename('rr_v_order_peerpay') . ' where orderid=:id and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid));
		...
		$order = pdo_fetch('select * from ' . tablename('rr_v_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid));
		...
		if ($_W['ispost']) {
			...
			$res = pdo_insert('rr_v_order_peerpay', $data);
		...
		$ordergoods = pdo_fetch('select * from ' . tablename('rr_v_order_goods') . ' where orderid=:id and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid));
		$goods = pdo_fetch('select * from ' . tablename('rr_v_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $ordergoods['goodsid'], ':uniacid' => $uniacid));
		$address = iunserializer($order['address']);
		$member = m('member')->getMember($openid, true);
	...
	public function peerpayshare()
	{
		...
		$uniacid = $_W['uniacid'];
		$peerpay = pdo_fetch('select p.*,o.openid from ' . tablename('rr_v_order_peerpay') . ' p join ' . tablename('rr_v_order') . ' o on o.id=p.orderid where p.orderid=:id and p.uniacid=:uniacid limit 1', array(':id' => $peerid, ':uniacid' => $uniacid));
		...
		$peerpay_info = (double) pdo_fetchcolumn('select SUM(price) price from ' . tablename('rr_v_order_peerpay_payinfo') . ' where pid=:pid limit 1', array(':pid' => $peerpay['id']));
		...
		$ordergoods = pdo_fetch('select * from ' . tablename('rr_v_order_goods') . ' where orderid=:id and uniacid=:uniacid limit 1', array(':id' => $peerpay['orderid'], ':uniacid' => $uniacid));
		$goods = pdo_fetch('select * from ' . tablename('rr_v_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $ordergoods['goodsid'], ':uniacid' => $uniacid));
	...
	public function peerpaydetail()
	{
		...
		$peerid = intval($_GPC['id']);
		$uniacid = $_W['uniacid'];
		$peerpay = pdo_fetch('select p.*,o.openid,o.address,o.id as oid from ' . tablename('rr_v_order_peerpay') . ' p join ' . tablename('rr_v_order') . ' o on o.id=p.orderid where p.orderid=:id and p.uniacid=:uniacid limit 1', array(':id' => $peerid, ':uniacid' => $uniacid));
		...
		if ($peerpay['peerpay_type'] == 0) {
			...
			$message = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_order_peerpay_payinfo') . ' WHERE pid = :pid ORDER BY id DESC LIMIT 3', array(':pid' => $peerpay['id']));
