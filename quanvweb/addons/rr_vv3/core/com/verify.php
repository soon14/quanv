...
class Verify_RrvV3ComModel extends ComModel
{
	...
	public function allow($orderid, $times = 0, $verifycode = '', $openid = '')
	{
		...
		if (empty($merchid)) {
			$saler = pdo_fetch('select * from ' . tablename('rr_v_saler') . ' where openid=:openid and uniacid=:uniacid and status=1 limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
		}
		else {
			if ($merch_plugin) {
				$saler = pdo_fetch('select * from ' . tablename('rr_v_merch_saler') . ' where openid=:openid and uniacid=:uniacid and status=1 and merchid=:merchid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid, ':merchid' => $merchid));
			}
		}
		...
		$allgoods = pdo_fetchall('select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,o.title as optiontitle,g.isverify,g.storeids,g.status' . $sqlstr . ' from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid ' . ' left join ' . tablename('rr_v_goods_option') . ' o on o.id=og.optionid ' . ' where og.orderid=:orderid and og.uniacid=:uniacid ', array(':uniacid' => $uniacid, ':orderid' => $orderid));
	...
	public function verify($orderid = 0, $times = 0, $verifycode = '', $openid = '')
	{
		...
		if ($order['isverify'] || $order['istrade']) {
			if (($order['verifytype'] == 0) || ($order['verifytype'] == 3)) {
				...
				pdo_update('rr_v_order', $change_data, array('id' => $order['id']));
				if ($newstore_plugin && !empty($order['istrade'])) {
					...
					pdo_insert('rr_v_newstore_trade_log', $insert_data);
				}
				...
				pdo_update('rr_v_order', array('verifyinfo' => iserializer($verifyinfo)), array('id' => $orderid));
				com_run('printer::sendOrderMessage', $orderid, array('type' => 1, 'times' => $times, 'lastverifys' => $data['lastverifys'] - $times));

				if ($order['status'] != 3) {
					pdo_update('rr_v_order', array('status' => 3, 'sendtime' => $current_time, 'finishtime' => $current_time), array('id' => $order['id']));
					$this->finish($openid, $order);
					m('order')->setGiveBalance($orderid, 1);
				}
				...
				if ($order['verifytype'] == 2) {
					...
					pdo_update('rr_v_order', array('verifyinfo' => iserializer($verifyinfo)), array('id' => $order['id']));

					if ($order['status'] != 3) {
						pdo_update('rr_v_order', array('status' => 3, 'sendtime' => $current_time, 'finishtime' => $current_time, 'verifytime' => $current_time, 'verified' => 1, 'verifyopenid' => $openid, 'verifystoreid' => $saler['storeid']), array('id' => $order['id']));
						$this->finish($openid, $order);
						m('order')->setGiveBalance($orderid, 1);
					}
			...
			if ($order['dispatchtype'] == 1) {
				pdo_update('rr_v_order', array('status' => 3, 'fetchtime' => $current_time, 'sendtime' => $current_time, 'finishtime' => $current_time, 'verifytime' => $current_time, 'verified' => 1, 'verifyopenid' => $openid, 'verifystoreid' => $saler['storeid']), array('id' => $order['id']));
				$this->finish($openid, $order);
				m('order')->setGiveBalance($orderid, 1);
				com_run('printer::sendOrderMessage', $orderid, array('type' => 0));
				m('member')->upgradeLevel($order['openid'], $orderid);
	...
	public function getSalerInfo($openid, $merchid = 0)
	{
		...
		if (empty($merchid)) {
			$table_name = tablename('rr_v_saler');
		}
		else {
			$table_name = tablename('rr_v_merch_saler');
			$condition .= ' and s.merchid = :merchid';
			$params['merchid'] = $merchid;
		}

		$sql = 'SELECT m.id as salerid,m.nickname as salernickname,s.salername FROM ' . $table_name . '  s ' . ' left join ' . tablename('rr_v_member') . ' m on s.openid=m.openid and m.uniacid = s.uniacid ' . ' WHERE ' . $condition . ' Limit 1';
	...
	public function getStoreInfo($storeid, $merchid = 0)
	{
		...
		if (empty($merchid)) {
			$table_name = tablename('rr_v_store');
		}
		else {
			$table_name = tablename('rr_v_merch_store');
