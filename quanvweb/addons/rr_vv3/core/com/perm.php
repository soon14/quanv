...
class Perm_RrvV3ComModel extends ComModel
{
	...
	protected function perm_store()
	{
		$perm = array(
			...
			'verifylog'   => array('text' => '记次时商品统计', 'main' => '查看列表')
			);
	...
	'category' => array('text' => '幻灯片管理', 'main' => '查看列表', 'view' => '查看内容', 'add' => '添加-log', 'edit' => '修改-log', 'delete' => '删除-log'),
	...
	public function is_perm_plugin($pluginname = '', $iscom = false)
	{
		...
		$perm_plugin = pdo_fetch('SELECT * FROM ' . tablename('rr_v_perm_plugin') . ' WHERE acid=:uniacid limit 1', array(':uniacid' => $acid));
	...
	private function check($permtype = '')
	{
		...
		$user = pdo_fetch('select u.status as userstatus,r.status as rolestatus,u.perms2 as userperms,r.perms2 as roleperms,u.roleid from ' . tablename('rr_v_perm_user') . ' u ' . ' left join ' . tablename('rr_v_perm_role') . ' r on u.roleid = r.id ' . ' where u.uid=:uid limit 1 ', array(':uid' => $uid));
	...
	public function check_plugin($pluginname = '')
	{
		...
		$allow = true;
		$acid = pdo_fetchcolumn('SELECT acid FROM ' . tablename('account_wechats') . ' WHERE `uniacid`=:uniacid LIMIT 1', array(':uniacid' => $_W['uniacid']));
		$ac_perm = pdo_fetch('select  plugins from ' . tablename('rr_v_perm_plugin') . ' where acid=:acid limit 1', array(':acid' => $acid));
	...
	public function check_com($comname = '')
	{
		...
		$allow = true;
		$acid = pdo_fetchcolumn('SELECT acid FROM ' . tablename('account_wechats') . ' WHERE `uniacid`=:uniacid LIMIT 1', array(':uniacid' => $_W['uniacid']));
		$ac_perm = pdo_fetch('select  coms from ' . tablename('rr_v_perm_plugin') . ' where acid=:acid limit 1', array(':acid' => $acid));
