...
class Sms_RrvV3ComModel extends ComModel
{
	/**
     * 发送短信
	...
	public function send($mobile, $tplid, $data, $replace = true)
	{
		...
		if ($template['type'] == 'juhe') {
			...
			include_once rr_vv3_VENDOR . 'dayu/TopSdk.php';
			...
			include_once rr_vv3_VENDOR . 'emay/SMSUtil.php';
	...
	{
		global $_W;
		return pdo_fetch('SELECT * FROM ' . tablename('rr_v_sms_set') . ' WHERE uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
	}

	public function sms_temp()
	{
		global $_W;
		$list = pdo_fetchall('SELECT id, `type`, `name` FROM ' . tablename('rr_v_sms') . ' WHERE status=1 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
	...
	public function sms_num($type, $smsset = NULL)
	{
		...
		if ($type == 'emay') {
			...
			if (!empty($smsset['emay_warn']) && !empty($smsset['emay_mobile']) && ($num < $smsset['emay_warn']) && (($smsset['emay_warn_time'] + (60 * 60 * 24)) < time())) {
				...
				if (empty($emayResult)) {
					pdo_update('rr_v_sms_set', array('emay_warn_time' => time()), array('id' => $smsset['id']));
				}
			}
	...
	{
		global $_W;
		$template = pdo_fetch('SELECT * FROM ' . tablename('rr_v_sms') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $tplid, ':uniacid' => $_W['uniacid']));
