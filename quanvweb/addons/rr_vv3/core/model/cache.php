		...
		if (function_exists('redis')) {
			...
			if (!is_error($redis)) {
				if (stripos($uniacid, 'global') !== false) {
					return 'rr_vv3_syscache_' . $_W['setting']['site']['key'] . '_global_' . $key;
				}
				...
				return 'rr_vv3_syscache_' . $_W['setting']['site']['key'] . '_' . $uniacid . '_' . $account_key . '_' . $key;
			}
		}

		return rr_vv3_PREFIX . md5($uniacid . '_new_' . $key);
	}
