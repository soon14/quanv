<?php
class Order_RrvV3Model
{
	public function fullback($orderid)
	{
		global $_W;
		$uniacid = $_W['uniacid'];
		$order_goods = pdo_fetchall('select o.openid,og.optionid,og.goodsid,og.price,og.total from ' . tablename('rr_v_order_goods') . " as og\r\n                    left join " . tablename('rr_v_order') . " as o on og.orderid = o.id\r\n                    where og.uniacid = " . $uniacid . ' and og.orderid = ' . $orderid . ' ');

		foreach ($order_goods as $key => $value) {
			if (0 < $value['optionid']) {
				$goods = pdo_fetch('select g.hasoption,g.id,go.goodsid,go.isfullback from ' . tablename('rr_v_goods') . " as g\r\n                left join " . tablename('rr_v_goods_option') . ' as go on go.goodsid = :id and go.id = ' . $value['optionid'] . "\r\n                 where g.id=:id and g.uniacid=:uniacid limit 1", array(':id' => $value['goodsid'], ':uniacid' => $uniacid));
			}
			else {
				$goods = pdo_fetch('select * from ' . tablename('rr_v_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $value['goodsid'], ':uniacid' => $uniacid));
			}

			if (0 < $goods['isfullback']) {
				$fullbackgoods = pdo_fetch("SELECT id,minallfullbackallprice,maxallfullbackallprice,minallfullbackallratio,maxallfullbackallratio,`day`,\r\n                          fullbackprice,fullbackratio,status,hasoption,marketprice,`type`,startday\r\n                          FROM " . tablename('rr_v_fullback_goods') . ' WHERE uniacid = ' . $uniacid . ' and goodsid = ' . $value['goodsid'] . ' limit 1');
				if (!empty($fullbackgoods) && $goods['hasoption'] && (0 < $value['optionid'])) {
					$option = pdo_fetch('select id,title,allfullbackprice,allfullbackratio,fullbackprice,fullbackratio,`day` from ' . tablename('rr_v_goods_option') . " \r\n                        where id=:id and goodsid=:goodsid and uniacid=:uniacid and isfullback = 1 limit 1", array(':uniacid' => $uniacid, ':goodsid' => $value['goodsid'], ':id' => $value['optionid']));
				...
				if (!empty($fullbackgoods)) {
					...
					pdo_insert('rr_v_fullback_log', $data);
				}
			}
		}
	}
	...
	public function fullbackstop($orderid)
	{
		...
		$uniacid = $_W['uniacid'];
		$shopset = $_S['shop'];
		$fullback_log = pdo_fetch('select * from ' . tablename('rr_v_fullback_log') . ' where uniacid = ' . $uniacid . ' and orderid = ' . $orderid . ' ');
		pdo_update('rr_v_fullback_log', array('isfullback' => 1), array('id' => $fullback_log['id'], 'uniacid' => $uniacid));
	}
	...
	public function payResult($params)
	{
		...
		$order = pdo_fetch('select id,ordersn, price,openid,dispatchtype,addressid,carrier,status,isverify,deductcredit2,`virtual`,isvirtual,couponid,isvirtualsend,isparent,paytype,merchid,agentid,createtime,buyagainprice,istrade,tradestatus from ' . tablename('rr_v_order') . ' where  ordersn=:ordersn and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':ordersn' => $ordersn));
		$orderid = $order['id'];
		$ispeerpay = $this->checkpeerpay($orderid);

		if (!empty($ispeerpay)) {
			$peerpay_info = (double) pdo_fetchcolumn('select SUM(price) price from ' . tablename('rr_v_order_peerpay_payinfo') . ' where pid=:pid limit 1', array(':pid' => $ispeerpay['id']));

			if ($peerpay_info < $ispeerpay['peerpay_realprice']) {
				return NULL;
			}

			pdo_update('rr_v_order', array('status' => 0), array('id' => $order['id']));
			$order['status'] = 0;
			pdo_update('rr_v_order_peerpay', array('status' => 1), array('id' => $ispeerpay['id']));
			$params['type'] = 'peerpay';
		}
		...
		if ($params['from'] == 'return') {
			...
			if (empty($order['dispatchtype'])) {
				$address = pdo_fetch('select realname,mobile,address from ' . tablename('rr_v_member_address') . ' where id=:id limit 1', array(':id' => $order['addressid']));
			}
			...
			if ($params['type'] == 'cash') {
				if ($order['isparent'] == 1) {
					...
					pdo_update('rr_v_order', $change_data, array('id' => $orderid));
			...
			if ($order['istrade'] == 0) {
				if ($order['status'] == 0) {
					...
					if (p('task')) {
						...
						$goodslist = pdo_fetchall('SELECT goodsid FROM ' . tablename('rr_v_order_goods') . ' WHERE orderid = :orderid AND uniacid = :uniacid', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
	...
	public function setChildOrderPayResult($order, $time, $type)
	{
		...
		if (!empty($list)) {
			...
			foreach ($list as $k => $v) {
				if ($v['status'] == 0) {
					pdo_update('rr_v_order', $change_data, array('id' => $v['id']));
				}
			}
		}
	}
	...
	public function setOrderPayType($orderid, $paytype, $ordersn = '')
	{
		...
		if (!empty($orderid)) {
			pdo_update('rr_v_order', array('paytype' => $paytype), array('parentid' => $orderid));
		}
	}
	...
	{
		global $_W;
		$list = pdo_fetchall('select id,ordersn,status,finishtime,couponid,merchid  from ' . tablename('rr_v_order') . ' where  parentid=:parentid and uniacid=:uniacid', array(':parentid' => $orderid, ':uniacid' => $_W['uniacid']));
		return $list;
	}
	...
	public function payVirtualSend($orderid = 0)
	{
		global $_W;
		global $_GPC;
		$order = pdo_fetch('select id,ordersn, price,openid,dispatchtype,addressid,carrier,status,isverify,deductcredit2,`virtual`,isvirtual,couponid from ' . tablename('rr_v_order') . ' where  id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $orderid));
		$order_goods = pdo_fetch('select g.virtualsend,g.virtualsendcontent from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid ' . ' where og.orderid=:orderid and og.uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
		$time = time();
		pdo_update('rr_v_order', array('virtualsend_info' => $order_goods['virtualsendcontent'], 'status' => '3', 'paytime' => $time, 'sendtime' => $time, 'finishtime' => $time), array('id' => $orderid));
	...
	{
		global $_W;
		$order = pdo_fetch('select id,ordersn,price,openid,dispatchtype,addressid,carrier,status from ' . tablename('rr_v_order') . ' where id=:id limit 1', array(':id' => $orderid));
		$goods = pdo_fetchall('select og.goodsid,og.total,g.totalcnf,og.realprice,g.money,og.optionid,g.total as goodstotal,og.optionid,g.sales,g.salesreal from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid ' . ' where og.orderid=:orderid and og.uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
		$balance = 0;
	...
	{
		global $_W;
		$order = pdo_fetch('select id,ordersn,price,openid,dispatchtype,addressid,carrier,status,isparent,paytype,isnewstore,storeid,istrade from ' . tablename('rr_v_order') . ' where id=:id limit 1', array(':id' => $orderid));
		...
		foreach ($goods as $g) {
			if (0 < $newstoreid) {
				...
				$goods_item = pdo_fetch('select total as goodstotal from' . tablename('rr_v_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $g['goodsid'], ':uniacid' => $_W['uniacid']));
			...
			if (!empty($stocktype)) {
				...
				if (!empty($g['optionid'])) {
					...
					if (!empty($option) && ($option['stock'] != -1)) {
						...
						if ($stock != -1) {
							if (0 < $newstoreid) {
								pdo_update('rr_v_newstore_goods_option', array('stock' => $stock), array('uniacid' => $_W['uniacid'], 'goodsid' => $g['goodsid'], 'id' => $store_goods_option['id']));
							}
							else {
				...
				if (!empty($g['goodstotal']) && ($g['goodstotal'] != -1)) {
					...
					if ($totalstock != -1) {
						if (0 < $newstoreid) {
							pdo_update('rr_v_newstore_goods', array('stotal' => $totalstock), array('uniacid' => $_W['uniacid'], 'id' => $store_goods['id']));
						}
						else {
	...
	public function getTotals($merch = 0)
	{
		...
		$totals['all'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('rr_v_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0 and deleted=0', $paras);
		$totals['status_1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('rr_v_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0 and status=-1 and refundtime=0 and deleted=0', $paras);
		$totals['status0'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('rr_v_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and status=0 and paytype<>3 and deleted=0', $paras);
		$totals['status1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('rr_v_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and ( status=1 or ( status=0 and paytype=3) ) and deleted=0', $paras);
		$totals['status2'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('rr_v_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and ( status=2 or (status = 1 and sendtype > 0) ) and deleted=0', $paras);
		$totals['status3'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('rr_v_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and status=3 and deleted=0', $paras);
		$totals['status4'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('rr_v_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and refundstate>0 and refundid<>0 and deleted=0', $paras);
		$totals['status5'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('rr_v_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0 and refundtime<>0 and deleted=0', $paras);
		return $totals;
	}
	...
	public function getGoodsDiscountPrice($g, $level, $type = 0)
	{
		...
		if (!empty($level['id'])) {
			$level = pdo_fetch('select * from ' . tablename('rr_v_member_level') . ' where id=:id and uniacid=:uniacid and enabled=1 limit 1', array(':id' => $level['id'], ':uniacid' => $_W['uniacid']));
	...
	public function changeParentOrderPrice($parent_order)
	{
		global $_W;
		$id = $parent_order['id'];
		$item = pdo_fetch('SELECT price,ordersn2,dispatchprice,changedispatchprice FROM ' . tablename('rr_v_order') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if (!empty($item)) {
			...
			if (!empty($orderupdate)) {
				pdo_update('rr_v_order', $orderupdate, array('id' => $id, 'uniacid' => $_W['uniacid']));
			}
		}
	}
	...
	public function getOrderCommission($orderid, $agentid = 0)
	{
		...
		if (empty($agentid)) {
			$item = pdo_fetch('select agentid from ' . tablename('rr_v_order') . ' where id=:id and uniacid=:uniacid Limit 1', array('id' => $orderid, ':uniacid' => $_W['uniacid']));
	...
	public function checkOrderGoods($orderid)
	{
		...
		$item = pdo_fetch('select * from ' . tablename('rr_v_order') . '  where  id = :id and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid));
		if ((empty($order['isnewstore']) || empty($order['storeid'])) && empty($order['istrade'])) {
			$order_goods = pdo_fetchall('select og.id,g.title, og.goodsid,og.optionid,g.total as stock,og.total as buycount,g.status,g.deleted,g.maxbuy,g.usermaxbuy,g.istime,g.timestart,g.timeend,g.buylevels,g.buygroups,g.totalcnf,og.seckill from  ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_goods') . ' g on og.goodsid = g.id ' . ' where og.orderid=:orderid and og.uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
				...
				if (($seckillinfo && ($seckillinfo['status'] == 0)) || !empty($ispeerpay)) {
					...
					if ($data['totalcnf'] == 1) {
						if (!empty($data['optionid'])) {
							$option = pdo_fetch('select id,title,marketprice,goodssn,productsn,stock,`virtual` from ' . tablename('rr_v_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $data['goodsid'], ':id' => $data['optionid']));
			...
			$sql = 'select g.id,g.title,ng.gstatus,g.deleted' . ' from ' . tablename('rr_v_order_goods') . ' og left join  ' . tablename('rr_v_goods') . ' g  on g.id=og.goodsid and g.uniacid=og.uniacid' . ' inner join ' . tablename('rr_v_newstore_goods') . ' ng on ng.goodsid = g.id AND ng.storeid=' . $item['storeid'] . ' where og.orderid=:orderid and og.uniacid=:uniacid';
	...
	{
		global $_W;
		$sql = 'SELECT p.*,o.openid FROM ' . tablename('rr_v_order_peerpay') . ' AS p JOIN ' . tablename('rr_v_order') . ' AS o ON p.orderid = o.id WHERE p.orderid = :orderid AND p.uniacid = :uniacid AND (p.status = 0 OR p.status=1) AND o.status >= 0 LIMIT 1';
	...
	public function peerStatus($param)
	{
		...
		if (!empty($param['tid'])) {
			$sql = 'SELECT id FROM ' . tablename('rr_v_order_peerpay_payinfo') . ' WHERE tid = :tid';
		...
		return pdo_insert('rr_v_order_peerpay_payinfo', $param);
	}

	public function getVerifyCardNumByOrderid($orderid)
	{
		global $_W;
		$num = pdo_fetchcolumn('select SUM(og.total)  from ' . tablename('rr_v_order_goods') . " og\r\n\t\t inner join " . tablename('rr_v_goods') . " g on og.goodsid = g.id\r\n\t\t where og.uniacid=:uniacid  and og.orderid =:orderid and g.cardid>0", array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
		return $num;
	}
	...
	public function checkisonlyverifygoods($orderid)
	{
		...
		$num2 = pdo_fetchcolumn('select COUNT(1)  from ' . tablename('rr_v_order_goods') . " og\r\n             inner join " . tablename('rr_v_goods') . " g on og.goodsid = g.id\r\n             where og.uniacid=:uniacid  and og.orderid =:orderid and g.type=5", array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
		$num2 = intval($num2);
	...
	{
		global $_W;
		$num = pdo_fetchcolumn('select COUNT(1)  from ' . tablename('rr_v_verifygoods_log') . " vl\r\n\t\t inner join " . tablename('rr_v_verifygoods') . " v on vl.verifygoodsid = v.id\r\n\t\t where v.uniacid=:uniacid  and v.orderid =:orderid ", array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
		$num = intval($num);
	...
	public function getOrderVirtual($order = array())
	{
		...
		$ordervirtual = array();
		$virtual_type = pdo_fetch('select fields from ' . tablename('rr_v_virtual_type') . ' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1 ', array(':id' => $order['virtual'], ':uniacid' => $_W['uniacid'], ':merchid' => $order['merchid']));
