...
class Member_RrvV3Model
{
	...
	public function getInfo($openid = '')
	{
		...
		if ($uid == 0) {
			$info = pdo_fetch('select * from ' . tablename('rr_v_member') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			...
			if (empty($info)) {
				...
				if (!empty($condition)) {
					$info = pdo_fetch('select * from ' . tablename('rr_v_member') . ' where ' . $condition . '  and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			...
			$info = pdo_fetch('select * from ' . tablename('rr_v_member') . ' where id=:id  and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $uid));
		}
	...
	public function getMember($openid = '')
	{
		...
		if ($uid == 0) {
			...
			$info = pdo_fetch('select * from ' . tablename('rr_v_member') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $openid));
		}
		...
		if (empty($info['uid'])) {
			...
			if ($followed) {
				...
				if (!empty($uid)) {
					...
					if (!empty($upgrade)) {
						pdo_update('rr_v_member', $upgrade, array('id' => $info['id']));
					}
				}
			}
		}
	...
	{
		global $_W;
		$info = pdo_fetch('select * from ' . tablename('rr_v_member') . ' where mobile=:mobile and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':mobile' => $mobile));
	...
	public function setCredit($openid = '', $credittype = 'credit1', $credits = 0, $log = array())
	{
		...
		$log_data = array('uid' => intval($uid), 'credittype' => $credittype, 'uniacid' => $_W['uniacid'], 'num' => $credits, 'createtime' => TIMESTAMP, 'module' => 'rr_vv3', 'operator' => intval($log[0]), 'remark' => $log[1]);
		...
		if (!empty($uid)) {
			...
			$value = pdo_fetchcolumn('SELECT ' . $credittype . ' FROM ' . tablename('rr_v_member') . ' WHERE  uniacid=:uniacid and openid=:openid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			$newcredit = $credits + $value;
			...
			pdo_update('rr_v_member', array($credittype => $newcredit), array('uniacid' => $_W['uniacid'], 'openid' => $openid));
	...
	public function getCredit($openid = '', $credittype = 'credit1')
	{
		...
		return pdo_fetchcolumn('SELECT ' . $credittype . ' FROM ' . tablename('rr_v_member') . ' WHERE (openid=:openid or openid_wa=:openid) and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
	}
	...
	public function getCredits($openid = '', $credittypes = array('credit1', 'credit2'))
	{
		...
		return pdo_fetch('SELECT ' . $types . ' FROM ' . tablename('rr_v_member') . ' WHERE  (openid=:openid or openid_wa=:openid) and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
	}
	...
	public function checkMember()
	{
		...
		if (empty($member) && !(empty($openid))) 
		{
				...
				//curl_file_get_contents 后面增加此函数	curl方法获取远程文件信息
				//

			$member = array(
				'uniacid' => $_W['uniacid'], 'uid' => $uid, 'openid' => $openid, 
				'realname' => (!(empty($mc['realname'])) ? $mc['realname'] : ''), 
				'mobile' => (!(empty($mc['mobile'])) ? $mc['mobile'] : ''), 
				'nickname' => (!(empty($mc['nickname'])) ? $mc['nickname'] : ''), 
				'MD5nickname' => MD5($mc['nickname']),
				'wxnickname' => base64_encode($mc['nickname']),
			...
			$upgrade['nickname'] = $mc['nickname'];
			$upgrade['MD5nickname'] = MD5($mc['nickname']);
			$upgrade['wxnickname'] = base64_encode($mc['nickname']);

			if (!empty($upgrade)) {
				pdo_update('rr_v_member', $upgrade, array('id' => $member['id']));
			}
		}
	...
	public function getLevels($all = true)
	{
		...
		return pdo_fetchall('select * from ' . tablename('rr_v_member_level') . ' where uniacid=:uniacid' . $condition . ' order by level asc', array(':uniacid' => $_W['uniacid']));
	}
	...
	public function getLevel($openid)
	{
		...
		if (!empty($member) && !empty($member['level'])) {
			$level = pdo_fetch('select * from ' . tablename('rr_v_member_level') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $member['level'], ':uniacid' => $_W['uniacid']));
	...
	public function getOneGoodsLevel($openid, $goodsid)
	{
		...
		$data = array();
		$levels = pdo_fetchall('select * from ' . tablename('rr_v_member_level') . ' where uniacid=:uniacid and buygoods=1 and level and level > :level order by level asc', array(':uniacid' => $uniacid, ':level' => $level));
	...
	public function getGoodsLevel($openid, $orderid)
	{
		global $_W;
		$uniacid = $_W['uniacid'];
		$order_goods = pdo_fetchall('select goodsid from ' . tablename('rr_v_order_goods') . ' where orderid=:orderid and uniacid=:uniacid', array(':uniacid' => $uniacid, ':orderid' => $orderid));
		$levels = array();
		$data = array();
	...
	public function upgradeLevel($openid, $orderid = 0)
	{
		...
		if (empty($leveltype)) {
			$ordermoney = pdo_fetchcolumn('select ifnull( sum(og.realprice),0) from ' . tablename('rr_v_order_goods') . ' og ' . ' left join ' . tablename('rr_v_order') . ' o on o.id=og.orderid ' . ' where o.openid=:openid and o.status=3 and o.uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
			$level = pdo_fetch('select * from ' . tablename('rr_v_member_level') . ' where uniacid=:uniacid  and enabled=1 and ' . $ordermoney . ' >= ordermoney and ordermoney>0  order by level desc limit 1', array(':uniacid' => $_W['uniacid']));
		}
		else {
			if ($leveltype == 1) {
				$ordercount = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_order') . ' where openid=:openid and status=3 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
				$level = pdo_fetch('select * from ' . tablename('rr_v_member_level') . ' where uniacid=:uniacid and enabled=1 and ' . $ordercount . ' >= ordercount and ordercount>0  order by level desc limit 1', array(':uniacid' => $_W['uniacid']));
			}
			if($leveltype == 2){
				$followcount = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_member_follow') . ' where doc_openid=:doc_openid and isdelete=0 and isfollow=1 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':doc_openid' => $member['openid']));
				$level = pdo_fetch('select * from ' . tablename('rr_v_member_level') . ' where uniacid=:uniacid and enabled=1 and ' . $followcount . ' >= followcount and followcount>0  order by level desc limit 1', array(':uniacid' => $_W['uniacid']));
			}
		}
		...
		if ($canupgrade) {
			pdo_update('rr_v_member', array('level' => $level['id']), array('id' => $member['id']));
			com_run('wxcard::updateMemberCardByOpenid', $openid);
	...
	public function upgradeLevelByLevelId($openid, $LevelID)
	{
		...
		$level = pdo_fetch('select *  from ' . tablename('rr_v_member_level') . ' where uniacid=:uniacid and enabled=1 and id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $LevelID));
	...
	{
		global $_W;
		return pdo_fetchall('select * from ' . tablename('rr_v_member_group') . ' where uniacid=:uniacid order by id asc', array(':uniacid' => $_W['uniacid']));
	}
	...
	public function checkMemberSNS($sns)
	{
		...
		if (empty($member)) {
			...
			pdo_insert('rr_v_member', $data);
			return pdo_insertid();
		}

		if (empty($member['bindsns']) || ($member['bindsns'] == $sns)) {
			pdo_update('rr_v_member', $data, array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
			return $member['id'];
		}
	}
