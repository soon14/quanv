...
class System_RrvV3Model
{
	...
	public function init()
	{
		...
		if (!$this->merch) {
			$arr['notice'] = pdo_fetchall('SELECT * FROM ' . tablename('rr_v_system_copyright_notice') . ' ORDER BY displayorder DESC,createtime DESC LIMIT 5');

			if ($this->cv('commission.apply.view1')) {
				$arr['commission1'] = pdo_fetchcolumn('select count(1) from' . tablename('rr_v_commission_apply') . ' a ' . ' left join ' . tablename('rr_v_member') . ' m on m.uid = a.mid' . ' left join ' . tablename('rr_v_commission_level') . ' l on l.id = m.agentlevel' . ' where a.uniacid=:uniacid and a.status=:status', array(':uniacid' => $_W['uniacid'], ':status' => 1));
			}

			if ($this->cv('commission.apply.view2')) {
				$arr['commission2'] = pdo_fetchcolumn('select count(1) from' . tablename('rr_v_commission_apply') . ' a ' . ' left join ' . tablename('rr_v_member') . ' m on m.uid = a.mid' . ' left join ' . tablename('rr_v_commission_level') . ' l on l.id = m.agentlevel' . ' where a.uniacid=:uniacid and a.status=:status', array(':uniacid' => $_W['uniacid'], ':status' => 2));
			}

			if ($this->cv('shop.comment')) {
				$arr['comment'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('rr_v_order_comment') . ' WHERE (`checked`=1 OR replychecked=1) AND deleted=0 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
			}
		}
		else {
			$arr['notice'] = 'none';
			...
			if ($this->cv('shop.comment')) {
				$arr['comment'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('rr_v_order_comment') . 'c LEFT JOIN ' . tablename('rr_v_goods') . ' g ON g.id=c.goodsid WHERE (c.checked=1 OR c.replychecked=1) AND c.deleted=0 AND c.uniacid=:uniacid AND g.merchid=:merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
			}
		}

		if (!empty($arr['funbar']['open'])) {
			$funbardata = pdo_fetch('select * from ' . tablename('rr_v_funbar') . ' where uid=:uid and uniacid=:uniacid limit 1', array(':uid' => $_W['uid'], ':uniacid' => $_W['uniacid']));
	...
	protected function getOrderTotal($status = 0)
	{
		...
		if ($status == 1) {
			...
			$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('rr_v_order') . ' WHERE ' . $condition, $params);
		}
		else {
			if ($status == 4) {
	...
	public function set_version($type = 0)
	{
		...
		if (!empty($closev2)) {
			...
			$set = pdo_fetch('SELECT * FROM ' . tablename('rr_v_version') . ' WHERE uid=:uid AND `type`=:type limit 1', array(':type' => $type, ':uid' => $uid));
