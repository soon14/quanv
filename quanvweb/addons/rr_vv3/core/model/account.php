...
class Account_RrvV3Model
{
	...
	public function checkOpenid()
	{
		...
		if (isset($_GPC[$key])) {
			...
			if (is_array($session)) {
				...
				if (is_array($member) && ($session['rr_vv3_member_hash'] == md5($member['pwd'] . $member['salt']))) {
					$GLOBALS['_W']['rr_vv3_member_hash'] = md5($member['pwd'] . $member['salt']);
					$GLOBALS['_W']['rr_vv3_member'] = $member;
	...
	public function setLogin($member)
	{
		...
		if (!empty($member)) {
			$member['rr_vv3_member_hash'] = md5($member['pwd'] . $member['salt']);
			$key = '__rr_vv3_member_session_' . $_W['uniacid'];
