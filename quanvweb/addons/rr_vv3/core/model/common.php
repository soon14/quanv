...
class Common_RrvV3Model
{
	...
	public function updateSysset($values, $uniacid = 0)
	{
		...
		$setdata = pdo_fetch('select id, sets from ' . tablename('rr_v_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));

		if (empty($setdata)) {
			pdo_insert('rr_v_sysset', array('sets' => iserializer($values), 'uniacid' => $uniacid));
		}
		else {
			...
			pdo_update('rr_v_sysset', array('sets' => iserializer($sets)), array('id' => $setdata['id']));
		}

		$setdata = pdo_fetch('select * from ' . tablename('rr_v_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
	...
	public function updatePluginset($values, $uniacid = 0)
	{
		...
		$setdata = pdo_fetch('select id, plugins from ' . tablename('rr_v_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));

		if (empty($setdata)) {
			pdo_insert('rr_v_sysset', array('plugins' => iserializer($values), 'uniacid' => $uniacid));
		}
		else {
			...
			pdo_update('rr_v_sysset', array('plugins' => iserializer($plugins)), array('id' => $setdata['id']));
		}
	...
	public function alipay_build($params, $alipay = array(), $type = 0, $openid = '')
	{
		...
		if (empty($type)) {
			$set['notify_url'] = $_W['siteroot'] . 'addons/rr_vv3/payment/alipay/notify.php';
	...
	public function publicAliPay($params = array(), $return = NULL)
	{
		...
		$url = (rr_vv3_DEBUG ? 'https://openapi.alipaydev.com/gateway.do' : 'https://openapi.alipay.com/gateway.do');
	...
	public function AliPayBarcode($params, $config)
	{
		...
		$config['notify_url'] = $_W['siteroot'] . 'addons/rr_vv3/payment/alipay/notify.php';
	...
	public function public_build($isapp = false)
	{
		...
		if (!empty($set['weixin_id']) && ($isapp == false)) {
			$payments = pdo_fetch('SELECT * FROM ' . tablename('rr_v_payment') . ' WHERE uniacid=:uniacid AND id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $set['weixin_id']));
	...
	public function wechat_jspay($params, $wechat, $type = 0)
	{
		...
		$package['notify_url'] = $_W['siteroot'] . 'addons/rr_vv3/payment/wechat/notify.php';
	...
	public function wechat_child_build($params, $wechat, $type = 0)
	{
		...
		$package['nonce_str'] = random(32);
		$package['body'] = $params['title'];
		$package['device_info'] = isset($params['device_info']) ? 'rr_vv3:' . $params['device_info'] : 'rr_vv3';
		...
		$package['notify_url'] = empty($params['notify_url']) ? $_W['siteroot'] . 'addons/rr_vv3/payment/wechat/notify.php' : $params['notify_url'];
		$package['trade_type'] = 'JSAPI';
	...
	{
		global $_W;
		$payment = pdo_fetch('SELECT * FROM ' . tablename('rr_v_payment') . ' WHERE uniacid=:uniacid AND `type`=\'0\'', array(':uniacid' => $_W['uniacid']));
		...
		$package = array();
		// $package['wxappid'] = $payment['sub_appid'];//公众号appid
		$package['wxappid'] = $_W['shopset']['app']['appid'];//小程序appid
		$package['mch_id'] = $payment['sub_mch_id'];
		$package['mch_billno'] = $params['tid'];
		$package['send_name'] = $params['send_name'];
		$package['nonce_str'] = random(32);
		$package['re_openid'] = str_replace('sns_wa_', '', $params['openid']);
	...
	public function createNO($table, $field, $prefix)
	{
		...
		while (1) {
			$count = pdo_fetchcolumn('select count(*) from ' . tablename('rr_v_' . $table) . ' where ' . $field . '=:billno limit 1', array(':billno' => $billno));
	...
	public function getSec($uniacid = 0)
	{
		...
		$set = pdo_fetch('select sec from ' . tablename('rr_v_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
	...
	public function paylog($log = '')
	{
		...
		if (!empty($openpaylog)) {
			$path = IA_ROOT . '/addons/rr_vv3/data/paylog/' . $_W['uniacid'] . '/' . date('Ymd');
	...
	public function getAreas()
	{
		...
		if (!empty($new_area)) {
			$file = IA_ROOT . '/addons/rr_vv3/static/js/dist/area/AreaNew.xml';
		}
		else {
			$file = IA_ROOT . '/addons/rr_vv3/static/js/dist/area/Area.xml';
		}
	...
	{
		return pdo_fetchall('SELECT  a.uniacid,a.name FROM ' . tablename('rr_v_sysset') . ' s  ' . ' left join ' . tablename('account_wechats') . ' a on a.uniacid = s.uniacid' . ' left join ' . tablename('account') . ' acc on acc.uniacid = a.uniacid where acc.isdeleted=0 GROUP BY uniacid');
	}
	...
	public function getCopyright($ismanage = false)
	{
		...
		if (!is_array($copyrights)) {
			$copyrights = pdo_fetchall('select *  from ' . tablename('rr_v_system_copyright'));
