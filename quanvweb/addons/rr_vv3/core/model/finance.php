...
class Finance_RrvV3Model
{
	...
	public function pay($openid = '', $paytype = 0, $money = 0, $trade_no = '', $desc = '', $return = true)
	{
		...
		$pars = array();
		// $pars['mch_appid'] = $payment['sub_appid'];//公众号appid
		$pars['mch_appid'] = $_W['shopset']['app']['appid'];//小程序appid
		$pars['mchid'] = $payment['sub_mch_id'];
		$pars['nonce_str'] = random(32);
		$pars['partner_trade_no'] = empty($trade_no) ? time() . random(4, true) : $trade_no;
		$pars['openid'] = str_replace('sns_wa_', '', $openid);
	...
	public function wxapp_refund($openid, $out_trade_no, $out_refund_no, $totalmoney, $refundmoney = 0, $app = false, $refund_account = false)
	{
		...
		if (is_array($certs)) {
			...
			$certfile = IA_ROOT . '/addons/rr_vv3/cert/' . random(64);
			file_put_contents($certfile, $certs['cert']);
			$keyfile = IA_ROOT . '/addons/rr_vv3/cert/' . random(64);
			file_put_contents($keyfile, $certs['key']);
			$rootfile = IA_ROOT . '/addons/rr_vv3/cert/' . random(64);
			file_put_contents($rootfile, $certs['root']);
			$extras['CURLOPT_SSLCERT'] = $certfile;
			$extras['CURLOPT_SSLKEY'] = $keyfile;
			$extras['CURLOPT_CAINFO'] = $rootfile;
	...
	public function AlipayRefund($params, $batch_no, $config)
	{
		...
		$parameter = array('service' => 'refund_fastpay_by_platform_pwd', 'partner' => $config['partner'], '_input_charset' => 'UTF-8', 'notify_url' => isset($params['notify_url']) ? $params['notify_url'] : $_W['siteroot'] . 'addons/rr_vv3/payment/alipay/notify.php', 'seller_user_id' => $config['partner'], 'seller_email' => $config['account'], 'refund_date' => date('Y-m-d H:i:s'), 'batch_no' => $batch_no, 'batch_num' => '1', 'detail_data' => $params['trade_no'] . '^' . $params['refund_price'] . '^' . $params['refund_reason']);
	...
	public function downloadbill($starttime, $endtime, $type = 'ALL', $datatype = 0)
	{
		...
		foreach ($dates as $date) {
			...
			if ($datatype && !strexists($dc, 'rr_vv3')) {
				continue;
			}
