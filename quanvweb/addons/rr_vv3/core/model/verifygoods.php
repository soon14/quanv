...
class Verifygoods_RrvV3Model
{
	...
	{
		global $_W;
		$verifygoods = pdo_fetchall('select  *   from  ' . tablename('rr_v_verifygoods') . ' where  orderid=:orderid ', array(':orderid' => $orderid));
		...
		$ordergoods = pdo_fetchall('select o.openid,o.uniacid,o.id as orderid , og.id as ordergoodsid,g.verifygoodsdays,g.verifygoodsnum,g.verifygoodslimittype,g.verifygoodslimitdate,og.total ' . $sql2 . ' from ' . tablename('rr_v_order_goods') . '   og inner join ' . tablename('rr_v_goods') . " g on og.goodsid = g.id\r\n          inner join " . tablename('rr_v_order') . " o on og.orderid = o.id\r\n          where   og.orderid =:orderid and g.type = 5", array(':orderid' => $orderid));
		$time = time();
		...
		foreach ($ordergoods as $ordergood) {
			...
			while ($i < $total) {
				...
				pdo_insert('rr_v_verifygoods', $data);
				++$i;
			}
		}
