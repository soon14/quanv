<?php
class Route_RrvV3Model
{
	public function run($isweb = true)
	{
		global $_GPC;
		global $_W;
		require_once IA_ROOT . '/addons/rr_vv3/core/inc/page.php';

		if ($isweb) {
			require_once rr_vv3_CORE . 'inc/page_web.php';
			require_once rr_vv3_CORE . 'inc/page_web_com.php';
		}
		else {
			require_once rr_vv3_CORE . 'inc/page_mobile.php';
			require_once rr_vv3_CORE . 'inc/page_mobile_login.php';
		}
		...
		$segs = count($routes);
		$method = 'main';
		$root = ($isweb ? rr_vv3_CORE_WEB : rr_vv3_CORE_MOBILE);
		$isMerch = false;
		$isNewstore = false;
		...
		if (strexists($_W['siteurl'], 'web/merchant.php')) {
			...
			$isplugin = !empty($r) && is_dir(rr_vv3_PLUGIN . $routes[0]);
		}

		if ($isplugin) {
			if ($isweb) {
				require_once rr_vv3_CORE . 'inc/page_web_plugin.php';
			}
			else {
				require_once rr_vv3_CORE . 'inc/page_mobile_plugin.php';
				require_once rr_vv3_CORE . 'inc/page_mobile_plugin_login.php';
				require_once rr_vv3_CORE . 'inc/page_mobile_plugin_pf.php';
			}

			$_W['plugin'] = $routes[0];
			$root = rr_vv3_PLUGIN . $routes[0] . '/core/' . ($isweb ? 'web' : 'mobile') . '/';

			if ($isMerch) {
				$_W['plugin'] = 'merch';
				$root = rr_vv3_PLUGIN . 'merch/core/web/manage/';
			}
			else if ($isNewstore) {
				$_W['plugin'] = 'newstore';
				$root = rr_vv3_PLUGIN . 'newstore/core/web/manage/';
			}
			else {
			...
			if ($routes[0] == 'system') {
				require_once rr_vv3_CORE . 'inc/page_system.php';
			}
		}
		...
		include $file;
		$class = ucfirst($class) . '_RrvV3Page';
		$instance = new $class();
