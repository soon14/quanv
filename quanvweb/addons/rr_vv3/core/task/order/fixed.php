...
foreach ($sets as $set) {
	...
	foreach ($orders as $o) {
		if ($o['status'] == 0) {
			...
			if (!empty($isPeerpay)) {
				$refundsql = 'SELECT * FROM ' . tablename('rr_v_order_peerpay_payinfo') . ' WHERE pid = :pid';
