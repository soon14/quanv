...
function goodsReceive($order, $sysday = 0)
{
	...
	if ($order['merchid'] == 0) {
		$goods = pdo_fetchall('select og.goodsid, g.autoreceive from' . tablename('rr_v_order_goods') . ' og left join ' . tablename('rr_v_goods') . ' g on g.id=og.goodsid where og.orderid=' . $order['id']);
...
foreach ($sets as $set) {
	...
	$p = p('commission');
	$pcoupon = com('coupon');
	$orders = pdo_fetchall('select id,couponid,openid,isparent,sendtime,price,merchid,isverify,addressid,isvirtualsend,virtual,dispatchtype from ' . tablename('rr_v_order') . ' where uniacid=' . $_W['uniacid'] . ' and status=2 ', array(), 'id');
	...
	if (!empty($orders)) {
		foreach ($orders as $orderid => $order) {
			...
			pdo_query('update ' . tablename('rr_v_order') . ' set status=3,finishtime=' . time() . ' where id=' . $orderid);
