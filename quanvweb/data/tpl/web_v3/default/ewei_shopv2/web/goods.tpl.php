...
<div class="page-header">
    当前位置：<span class="text-primary">模具管理</span>
</div>
<div class="page-content">
    ...
    <form action="./index.php" method="get" class="form-horizontal form-search" role="form">
        ...
        <div class="page-toolbar">
            <span class="pull-left" style="margin-right:30px;">
                <?php if(cv('goods.add')) { ?>
                <a class='btn btn-sm btn-primary' href="<?php  echo webUrl('goods/add')?>"><i class='fa fa-plus'></i> 添加模具</a>
                <?php  } ?>
               <!--  <?php if(cv('goods.add')) { ?>
                <a class='btn btn-sm btn-primary' href="<?php  echo webUrl('goods/create')?>"><i class='fa fa-plus'></i> 快速添加模具</a>
                <?php  } ?>

                <?php  if(p('taobao')) { ?>
                    <?php if(cv('taobao')) { ?>
                        <a class='btn btn-sm btn-primary' href="<?php  echo webUrl('taobao')?>"><i class='fa fa-plus'></i> 导入模具</a>
                    <?php  } ?>
                <?php  } ?> -->
            </span>
            <div class="input-group col-sm-6 pull-right">
                <span class="input-group-select">
                    <select name="cate" class='form-control select2' style="width:200px;" data-placeholder="模具分类">
                        <option value="" <?php  if(empty($_GPC['cate'])) { ?>selected<?php  } ?> >模具分类</option>
                ...
                <input type="text" class="input-sm form-control" name='keyword' value="<?php  echo $_GPC['keyword'];?>" placeholder="ID/名称/编号/条码<?php  if($merch_plugin) { ?>/企业名称<?php  } ?>">
                ...
                <div class="btn-group">
                    ...
                    <?php  if($_GPC['goodsfrom']=='cycle') { ?>
                        <?php if(cv('goods.delete1')) { ?>
                            <button class="btn btn-default btn-sm  btn-operation" type="button" data-toggle='batch-remove' data-confirm="如果模具存在承租记录，会无法关联到模具, 确认要彻底删除吗?" data-href="<?php  echo webUrl('goods/delete1')?>">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <?php  if(!empty($item['hoteldaystock'])) { ?>
                        <span data-toggle='tooltip' title='民宿类模具显示每日库存'><?php  echo $item['hoteldaystock'];?>/日</span>
                   ...
                   <!--  <td  class="goods_attribute">
                    ...
                    <td  style="overflow:visible;position:relative">
                            ...
                            <?php  if($_GPC['goodsfrom']=='cycle') { ?>
                                ...
                                <?php  } ?>
                                <?php if(cv('goods.delete1')) { ?>
                                <a  class='btn  btn-op btn-operation' data-toggle='ajaxRemove' href="<?php  echo webUrl('goods/delete1', array('id' => $item['id']))?>" data-confirm='如果此模具存在承租记录，会无法关联到模具, 确认要彻底删除吗?？'>
                            ...
                            <?php  } else { ?>
                            <?php if(cv('goods.delete')) { ?>
                            <a  class='btn  btn-op btn-operation' data-toggle='ajaxRemove' href="<?php  echo webUrl('goods/delete', array('id' => $item['id']))?>" data-confirm='确认彻底删除此模具？'>
            ...
            <tr style="background: #f9f9f9">
                <td colspan='<?php  if($goodsfrom=='cycle') { ?>9<?php  } else { ?>10<?php  } ?>' style='text-align: left;border-top:none;padding:5px 0;' class='aops'>
                    <span class="text-default" style="margin-left: 10px;">企业名称：</span><span class="text-info"><?php  echo $item['merchname'];?></span>
                </td>
            </tr>
            <?php  } ?>
            <?php  } } ?>
  ...
  <?php  } else { ?>
    <div class="panel panel-default">
        <div class="panel-body empty-data">暂时没有任何模具!</div>
    </div>
  <?php  } ?>
</div>
