...
<script language='javascript'>
 ...
 <?php  if($_W['isfounder'] && $_W['routes']!='system.auth.upgrade') { ?>
    function check_rr_vv3_upgrade() {
  
        require(['util'], function (util) {  
            if (util.cookie.get('checkrrvv3upgrade_sys')) {
                return;
            }
	...
	var result = ret.result;	    
                    if(result.filecount>0 || result.database || result.upgrades){
                        ...
                        var html = '<div id="rr-vv3-upgrade-tips" class="tips-upgrade">';
	   html+='<span class="tclose" onclick="check_rr_vv3_upgrade_hide();"><i class="fa fa-times-circle fa-2x"></i></span>';
      ...
      function check_rr_vv3_upgrade_hide() {
        require(['util'], function (util) {
            util.cookie.set('checkrrvv3upgrade_sys', 1, 3600);
            $('#rr-vv3-upgrade-tips').fadeOut(150);
        });
    }
    $(function () {
        setTimeout( function() {
            check_rr_vv3_upgrade();
        },4000);
    });
<?php  } ?>
    ...
    $(function () {
        ...
        $('.img-thumbnail').each(function () {
            if ($(this).attr('src').indexOf('nopic.jpg') != -1) {
                $(this).attr('src', "<?php  echo rr_vv3_LOCAL?>static/images/nopic.jpg");
            }
        })
