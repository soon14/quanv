...
<div class="page-heading"> 
    ...
    <span>会员数:  <span class='text-danger'><?php  echo $membertotal;?></span>
	...
	<select name='level' class='form-control  input-sm select-md' style="width:140px;"  >
        ...
        <table class="table table-hover table-responsive">
			...
			<div  >
                    ...
                    <td>
                        <?php  if(strexists($row['openid'],'sns_wa_')) { ?>
                            <?php  if($row['isaudit']==1) { ?>
                                <label class='label label-danger'>待审核</label>
                            <?php  } else { ?>
                                <?php  if($row['identity']==0) { ?>

                                    <label class='label label-success'>患者</label>

                                <?php  } ?>

                                <?php  if($row['identity']==1) { ?>

                                    <label class='label label-primary'>医生</label>

                                <?php  } ?>
                            <?php  } ?>
                            <br/>
                        <?php  } ?>
                         <?php  echo $row['realname'];?><br/><span <?php  if(!empty($row['mobileverify'])) { ?>class="text-info" title="已绑定"<?php  } ?>><?php  echo $row['mobile'];?></span>
					...
					>
                        ...
                        <div class="btn-group btn-group-sm" >
                                <a class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-expanded="false" href="javascript:;">
                                ...
                                <?php if(cv('member.list.detail')) { ?>
                                	<li>
                                        <a href="<?php  echo webUrl('member/list/detail',array('id' => $row['id']));?>" title="会员详情">
                                ...
                                <?php  if(strexists($row['openid'],'sns_wa_')) { ?>
                                    <?php if(cv('order.list')) { ?>
                                    	<li>
                                            <a href="<?php  echo webUrl('order/list', array('searchfield'=>'mid','keyword'=>$row['id']))?>" title='会员订单'>
                                    ...
                                    <?php if(cv('finance.recharge.credit1')) { ?>
                                        <li>
                                            <a data-toggle="ajaxModal" href="<?php  echo webUrl('finance/recharge', array('type'=>'credit1','id'=>$row['id']))?>" title='充值积分'>
                                    ...
                                    <?php if(cv('finance.recharge.credit2')) { ?>
                                        <li>
                                            <a data-toggle="ajaxModal" href="<?php  echo webUrl('finance/recharge', array('type'=>'credit2','id'=>$row['id']))?>" title='充值余额'>
                                                <i class='fa fa-money'></i> 充值余额 
                                            </a>
                                        </li>
                                    <?php  } ?>
                                <?php  } ?>
                                <?php if(cv('member.list.delete')) { ?>
                                    <li>
                                        <a  data-toggle='ajaxRemove'  href="<?php  echo webUrl('member/list/delete',array('id' => $row['id']));?>" title='删除会员' data-confirm="确定要删除该会员吗？">
