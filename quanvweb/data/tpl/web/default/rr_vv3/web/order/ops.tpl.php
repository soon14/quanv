...
<?php  if($detial_flag == 1 && $item['status']!=3) { ?>
...
<?php  } else { ?>
    ...
    <?php  if($isonly) { ?>
        <!-- <?php if(cv('order.op.finish')) { ?><?php  if(strexists($_W['action'],'order.list')) { ?><?php  } ?><a class="btn btn-primary btn-xs" data-toggle='ajaxPost'  href="<?php  echo webUrl('order/op/finish', array('id' => $item['id']))?>" data-confirm="确认完成订单吗？">确认完成</a><?php  } ?> -->
    <?php  } ?>
