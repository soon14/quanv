    ...
    <div class="page-toolbar row m-b-sm m-t-sm">
        <div class="col-sm-4">
            <div class="input-group-btn">
                ...
                <?php if(cv('app.shop.banner.edit')) { ?>
                	<button class="btn btn-default btn-sm" type="button" data-toggle='batch' data-href="<?php  echo webUrl('app/shop/banner/enabled',array('enabled'=>1))?>"><i class='fa fa-circle'></i> 显示</button>
                	<button class="btn btn-default btn-sm" type="button" data-toggle='batch'  data-href="<?php  echo webUrl('app/shop/banner/enabled',array('enabled'=>0))?>"><i class='fa fa-circle-o'></i> 隐藏</button>
                <?php  } ?>
                <?php if(cv('app.shop.banner.delete')) { ?>	
                	<button class="btn btn-default btn-sm" type="button" data-toggle='batch-remove' data-confirm="确认要删除?" data-href="<?php  echo webUrl('app/shop/banner/delete')?>"><i class='fa fa-trash'></i> 删除</button>
                <?php  } ?>
    ...
    <table class="table table-responsive table-hover" >
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <?php if(cv('app.shop.banner.edit')) { ?>
                    	<a href='javascript:;' data-toggle='ajaxEdit' data-href="<?php  echo webUrl('app/shop/banner/displayorder',array('id'=>$row['id']))?>" ><?php  echo $row['displayorder'];?></a>
                    <?php  } else { ?>
                ...
                <td>
                    <label class="label <?php  if($row['show_mode']==0) { ?>label-primary<?php  } else { ?>label-success<?php  } ?>">
                        <?php  if($row['show_mode']==0) { ?>医生<?php  } else { ?>患者<?php  } ?>
                    </label>
                </td>
                ...
                <td>
                    ...
                    <span class='label <?php  if($row['enabled']==1) { ?>label-success<?php  } else { ?>label-default<?php  } ?>' 
                          <?php if(cv('app.shop.banner.edit')) { ?>
	                          ...
	                          data-switch-value1='1|显示|label label-success|<?php  echo webUrl('app/shop/banner/enabled',array('enabled'=>0,'id'=>$row['id']))?>'  
                          <?php  } ?>>
                    ...
                    <td style="text-align:left;">
                        <?php if(cv('app.shop.banner.view|app.shop.banner.edit')) { ?>
                        	<a href="<?php  echo webUrl('app/shop/banner/edit', array('id' => $row['id']))?>" class="btn btn-default btn-sm">
                        		<i class='fa fa-edit'></i> <?php if(cv('app.shop.banner.edit')) { ?>修改<?php  } else { ?>查看<?php  } ?>
                        	</a>
                        <?php  } ?>
                        <?php if(cv('app.shop.banner.delete')) { ?>
                        	<a data-toggle='ajaxRemove' href="<?php  echo webUrl('app/shop/banner/delete', array('id' => $row['id']))?>"class="btn btn-default btn-sm" data-confirm='确认要删除此广告吗?'><i class="fa fa-trash"></i> 删除</a>
                        <?php  } ?>
                    </td>
                </tr>
                <?php  } } ?> 
