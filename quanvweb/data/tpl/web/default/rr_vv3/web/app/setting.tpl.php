...
<form <?php if(cv('app.setting.edit')) { ?>action="" method="post"<?php  } ?> class="form-horizontal form-validate" enctype="multipart/form-data" >
    ...
    <div class="form-group">
        <label class="col-xs-12 col-sm-3 col-md-2 control-label">开启商品展示</label>
        <div class="col-sm-9">
            <label class="radio-inline">
                <input class="toggle" type="radio" data-show="1" data-class="goodsswitch" value="1" name="data[goodsswitch]" <?php  if($data['goodsswitch'] == 1) { ?>checked<?php  } ?>> 开启
            </label>
            <label class="radio-inline">
                <input class="toggle" type="radio" data-show="0" data-class="goodsswitch" value="0" name="data[goodsswitch]" <?php  if($data['goodsswitch'] == 0) { ?>checked<?php  } ?>>关闭
            </label>
            <label class="radio-inline">
                <input class="toggle" type="radio" data-show="0" data-class="goodsswitch" value="2" name="data[goodsswitch]" <?php  if($data['goodsswitch'] == 2) { ?>checked<?php  } ?>>开发版
            </label>
            <div class="help-block">提示：此处关闭后则不在前端显示商品信息</div>
        </div>
    </div>

    <div class="form-group goodsswitch" <?php  if(empty($data['goodsswitch'])) { ?>style="display: none"<?php  } ?>>
        <label class="col-sm-2 control-label must">版本号</label>
        <div class="col-sm-9">
            <input class="form-control valid" value="<?php  echo $data['version'];?>" name="data[version]" />
        </div>
    </div>
