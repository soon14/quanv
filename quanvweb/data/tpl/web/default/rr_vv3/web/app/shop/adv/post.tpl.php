...
<div class="page-heading"> 
	<span class='pull-right'>
		<?php if(cv('app.shop.adv.add')) { ?>
			<a class="btn btn-primary btn-sm" href="<?php  echo webUrl('app/shop/adv/add')?>">添加新幻灯片</a>
		<?php  } ?>
		<a class="btn btn-default  btn-sm" href="<?php  echo webUrl('app/shop/adv')?>">返回列表</a>
	</span>
	<h2><?php  if(!empty($item['id'])) { ?>编辑<?php  } else { ?>添加<?php  } ?>幻灯片 <small><?php  if(!empty($item['id'])) { ?>修改【<?php  echo $item['advname'];?>】<?php  } ?></small></h2> 
</div>
 
    <form <?php if( ce('app.shop.adv' ,$item) ) { ?>action="" method="post"<?php  } ?> class="form-horizontal form-validate" enctype="multipart/form-data">
        <input type="hidden" name="id" value="<?php  echo $item['id'];?>" />
                <div class="form-group">
                    <label class="col-sm-2 control-label">排序</label>
                    <div class="col-sm-9 col-xs-12">
                        <?php if( ce('app.shop.adv' ,$item) ) { ?>
                    	...
                    	<?php if( ce('app.shop.adv' ,$item) ) { ?>
                    		<div class="input-group form-group" style="margin: 0;">
                    			<input type="text" value="<?php  echo $item['link'];?>" class="form-control valid" name="link" placeholder="" id="advlink">
                    				<span data-input="#advlink" data-toggle="selectUrl" data-platform="wxapp" class="input-group-addon btn btn-default">选择链接</span>
                    		</div>
                        <?php  } else { ?>
                    ...
                    <div class="col-sm-9 col-xs-12">
                    	<?php if( ce('app.shop.adv' ,$item) ) { ?>
                    		<label class='radio-inline'>
                            	<input type='radio' name='showtype' value='1' <?php  if($item['showtype']==1) { ?>checked<?php  } ?> /> 医生端
                            </label>
                        	<label class='radio-inline'>
                        		<input type='radio' name='showtype' value='0' <?php  if($item['showtype']==0) { ?>checked<?php  } ?> /> 患者端
                        	</label>
                        <?php  } else { ?>
                            <div class='form-control-static'><?php  if($item['showtype'] == 1) { ?>医生端<?php  } else { ?>患者端<?php  } ?></div>
                        <?php  } ?>
                    </div>
                </div>
            ...
            <div class="form-group">
                    ...
                    <div class="col-sm-9 col-xs-12">
                    	...
                    	<?php  } ?>
                       <input type="button" name="back" onclick='history.back()' <?php if(cv('app.shop.adv.add|app.shop.adv.edit')) { ?>style='margin-left:10px;'<?php  } ?> value="返回列表" class="btn btn-default" />
                    </div>
            </div>
