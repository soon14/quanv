...
<div class="system-login" <?php  if(!empty($_W['setting']['copyright']['background_img'])) { ?> style="background-image:url('<?php  echo tomedia($_W['setting']['copyright']['background_img']);?>')" <?php  } else { ?> style="background-image: url('./resource/images/bg-login.png');" <?php  } ?>>
	...
	<div class="login-panel">
		<div class="title">
			<a href="<?php  echo url('user/login', array('login_type' => 'system'))?>" <?php  if(empty($_GPC['login_type']) || $_GPC['login_type'] == 'system') { ?>class="active" <?php  } ?>>账号密码</a>
			<?php  if(!empty($_W['setting']['copyright']['mobile_status'])) { ?>
			<a href="<?php  echo url('user/login', array('login_type' => 'mobile'))?>" <?php  if($_GPC['login_type'] == 'mobile') { ?>class="active" <?php  } ?>>手机</a>
			<?php  } ?>
		</div>
